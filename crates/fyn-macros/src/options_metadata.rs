    ...
    match data {
        Data::Struct(DataStruct {
            ...
            Ok(quote! {
                ...
                impl fyn_options_metadata::OptionsMetadata for #ident {
                    fn record(visit: &mut dyn fyn_options_metadata::Visit) {
                        #(#output);*
                    }
    ...
    match &field.ty {
        Type::Path(TypePath {
            ...
            Some(PathSegment {
                ...
                Ok(quote_spanned!(
                    ident.span() => (visit.record_set(#kebab_name, fyn_options_metadata::OptionSet::of::<#path>()))
                ))
            }
            _ => Err(syn::Error::new(
                ident.span(),
...
fn handle_option(field: &Field, attr: &Attribute) -> syn::Result<TokenStream> {
        ...
        .find(|attr| attr.path().is_ident("deprecated"))
    {
        ...
        quote!(Some(fyn_options_metadata::Deprecated { since: #since, message: #note }))
    } else {
        quote!(None)
    };
        ...
        quote!(
            Some(
                <#inner_type as clap::ValueEnum>::value_variants()
                    ...
                    .map(|value| fyn_options_metadata::PossibleValue {
    ...
    Ok(quote_spanned!(
        ident.span() => {
            visit.record_field(#kebab_name, fyn_options_metadata::OptionField{
                doc: &#doc,
                default: &#default,
    ...
    attribute.parse_nested_meta(|meta| {
        if meta.path.is_ident("default") {
            ...
            return Err(syn::Error::new(
                meta.path.span(),
                format!(
                    "Deprecated meta {:?} is not supported by fyn's option macro.",
                    meta.path.get_ident()
                ),
            ));
        }
