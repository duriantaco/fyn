...
///
/// These represent a subset of the `pip-compile` interface that fv supports by default.
/// For example, users often pass `--allow-unsafe`, which is unnecessary with fv. But it's a
...
impl CompatArgs for PipCompileCompatArgs {
    ...
    ///
    /// This method will warn when an argument is passed that has no effect but matches fv's
    /// behavior. If an argument is passed that does _not_ match fv's behavior (e.g.,
    /// `--no-build-isolation`), this method will return an error.
    fn validate(&self) -> Result<()> {
        if self.allow_unsafe {
            warn_user!(
                "pip-compile's `--allow-unsafe` has no effect (fv can safely pin `pip` and other packages)"
            );
        }

        if self.no_allow_unsafe {
            warn_user!(
                "pip-compile's `--no-allow-unsafe` has no effect (fv can safely pin `pip` and other packages)"
            );
        }

        if self.reuse_hashes {
            return Err(anyhow!(
                "pip-compile's `--reuse-hashes` is unsupported (fv doesn't reuse hashes)"
            ));
        }

        if self.no_reuse_hashes {
            warn_user!("pip-compile's `--no-reuse-hashes` has no effect (fv doesn't reuse hashes)");
        }
        ...
        if let Some(resolver) = self.resolver {
            match resolver {
                Resolver::Backtracking => {
                    warn_user!(
                        "pip-compile's `--resolver=backtracking` has no effect (fv always backtracks)"
                    );
                }
                Resolver::Legacy => {
                    return Err(anyhow!(
        ...
        if self.client_cert.is_some() {
            return Err(anyhow!(
                "pip-compile's `--client-cert` is unsupported (fv doesn't support dedicated client certificates)"
            ));
        }
        ...
        if self.no_emit_trusted_host {
            warn_user!(
                "pip-compile's `--no-emit-trusted-host` has no effect (fv never emits trusted hosts)"
            );
        }

        if self.config.is_some() {
            return Err(anyhow!(
                "pip-compile's `--config` is unsupported (fv does not use a configuration file)"
            ));
        }

        if self.emit_options {
            return Err(anyhow!(
                "pip-compile's `--emit-options` is unsupported (fv never emits options)"
            ));
        }

        if self.no_emit_options {
            warn_user!("pip-compile's `--no-emit-options` has no effect (fv never emits options)");
        }

        if self.pip_args.is_some() {
            return Err(anyhow!(
                "pip-compile's `--pip-args` is unsupported (try passing arguments to fv directly)"
            ));
        }
...
///
/// These represent a subset of the `pip list` interface that fv supports by default.
#[derive(Args)]
...
///
/// These represent a subset of the `pip-sync` interface that fv supports by default.
#[derive(Args)]
...
impl CompatArgs for PipSyncCompatArgs {
    ...
    ///
    /// This method will warn when an argument is passed that has no effect but matches fv's
    /// behavior. If an argument is passed that does _not_ match fv's behavior, this method will
    /// return an error.
    fn validate(&self) -> Result<()> {
        if self.ask {
            return Err(anyhow!(
                "pip-sync's `--ask` is unsupported (fv never asks for confirmation)"
            ));
        }
        ...
        if self.client_cert.is_some() {
            return Err(anyhow!(
                "pip-sync's `--client-cert` is unsupported (fv doesn't support dedicated client certificates)"
            ));
        }

        if self.config.is_some() {
            return Err(anyhow!(
                "pip-sync's `--config` is unsupported (fv does not use a configuration file)"
            ));
        }

        if self.pip_args.is_some() {
            return Err(anyhow!(
                "pip-sync's `--pip-args` is unsupported (try passing arguments to fv directly)"
            ));
        }
...
///
/// These represent a subset of the `virtualenv` interface that fv supports by default.
#[derive(Args)]
...
impl CompatArgs for VenvCompatArgs {
    ...
    fn validate(&self) -> Result<()> {
        if self.no_seed {
            warn_user!(
                "virtualenv's `--no-seed` has no effect (fv omits seed packages by default)"
            );
        }

        if self.no_pip {
            warn_user!("virtualenv's `--no-pip` has no effect (fv omits `pip` by default)");
        }

        if self.no_setuptools {
            warn_user!(
                "virtualenv's `--no-setuptools` has no effect (fv omits `setuptools` by default)"
            );
        }

        if self.no_wheel {
            warn_user!("virtualenv's `--no-wheel` has no effect (fv omits `wheel` by default)");
        }
...
///
/// These represent a subset of the `pip install` interface that fv supports by default.
#[derive(Args)]
