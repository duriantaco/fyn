...
use fv_pep508::fv_pep440::Version;

/// Information about the git repository where fv was built from.
#[derive(Serialize)]
...
/// Version information for fv itself (e.g., in `fv self version`).
#[derive(Serialize)]
pub struct SelfVersionInfo {
    /// Name of the package (always "fv").
    package_name: String,
    ...
    /// The target triple for which fv was built (e.g., `x86_64-unknown-linux-gnu`).
    target_triple: String,
}

/// Version information for a project (`fv version`).
#[derive(Serialize)]
pub struct ProjectVersionInfo {
    ...
    /// Version, such as "0.5.1".
    version: String,
    /// Information about the git commit fv was built from.
    ///
...
impl fmt::Display for SelfVersionInfo {
    ...
    ///
    /// This is intended for consumption by `clap` to provide `fv --version`,
...
/// Returns information about fv's version.
pub fn uv_self_version() -> SelfVersionInfo {
    ...
    let version = fv_version::version().to_string();
