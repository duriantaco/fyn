...
use fv_auth::Service;
use fv_cache::CacheArgs;
use fv_configuration::{
...
use fv_normalize::{ExtraName, GroupName, PackageName, PipGroupName};
use fv_pep508::{MarkerTree, Requirement};
use fv_preview::PreviewFeature;
use fv_pypi_types::VerbatimParsedUrl;
use fv_python::{PythonDownloads, PythonPreference, PythonVersion};
use fv_redacted::DisplaySafeUrl;
use fv_resolver::{
    AnnotationStyle, ExcludeNewerPackageEntry, ExcludeNewerValue, ForkStrategy, PrereleaseMode,
    ResolutionMode,
};
use fv_settings::PythonInstallMirrors;
use fv_static::EnvVars;
use fv_torch::TorchMode;
use fv_workspace::pyproject_mut::AddBoundsKind;
...
#[derive(Parser)]
#[command(name = "fv", author, long_version = crate::version::uv_self_version())]
#[command(about = "An extremely fast Python package manager.")]
#[command(
    after_help = "Use `fv help` for more details.",
    after_long_help = "",
...
pub struct TopLevelArgs {
    ...
    /// The path to a `fv.toml` file to use for configuration.
    ///
    /// While fv configuration can be included in a `pyproject.toml` file, it is
    ...
    /// Avoid discovering configuration files (`pyproject.toml`, `fv.toml`).
    ///
...
pub struct GlobalArgs {
    ...
    /// Require use of fv-managed Python versions [env: UV_MANAGED_PYTHON=]
    ///
    /// By default, fv prefers using Python versions it manages. However, it will use system Python
    /// versions if a fv-managed Python is not installed. This option disables use of system Python
    /// versions.
    #[arg(
        global = true,
        long,
    ...
    /// Disable use of fv-managed Python versions [env: UV_NO_MANAGED_PYTHON=]
    ///
    /// Instead, fv will search for a suitable Python version on the system.
    #[arg(
        global = true,
        long,
        help_heading = "Python options",
    ...
    /// fv will write no output to stdout.
    ...
    ///
    /// By default, fv will automatically detect support for colors when writing to a terminal.
    #[arg(
        global = true,
        long,
        value_enum,
    ...
    ///
    /// By default, fv loads certificates from the bundled `webpki-roots` crate. The
    /// `webpki-roots` are a reliable set of trust roots from Mozilla, and including them in fv
    ...
    ///
    /// When disabled, fv will only use locally cached data and locally available files.
    ...
    /// Avoid discovering a `pyproject.toml` or `fv.toml` file [env: UV_ISOLATED=]
    ///
    /// Normally, configuration files are discovered in the current directory,
    /// parent directories, or user configuration directories.
    ///
    ...
    /// Skip writing `fv` installer metadata files (e.g., `INSTALLER`, `REQUESTED`, and
    ...
    ///
    /// All `pyproject.toml`, `fv.toml`, and `.python-version` files will be discovered by walking
    ...
    ///
    /// This setting has no effect when used in the `fv pip` interface.
...
pub enum Commands {
    ...
    #[command(
        after_help = "Use `fv help auth` for more details.",
        after_long_help = ""
    )]
    Auth(AuthNamespace),
    ...
    #[command(
        after_help = "Use `fv help tool` for more details.",
        after_long_help = ""
    )]
    Tool(ToolNamespace),
    ...
    ///
    /// Generally, fv first searches for Python in a virtual environment, either active or in a
    /// `.venv` directory in the current working directory or any parent directory. If a virtual
    /// environment is not required, fv will then search for a Python interpreter. Python
    ...
    ///
    /// By default, fv will download Python if a version cannot be found. This behavior can be
    ...
    /// are checked for compatibility with the request, e.g., if `pypy` is requested, fv will first
    ...
    ///
    /// fv supports discovering CPython, PyPy, and GraalPy interpreters. Unsupported interpreters
    /// will be skipped during discovery. If an unsupported interpreter implementation is requested,
    /// fv will exit with an error.
    #[clap(verbatim_doc_comment)]
    #[command(
        after_help = "Use `fv help python` for more details.",
        after_long_help = ""
    )]
    Python(PythonNamespace),
    /// Manage Python packages with a pip-compatible interface.
    #[command(
        after_help = "Use `fv help pip` for more details.",
        after_long_help = ""
    )]
    Pip(PipNamespace),
    ...
    ///
    /// When using fv, the virtual environment does not need to be activated. fv
    /// will find a virtual environment (named `.venv`) in the working directory
    /// or any parent directories.
    #[command(
        alias = "virtualenv",
        alias = "v",
        after_help = "Use `fv help venv` for more details.",
        after_long_help = ""
    )]
    Venv(VenvArgs),
    /// Activate the virtual environment in a new shell.
    ///
    /// Spawns a new shell process with the project's virtual environment
    /// activated. The virtual environment's `bin` directory is prepended
    /// to `PATH` and `VIRTUAL_ENV` is set.
    ///
    /// Use `exit` or Ctrl-D to leave the activated shell.
    #[command(
        after_help = "Use `fv help shell` for more details.",
        after_long_help = ""
    )]
    Shell(ShellArgs),
    /// Build Python packages into source distributions and wheels.
    ///
    /// `fv build` accepts a path to a directory or source distribution,
    /// which defaults to the current working directory.
    ///
    /// By default, if passed a directory, `fv build` will build a source
    /// distribution ("sdist") from the source directory, and a binary
    /// distribution ("wheel") from the source distribution.
    ///
    /// `fv build --sdist` can be used to build only the source distribution,
    /// `fv build --wheel` can be used to build only the binary distribution,
    /// and `fv build --sdist --wheel` can be used to build both distributions
    /// from source.
    ///
    /// If passed a source distribution, `fv build --wheel` will build a wheel
    /// from the source distribution.
    #[command(
        after_help = "Use `fv help build` for more details.",
        after_long_help = ""
    )]
    Build(BuildArgs),
    /// Upload distributions to an index.
    Publish(PublishArgs),
    /// Inspect fv workspaces.
    #[command(
        after_help = "Use `fv help workspace` for more details.",
        after_long_help = "",
        hide = true
    )]
    ...
    /// frontend (PEP 517) which calls the Python shims which calls back into fv with this method.
    #[command(hide = true)]
    BuildBackend {
        #[command(subcommand)]
    ...
    /// Manage fv's cache.
    #[command(
        after_help = "Use `fv help cache` for more details.",
        after_long_help = ""
    )]
    Cache(CacheNamespace),
    /// Manage the fv executable.
    #[command(name = "self")]
    Self_(SelfNamespace),
...
pub struct VersionArgs {
    ...
    ///
    /// By default, fv will show the project name before the version.
    #[arg(long)]
    pub short: bool,
    ...
    /// Assert that the `fv.lock` will remain unchanged [env: UV_LOCKED=]
    ///
    ...
    ///
    /// See `fv help python` for details on Python discovery and supported request formats.
    #[arg(
        long,
        short,
...
pub struct SelfUpdateArgs {
    /// Update to the specified version. If not provided, fv will update to the latest version.
    #[arg(value_hint = ValueHint::Other)]
...
pub enum CacheCommand {
    ...
    ///
    /// By default, the cache is stored in `$XDG_CACHE_HOME/uv` or `$HOME/.cache/fv` on Unix and
    ...
    /// file system as the Python environment fv is operating on.
    Dir,
    /// Show the cache size.
    ///
...
pub struct CleanArgs {
    ...
    ///
    /// By default, `fv cache clean` will block until no process is reading the cache. When
    /// `--force` is used, `fv cache clean` will proceed without taking a lock.
    #[arg(long)]
    pub force: bool,
}
...
pub struct PruneArgs {
    ...
    /// Actions.
    ///
    /// By default, fv caches both the wheels that it builds from source and the pre-built wheels
    ...
    /// modules.
    ///
    /// In `--ci` mode, fv will prune any pre-built wheels from the cache, but retain any wheels
    ...
    /// Force removal of the cache, ignoring in-use checks.
    ///
    /// By default, `fv cache prune` will block until no process is reading the cache. When
    /// `--force` is used, `fv cache prune` will proceed without taking a lock.
    #[arg(long)]
    pub force: bool,
}
...
pub enum PipCommand {
    ...
    #[command(
        after_help = "Use `fv help pip compile` for more details.",
        after_long_help = ""
    )]
    ...
    /// `pylock.toml` file will be removed. To retain extraneous packages, use `fv pip install`
    /// instead.
    ///
    /// The input file is presumed to be the output of a `pip compile` or `fv export` operation,
    ...
    #[command(
        after_help = "Use `fv help pip sync` for more details.",
        after_long_help = ""
    )]
    Sync(Box<PipSyncArgs>),
    /// Install packages into an environment.
    #[command(
        after_help = "Use `fv help pip install` for more details.",
        after_long_help = ""
    )]
    Install(PipInstallArgs),
    /// Uninstall packages from an environment.
    #[command(
        after_help = "Use `fv help pip uninstall` for more details.",
        after_long_help = ""
    )]
    Uninstall(PipUninstallArgs),
    /// List, in requirements format, packages installed in an environment.
    #[command(
        after_help = "Use `fv help pip freeze` for more details.",
        after_long_help = ""
    )]
    Freeze(PipFreezeArgs),
    /// List, in tabular format, packages installed in an environment.
    #[command(
        after_help = "Use `fv help pip list` for more details.",
        after_long_help = "",
        alias = "ls"
    )]
    List(PipListArgs),
    ...
    #[command(
        after_help = "Use `fv help pip show` for more details.",
        after_long_help = ""
    )]
    Show(PipShowArgs),
    /// Display the dependency tree for an environment.
    #[command(
        after_help = "Use `fv help pip tree` for more details.",
        after_long_help = ""
    )]
    Tree(PipTreeArgs),
    /// Verify installed packages have compatible dependencies.
    #[command(
        after_help = "Use `fv help pip check` for more details.",
        after_long_help = ""
    )]
    Check(PipCheckArgs),
...
pub enum ProjectCommand {
    ...
    /// script and run with a Python interpreter, i.e., `fv run file.py` is equivalent to `fv run
    ...
    ///
    /// Arguments following the command (or script) are not interpreted as arguments to fv. All
    /// options to fv must be provided before the command, e.g., `fv run --verbose foo`. A `--` can
    /// be used to separate the command from fv options for clarity, e.g., `fv run --python 3.12 --
    /// python`.
    #[command(
        after_help = "Use `fv help run` for more details.",
        after_long_help = ""
    )]
    Run(RunArgs),
    ...
    ///
    /// If a `pyproject.toml` already exists at the target, fv will exit with an error.
    ///
    ...
    /// (`.venv`) and lockfile (`fv.lock`) are lazily created during the first sync.
    Init(InitArgs),
    ...
    /// `--no-sync`.
    ///
    /// If any of the requested dependencies cannot be found, fv will exit with an error, unless the
    /// `--frozen` flag is provided, in which case fv will add the dependencies verbatim without
    /// checking that they exist or are compatible with the project.
    ///
    /// fv will search for a project in the current directory or any parent directory. If a project
    /// cannot be found, fv will exit with an error.
    #[command(
        after_help = "Use `fv help add` for more details.",
        after_long_help = ""
    )]
    Add(AddArgs),
    ...
    /// `--no-sync`.
    ///
    /// If any of the requested dependencies are not present in the project, fv will exit with an
    /// error.
    ///
    /// If a package has been manually installed in the environment, i.e., with `fv pip install`, it
    /// will not be removed by `fv remove`.
    ///
    /// fv will search for a project in the current directory or any parent directory. If a project
    /// cannot be found, fv will exit with an error.
    #[command(
        after_help = "Use `fv help remove` for more details.",
        after_long_help = ""
    )]
    Remove(RemoveArgs),
    ...
    /// lockfile.
    ///
    /// By default, an exact sync is performed: fv removes packages that are not declared as
    ...
    /// Additionally, if `--no-build-isolation` is used, fv will not remove extraneous packages to
    ...
    /// cannot be found, fv will exit with an error.
    ///
    /// Note that, when installing from a lockfile, fv will not provide warnings for yanked package
    /// versions.
    #[command(
        after_help = "Use `fv help sync` for more details.",
        after_long_help = ""
    )]
    Sync(SyncArgs),
    /// Update the project's lockfile.
    ///
    /// If the project lockfile (`fv.lock`) does not exist, it will be created. If a lockfile is
    ...
    #[command(
        after_help = "Use `fv help lock` for more details.",
        after_long_help = ""
    )]
    Lock(LockArgs),
    /// Upgrade project dependencies.
    ///
    /// Re-resolves all (or specified) project dependencies to their latest
    /// compatible versions, updates the lockfile, and syncs the environment.
    ///
    /// This is equivalent to running `fv lock --upgrade` followed by `fv sync`.
    ///
    /// To upgrade specific packages, pass them as arguments, e.g.,
    /// `fv upgrade requests flask`.
    #[command(
        after_help = "Use `fv help upgrade` for more details.",
        after_long_help = ""
    )]
    Upgrade(UpgradeArgs),
    ...
    #[command(
        after_help = "Use `fv help export` for more details.",
        after_long_help = ""
    )]
    Export(ExportArgs),
    ...
    #[command(
        after_help = "Use `fv help format` for more details.",
        after_long_help = ""
    )]
    Format(FormatArgs),
    ...
    #[command(
        after_help = "Use `fv help audit` for more details.",
        after_long_help = ""
    )]
    Audit(AuditArgs),
}
...
pub struct PipCompileArgs {
    ...
    ///
    /// If a `pyproject.toml`, `setup.py`, or `setup.cfg` file is provided, fv will extract the
    ...
    ///
    /// fv will infer the output format from the file extension of the output file, if
    ...
    ///
    /// By default, fv strips extras, as any packages pulled in by the extras are already included
    ...
    ///
    /// By default, fv strips environment markers, as the resolution generated by `compile` is
    ...
    /// The header comment to include at the top of the output file generated by `fv pip compile`.
    ///
    /// Used to reflect custom build scripts and commands that wrap `fv pip compile`.
    ...
    ///
    /// By default, fv uses the virtual environment in the current working directory or any parent
    /// directory, falling back to searching for a Python executable in `PATH`. The `--system`
    /// option instructs fv to avoid using a virtual environment Python and restrict its search to
    /// the system path.
    #[arg(
        long,
    ...
    ///
    /// When set, fv will ignore the configured index URLs for packages in the PyTorch ecosystem,
    /// and will instead use the defined backend.
    ///
    /// For example, when set to `cpu`, fv will use the CPU-only PyTorch index; when set to `cu126`,
    /// fv will use the PyTorch index for CUDA 12.6.
    ///
...
pub struct PipSyncArgs {
    ...
    /// `pylock.toml`, `pyproject.toml`, `setup.py`, and `setup.cfg`.
    ///
    /// If a `pyproject.toml`, `setup.py`, or `setup.cfg` file is provided, fv will
    ...
    ///
    /// By default, fv will verify any available hashes in the requirements file, but will not
    ...
    /// Install packages into the system Python environment.
    ///
    /// By default, fv installs into the virtual environment in the current working directory or any
    /// parent directory. The `--system` option instructs fv to instead use the first Python found
    /// in the system `PATH`.
    ///
    ...
    /// Allow fv to modify an `EXTERNALLY-MANAGED` Python installation.
    ///
    /// WARNING: `--break-system-packages` is intended for use in continuous integration (CI)
    /// environments, when installing into Python installations that are managed by an external
    /// package manager, like `apt`. It should be used with caution, as such Python installations
    /// explicitly recommend against modifications by other package managers (like fv or `pip`).
    #[arg(
        long,
    ...
    /// If a suitable Python interpreter cannot be found, fv will install one.
    ...
    /// `ANDROID_API_LEVEL` to specify a different minimum version, e.g., `26`.
    ///
    /// WARNING: When specified, fv will select wheels that are compatible with the _target_
...
pub struct PipInstallArgs {
    ...
    /// When enabled, fv will update the environment to exactly match the requirements, removing
...
pub struct PipUninstallArgs {
    ...
    ///
    /// At present, only `--keyring-provider subprocess` is supported, which configures fv to use
    ...
    ///
    /// By default, fv uninstalls from the virtual environment in the current working directory or
    /// any parent directory. The `--system` option instructs fv to instead use the first Python
...
pub struct PipFreezeArgs {
    ...
    ///
    /// By default, fv lists packages in a virtual environment but will show packages in a system
    ...
    ///
    /// See `fv help python` for details on Python discovery.
    #[arg(
        long,
        env = EnvVars::UV_SYSTEM_PYTHON,
        value_parser = clap::builder::BoolishValueParser::new(),
...
pub struct PipCheckArgs {
    ...
    ///
    /// By default, fv checks packages in a virtual environment but will check packages in a system
    /// Python environment if no virtual environment is found.
    ///
    /// See `fv help python` for details on Python discovery and supported request formats.
    #[arg(
...
pub struct PipShowArgs {
    ...
    ///
    /// By default, fv looks for packages in a virtual environment but will look for packages in a
...
pub struct BuildArgs {
    ...
    ///
    /// If the workspace member does not exist, fv will exit with an error.
    ...
    /// When using the fv build backend, list the files that would be included when building.
    ///
    ...
    /// works with the fv build backend, there is no PEP 517 file list build hook.
    ///
    ...
    /// Always build through PEP 517, don't use the fast path for the fv build backend.
    ///
    /// By default, fv won't create a PEP 517 build environment for packages using the fv build
    ...
    ///
    /// By default, fv creates a `.gitignore` file in the output directory to exclude build
    ...
    /// the platform.
    ///
    /// See `fv help python` to view supported request formats.
    #[arg(
        long,
        short,
        env = EnvVars::UV_PYTHON,
...
pub struct VenvArgs {
    ...
    ///
    /// During virtual environment creation, fv will not look for Python interpreters in virtual
    /// environments.
    ///
    /// See `fv help python` for details on Python discovery and supported request formats.
    #[arg(
    ...
    ///
    /// fv will never search for interpreters in virtual environments when creating a virtual
    /// environment.
    #[arg(long, overrides_with("system"), hide = true)]
    pub no_system: bool,
    ...
    ///
    /// By default, fv searches for projects in the current directory or any parent directory to
    ...
    ///
    /// By default, `fv venv` will exit with an error if the given path is non-empty. The
    ...
    ///
    /// By default, when a TTY is available, `fv venv` will prompt to clear a non-empty directory.
    ...
    ///
    /// By default, the prompt is dependent on whether a path was provided to `fv venv`. If provided
    /// (e.g, `fv venv project`), the prompt is set to the directory name. If not provided
    /// (`fv venv`), the prompt is set to the current directory's name.
    ///
    ...
    ///
    /// Unlike `pip`, when a virtual environment is created with `--system-site-packages`, fv will
    /// _not_ take system site packages into account when running commands like `fv pip list` or `uv
    ...
    ///
    /// By default, fv will stop at the first index on which a given package is available, and
    ...
    /// the cache and the target environment. For example, clearing the cache (`fv cache clean`)
    ...
    pub link_mode: Option<fv_install_wheel::LinkMode>,
...
pub struct ShellArgs {
    ...
    ///
    /// If not provided, fv will look for a `.venv` directory in the current
    /// directory or any parent directory. The `VIRTUAL_ENV` environment
    ...
    /// Avoid discovering a project or workspace.
    ///
    /// By default, fv searches for projects in the current directory or any
    /// parent directory to determine the virtual environment path.
    #[arg(long, alias = "no-workspace")]
    pub no_project: bool,
}
...
pub struct InitArgs {
    ...
    ///
    /// PEP 723 scripts can be executed directly with `fv run`.
    ///
    ...
    ///
    /// By default, fv will initialize a Git repository (`git`). Use `--vcs none` to explicitly
    ...
    ///
    /// By default, fv will attempt to infer the author information from some sources (e.g., Git)
    ...
    ///
    /// By default, fv will create a `.python-version` file containing the minor version of the
    /// discovered Python interpreter, which will cause subsequent fv commands to use that version.
    #[arg(long)]
...
pub struct RunArgs {
    ...
    /// `tool.fv.dev-dependencies` in a `pyproject.toml`.
    ///
    ...
    ///
    /// fv includes the groups defined in `tool.fv.default-groups` by default.
    ...
    /// Perform an exact sync, removing extraneous packages.
    ///
    /// When enabled, fv will remove any extraneous packages from the environment. By default, `uv
    ...
    /// needs to be updated, fv will exit with an error.
    #[arg(long, conflicts_with_all = ["frozen", "upgrade"])]
    pub locked: bool,

    /// Run without updating the `fv.lock` file [env: UV_FROZEN=]
    ///
    /// Instead of checking if the lockfile is up-to-date, uses the versions in the lockfile as the
    /// source of truth. If the lockfile is missing, fv will exit with an error. If the
    ...
    /// Number of times that `fv run` will allow recursive invocations.
    ///
    /// The current recursion depth is tracked by environment variable. If environment variables are
    /// cleared, fv will fail to detect the recursion depth.
    ///
    ...
    /// List available tasks defined in `[tool.fv.tasks]` and exit.
    #[arg(long)]
    pub list_tasks: bool,
}
...
pub struct SyncArgs {
    ...
    /// May be provided more than once.
    ///
    /// When multiple extras or groups are specified that appear in `tool.fv.conflicts`, fv will
    /// report an error.
    ///
    ...
    /// Include all optional dependencies.
    ///
    /// When two or more extras are declared as conflicting in `tool.fv.conflicts`, using this flag
    ...
    /// `tool.fv.conflicts`, fv will report an error.
    ///
    /// May be provided multiple times.
    #[arg(long, conflicts_with_all = ["only_group", "only_dev"], value_hint = ValueHint::Other)]
    pub group: Vec<GroupName>,
    ...
    /// Do not remove extraneous packages present in the environment.
    ///
    /// When enabled, fv will make the minimum necessary changes to satisfy the requirements.
    ...
    /// Sync without updating the `fv.lock` file [env: UV_FROZEN=]
    ///
    /// Instead of checking if the lockfile is up-to-date, uses the versions in the lockfile as the
    /// source of truth. If the lockfile is missing, fv will exit with an error. If the
    /// `pyproject.toml` includes changes to dependencies that have not been included in the
    ...
    ///
    /// In dry-run mode, fv will resolve the project's dependencies and report on the resulting
    ...
    ///
    /// If any workspace member does not exist, fv will exit with an error.
    #[arg(long, conflicts_with = "all_packages", value_hint = ValueHint::Other)]
    pub package: Vec<PackageName>,

    /// Sync the environment for a Python script, rather than the current project.
    ///
    /// If provided, fv will sync the dependencies based on the script's inline metadata table, in
    ...
    ///
    /// If the environment is not up to date, fv will exit with an error.
...
pub struct LockArgs {
    ...
    ///
    /// Asserts that the `fv.lock` would remain unchanged after a resolution. If the lockfile is
    /// missing or needs to be updated, fv will exit with an error.
    ///
    ...
    /// Assert that a `fv.lock` exists without checking if it is up-to-date [env: UV_FROZEN=]
    ///
    ...
    ///
    /// If provided, fv will lock the script (based on its inline metadata table, in adherence with
...
pub struct AddArgs {
    ...
    ///
    /// By default, fv will use the `tool.fv.sources` section to record source information for Git,
    /// local, editable, and direct URL requirements. When `--raw` is provided, fv will add source
    /// requirements to `project.dependencies`, rather than `tool.fv.sources`.
    ///
    /// Additionally, by default, fv will add bounds to your dependency, e.g., `foo>=1.0.0`. When
    /// `--raw` is provided, fv will add the dependency without bounds.
    #[arg(
        long,
    ...
    ///
    /// If provided, fv will add the dependency to the script's inline metadata table, in adherence
    /// with PEP 723. If no such inline metadata table is present, a new one will be created and
    /// added to the script. When executed via `fv run`, fv will create a temporary environment for
    ...
    ///
    /// By default, fv will add path dependencies that are within the workspace directory
    ...
    /// directory, fv will add it as a workspace member; pass `--no-workspace` to add the package
...
pub struct RemoveArgs {
    ...
    ///
    /// If provided, fv will remove the dependency from the script's inline metadata table, in
    /// adherence with PEP 723.
    #[arg(long, value_hint = ValueHint::FilePath)]
    pub script: Option<PathBuf>,
...
pub struct TreeArgs {
    ...
    ///
    /// If the lockfile is missing, fv will exit with an error.
    #[arg(long, conflicts_with_all = ["locked", "upgrade", "no_sources"])]
    pub frozen: bool,
    ...
    /// project.
    ///
    /// If provided, fv will resolve the dependencies based on its inline metadata table, in
    /// adherence with PEP 723.
    #[arg(long, value_hint = ValueHint::FilePath)]
    pub script: Option<PathBuf>,
...
pub struct ExportArgs {
    /// The format to which `fv.lock` should be exported.
    ///
    ...
    /// Do not update the `fv.lock` before exporting [env: UV_FROZEN=]
    ///
    /// If a `fv.lock` does not exist, fv will exit with an error.
    #[arg(long, conflicts_with_all = ["locked", "upgrade", "no_sources"])]
    pub frozen: bool,
...
pub struct FormatArgs {
    ...
    ///
    /// For example, use `fv format -- --line-length 100` to set the line length or
    /// `fv format -- src/module/foo.py` to format a specific file.
...
pub struct AuditArgs {
    ...
    /// project.
    ///
    /// The specified script must be locked, i.e. with `fv lock --script <script>`
...
pub enum AuthCommand {
    ...
    Token(AuthTokenArgs),
    /// Show the path to the fv credentials directory.
    ///
    /// By default, credentials are stored in the fv data directory at
    /// `$XDG_DATA_HOME/fv/credentials` or `$HOME/.local/share/fv/credentials` on Unix and
...
pub enum ToolCommand {
    ...
    /// e.g., `fv tool run ruff@0.3.0`. If more complex version specification is desired or if the
    /// command is provided by a different package, use `--from`.
    ///
    /// `fvx` can be used to invoke Python, e.g., with `fvx python` or `fvx python@<version>`. A
    /// Python interpreter will be started in an isolated virtual environment.
    ///
    /// If the tool was previously installed, i.e., via `fv tool install`, the installed version
    /// will be used unless a version is requested or the `--isolated` flag is used.
    ///
    /// `fvx` is provided as a convenient alias for `fv tool run`, their behavior is identical.
    ///
    /// If no command is provided, the installed tools are displayed.
    ///
    /// Packages are installed into an ephemeral virtual environment in the fv cache directory.
    #[command(
        after_help = "Use `fvx` as a shortcut for `fv tool run`.\n\n\
        Use `fv help tool run` for more details.",
        after_long_help = ""
    )]
    Run(ToolRunArgs),
    /// Hidden alias for `fv tool run` for the `fvx` command
    #[command(
        hide = true,
        override_usage = "fvx [OPTIONS] [COMMAND]",
        about = "Run a command provided by a Python package.",
        after_help = "Use `fv help tool run` for more details.",
        after_long_help = "",
        display_name = "fvx",
    ...
    ///
    /// Packages are installed into an isolated virtual environment in the fv tools directory. The
    /// executables are linked the tool executable directory, which is determined according to the
    /// XDG standard and can be retrieved with `fv tool dir --bin`.
    ///
    ...
    /// upgrade a tool beyond the originally provided constraints, use `fv tool install` again.
    ///
    ...
    ///
    /// If the tool executable directory is not present on the `PATH`, fv will attempt to add it to
    ...
    /// the path, but the directory is not present on the `PATH`, fv will exit with an error.
    ///
    /// The tool executable directory is determined according to the XDG standard and can be
    /// retrieved with `fv tool dir --bin`.
    #[command(alias = "ensurepath")]
    UpdateShell,
    /// Show the path to the fv tools directory.
    ///
    /// The tools directory is used to store environments and metadata for installed tools.
    ///
    /// By default, tools are stored in the fv data directory at `$XDG_DATA_HOME/fv/tools` or
    /// `$HOME/.local/share/fv/tools` on Unix and `%APPDATA%\uv\data\tools` on Windows.
    ///
    /// The tool installation directory may be overridden with `$UV_TOOL_DIR`.
    ///
    /// To instead view the directory fv installs executables into, use the `--bin` flag.
    Dir(ToolDirArgs),
}
...
pub struct ToolRunArgs {
    ...
    /// Run with the given packages installed in editable mode
    ///
    /// When used in a project, these dependencies will be layered on top of the fv tool's
...
pub struct ToolInstallArgs {
    ...
    /// The package to install commands from.
    ///
    /// This option is provided for parity with `fv tool run`, but is redundant with `package`.
...
pub struct ToolDirArgs {
    /// Show the directory into which `fv tool` will install executables.
    ///
    /// By default, `fv tool dir` shows the directory into which the tool Python environments
...
pub struct ToolUpgradeArgs {
    ...
    /// package requirement.
    ///
    /// By default, fv will use the latest compatible version of each package (`highest`).
    #[arg(
        long,
        value_enum,
    ...
    ///
    /// By default, fv will accept pre-releases for packages that _only_ publish pre-releases, along
    ...
    ///
    /// By default, fv will optimize for selecting the latest version of each package for each
    /// supported Python version (`requires-python`), while minimizing the number of selected
    /// versions across platforms.
    ///
    /// Under `fewest`, fv will minimize the number of selected versions for each package,
    ...
    ///
    /// By default, fv does not compile Python (`.py`) files to bytecode (`__pycache__/*.pyc`);
    /// instead, compilation is performed lazily the first time a module is imported. For use-cases
    /// in which start time is critical, such as CLI applications and Docker containers, this option
    /// can be enabled to trade longer installation times for faster start times.
    ///
    /// When enabled, fv will process the entire site-packages directory (including packages that
    ...
    /// Ignore the `tool.fv.sources` table when resolving dependencies. Used to lock against the
    ...
    /// Don't use sources from the `tool.fv.sources` table for the specified packages.
...
pub enum PythonCommand {
    ...
    /// available Python versions are bundled with each fv release. To install new Python versions,
    /// you may need upgrade uv.
    ///
    /// Python versions are installed into the fv Python directory, which can be retrieved with `uv
    /// python dir`.
    ///
    ...
    /// `fv python dir --bin` to see the target directory.
    ///
    ...
    ///
    /// If no target version is provided, then fv will upgrade all managed CPython versions.
    ///
    /// During an upgrade, fv will not uninstall outdated patch versions.
    ///
    /// When an upgrade is performed, virtual environments created by fv will automatically
    ...
    ///
    /// See `fv help python` to view supported request formats and details on discovery behavior.
    Find(PythonFindArgs),

    /// Pin to a specific Python version.
    ///
    /// Writes the pinned Python version to a `.python-version` file, which is used by other fv
    /// commands to determine the required Python version.
    ///
    /// If no version is provided, fv will look for an existing `.python-version` file and display
    /// the currently pinned version. If no `.python-version` file is found, fv will exit with an
    /// error.
    ///
    /// See `fv help python` to view supported request formats.
    Pin(PythonPinArgs),

    /// Show the fv Python installation directory.
    ///
    /// By default, Python installations are stored in the fv data directory at
    /// `$XDG_DATA_HOME/fv/python` or `$HOME/.local/share/fv/python` on Unix and
    /// `%APPDATA%\uv\data\python` on Windows.
    ///
    /// The Python installation directory may be overridden with `$UV_PYTHON_INSTALL_DIR`.
    ///
    /// To view the directory where fv installs Python executables instead, use the `--bin` flag.
    ...
    ///
    /// If the Python executable directory is not present on the `PATH`, fv will attempt to add it to
    /// the relevant shell configuration files.
    ///
    /// If the shell configuration files already include a blurb to add the executable directory to
    /// the path, but the directory is not present on the `PATH`, fv will exit with an error.
    ...
    /// retrieved with `fv python dir --bin`.
    #[command(alias = "ensurepath")]
    UpdateShell,
}
...
pub struct PythonDirArgs {
    /// Show the directory into which `fv python` will install Python executables.
    ///
...
pub struct PythonInstallCompileBytecodeArgs {
    ...
    /// faster start times.
    ///
    /// When enabled, fv will process the Python version's `stdlib` directory. It will ignore any
    /// compilation errors.
    #[arg(
        long,
        alias = "compile",
...
pub struct PythonInstallArgs {
    ...
    ///
    /// If provided, `UV_PYTHON_INSTALL_DIR` will need to be set for subsequent operations for fv to
    /// discover the Python installation.
    ///
    /// See `fv python dir` to view the current Python installation directory. Defaults to
    /// `~/.local/share/fv/python`.
    ...
    ///
    /// This is the default behavior. If this flag is provided explicitly, fv will error if the
    ...
    ///
    /// This is the default behavior on Windows. If this flag is provided explicitly, fv will error if the
    ...
    /// above are present, fv will check if it has installed any Python versions. If not, it will
    ...
    ///
    /// By default, fv will exit successfully if the version is already
    /// installed.
    #[arg(long, short)]
    pub reinstall: bool,
    ...
    ///
    /// By default, fv will refuse to replace executables that it does not manage.
    ///
    /// Implies `--reinstall`.
    #[arg(long, short)]
    pub force: bool,
    ...
    ///
    /// By default, fv will not upgrade already-installed Python versions to newer patch releases.
    /// With `--upgrade`, fv will upgrade to the latest available patch version for the specified
    /// minor version(s).
    ///
    /// If the requested versions are not yet installed, fv will install them.
    ///
    /// This option is only supported for minor version requests, e.g., `3.12`; fv will exit with an
    ...
    /// `python3` and `python`.
    ///
    /// If multiple Python versions are requested, fv will exit with an error.
...
pub struct PythonUpgradeArgs {
    ...
    ///
    /// By default, fv will exit successfully if the latest patch is already
    /// installed.
    #[arg(long, short)]
    pub reinstall: bool,
...
pub struct PythonFindArgs {
    ...
    ///
    /// By default, fv will report the first Python interpreter it would use, including those in an
    /// active virtual environment or a virtual environment in the current working directory or any
    /// parent directory.
    ///
    /// The `--system` option instructs fv to skip virtual environment Python interpreters and
...
pub struct PythonPinArgs {
    ...
    ///
    /// fv supports more formats than other tools that read `.python-version` files, i.e., `pyenv`.
    ...
    ///
    /// Writes the pinned Python version to a `.python-version` file in the fv user configuration
...
pub struct AuthLogoutArgs {
    ...
    /// via an integration built into fv.
    #[arg(
        long,
        value_enum,
        env = EnvVars::UV_KEYRING_PROVIDER,
...
/// Credential helper protocols supported by fv
...
pub struct PublishArgs {
    ...
    ///
    /// ```shell
    /// fv publish --index pypi
    /// fv publish --publish-url https://upload.pypi.org/legacy/ --check-url https://pypi.org/simple
    /// ```
    #[arg(
        long,
        verbatim_doc_comment,
    ...
    ///
    /// By default, fv checks for trusted publishing when running in a supported environment, but
    ...
    ///
    /// By default, fv attempts to upload matching PEP 740 attestations with each distribution
    /// that is published.
...
pub enum WorkspaceCommand {
    ...
    ///
    /// If used outside of a workspace, i.e., if a `pyproject.toml` cannot be found, fv will exit with an error.
