...
use fv_auth::RealmRef;
use fv_cache_key::CanonicalUrl;
use fv_pep508::{Scheme, VerbatimUrl, VerbatimUrlError, split_scheme};
...
/// along with the fv-specific `--index` and `--default-index`.
...
impl From<&IndexLocations> for fv_auth::Indexes {
...
/// fv-specific `--index` and `--default-index`.
...
impl<'a> IndexUrls {
    ...
    ///
    /// Prioritizes the `[tool.fv.index]` definitions over the `--extra-index-url` definitions
    ...
    fn test_cache_control_lookup() {
        ...
        let indexes = vec![
            Index {
                ...
                publish_url: None,
                authenticate: fv_auth::AuthPolicy::default(),
