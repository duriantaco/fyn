...
use fv_auth::{AuthPolicy, Credentials};
...
pub struct Index {
    ...
    ///
    /// Explicit indexes will _only_ be used when explicitly requested via a `[tool.fv.sources]`
    /// definition, as in:
    ///
    /// ```toml
    /// [[tool.fv.index]]
    ...
    ///
    /// By default, fv uses PyPI as the default index, such that even if additional indexes are
    /// defined via `[[tool.fv.index]]`, PyPI will still be used as a fallback for packages that
    ...
    ///
    /// When using `fv publish --index <name>`, this URL is used for publishing.
    ///
    ...
    /// When fv should use authentication for requests to the index.
    ///
    /// ```toml
    /// [[tool.fv.index]]
    /// name = "my-index"
    ...
    /// Status codes that fv should ignore when deciding whether
...
impl Index {
    ...
    /// (e.g., `fv tool upgrade`) know that authentication is required even after the credentials
