...
use fv_distribution_filename::{EggInfoFilename, ExpandedTags};
use fv_fs::Simplified;
use fv_install_wheel::WheelFile;
use fv_normalize::PackageName;
use fv_pep440::Version;
use fv_pypi_types::{DirectUrl, MetadataError};
...
pub enum InstalledDistError {
    ...
    #[error(transparent)]
    EggInfoParse(#[from] fv_distribution_filename::EggInfoFilenameError),

    #[error(transparent)]
    VersionParse(#[from] fv_pep440::VersionParseError),

    #[error(transparent)]
    PackageNameParse(#[from] fv_normalize::InvalidNameError),

    #[error(transparent)]
    WheelFileParse(#[from] fv_install_wheel::Error),

    #[error(transparent)]
    ExpandedTagParse(#[from] fv_distribution_filename::ExpandedTagError),
...
pub struct InstalledDist {
    ...
    metadata_cache: OnceLock<fv_pypi_types::ResolutionMetadata>,
...
impl InstalledDist {
    ...
    /// Read the `fv_cache.json` file from a `.dist-info` directory.
    pub fn read_cache_info(path: &Path) -> Result<Option<CacheInfo>, InstalledDistError> {
        let path = path.join("fv_cache.json");
        let file = match fs_err::File::open(&path) {
            Ok(file) => file,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(None),
            Err(err) => return Err(err.into()),
    ...
    /// Read the `fv_build.json` file from a `.dist-info` directory.
    pub fn read_build_info(path: &Path) -> Result<Option<BuildInfo>, InstalledDistError> {
        let path = path.join("fv_build.json");
        let file = match fs_err::File::open(&path) {
            Ok(file) => file,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(None),
            Err(err) => return Err(err.into()),
    ...
    pub fn read_metadata(&self) -> Result<&fv_pypi_types::ResolutionMetadata, InstalledDistError> {
        ...
        let metadata = match &self.kind {
            InstalledDistKind::Registry(_) | InstalledDistKind::Url(_) => {
                ...
                fv_pypi_types::ResolutionMetadata::parse_metadata(&contents).map_err(|err| {
...
fn read_metadata(path: &Path) -> Option<fv_pypi_types::Metadata10> {
    ...
    let metadata = match fv_pypi_types::Metadata10::parse_pkg_info(&content) {
        Ok(metadata) => metadata,
        Err(err) => {
