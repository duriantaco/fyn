...
pub trait IsBuildBackendError: std::error::Error + Send + Sync + 'static {
    /// Returns whether the build backend failed to build the package, so it's not a fv error.
...
impl DerivationChain {
    ...
    ///
    /// This is used to construct a derivation chain upon install failure in the `fv pip` context,
