...
use fv_distribution_types::{CompatibleDist, IncompatibleDist, IncompatibleSource, IndexUrl};
use fv_distribution_types::{DistributionMetadata, IncompatibleWheel, Name, PrioritizedDist};
...
pub(crate) enum VersionChoiceKind {
    /// A preference from an output file such as `-o requirements.txt` or `fv.lock`.
    Preference,
...
impl DistributionMetadata for Candidate<'_> {
    fn version_or_url(&self) -> fv_distribution_types::VersionOrUrlRef<'_> {
        fv_distribution_types::VersionOrUrlRef::Version(self.version)
    }
}
