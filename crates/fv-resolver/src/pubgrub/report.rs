...
use fv_configuration::{IndexStrategy, NoBinary, NoBuild};
...
use fv_pep508::{MarkerEnvironment, MarkerExpression, MarkerTree, MarkerValueVersion};
use fv_platform_tags::{AbiTag, IncompatibleTag, LanguageTag, PlatformTag, Tags};
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::BuildPrereleaseAvailable { package, version } => {
                ...
                write!(
                    f,
                    "{}{} Only pre-releases of `{}` (e.g., {}) match these build requirements, and build environments can't enable pre-releases automatically. Add `{}` to `build-system.requires`, `[tool.fv.extra-build-dependencies]`, or supply it via `fv build --build-constraint`.",
                    "hint".bold().cyan(),
                    ":".bold(),
                    package.cyan(),
                    version.cyan(),
            ...
            Self::BuildPrereleaseRequested { name, range } => {
                write!(
                    f,
                    "{}{} `{}` was requested with a pre-release marker (e.g., {}), but build environments can't opt into pre-releases automatically.  Add `{}` to `build-system.requires`, `[tool.fv.extra-build-dependencies]`, or supply it via `fv build --build-constraint`.",
                    "hint".bold().cyan(),
                    ":".bold(),
                    name.cyan(),
                    PackageRange::compatibility(&PubGrubPackage::base(name), range, None).cyan(),
            ...
            Self::DisjointEnvironment => {
                write!(
                    ...
                    consider limiting the environments with `tool.fv.environments`.",
                    "hint".bold().cyan(),
                    ":".bold(),
                )
            }
