...
use fv_distribution_types::{IndexMetadata, Requirement, RequirementSource};
...
impl DependencySource {
    ...
    ///
    /// Registry requirements carry an explicit index when they have one assigned
    /// via `[tool.fv.sources]`, regardless of whether they're direct or transitive.
    /// Direct URL-like requirements always preserve their verbatim URL.
    pub(crate) fn from_requirement(requirement: &Requirement) -> Self {
        match &requirement.source {
            RequirementSource::Registry {
                index: Some(index), ..
            } => Self::ExplicitIndex(index.clone()),
