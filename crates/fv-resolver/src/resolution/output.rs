...
use fv_configuration::{Constraints, Overrides};
use fv_distribution::Metadata;
...
use fv_pep440::{Version, VersionSpecifier};
use fv_pep508::{MarkerEnvironment, MarkerTree, MarkerTreeKind};
use fv_pypi_types::{Conflicts, HashDigests, ParsedUrlError, VerbatimParsedUrl, Yanked};
...
impl ResolverOutput {
    ...
    pub(crate) fn from_state(
        ...
        // the same time. At which point, fv will report an error,
        ...
        if conflicts.is_empty() {
            ...
            // Returning an error in that case would make `fv` unusable whenever
            // the bug occurs, but letting it through means `fv` *could* still be
            // usable.
...
/// Convert a [`ResolverOutput`] into a [`fv_distribution_types::Resolution`].
///
...
impl From<ResolverOutput> for fv_distribution_types::Resolution {
