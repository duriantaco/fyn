...
impl<'a> ComponentBuilder<'a> {
    ...
    fn create_synthetic_root_component(&mut self, root: Option<&Package>) -> Component {
        let name = root.map(Self::get_package_name).unwrap_or("fv-workspace");
        let bom_ref = self.create_bom_ref(name, None);
        let properties = Properties(vec![Property::new("fv:package:is_synthetic_root", "true")]);
    ...
    fn create_component_from_package(
        ...
        match package_type {
            ...
            PackageType::Root => {
                properties.push(Property::new("fv:package:is_project_root", "true"));
            }
...
pub fn from_lock<'lock>(
    ...
    if !preview.is_enabled(PreviewFeature::SbomExport) {
        warn_user!(
            "`fv export --format=cyclonedx1.5` is experimental and may change without warning. Pass `--preview-features {}` to disable this warning.",
    ...
    let mut metadata = Metadata {
        ...
        tools: Some(Tools::List(vec![Tool {
            ...
            name: Some(NormalizedString::new("fv")),
            version: Some(NormalizedString::new(fv_version::version())),
            hashes: None,
            external_references: None,
        }])),
        ..Metadata::default()
