...
use fv_distribution_types::{RequiresPython, RequiresPythonRange};
use fv_pep440::VersionSpecifiers;
use fv_pep508::{MarkerEnvironment, MarkerTree};
use fv_pypi_types::{
...
/// a specific marker environment for `fv pip`-style resolutions and ask for a
/// universal resolution for fv's project based commands like `fv lock`.
///
...
enum Kind {
    ...
    ///
    /// Generally, this is what's done for `fv pip`. For the project based
    /// commands, like `fv lock`, we do universal resolution.
    Specific {
...
impl ResolverEnvironment {
    ...
    /// environment.
    ///
    /// This enables `fv pip`-style resolutions. That is, the resolution
...
mod tests {
    ...
    use fv_pep440::{LowerBound, UpperBound, Version};
    ...
    /// copied from our fv microbenchmarks.
