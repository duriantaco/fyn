use fv_client::MetadataFormat;
use fv_configuration::BuildOptions;
use fv_distribution::{ArchiveMetadata, DistributionDatabase, Reporter};
...
pub type PackageVersionsResult = Result<VersionsResponse, fv_client::Error>;
pub type WheelMetadataResult = Result<MetadataResponse, fv_distribution::Error>;
...
pub enum MetadataResponse {
    ...
    Error(Box<RequestedDist>, Arc<fv_distribution::Error>),
}
...
pub enum MetadataUnavailable {
    ...
    InvalidMetadata(Arc<fv_pypi_types::MetadataError>),
    /// The wheel metadata was found, but the metadata was inconsistent.
    InconsistentMetadata(Arc<fv_distribution::Error>),
    /// The wheel has an invalid structure.
    InvalidStructure(Arc<fv_metadata::Error>),
...
impl<Context: BuildContext> ResolverProvider for DefaultResolverProvider<'_, Context> {
    ...
    async fn get_package_versions<'io>(
        ...
        match result {
            ...
            Err(err) => match err.kind() {
                fv_client::ErrorKind::RemotePackageNotFound(_) => {
    ...
    async fn get_or_build_wheel_metadata<'io>(&'io self, dist: &'io Dist) -> WheelMetadataResult {
            ...
            .await
        {
            ...
            Err(err) => match err {
                fv_distribution::Error::Client(client) => {
                    ...
                    match client.into_kind() {
                        ...
                        fv_client::ErrorKind::MetadataParseError(_, _, err) => {
                            Ok(MetadataResponse::Unavailable(
                                MetadataUnavailable::InvalidMetadata(Arc::new(*err)),
                            ))
                        }
                        fv_client::ErrorKind::Metadata(_, err) => {
                            Ok(MetadataResponse::Unavailable(
                                MetadataUnavailable::InvalidStructure(Arc::new(err)),
                            ))
                        }
                        kind => Err(fv_client::Error::new(kind, retries, duration).into()),
                    }
                }
                fv_distribution::Error::WheelMetadataVersionMismatch { .. } => {
                    Ok(MetadataResponse::Unavailable(
                        MetadataUnavailable::InconsistentMetadata(Arc::new(err)),
                    ))
                }
                fv_distribution::Error::WheelMetadataNameMismatch { .. } => {
                    Ok(MetadataResponse::Unavailable(
                        MetadataUnavailable::InconsistentMetadata(Arc::new(err)),
                    ))
                }
                fv_distribution::Error::Metadata(err) => Ok(MetadataResponse::Unavailable(
                    MetadataUnavailable::InvalidMetadata(Arc::new(err)),
                )),
                fv_distribution::Error::WheelMetadata(_, err) => Ok(MetadataResponse::Unavailable(
                    MetadataUnavailable::InvalidStructure(Arc::new(*err)),
                )),
                fv_distribution::Error::RequiresPython(requires_python, version) => {
                    Ok(MetadataResponse::Unavailable(
