...
use fv_configuration::{Constraints, Excludes, Overrides};
use fv_distribution::{ArchiveMetadata, DistributionDatabase};
...
use fv_pep440::{MIN_VERSION, Version, VersionSpecifiers, release_specifiers_to_ranges};
use fv_pep508::{
    MarkerEnvironment, MarkerExpression, MarkerOperator, MarkerTree, MarkerValueString,
};
use fv_platform_tags::{IncompatibleTag, Tags};
use fv_pypi_types::{ConflictItem, ConflictItemRef, ConflictKindRef, Conflicts, VerbatimParsedUrl};
use fv_torch::TorchStrategy;
use fv_types::{BuildContext, HashStrategy, InstalledPackagesProvider};
    ...
    fn new(
        ...
        for (name, mut deps) in name_to_deps {
            ...
            if let [dep] = deps.as_slice() {
                ...
                // resolution.
                //
                // For example, given `requires-python = ">=3.7"` and `fv ; python_version >= "3.8"`,
                // where fv itself only supports Python 3.8 and later, we need to fork to ensure
