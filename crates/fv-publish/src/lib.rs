...
use fv_auth::{Credentials, PyxTokenStore, Realm};
...
use fv_configuration::{KeyringProviderType, TrustedPublishing};
use fv_distribution_filename::{DistFilename, SourceDistExtension, SourceDistFilename};
use fv_distribution_types::{IndexCapabilities, IndexUrl};
use fv_extract::hash::{HashReader, Hasher};
use fv_fs::{ProgressReader, Simplified};
use fv_metadata::read_metadata_async_seek;
use fv_pypi_types::{HashAlgorithm, HashDigest, Metadata23, MetadataError};
...
pub enum PublishError {
    ...
    CheckUrlIndex(#[source] fv_client::Error),
    #[error(
...
pub enum PublishPrepareError {
    ...
    Metadata(#[from] fv_metadata::Error),
    #[error("Failed to read metadata")]
...
// Not in `fv-metadata` because we only support tar files here.
...
async fn handle_response(
    ...
    if content_type.as_deref() == Some(fv_client::ProblemDetails::CONTENT_TYPE)
        && let Some(problem) =
            fv_client::ProblemDetails::try_from_response_body(upload_error.as_bytes())
...
mod tests {
    ...
    use fv_auth::Credentials;
    use fv_client::{AuthIntegration, BaseClientBuilder, RedirectPolicy};
    ...
    use fv_warnings::owo_colors::AnsiColors;
    use fv_warnings::write_error_chain;
    use tokio::sync::Semaphore;
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(
                ResponseTemplate::new(400)
                    .insert_header(
                        "content-type",
                        fv_client::ProblemDetails::CONTENT_TYPE,
                    )
                    .set_body_raw(
