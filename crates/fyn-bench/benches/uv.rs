...
fn resolve_warm_airflow(c: &mut Criterion<WallTime>) {
    let manifest = Manifest::simple(vec![
        Requirement::from(fyn_pep508::Requirement::from_str("apache-airflow[all]==2.9.3").unwrap()),
        Requirement::from(
            fyn_pep508::Requirement::from_str("apache-airflow-providers-apache-beam>3.0.0")
                .unwrap(),
        ),
    ]);
    let run = setup(manifest, false);
...
//     let manifest = Manifest::simple(vec![
//         Requirement::from(fyn_pep508::Requirement::from_str("apache-airflow[all]").unwrap()),
//         Requirement::from(
//             fyn_pep508::Requirement::from_str("apache-airflow-providers-apache-beam>3.0.0").unwrap(),
//         ),
//     ]);
//     let run = setup(manifest, true);
...
mod resolver {
    ...
    use fyn_configuration::{BuildOptions, Concurrency, Constraints, IndexStrategy, NoSources};
