...
use fv_windows::{Job, install_ctrl_handler};
...
fn make_child_cmdline() -> CString {
    let executable_name = std::env::current_exe().unwrap_or_else(|_| {
        error_and_exit("fv trampoline failed to determine executable path");
    });
    ...
    let trampoline_kind = load_resource(RESOURCE_TRAMPOLINE_KIND)
        ...
        .unwrap_or_else(|| {
            error_and_exit("fv trampoline failed to load trampoline kind from resources")
        });
    ...
    let python_path = load_resource(RESOURCE_PYTHON_PATH)
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            error_and_exit("fv trampoline failed to load Python path from resources")
        });
    ...
    let python_exe = if python_path.is_absolute() {
        ...
        let parent_dir = match executable_name.parent() {
            Some(parent) => parent,
            None => {
                error_and_exit("fv trampoline executable path has no parent directory");
            }
        };
    ...
    let python_exe =
        if !python_exe.is_absolute() || matches!(trampoline_kind, TrampolineKind::Script) {
            ...
            dunce::canonicalize(python_exe.as_path()).unwrap_or_else(|_| {
                error_and_exit("fv trampoline failed to canonicalize script path");
            })
        } else {
    ...
    match trampoline_kind {
        TrampolineKind::Python => {
            ...
            unsafe {
                ...
                // whether `PYTHONHOME` was set by fv. This allows us to:
                ...
                if !is_virtualenv(python_exe.as_path()) {
                    ...
                    // - It's not set, OR
                    // - It was set by fv (marker matches current `PYTHONHOME`)
    ...
    CString::from_vec_with_nul(child_cmdline).unwrap_or_else(|_| {
        error_and_exit("fv trampoline child command line is not correctly null terminated");
    })
}
...
#[cold]
fn print_job_error_and_exit(message: &str, err: fv_windows::JobError) -> ! {
    error!(
        "{}\n  Caused by: {} (os error {})",
        message,
        err.message(),
...
#[cold]
fn print_ctrl_handler_error_and_exit(err: fv_windows::CtrlHandlerError) -> ! {
    error!(
        "fv trampoline failed to set control handler\n  Caused by: os error {}",
        err.code()
    );
    exit_with_status(1);
}
...
fn spawn_child(si: &STARTUPINFOA, child_cmdline: CString) -> HANDLE {
    ...
    .unwrap_or_else(|_| {
        print_last_error_and_exit("fv trampoline failed to spawn Python child process");
    });
    unsafe { CloseHandle(child_process_info.hThread) }.unwrap_or_else(|_| {
        print_last_error_and_exit(
            "fv trampoline failed to close Python child process thread handle",
        );
    });
    ...
    unsafe {
        ...
        if let Ok(hwnd) = CreateWindowExA(
            ...
            DestroyWindow(hwnd).unwrap_or_else(|_| {
                print_last_error_and_exit("fv trampoline failed to destroy temporary window");
            });
        }
    }
}
...
pub fn bounce(is_gui: bool) -> ! {
    ...
    let job = Job::new().unwrap_or_else(|e| {
        print_job_error_and_exit("fv trampoline failed to create job object", e);
    });
    ...
    if let Err(e) = unsafe { job.assign_process(child_handle) } {
        warn!(
            "fv trampoline failed to assign child process to job object\n  Caused by: {} (os error {})",
            e.message(),
            e.code(),
        );
    }
    ...
    if unsafe { GetExitCodeProcess(child_handle, &mut exit_code) }.is_err() {
        print_last_error_and_exit("fv trampoline failed to get exit code of child process");
    }
