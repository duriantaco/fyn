...
fn resolve_warm_airflow(c: &mut Criterion<WallTime>) {
    let manifest = Manifest::simple(vec![
        Requirement::from(fv_pep508::Requirement::from_str("apache-airflow[all]==2.9.3").unwrap()),
        Requirement::from(
            fv_pep508::Requirement::from_str("apache-airflow-providers-apache-beam>3.0.0").unwrap(),
        ),
    ]);
    let run = setup(manifest, false);
...
//     let manifest = Manifest::simple(vec![
//         Requirement::from(fv_pep508::Requirement::from_str("apache-airflow[all]").unwrap()),
//         Requirement::from(
//             fv_pep508::Requirement::from_str("apache-airflow-providers-apache-beam>3.0.0").unwrap(),
//         ),
//     ]);
...
mod resolver {
    ...
    use fv_configuration::{BuildOptions, Concurrency, Constraints, IndexStrategy, NoSources};
    ...
    use fv_pep508::{MarkerEnvironment, MarkerEnvironmentBuilder};
    use fv_platform_tags::{Arch, Os, Platform, Tags};
    use fv_preview::Preview;
    use fv_pypi_types::{Conflicts, ResolverMarkerEnvironment};
    use fv_python::Interpreter;
    use fv_resolver::{
