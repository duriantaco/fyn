...
impl Cache {
    ...
    /// 4. A `.fyn_cache` directory in the current working directory.
    ///
    ...
    pub fn from_settings(no_cache: bool, cache_dir: Option<PathBuf>) -> Result<Self, io::Error> {
        ...
        } else if let Some(cache_dir) = fyn_dirs::legacy_user_cache_dir().filter(|dir| dir.exists())
        {
            // If the user has an existing directory at (e.g.) `/Users/user/Library/Caches/fyn`,
            // respect it for backwards compatibility. Otherwise, prefer the XDG strategy, even on
            // macOS.
            Ok(Self::from_path(cache_dir))
        } else if let Some(cache_dir) = fyn_dirs::user_cache_dir() {
            if cfg!(windows) {
                // On Windows, we append `cache` to the LocalAppData directory, i.e., prefer
                // `C:\Users\User\AppData/Local/fyn\cache` over `C:\Users\User\AppData/Local/fyn`.
                //
...
/// Migrate the Windows cache from `C:\Users\User\AppData/Local/fyn` to `C:\Users\User\AppData/Local/fyn\cache`.
