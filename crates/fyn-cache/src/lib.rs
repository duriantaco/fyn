...
use fyn_fs::{LockedFile, LockedFileError, LockedFileMode, Simplified, cachedir, directories};
use fyn_normalize::PackageName;
use fyn_pypi_types::ResolutionMetadata;
...
pub struct Cache {
    ...
    /// Ensure that `fyn cache` operations don't remove items from the cache that are used by another
    /// fyn process.
...
impl Cache {
    ...
    pub async fn persist(
        ...
        fyn_fs::rename_with_retry(temp_dir.as_ref(), archive_entry.path()).await?;
                    ...
                    .is_some_and(|err| err.kind() == io::ErrorKind::Unsupported) =>
            {
                warn!(
                    "Shared locking is not supported by the current platform or filesystem, \
                        reduced parallel process safety with `fyn cache clean` and `fyn cache prune`."
                );
                None
            }
    ...
    /// Initialize the [`Cache`], assuming that there are no other fyn processes running.
    ...
    /// Parse a size string like "2G", "500M", "1024K" into bytes.
    ...
    /// Enforce a maximum cache size by removing oldest entries.
    ///
    /// Returns the amount of space freed, if any pruning was needed.
    pub fn enforce_max_size(&self, max_bytes: u64) -> Result<Option<Removal>, io::Error> {
        use walkdir::WalkDir;

        // Collect all files with their modification times and sizes.
        ...
        for entry in WalkDir::new(&self.root).into_iter().filter_map(Result::ok) {
            if entry.file_type().is_file() {
                if let Ok(metadata) = entry.metadata() {
                    ...
                    total_size += size;
                    entries.push((modified, size, entry.into_path()));
                }
            }
        }
        ...
        for (_, size, path) in &entries {
            ...
            if let Err(err) = fs_err::remove_file(path) {
                debug!("Failed to remove cache entry: {err}");
                continue;
            }
            freed += size;
            summary.num_files += 1;
            ...
            .filter_map(Result::ok)
        {
            if entry.file_type().is_dir() {
                if let Ok(mut read_dir) = fs_err::read_dir(entry.path()) {
                    if read_dir.next().is_none() {
                        let _ = fs_err::remove_dir(entry.path());
                        summary.num_dirs += 1;
                    }
                }
            }
        ...
        debug!(
            "Cache size limit enforced: removed {} files, freed {} bytes",
            summary.num_files, summary.total_bytes
        );
...
pub enum CacheBucket {
    ...
    ///
    /// See `fyn_client::RegistryClient::wheel_metadata` for information on how wheel metadata
    /// is fetched.
    ///
    /// # Example
    ///
    ...
    ///     "sys_executable": "/home/ferris/projects/fyn/.venv/bin/python"
    ///   }
    /// }
    /// ```
    ///
    ...
    ///
    /// The response is parsed into `fyn_client::SimpleDetailMetadata` before storage.
    Simple,
