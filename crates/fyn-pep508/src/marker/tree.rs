...
use fyn_normalize::{ExtraName, GroupName};
use fyn_pep440::{Version, VersionParseError, VersionSpecifier};
...
impl MarkerOperator {
    ...
    pub(crate) fn to_pep440_operator(self) -> Option<fyn_pep440::Operator> {
        match self {
            Self::Equal => Some(fyn_pep440::Operator::Equal),
            Self::NotEqual => Some(fyn_pep440::Operator::NotEqual),
            Self::GreaterThan => Some(fyn_pep440::Operator::GreaterThan),
            Self::GreaterEqual => Some(fyn_pep440::Operator::GreaterThanEqual),
            Self::LessThan => Some(fyn_pep440::Operator::LessThan),
            Self::LessEqual => Some(fyn_pep440::Operator::LessThanEqual),
            Self::TildeEqual => Some(fyn_pep440::Operator::TildeEqual),
            _ => None,
        }
    }
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::Version { key, specifier } => {
                ...
                if op == &fyn_pep440::Operator::EqualStar
                    || op == &fyn_pep440::Operator::NotEqualStar
                {
    ...
    fn warnings4() {
        ...
        logs_assert(|lines: &[&str]| {
            let lines: Vec<_> = lines
                ...
                .collect();
            let expected = [
                "WARN warnings4: fyn_pep508: os.name is deprecated in favor of os_name",
                "WARN warnings4: fyn_pep508: platform.machine is deprecated in favor of platform_machine",
                "WARN warnings4: fyn_pep508: platform.python_implementation is deprecated in favor of platform_python_implementation",
                "WARN warnings4: fyn_pep508: platform.version is deprecated in favor of platform_version",
                "WARN warnings4: fyn_pep508: sys.platform is deprecated in favor of sys_platform",
                "WARN warnings4: fyn_pep508: Comparing linux and posix lexicographically",
            ];
            if lines == expected {
                Ok(())
            } else {
