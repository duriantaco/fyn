...
use fv_configuration::BuildOutput;
use fv_distribution_types::IsBuildBackendError;
...
pub enum Error {
    ...
    Io(#[from] io::Error),
    #[error(transparent)]
    Lowering(#[from] fv_distribution::MetadataError),
...
fn hint_build_dependency(
    ...
    write!(
        ...
        table_key.cyan(),
        package.cyan(),
        format!("fv pip install {package}").green(),
...
mod test {
    ...
    fn missing_wheel_package() {
        ...
        or `fv pip install wheel` into the environment and re-run with `--no-build-isolation`.
        "#);
    }
