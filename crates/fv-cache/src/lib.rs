...
use fv_fs::{LockedFile, LockedFileError, LockedFileMode, Simplified, cachedir, directories};
use fv_normalize::PackageName;
use fv_pypi_types::ResolutionMetadata;
...
pub struct Cache {
    ...
    /// Ensure that `fv cache` operations don't remove items from the cache that are used by another
    /// fv process.
...
impl Cache {
    ...
    pub async fn persist(
        ...
        fv_fs::rename_with_retry(temp_dir.as_ref(), archive_entry.path()).await?;
                    ...
                    .is_some_and(|err| err.kind() == io::ErrorKind::Unsupported) =>
            {
                warn!(
                    "Shared locking is not supported by the current platform or filesystem, \
                        reduced parallel process safety with `fv cache clean` and `fv cache prune`."
                );
                None
            }
    ...
    /// Initialize the [`Cache`], assuming that there are no other fv processes running.
...
pub enum CacheBucket {
    ...
    ///
    /// See `fv_client::RegistryClient::wheel_metadata` for information on how wheel metadata
    /// is fetched.
    ///
    /// # Example
    ///
    ...
    ///     "sys_executable": "/home/ferris/projects/fv/.venv/bin/python"
    ///   }
    /// }
    /// ```
    ///
    ...
    ///
    /// The response is parsed into `fv_client::SimpleDetailMetadata` before storage.
    Simple,
