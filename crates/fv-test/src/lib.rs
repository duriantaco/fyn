...
use fv_python::downloads::ManagedPythonDownloadList;
...
use fv_python::managed::ManagedPythonInstallations;
use fv_python::{
...
///
/// This macro captures the fv binary path at compile time using `env!("CARGO_BIN_EXE_fv")`,
...
macro_rules! test_context {
    ($python_version:expr) => {
        $crate::TestContext::new_with_bin(
            $python_version,
            std::path::PathBuf::from(env!("CARGO_BIN_EXE_fv")),
        )
    };
}
...
/// Return the path to the fv binary.
///
/// This macro captures the fv binary path at compile time using `env!("CARGO_BIN_EXE_fv")`,
/// which is only available in the test crate.
#[macro_export]
...
pub const INSTA_FILTERS: &[(&str, &str)] = &[
    ...
    // fv version display
    (
        r"fv(-.*)? \d+\.\d+\.\d+(-(alpha|beta|rc)\.\d+)?(\+\d+)?( \([^)]*\))?",
        r"fv [VERSION] ([COMMIT] DATE)",
    ),
...
impl TestContext {
    /// Create a new test context with a virtual environment and explicit fv binary path.
    ///
    ...
    pub fn with_pyvenv_cfg_filters(mut self) -> Self {
        let added_filters = [
            ...
            (
                r"fv = \d+\.\d+\.\d+(-(alpha|beta|rc)\.\d+)?(\+\d+)?".to_string(),
                "fv = [FV_VERSION]".to_string(),
            ),
            (
    ...
    /// context, e.g., for use in `fv python list`.
    #[must_use]
...
";
    ...
    pub fn with_cyclonedx_filters(mut self) -> Self {
        ...
        self.filters.push((
            r#""name": "fv",\s*"version": "\d+\.\d+\.\d+(-(alpha|beta|rc)\.\d+)?(\+\d+)?""#
                .to_string(),
            r#""name": "fv",
        "version": "[VERSION]""#
                .to_string(),
    ...
    pub fn with_python_download_cache(mut self) -> Self {
        self.extra_env.push((
            ...
            env::var_os(EnvVars::UV_PYTHON_CACHE_DIR).unwrap_or_else(|| {
                fv_cache::Cache::from_settings(false, None)
                    .unwrap()
    ...
    /// Create a new test context with multiple Python versions and explicit fv binary path.
    ///
    ...
    pub fn new_with_versions_and_bin(python_versions: &[&str], uv_bin: PathBuf) -> Self {
        ...
        // Avoid breaking the tests when bumping the fv version
        filters.push((
            format!(
                r#"requires = \["fv_build>={},<[0-9.]+"\]"#,
                fv_version::version()
            ),
            r#"requires = ["fv_build>=[CURRENT_VERSION],<[NEXT_BREAKING]"]"#.to_string(),
        ));
    ...
    /// Create a fv command for testing.
    ...
    pub fn add_shared_env(&self, command: &mut Command, activate_venv: bool) {
        ...
        command
            ...
            // Disable wrapping of fv output for readability / determinism in snapshots.
            .env(EnvVars::UV_NO_WRAP, "1")
            // While we disable wrapping in fv above, invoked tools may still wrap their output so
            ...
            // other git repos.
            //
            // If one wants to write a test specifically targeting fv within a
    ...
    /// Create a `fv help` command with options shared across scenarios.
    ...
    /// Create a `fv init` command with options shared across scenarios and
    ...
    /// Create a `fv sync` command with options shared across scenarios.
    ...
    /// Create a `fv lock` command with options shared across scenarios.
    ...
    /// Create a `fv workspace metadata` command with options shared across scenarios.
    ...
    /// Create a `fv workspace dir` command with options shared across scenarios.
    ...
    /// Create a `fv workspace list` command with options shared across scenarios.
    ...
    /// Create a `fv export` command with options shared across scenarios.
    ...
    /// Create a `fv format` command with options shared across scenarios.
    ...
    /// Create a `fv build` command with options shared across scenarios.
    ...
    /// Create a `fv publish` command with options shared across scenarios.
    ...
    /// Create a `fv python find` command with options shared across scenarios.
    ...
    /// Create a `fv python list` command with options shared across scenarios.
    ...
    /// Create a `fv python install` command with options shared across scenarios.
    ...
    /// Create a `fv python uninstall` command with options shared across scenarios.
    ...
    /// Create a `fv python upgrade` command with options shared across scenarios.
    ...
    /// Create a `fv python pin` command with options shared across scenarios.
    ...
    /// Create a `fv python dir` command with options shared across scenarios.
    ...
    /// Create a `fv run` command with options shared across scenarios.
    ...
    /// Create a `fv tool run` command with options shared across scenarios.
    ...
    /// Create a `fv upgrade run` command with options shared across scenarios.
    ...
    /// Create a `fv tool install` command with options shared across scenarios.
    ...
    /// Create a `fv tool list` command with options shared across scenarios.
    ...
    /// Create a `fv tool dir` command with options shared across scenarios.
    ...
    /// Create a `fv tool uninstall` command with options shared across scenarios.
    ...
    /// Create a `fv add` command for the given requirements.
    ...
    /// Create a `fv remove` command for the given requirements.
    ...
    /// Create a `fv tree` command with options shared across scenarios.
    ...
    /// Create a `fv cache clean` command.
    ...
    /// Create a `fv cache prune` command.
    ...
    /// Create a `fv cache size` command.
    ...
    /// Create a `fv build_backend` command.
    ///
    ...
    /// Create a `fv auth login` command.
    ...
    /// Create a `fv auth logout` command.
    ...
    /// Create a `fv auth helper --protocol bazel get` command.
    ...
    /// Create a `fv auth token` command.
    ...
    pub fn copy_ecosystem_project(&self, name: &str) {
        ...
        if let Err(err) = fs_err::remove_file(self.temp_dir.join("fv.lock")) {
            assert_eq!(
                err.kind(),
                io::ErrorKind::NotFound,
                "Failed to remove fv.lock: {err}"
            );
        }
    }
    ...
    pub fn diff_lock(&self, change: impl Fn(&Self) -> Command) -> String {
        ...
        let lock_path = ChildPath::new(self.temp_dir.join("fv.lock"));
...
/// Helper method to apply filters to a string. Useful when `!fv_snapshot` cannot be used.
...
pub async fn download_to_disk(url: &str, path: &Path) {
    let trusted_hosts: Vec<_> = env::var(EnvVars::UV_INSECURE_HOST)
        .unwrap_or_default()
        .split(' ')
        .map(|h| fv_configuration::TrustedHost::from_str(h).unwrap())
        .collect();

    let client = fv_client::BaseClientBuilder::default()
...
#[macro_export]
macro_rules! fv_snapshot {
    ($spawnable:expr, @$snapshot:literal) => {{
        fv_snapshot!($crate::INSTA_FILTERS.to_vec(), $spawnable, @$snapshot)
    }};
