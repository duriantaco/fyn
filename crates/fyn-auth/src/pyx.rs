...
impl PyxTokens {
    ...
    fn check_fresh(&self, tolerance_secs: u64) -> Result<jiff::Timestamp, ExpiredTokenReason> {
        let Ok(jwt) = Pyfynwt::decode(self.access_token()) else {
...
struct PyxDirectories {
    ...
    root: PathBuf,
    /// The subdirectory for the token store (e.g., `/Users/ferris/.local/share/fyn/credentials/3859a629b26fda96`).
...
impl PyxDirectories {
    ...
    fn from_api(api: &DisplaySafeUrl) -> Result<Self, io::Error> {
        ...
        let digest = fyn_cache_key::cache_digest(&CanonicalUrl::new(api));
        ...
        // If the user has pyx credentials in their fyn credentials directory, read them for
...
impl PyxTokenStore {
    ...
    pub async fn write(&self, tokens: &PyxTokens) -> Result<(), TokenStoreError> {
        ...
        match tokens {
            ...
            PyxTokens::ApiKey(tokens) => {
                ...
                let digest = fyn_cache_key::cache_digest(&tokens.api_key);
                fs_err::tokio::write(
    ...
    pub async fn read(&self) -> Result<Option<PyxTokens>, TokenStoreError> {
        if let Some(api_key) = read_pyx_api_key() {
            ...
            let digest = fyn_cache_key::cache_digest(&api_key);
