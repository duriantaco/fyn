...
pub enum Error {
    #[error(transparent)]
    Keyring(#[from] fyn_keyring::Error),
...
impl KeyringProvider {
    ...
    async fn store_native(
        ...
        let entry = fyn_keyring::Entry::new(&prefixed_service, username)?;
    ...
    async fn fetch_native(
        ...
        let Ok(entry) = fyn_keyring::Entry::new(&prefixed_service, username) else {
            return None;
        };
        match entry.get_password().await {
            Ok(password) => return Some((username.to_string(), password)),
            Err(fyn_keyring::Error::NoEntry) => {
