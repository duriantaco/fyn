...
pub enum AuthPolicy {
    ...
    /// be attempted first. If the request fails, fyn will search for credentials. If credentials are
    /// found, an authenticated request will be attempted.
    #[default]
    Auto,
    /// Always authenticate.
    ///
    /// If credentials are not provided, fyn will eagerly search for credentials. If credentials
    /// cannot be found, fyn will error instead of attempting an unauthenticated request.
    Always,
    /// Never authenticate.
    ///
    /// If credentials are provided, fyn will error. fyn will not search for credentials.
    Never,
}
...
// TODO(john): We are not using `fyn_distribution_types::Index` directly
