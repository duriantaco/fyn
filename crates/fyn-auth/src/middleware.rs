...
pub struct AuthMiddleware {
    ...
    /// Global authentication cache for a fyn invocation to share credentials across fyn clients.
    ...
    async fn handle(
            ...
            .flatten()
        {
            ...
            Err(Error::Middleware(format_err!(
                "Run `{}` to authenticate fyn with pyx",
                format!("fyn auth login {domain}").green()
            )))
        } else {
            Err(Error::Middleware(format_err!(
