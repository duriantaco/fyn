...
use fyn_distribution_types::{IndexCapabilities, IndexUrl};
use fyn_extract::hash::{HashReader, Hasher};
use fyn_fs::{ProgressReader, Simplified};
use fyn_metadata::read_metadata_async_seek;
use fyn_pypi_types::{HashAlgorithm, HashDigest, Metadata23, MetadataError};
...
pub enum PublishPrepareError {
    ...
    Metadata(#[from] fyn_metadata::Error),
    #[error("Failed to read metadata")]
...
// Not in `fyn-metadata` because we only support tar files here.
...
async fn handle_response(
    ...
    if content_type.as_deref() == Some(fyn_client::ProblemDetails::CONTENT_TYPE)
        && let Some(problem) =
            fyn_client::ProblemDetails::try_from_response_body(upload_error.as_bytes())
...
mod tests {
    ...
    use fyn_client::{AuthIntegration, BaseClientBuilder, RedirectPolicy};
    ...
    use fyn_warnings::owo_colors::AnsiColors;
    use fyn_warnings::write_error_chain;
    use tokio::sync::Semaphore;
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(
                ResponseTemplate::new(400)
                    .insert_header(
                        "content-type",
                        fyn_client::ProblemDetails::CONTENT_TYPE,
                    )
                    .set_body_raw(
