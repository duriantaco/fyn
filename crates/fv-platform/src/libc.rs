...
impl From<&fv_platform_tags::Os> for Libc {
    fn from(value: &fv_platform_tags::Os) -> Self {
        match value {
            fv_platform_tags::Os::Manylinux { .. } => Self::Some(target_lexicon::Environment::Gnu),
            fv_platform_tags::Os::Musllinux { .. } => Self::Some(target_lexicon::Environment::Musl),
            fv_platform_tags::Os::Pyodide { .. } => Self::Some(target_lexicon::Environment::Musl),
            _ => Self::None,
        }
    }
}
