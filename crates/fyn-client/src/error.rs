    ...
    pub fn is_http_range_requests_unsupported(
        ...
        match &*self.kind {
            ...
            ErrorKind::AsyncHttpRangeReader(
                ...
                warn!(
                    "Invalid range request response from server that declares HTTP range request support, falling back to streaming: {url}"
                );
                return true;
            }
...
pub enum ErrorKind {
    ...
    Metadata(String, #[source] fyn_metadata::Error),
    ...
    MetadataParseError(
        WheelFilename,
        String,
        #[source] Box<fyn_pypi_types::MetadataError>,
    ),
...
impl WrappedReqwestError {
    ...
    /// and from fyn's outer retry loop for streaming bodies. Stripping the inner count from the
