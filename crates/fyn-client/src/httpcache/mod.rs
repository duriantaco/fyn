...
request by adding a `max-stale` cache-control directive, but nothing in fyn
...
/// absent from this (among other things that fyn probably doesn't care
...
impl ArchivedCachePolicy {
    ...
    fn freshness_lifetime(&self) -> Duration {
        ...
        if self.response.headers.last_modified_unix_timestamp.is_some() {
            ...
            // lifetimes[1] that lead to fyn caching too aggressively.
            //
