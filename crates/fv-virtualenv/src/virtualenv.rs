...
use fv_fs::{CWD, Simplified, cachedir};
use fv_platform_tags::Os;
use fv_pypi_types::Scheme;
use fv_python::managed::{
    ManagedPythonInstallation, PythonMinorVersionLink, replace_link_to_executable,
};
use fv_python::{Interpreter, VirtualEnvironment};
use fv_shell::escape_posix_for_single_quotes;
use fv_version::version;
...
pub(crate) fn create(
    ...
    // using a fv-managed Python (in which case, we can do better for symlinked executables).
    ...
    match location.metadata() {
        ...
        Ok(metadata) if metadata.is_dir() => {
            let is_virtualenv = fv_fs::is_virtualenv_base(location);
    ...
    #[cfg(unix)]
    {
        fv_fs::replace_symlink(&executable_target, &executable)?;
        fv_fs::replace_symlink(
            "python",
        ...
        if interpreter.markers().implementation_name() == "pypy" {
            ...
            fv_fs::replace_symlink("python", scripts.join("pypy"))?;
        }

        if interpreter.markers().implementation_name() == "graalpy" {
            fv_fs::replace_symlink("python", scripts.join("graalpy"))?;
        }
    }
    ...
    let mut pyvenv_cfg_data: Vec<(String, String)> = vec![
        ...
        ("fv".to_string(), version().to_string()),
        (
...
fn confirm_clear(location: &Path, name: &'static str) -> Result<Option<bool>, io::Error> {
    ...
    if term.is_term() {
        ...
        Ok(Some(fv_console::confirm_with_hint(
...
pub fn remove_virtualenv(location: &Path) -> Result<(), Error> {
    ...
    // fv can still identify it as a Python virtual environment that can be deleted.
...
pub enum RemovalReason {
    ...
    /// The environment can be removed because it is managed by fv, e.g., a project or tool
    /// environment.
    ManagedEnvironment,
}
...
impl std::fmt::Display for RemovalReason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ManagedEnvironment => f.write_str("environment is managed by fv"),
