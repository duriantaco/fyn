    ...
    pub fn with_features(features: &[super::PreviewFeature]) -> FeaturesGuard {
        ...
        match state {
            ...
            PreviewMode::Normal(_) => {
                panic!(
                    "Cannot use `{}::with_features` after `fyn_preview::init` has been called",
                    module_path!()
                );
            }
        }
...
mod tests {
    ...
    fn test_preview_feature_as_str() {
        ...
        assert_eq!(PreviewFeature::Audit.as_str(), "audit");
        assert_eq!(
    ...
    #[test]
    #[should_panic(
        expected = "Additional calls to `fyn_preview::test::with_features` are not allowed while holding a `FeaturesGuard`"
    )]
    ...
    #[test]
    #[should_panic(expected = "fyn_preview::test::with_features")]
