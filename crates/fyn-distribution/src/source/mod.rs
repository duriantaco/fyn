...
use fyn_cache::{Cache, CacheBucket, CacheEntry, CacheShard, Removal, WheelCache};
use fyn_cache_info::CacheInfo;
use fyn_client::{
    CacheControl, CachedClientError, Connectivity, DataWithCachePolicy, RegistryClient,
};
use fyn_configuration::{BuildKind, BuildOutput, NoSources};
use fyn_distribution_filename::{SourceDistExtension, WheelFilename};
...
use fyn_fs::{Simplified, rename_with_retry, write_atomic};
use fyn_git::{GIT_LFS, GitError};
use fyn_git_types::{GitHubRepository, GitOid};
use fyn_metadata::read_archive_metadata;
use fyn_normalize::PackageName;
use fyn_pep440::{Version, release_specifiers_to_ranges};
use fyn_platform_tags::Tags;
use fyn_pypi_types::{HashAlgorithm, HashDigest, HashDigests, PyProjectToml, ResolutionMetadata};
use fyn_redacted::DisplaySafeUrl;
use fyn_types::{BuildContext, BuildKey, BuildStack, SourceBuildTrait};
use fyn_workspace::pyproject::ToolfynSources;
...
impl<'a, T: BuildContext> SourceDistributionBuilder<'a, T> {
    ...
    pub(crate) async fn source_tree_requires_dist(
        ...
        match fyn_pypi_types::RequiresDist::from_pyproject_toml(pyproject_toml.clone()) {
            Ok(requires_dist) => {
            ...
            Err(
                err @ (fyn_pypi_types::MetadataError::Pep508Error(_)
                | fyn_pypi_types::MetadataError::DynamicField(_)
                | fyn_pypi_types::MetadataError::FieldNotFound(_)
                | fyn_pypi_types::MetadataError::PoetrySyntax),
            ) => {
                debug!(
    ...
    async fn github_metadata(
        ...
        // have `tool.fyn.sources` entries that it inherits from the workspace root.
        ...
        let pyproject_toml = match PyProjectToml::from_toml(&content, source) {
            ...
            Err(
                fyn_pypi_types::MetadataError::InvalidPyprojectTomlSyntax(..)
                | fyn_pypi_types::MetadataError::InvalidPyprojectTomlSchema(..),
            ) => {
        ...
        let metadata =
            match ResolutionMetadata::parse_pyproject_toml(pyproject_toml, source.version()) {
                Ok(metadata) => metadata,
                Err(
                    fyn_pypi_types::MetadataError::Pep508Error(..)
                    | fyn_pypi_types::MetadataError::DynamicField(..)
                    | fyn_pypi_types::MetadataError::FieldNotFound(..)
                    | fyn_pypi_types::MetadataError::PoetrySyntax,
                ) => {
        ...
        // Determine whether the project has `tool.fyn.sources`. If the project has sources, it must
        ...
        match has_sources(&content) {
            ...
            Err(err) => {
                debug!("Failed to parse `tool.fyn.sources` from GitHub API for: {url} ({err})");
                return Ok(None);
            }
        }
    ...
    async fn download_archive(
        ...
        let mut hashers = algorithms
            ...
            .map(Hasher::from)
            .collect::<Vec<_>>();
        let mut hasher = fyn_extract::hash::HashReader::new(reader.compat(), &mut hashers);
        ...
        fyn_extract::stream::archive(query_url, &mut hasher, ext, temp_dir.path())
            .await
        ...
        let extracted = match fyn_extract::strip_component(temp_dir.path()) {
            Ok(top_level) => top_level,
            Err(fyn_extract::Error::NonSingularArchive(_)) => temp_dir.keep(),
            Err(err) => {
    ...
    async fn persist_archive(
        ...
        let mut hashers = algorithms
            ...
            .map(Hasher::from)
            .collect::<Vec<_>>();
        let mut hasher = fyn_extract::hash::HashReader::new(reader, &mut hashers);

        // Unzip the archive into a temporary directory.
        fyn_extract::stream::archive(path.display(), &mut hasher, ext, &temp_dir.path())
            .await
        ...
        let extracted = match fyn_extract::strip_component(temp_dir.path()) {
            Ok(top_level) => top_level,
            Err(fyn_extract::Error::NonSingularArchive(_)) => temp_dir.path().to_path_buf(),
            Err(err) => {
                return Err(Error::Extract(
                    temp_dir.path().to_string_lossy().into_owned(),
                    err,
    ...
    async fn build_distribution(
        ...
        // Try a direct build if that isn't disabled and the fyn build backend is used.
        ...
        {
            // In the fyn build backend, the normalized filename and the disk filename are the same.
            name.to_string()
        } else {
            ...
            if let Some(builder) = self.build_context.build_arena().remove(&build_key) {
                ...
                let builder = self
                    ...
                    .setup_build(
                        ...
                        if fyn_flags::contains(fyn_flags::EnvironmentFlags::HIDE_BUILD_OUTPUT) {
                            BuildOutput::Quiet
                        } else {
                            BuildOutput::Debug
                        },
                    ...
                    for sibling in entry.path().read_dir().map_err(Error::CacheRead)? {
                        ...
                        if sibling.file_type().map_err(Error::CacheRead)?.is_dir() {
                            ...
                            if sibling_name != pointer.revision.id().as_str() {
                                ...
                                removal +=
                                    fyn_cache::rm_rf(sibling.path()).map_err(Error::CacheWrite)?;
                            }
                        }
                    }
                }
    ...
    async fn read(
        ...
        match read_pkg_info(source_root, subdirectory).await {
            ...
            Err(
                ...
                | Error::PkgInfo(
                    fyn_pypi_types::MetadataError::Pep508Error(_)
                    | fyn_pypi_types::MetadataError::DynamicField(_)
                    | fyn_pypi_types::MetadataError::FieldNotFound(_)
                    | fyn_pypi_types::MetadataError::UnsupportedMetadataVersion(_),
                )),
            ) => {
...
/// Returns `true` if a `pyproject.toml` has `tool.fyn.sources`.
