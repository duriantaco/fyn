...
enum RequirementOrigin {
    /// The `tool.fyn.sources` were read from the project.
    Project,
    /// The `tool.fyn.sources` were read from the workspace root.
    Workspace,
}

impl LoweredRequirement {
    /// Combine `project.dependencies` or `project.optional-dependencies` with `tool.fyn.sources`.
    pub(crate) fn from_requirement<'data>(
        ...
        // Identify the source from the `tool.fyn.sources` table.
                    ...
                    let (source, mut marker) = match source {
                        ...
                        Source::Workspace {
                            ...
                            let source = if let Some(git_member) = &git_member {
                                ...
                                let subdirectory =
                                    fyn_fs::relative_to(member.root(), git_member.fetch_root)
    ...
    /// Lower a [`fyn_pep508::Requirement`] in a non-workspace setting (for example, in a PEP 723
...
pub enum LoweringError {
    #[error(
        "`{0}` is included as a workspace member, but is missing an entry in `tool.fyn.sources` (e.g., `{0} = {{ workspace = true }}`)"
    )]
    MissingWorkspaceSource(PackageName),
    #[error(
        "`{0}` is included as a workspace member, but references a {1} in `tool.fyn.sources`. Workspace members must be declared as workspace sources (e.g., `{0} = {{ workspace = true }}`)."
    )]
    NonWorkspaceSource(PackageName, SourceKind),
    #[error(
        "`{0}` references a workspace in `tool.fyn.sources` (e.g., `{0} = {{ workspace = true }}`), but is not a workspace member"
    )]
...
/// (e.g., `fyn.toml`) rather than in the project's `pyproject.toml`.
fn missing_index_hint(locations: &IndexLocations, index: &IndexName) -> Option<String> {
    ...
    config_index.and_then(|idx| {
        let source = match idx.origin {
            Some(Origin::User) => "a user-level `fyn.toml`",
            Some(Origin::System) => "a system-level `fyn.toml`",
            Some(Origin::Project) => "a project-level `fyn.toml`",
            Some(Origin::Cli | Origin::RequirementsTxt) | None => return None,
        };
        Some(format!(
            "Index `{index}` was found in {source}, but indexes \
             referenced via `tool.fyn.sources` must be defined in the project's \
             `pyproject.toml`"
        ))
    })
}
...
fn path_source(
    ...
    if is_dir {
        ...
        if editable == Some(true) {
            ...
            // If the `package` option is unset, check if `tool.fyn.package` is set
