...
use fyn_configuration::NoSources;
use fyn_distribution_types::{IndexLocations, Requirement};
use fyn_normalize::{ExtraName, GroupName, PackageName};
use fyn_pep508::MarkerTree;
use fyn_workspace::dependency_groups::FlatDependencyGroups;
use fyn_workspace::pyproject::{Sources, ToolfynSources};
use fyn_workspace::{DiscoveryOptions, MemberDiscovery, ProjectWorkspace, WorkspaceCache};
...
impl RequiresDist {
    /// Lower without considering `tool.fyn` in `pyproject.toml`, used for index and other archive
    /// dependencies.
    pub fn from_metadata23(metadata: fyn_pypi_types::RequiresDist) -> Self {
        Self {
            name: metadata.name,
    ...
    /// Lower by considering `tool.fyn` in `pyproject.toml` if present, used for Git and directory
    /// dependencies.
    ...
    fn from_project_workspace(
        ...
        // Collect any `tool.fyn.index` entries.
        let empty = vec![];
        ...
        // Collect any `tool.fyn.sources` and `tool.fyn.dev_dependencies` from `pyproject.toml`.
    ...
    /// Validate the sources for a given [`fyn_pypi_types::RequiresDist`].
    ///
...
/// Like [`fyn_pypi_types::RequiresDist`], but with any recursive (or self-referential) dependencies
/// resolved.
///
/// For example, given:
/// ```toml
...
mod test {
    ...
    use fyn_auth::CredentialsCache;
    use fyn_configuration::NoSources;
    use fyn_distribution_types::IndexLocations;
    use fyn_normalize::PackageName;
    use fyn_pep508::Requirement;
    use fyn_workspace::{DiscoveryOptions, ProjectWorkspace, WorkspaceCache};
    ...
    async fn requires_dist_from_pyproject_toml(
        ...
        .await?;
        let pyproject_toml = fyn_pypi_types::PyProjectToml::from_toml(contents, "pyproject.toml")?;
        let requires_dist = fyn_pypi_types::RequiresDist::from_pyproject_toml(pyproject_toml)?;
    ...
    async fn workspace_and_url_spec() {
        ...
        error: Failed to parse entry: `tqdm`
          Caused by: `tqdm` references a workspace in `tool.fyn.sources` (e.g., `tqdm = { workspace = true }`), but is not a workspace member
        ");
    }
