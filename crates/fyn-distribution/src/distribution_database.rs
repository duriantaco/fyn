...
use fyn_cache::{ArchiveId, CacheBucket, CacheEntry, WheelCache};
use fyn_cache_info::{CacheInfo, Timestamp};
use fyn_client::{
...
use fyn_types::{BuildContext, BuildStack};
...
impl<'a, Context: BuildContext> DistributionDatabase<'a, Context> {
    ...
    async fn build_wheel_artifact(
            ...
            )
        {
            if matches!(dist, SourceDist::Registry(_)) {
                ...
                warn_user_once!(
                    "{dist} uses a legacy source distribution format ('.{extension}') that is not compliant with PEP 625. A future version of fyn will reject this source distribution. Consider upgrading to a newer version of {package}",
                    package = dist.name(),
                );
            } else {
                warn_user_once!(
                    "{dist} is not a standards-compliant source distribution: expected '.tar.gz' but found '.{extension}'. A future version of fyn will reject source distributions that do not meet the requirements specified in PEP 625",
                );
            }
        }
        ...
        if !built_wheel.filename.is_compatible(tags) {
            return if tags.is_cross() {
                Err(Error::BuiltWheelIncompatibleTargetPlatform {
                    ...
                    python_version: crate::error::PythonVersion {
    ...
    async fn get_wheel_metadata(
        ...
        // wheel.
        //
        // Historically, for `fyn pip compile --universal`, we also generate hashes for
