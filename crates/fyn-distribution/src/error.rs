...
use fyn_cache::Error as CacheError;
use fyn_client::WrappedReqwestError;
use fyn_distribution_filename::{WheelFilename, WheelFilenameError};
...
pub enum Error {
    ...
    // Network error
    #[error(transparent)]
    InvalidUrl(#[from] fyn_distribution_types::ToUrlError),
    #[error("Expected a file URL, but received: {0}")]
    NonFileUrl(DisplaySafeUrl),
    #[error(transparent)]
    Git(#[from] fyn_git::GitResolverError),
    #[error(transparent)]
    ...
    WheelMetadata(PathBuf, #[source] Box<fyn_metadata::Error>),
    ...
    Extract(String, #[source] fyn_extract::Error),
    ...
    PyprojectToml(#[source] fyn_pypi_types::MetadataError),
    #[error(transparent)]
    MetadataLowering(#[from] MetadataError),
    #[error("Distribution not found at: {0}")]
    NotFound(DisplaySafeUrl),
    #[error("Attempted to re-extract the source distribution for `{}`, but the {} hash didn't match. Run `{}` to clear the cache.", _0, _1, "fyn cache clean".green())]
