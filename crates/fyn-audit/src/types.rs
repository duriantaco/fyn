...
impl Vulnerability {
    ...
    /// Return an iterator over all identifiers for this vulnerability, including the primary ID
    /// and all aliases.
    fn ids(&self) -> impl Iterator<Item = &VulnerabilityID> {
        std::iter::once(&self.id).chain(self.aliases.iter())
    }

    /// Returns `true` if any of this vulnerability's identifiers (primary ID or aliases) match
    /// the given ID.
