...
//! operations are unimplemented, so callers may want to take a different code
//! path when they detect Wine.
//!
//! Detection is based on the canonical method recommended by the Wine project:
//! probing `ntdll.dll` for the Wine-only `wine_get_version` export.
