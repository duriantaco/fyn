    ...
    pub fn with_features(features: &[super::PreviewFeature]) -> FeaturesGuard {
        ...
        match state {
            ...
            PreviewMode::Normal(_) => {
                panic!(
                    "Cannot use `{}::with_features` after `fv_preview::init` has been called",
                    module_path!()
                );
            }
        }
...
mod tests {
    ...
    #[test]
    #[should_panic(
        expected = "Additional calls to `fv_preview::test::with_features` are not allowed while holding a `FeaturesGuard`"
    )]
    ...
    #[test]
    #[should_panic(expected = "fv_preview::test::with_features")]
