...
use fv_cache::Cache;
use fv_configuration::{BuildKind, BuildOptions, BuildOutput, NoSources};
...
/// ```
///
/// Put in a different way, the types here allow `fv-resolver` to depend on `fv-build` and
/// `fv-build-frontend` to depend on `fv-resolver` without having actual crate dependencies between
/// them.
pub trait BuildContext {
    ...
    /// Whether to incorporate `tool.fv.sources` when resolving requirements.
    ...
    /// Build by calling directly into the fv build backend without PEP 517, if possible.
    ///
    /// Checks if the source tree uses fv as build backend. If not, it returns `Ok(None)`, otherwise
...
/// A wrapper for `fv_build::SourceBuild` to avoid cyclical crate dependencies.
///
...
pub trait SourceBuildTrait {
    /// A wrapper for `fv_build::SourceBuild::get_metadata_without_build`.
    ///
    ...
    /// A wrapper for `fv_build::SourceBuild::build`.
    ///
...
/// A wrapper for [`fv_installer::SitePackages`]
...
///
/// The errors types have the same problem as [`BuildDispatch`] generally: The `fv-resolver`,
/// `fv-installer` and `fv-build-frontend` error types all reference each other:
