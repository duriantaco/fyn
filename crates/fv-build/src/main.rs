...
use fv_static::{EnvVars, parse_boolish_environment_variable};
use std::env;
use std::io::Write;
...
use fv_logging::UvFormat;

/// Entrypoint for the `fv-build` Python package.
fn main() -> Result<()> {
    // Support configuring the log level with `RUST_LOG` (shows only the error level by default) and
    // color.
    //
    // This configuration is a simplified version of the fv logging configuration. When using
    // fv_build through fv proper, the fv logging configuration applies.
    ...
    // Ad-hoc preview features parsing due to a lack of clap CLI in fv-build.
    ...
    match command.as_str() {
        "build-sdist" => {
            ...
            let filename = fv_build_backend::build_source_dist(
                &env::current_dir()?,
                &sdist_directory,
                fv_version::version(),
                false,
        ...
        "build-wheel" => {
            ...
            let filename = fv_build_backend::build_wheel(
                &env::current_dir()?,
                &wheel_directory,
        ...
        "build-editable" => {
            let wheel_directory = PathBuf::from(args.next().context("Missing wheel directory")?);
            let metadata_directory = args.next().map(PathBuf::from);
            let filename = fv_build_backend::build_editable(
                &env::current_dir()?,
                &wheel_directory,
                metadata_directory.as_deref(),
                fv_version::version(),
        ...
        "prepare-metadata-for-build-wheel" => {
            let wheel_directory = PathBuf::from(args.next().context("Missing wheel directory")?);
            let filename = fv_build_backend::metadata(
                &env::current_dir()?,
                &wheel_directory,
                fv_version::version(),
            )?;
        ...
        "--help" => {
            // This works both as redirect to use the proper fv package and as smoke test.
            writeln!(
                &mut std::io::stderr(),
                "fv_build contains only the PEP 517 build backend for fv and can't be used on the CLI. \
                Use `fv build` or another build frontend instead."
