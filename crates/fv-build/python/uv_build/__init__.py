...
# Use the `fv build-backend` command rather than `fv-build`. This option is provided
# for downstream distributions who provide `fv` and wish to avoid building a partially
# overlapping `fv-build` executable.
USE_FV_EXECUTABLE = False
...
def call(
    ...
    """Invoke a fv subprocess and return the filename from stdout."""
    import shutil
    import subprocess
    import sys
    ...
    fv_bin_name = "fv" if USE_FV_EXECUTABLE else "fv-build"
    # Unlike `find_fv_bin`, this mechanism must work according to PEP 517
    fv_bin = shutil.which(fv_bin_name)
    if fv_bin is None:
        raise RuntimeError(f"{fv_bin_name} was not properly installed")
    build_backend_args = ["build-backend"] if USE_FV_EXECUTABLE else []
    # Forward stderr, capture stdout for the filename
    result = subprocess.run(
        [fv_bin, *build_backend_args, *args], stdout=subprocess.PIPE
    )
    ...
    if not stdout:
        print(
            f"{fv_bin_name} subprocess did not return a filename on stdout",
            file=sys.stderr,
        )
        sys.exit(1)
