...
use fv_platform_tags::{AbiTag, LanguageTag, PlatformTag};
...
pub fn build_wheel(
    ...
    for warning in pyproject_toml.check_build_system(fv_version) {
    ...
    let temp_file = fv_fs::tempfile_in(wheel_dir)?;
...
fn write_wheel(
    ...
    for (name, directory) in settings.data.iter() {
        ...
        wheel_subdir_from_globs(
            ...
            &["**".to_string()],
            &mut wheel_writer,
            &format!("tool.fv.build-backend.data.{name}"),
        )?;
    }
    ...
    for exclude in excludes {
        ...
        let glob = PortableGlobParser::Uv
            ...
            .map_err(|err| Error::PortableGlob {
                field: "tool.fv.build-backend.*-exclude".to_string(),
                source: err,
            })?;
...
fn write_dist_info(
    ...
    let wheel_info = WheelInfo::new(filename, fv_version);
    writer.write_bytes(
        &format!("{dist_info_dir}/WHEEL"),
        wheel_info.to_string().as_bytes(),
    )?;
    if fv_preview::is_enabled(PreviewFeature::MetadataJson) {
        writer.write_bytes(
...
impl WheelInfo {
    fn new(filename: &WheelFilename, fv_version: &str) -> Self {
        ...
        Self {
            ...
            generator: format!("fv {fv_version}"),
...
mod test {
    ...
    use fv_platform_tags::{AbiTag, PlatformTag};
    ...
    fn test_prepare_metadata() {
        ...
        Name: built-by-uv
        Version: 0.1.0
        Summary: A package to be built with the fv build backend that uses all features exposed by the build backend
        ...
        A package to be built with the fv build backend that uses all features exposed by the build backend.
        ");
