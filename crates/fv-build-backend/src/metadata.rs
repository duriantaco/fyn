use fv_warnings::warn_user_once;
...
use fv_globfilter::{GlobDirFilter, PortableGlobParser};
use fv_normalize::{ExtraName, PackageName};
...
use fv_pypi_types::{Keywords, Metadata23, ProjectUrls, VerbatimParsedUrl};
...
/// No breaking changes were introduced to the fv build backend since these releases, so we can use
...
/// The project is not compatible with a direct fv build.
///
...
pub enum DirectBuildIncompatibility {
    ...
    #[error("`build_system.build-backend` is not `fv_build`, but `{0}`")]
    WrongBackend(String),
    #[error("`build-system.requires` is not exactly `fv_build`, but `{0}`")]
    MultipleRequires(String),
    #[error("`build-system.requires` is not `fv_build`, but `{0}`")]
    WrongPackage(PackageName),
    #[error("`build_system.requires` uses a URL requirement")]
    UrlRequirement,
    #[error("`fv_build{0}` is not a known compatible range")]
    IncompatibleRange(VersionSpecifiers),
}

/// Check if the build backend is matching the currently running fv version.
///
/// Example table compatible with fv 0.4.21:
///
/// ```toml
/// [build-system]
/// requires = ["fv_build>=0.4.15,<0.5.0"]
...
pub fn check_direct_build(
    ...
    if pyproject_toml.build_system.build_backend.as_deref() != Some("fv_build") {
    ...
    match &uv_requirement.version_or_url {
        None => {
            ...
            // path, their build may equally fail if the index version of `fv_build`.
        }
        ...
        Some(VersionOrUrl::VersionSpecifier(specifier)) => {
            ...
            // path, their build may equally fail if the index version of `fv_build`, so we allow
...
impl PyProjectToml {
    ...
    pub(crate) fn parse(path: &Path) -> Result<Self, Error> {
        ...
        let pyproject_toml =
            tracing::info_span!("toml::from_str fv build backend", path = %path.display())
    ...
    pub(crate) fn settings(&self) -> Option<&BuildBackendSettings> {
        self.tool.as_ref()?.fv.as_ref()?.build_backend.as_ref()
    }

    /// See [`BuildSystem::check_build_system`].
    pub fn check_build_system(&self, fv_version: &str) -> Vec<String> {
        self.build_system.check_build_system(fv_version)
    }
...
/// The `tool.fv` section with build configuration.
#[derive(Deserialize, Debug, Clone)]
#[serde(rename_all = "kebab-case")]
pub(crate) struct ToolFv {
    /// Configuration for building source distributions and wheels with the fv build backend
...
impl BuildSystem {
    /// Check if the `[build-system]` table matches the fv build backend expectations and return
    ...
    /// build-backend = "fv_build"
    /// ```
    pub(crate) fn check_build_system(&self, fv_version: &str) -> Vec<String> {
        let mut warnings = Vec::new();
        if self.build_backend.as_deref() != Some("fv_build") {
            warnings.push(format!(
                r#"`build_system.build-backend` was expected to be `"fv_build"`, not `"{}"`"#,
                self.build_backend.clone().unwrap_or_default()
            ));
        }

        let fv_version =
            Version::from_str(fv_version).expect("uv's own version is not PEP 440 compliant");
        let next_minor = fv_version.release().get(1).copied().unwrap_or_default() + 1;
        let next_breaking = Version::new([0, next_minor]);

        let [uv_requirement] = &self.requires.as_slice() else {
            warnings.push(format!(
                "Expected `build-system.requires` to contain only `fv_build`, found `{}`",
        ...
        if uv_requirement.name.as_str() != "uv-build" {
            warnings.push(format!(
                "Expected `build-system.requires` to be `fv_build`, found `{}`",
                self.requires.iter().map(ToString::to_string).join("`, `")
            ));
            return warnings;
        }
        let bounded = match &uv_requirement.version_or_url {
            ...
            Some(VersionOrUrl::VersionSpecifier(specifier)) => {
                ...
                // is very important to allow making breaking changes in fv without breaking
                // the existing immutable source distributions on pypi.
                if !specifier.contains(&fv_version) {
        ...
        if !bounded {
            warnings.push(format!(
                ...
                upper bound on the `fv_build` version such as `<{next_breaking}`. \
                Without bounding the `fv_build` version, the source distribution will break \
                when a future, breaking version of `fv_build` is released.",
...
mod tests {
    ...
    fn build_system_no_bound() {
        ...
        assert_snapshot!(
            pyproject_toml.check_build_system("0.4.15+test").join("\n"),
            @r#"`build_system.requires = ["fv_build"]` is missing an upper bound on the `fv_build` version such as `<0.5`. Without bounding the `fv_build` version, the source distribution will break when a future, breaking version of `fv_build` is released."#
        );
    }
    ...
    fn build_system_multiple_packages() {
        let contents = indoc! {r#"
            ...
            [build-system]
            requires = ["fv_build>=0.4.15,<0.5.0", "wheel"]
            build-backend = "fv_build"
        "#};
        let pyproject_toml: PyProjectToml = toml::from_str(contents).unwrap();
        assert_snapshot!(
            pyproject_toml.check_build_system("0.4.15+test").join("\n"),
            @"Expected `build-system.requires` to contain only `fv_build`, found `uv-build>=0.4.15,<0.5.0`, `wheel`"
        );
    }
    ...
    fn build_system_no_requires_uv() {
        ...
        assert_snapshot!(
            pyproject_toml.check_build_system("0.4.15+test").join("\n"),
            @"Expected `build-system.requires` to be `fv_build`, found `setuptools`"
        );
    }
    ...
    fn build_system_not_uv() {
        ...
        assert_snapshot!(
            pyproject_toml.check_build_system("0.4.15+test").join("\n"),
            @r#"`build_system.build-backend` was expected to be `"fv_build"`, not `"setuptools"`"#
        );
    }
    ...
    fn check_direct_build_wrong_backend() {
        ...
        assert_snapshot!(
            check_direct_build(temp_dir.path(), "0.10.0").unwrap_err(),
            @"`build_system.build-backend` is not `fv_build`, but `setuptools`"
        );
    }
    ...
    fn check_direct_build_multiple_requires() {
        ...
        fs_err::write(
            ...
            indoc! {r#"
                ...
                [build-system]
                requires = ["fv_build>=0.10.0,<0.11", "wheel"]
                build-backend = "fv_build"
            "#},
        )
        .unwrap();
        assert_snapshot!(
            check_direct_build(temp_dir.path(), "0.10.0").unwrap_err(),
            @"`build-system.requires` is not exactly `fv_build`, but `uv-build>=0.10.0,<0.11`, `wheel`"
        );
    }
    ...
    fn check_direct_build_wrong_package() {
        ...
        assert_snapshot!(
            check_direct_build(temp_dir.path(), "0.10.0").unwrap_err(),
            @"`build-system.requires` is not `fv_build`, but `setuptools`"
        );
    }
    ...
    fn check_direct_build_url_requirement() {
        ...
        fs_err::write(
            ...
            indoc! {r#"
                ...
                [build-system]
                requires = ["fv_build @ https://example.com/fv_build-0.10.0-py3-none-any.whl"]
                build-backend = "fv_build"
            "#},
        )
        .unwrap();
    ...
    fn check_direct_build_incompatible_range() {
        ...
        assert_snapshot!(
            check_direct_build(temp_dir.path(), "0.10.0").unwrap_err(),
            @"`fv_build>=0.5.0, <0.6` is not a known compatible range"
        );
    }
}
