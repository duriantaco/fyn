...
use fs_err::File;
use fv_distribution_filename::{SourceDistExtension, SourceDistFilename};
...
pub fn build_source_dist(
    ...
    let temp_file = fv_fs::tempfile_in(source_dist_directory)?;
    let writer = TarGzWriter::new(temp_file.as_file(), &source_dist_path);
    write_source_dist(source_tree, writer, fv_version, show_warnings)?;
    temp_file
...
fn source_dist_matcher(
    ...
    for module_relative in modules_relative {
        ...
        let import_path = fv_fs::normalize_path(
            &fv_fs::relative_to(src_root.join(module_relative), source_tree)
    ...
    for include in includes {
        let glob = PortableGlobParser::Uv
            ...
            .map_err(|err| Error::PortableGlob {
                field: "tool.fv.build-backend.source-include".to_string(),
                source: err,
            })?;
    ...
    {
        let readme = fv_fs::normalize_path(readme);
    ...
    for (name, directory) in settings.data.iter() {
        let directory = fv_fs::normalize_path(directory);
        ...
        let glob = PortableGlobParser::Uv
            ...
            .map_err(|err| Error::PortableGlob {
                field: format!("tool.fv.build-backend.data.{name}"),
                source: err,
            })?;
        include_globs.push(glob);
    }
