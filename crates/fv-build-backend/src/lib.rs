...
use fv_globfilter::PortableGlobError;
use fv_normalize::PackageName;
use fv_pypi_types::{Identifier, IdentifierParseError};
...
fn prune_redundant_modules_warn(names: &[String], show_warnings: bool) -> Vec<String> {
    ...
    if show_warnings && names.len() != pruned.len() {
        ...
        warn_user_once!(
            "Ignoring redundant module name{s} in `tool.fv.build-backend.module-name`: `{}`",
...
/// `tool.fv.build-backend.module-name`.
///
...
fn find_roots(
    ...
    let relative_module_root = fv_fs::normalize_path(relative_module_root);
    // Check that even if a path contains `..`, we only include files below the module root.
    if !fv_fs::normalize_path(&source_tree.join(&relative_module_root))
        .starts_with(fv_fs::normalize_path(source_tree))
    {
...
mod tests {
    ...
    use fs_err::File;
    use fv_distribution_filename::{SourceDistFilename, WheelFilename};
    use fv_fs::{copy_dir_all, relative_to};
    ...
    fn build(
        ...
        let (_name, direct_wheel_list_files) = {
            let _preview = fv_preview::test::with_features(preview_features);
    ...
    fn built_by_fv_building() {
        ...
        // This is a caveat to the determinism of the fv build backend: When a file has the
        ...
        // Redact the fv_build version to keep the hash stable across releases
        let pyproject_toml = fs_err::read_to_string(src.path().join("pyproject.toml")).unwrap();
        let current_requires =
            Regex::new(r#"requires = \["fv_build>=[0-9.]+,<[0-9.]+"\]"#).unwrap();
        let mocked_requires = r#"requires = ["fv_build>=1,<2"]"#;
    ...
    fn license_file_pre_pep639() {
        ...
        fs_err::write(
            ...
            [build-system]
            requires = ["fv_build>=0.5.15,<0.6.0"]
        ...
        {
            let _preview = fv_preview::test::with_features(&[]);
            build_wheel(
