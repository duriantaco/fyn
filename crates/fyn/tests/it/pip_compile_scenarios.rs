...
use fyn_test::{
    TestContext, build_vendor_links_url, fyn_snapshot, get_bin, packse_index_url,
...
fn compatible_python_incompatible_override() -> Result<()> {
    ...
    let output = fyn_snapshot!(filters, command(&context, python_versions)
    ...
    warning: The requested Python version 3.9 is not available; 3.11.[X] will be used to build dependencies instead.
      ...
      ╰─▶ Because the requested Python version (>=3.9) does not satisfy Python>=3.10 and package-a==1.0.0 depends on Python>=3.10, we can conclude that package-a==1.0.0 cannot be used.
          ...
          The `--python-version` value (>=3.9) includes Python versions that are not supported by your dependencies (e.g., package-a==1.0.0 only supports >=3.10). Consider using a higher `--python-version` value.
    "
    );
...
fn incompatible_python_compatible_override_available_no_wheels() -> Result<()> {
    ...
    #    fyn pip compile requirements.in --cache-dir [CACHE_DIR] --python-version=3.11
    package-a==1.0.0
...
fn python_patch_override_no_patch() -> Result<()> {
    let context = fyn_test::test_context!("3.9.21");
    let python_versions = &[];
    ...
    ----- stderr -----
      ...
      ╰─▶ Because the requested Python version (>=3.9) does not satisfy Python>=3.9.4 and package-a==1.0.0 depends on Python>=3.9.4, we can conclude that package-a==1.0.0 cannot be used.
          ...
          The `--python-version` value (>=3.9) includes Python versions that are not supported by your dependencies (e.g., package-a==1.0.0 only supports >=3.9.4). Consider using a higher `--python-version` value.
    "
    );
...
fn python_patch_override_patch_compatible() -> Result<()> {
    ...
    ----- stdout -----
    # This file was autogenerated by fyn via the following command:
    #    fyn pip compile requirements.in --cache-dir [CACHE_DIR] --python-version=3.9.0
    package-a==1.0.0
        # via -r requirements.in
