...
// These tests just run `fyn lock` on an assorted of ecosystem
// projects.
//
...
// Currently ignored because the project doesn't build with `fyn` yet.
//
// Source: https://github.com/apache/airflow/blob/c55438d9b2eb9b6680641eefdd0cbc67a28d1d29/pyproject.toml
#[test]
#[ignore = "Airflow doesn't build with `fyn` yet"]
fn airflow() -> Result<()> {
...
/// root of the `fyn` repository.
fn lock_ecosystem_package(python_version: &str, name: &str) -> Result<()> {
    let context = fyn_test::test_context!(python_version);
    ...
    let (snapshot, _) = fyn_test::run_and_format(
        &mut command,
        context.filters(),
        name,
        Some(fyn_test::WindowsFilters::Platform),
        None,
    );
