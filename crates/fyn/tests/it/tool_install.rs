...
fn tool_install() {
    ...
    insta::with_settings!({
        ...
        assert_snapshot!(fs_err::read_to_string(tool_dir.join("black").join("fyn-receipt.toml")).unwrap(), @r#"
        [tool]
    ...
    fyn_snapshot!(context.filters(), Command::new("black").arg("--version").env(EnvVars::PATH, bin_dir.as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), Command::new("flask").arg("--version").env(EnvVars::PATH, bin_dir.as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Python 3.12.[X]
    ...
    insta::with_settings!({
        ...
        assert_snapshot!(fs_err::read_to_string(tool_dir.join("flask").join("fyn-receipt.toml")).unwrap(), @r#"
        [tool]
...
fn tool_install_suggest_other_packages_with_executable() {
    ...
    hint: An executable with the name `fastapi` is available via dependency `fastapi-cli`.
          Did you mean `fyn tool install fastapi-cli`?
...
/// Test installing a tool with `fyn tool install --from`
#[test]
...
fn tool_install_force() {
    ...
    assert!(!tool_dir.join("black").join("fyn-receipt.toml").exists());
...
/// Test `fyn tool install` when the bin directory is inferred from `$HOME`
///
...
/// Test `fyn tool install` when the bin directory is inferred from `$XDG_DATA_HOME`
#[test]
...
/// Test `fyn tool install` when the bin directory is set by `$XDG_BIN_HOME`
#[test]
...
fn tool_install_uninstallable() {
    ...
    let filters = context
        ...
        .collect::<Vec<_>>();
    fyn_snapshot!(filters, context.tool_install()
        .arg("pyenv")
        .env(EnvVars::UV_TOOL_DIR, tool_dir.as_os_str())
        .env(EnvVars::XDG_BIN_HOME, bin_dir.as_os_str())
        .env(EnvVars::PATH, bin_dir.as_os_str()), @"
...
fn tool_install_git_lfs() {
    ...
    insta::with_settings!({
        ...
        // We should have a tool receipt
        assert_snapshot!(fs_err::read_to_string(tool_dir.join("test-lfs-repo").join("fyn-receipt.toml")).unwrap(), @r#"
        [tool]
    ...
    fyn_snapshot!(context.filters(), Command::new("test-lfs-repo").env(EnvVars::PATH, bin_dir.as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), Command::new("test-lfs-repo-assets").env(EnvVars::PATH, bin_dir.as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    #[cfg(not(windows))]
    fyn_snapshot!(context.filters(), Command::new("test-lfs-repo-assets").env(EnvVars::PATH, bin_dir.as_os_str()), @r#"
    success: false
    exit_code: [ERROR_CODE]
    ----- stdout -----
...
fn tool_install_warn_path() {
    ...
    Installed [N] packages in [TIME]
     ...
     + platformdirs==4.2.0
    Installed 2 executables: black, blackd
    warning: `[TEMP_DIR]/bin` is not on your PATH. To use installed tools, run `export PATH="[TEMP_DIR]/bin:$PATH"` or `fyn tool update-shell`.
    "#);
}
...
fn tool_install_bad_receipt() -> Result<()> {
    ...
    // Override the `fyn-receipt.toml` file with an invalid receipt.
    tool_dir
        .child("black")
        .child("fyn-receipt.toml")
        .write_str("invalid")?;
...
fn tool_install_malformed_dist_info() {
    ...
    insta::with_settings!({
        ...
        // We should have a tool receipt
        assert_snapshot!(fs_err::read_to_string(tool_dir.join("executable-application").join("fyn-receipt.toml")).unwrap(), @r#"
        [tool]
...
/// Test installing a tool with `fyn tool install {package}@{version}`.
#[test]
...
/// Test installing a tool with `fyn tool install {package}@latest`.
#[test]
...
/// Test installing a tool with `fyn tool install {package} --from {package}@latest`.
#[test]
...
/// Test installing a tool with `fyn tool install {package} --from {package}@{version}`.
#[test]
...
/// `fyn tool install python` is not allowed
#[test]
fn tool_install_python() {
    ...
    ----- stderr -----
    error: Cannot install Python with `fyn tool install`. Did you mean to use `fyn python install`?
    ");
...
async fn tool_install_default_credentials() -> Result<()> {
    ...
    // Write a `fyn.toml` with a default index that has credentials.
    ...
    fyn_snapshot!(context.filters(), context.tool_upgrade()
        .arg("executable-application")
        .env(EnvVars::UV_TOOL_DIR, tool_dir.as_os_str())
        .env(EnvVars::XDG_BIN_HOME, bin_dir.as_os_str())
        .env(EnvVars::PATH, bin_dir.as_os_str()), @"
...
fn tool_install_with_executables_from() {
    ...
    insta::with_settings!({
        ...
        assert_snapshot!(fs_err::read_to_string(tool_dir.join("ansible").join("fyn-receipt.toml")).unwrap(), @r#"
        [tool]
        requirements = [
    ...
    fyn_snapshot!(context.filters(), context.tool_uninstall()
        .arg("ansible")
        .env(EnvVars::UV_TOOL_DIR, tool_dir.as_os_str())
        .env(EnvVars::XDG_BIN_HOME, bin_dir.as_os_str())
        .env(EnvVars::PATH, bin_dir.as_os_str()), @"
...
fn tool_install_find_links() {
    let context = fyn_test::test_context!("3.13").with_filtered_exe_suffix();
    let tool_dir = context.temp_dir.child("tools");
    let bin_dir = context.temp_dir.child("bin");
