...
use fyn_fs::{Simplified, copy_dir_all};
use fyn_static::EnvVars;
use fyn_test::{download_to_disk, fyn_snapshot, site_packages_path};
...
fn missing_venv() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.pip_sync().arg("requirements.txt"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.pip_sync().arg("requirements.txt").env_remove(EnvVars::VIRTUAL_ENV), @"
    success: false
    exit_code: 2
    ----- stdout -----

    ----- stderr -----
    error: No virtual environment found; run `fyn venv` to create an environment, or pass `--system` to install into a non-virtual environment
    ");
...
fn missing_system() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.pip_sync().arg("requirements.txt").arg("--system"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn install_hardlink_after_emlink() -> anyhow::Result<()> {
    ...
    let Some(context) = fyn_test::test_context!("3.12").with_cache_on_lowlinks_fs()? else {
        return Ok(());
    };
...
fn install_sdist_archive_type_bz2() -> Result<()> {
    ...
    ----- stderr -----
    Resolved 1 package in [TIME]
    warning: bz2 @ file://[WORKSPACE]/test/links/bz2-1.0.0.tar.bz2 is not a standards-compliant source distribution: expected '.tar.gz' but found '.tar.bz2'. A future version of fyn will reject source distributions that do not meet the requirements specified in PEP 625
    Prepared 1 package in [TIME]
    Installed 1 package in [TIME]
...
fn warn_on_yanked() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), windows_filters=false, context.pip_sync()
        .arg("requirements.txt")
        .arg("--strict"), @r#"
    success: true
    exit_code: 0
...
fn install_git_source_dist_cached() -> Result<()> {
    ...
    fyn_snapshot!(filters, context.clean()
        .arg("werkzeug"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn install_url_built_dist_cached() -> Result<()> {
    ...
    fyn_snapshot!(context_filters, context.pip_sync()
        .arg("requirements.txt")
        .arg("--strict"), @"
    success: true
    exit_code: 0
