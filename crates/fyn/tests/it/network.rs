...
/// IO error.
///
/// Tests different errors paths inside fyn, which retries 3 times by default, for a total for 4
/// requests.
...
/// Test that invalid proxy URL in fyn.toml produces a helpful error message.
#[tokio::test]
...
/// Test that https-proxy in fyn.toml routes HTTPS requests through a CONNECT tunnel proxy.
#[cfg(feature = "test-pypi")]
#[test]
...
/// Test that no-proxy in fyn.toml bypasses the proxy for specified hosts.
#[cfg(feature = "test-pypi")]
#[tokio::test]
...
/// Test that proxy URLs without a scheme in fyn.toml default to http://.
#[cfg(feature = "test-pypi")]
#[tokio::test]
