...
use fyn_test::{fyn_snapshot, get_bin};
...
fn check_self_update() {
    ...
    let args = RuntestArgs {
        app_name: "fyn".to_owned(),
        package: "fyn".to_owned(),
        owner: "astral-sh".to_owned(),
        bin: get_bin!(),
        binaries: vec!["fyn".to_owned()],
    ...
    let status = Command::new(installed_bin)
        .arg("--version")
        .status()
        .expect("failed to run 'fyn --version'");
    assert!(status.success(), "'fyn --version' returned non-zero");
}
...
fn test_self_update_offline_error() {
    ...
    fyn_snapshot!(context.self_update().arg("--offline"),
    @r"
    success: false
    exit_code: 1
    ----- stdout -----
