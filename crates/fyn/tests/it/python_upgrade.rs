...
fn python_upgrade() {
    ...
    fyn_snapshot!(context.filters(), context.python_upgrade().arg("3.10.17"), @"
    success: false
    exit_code: 1
    ----- stdout -----

    ----- stderr -----
    error: `fyn python upgrade` only accepts minor versions, got: 3.10.17
    ");

    // Upgrade patch version
    fyn_snapshot!(context.filters(), context.python_upgrade().arg("3.10"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_upgrade().arg("3.10").arg("--reinstall"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_install().arg("3.14.0rc2"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_upgrade(), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn python_upgrade_without_version() {
    ...
    fyn_snapshot!(context.filters(), context.python_install().arg("3.11.8").arg("3.12.8").arg("3.13.1"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_upgrade().arg("3.13"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Providing no minor version to `fyn python upgrade` should upgrade the rest
...
fn python_upgrade_transparent_from_venv() {
    ...
    fyn_snapshot!(context.filters(), context.venv().arg(second_venv).arg("-p").arg("3.10"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn python_upgrade_ignored_with_python_pin() {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("3.10.17"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
// Virtual environments record patch versions. `fyn venv -p 3.x.y` will
...
fn python_no_transparent_upgrade_with_venv_patch_specification() {
    ...
    fyn_snapshot!(context.filters(), context.venv().arg("-p").arg("3.10.17"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn python_upgrade_transparent_from_venv_module() {
    ...
    fyn_snapshot!(context.filters(), context.python_install().arg("3.12.9"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Upgrade patch version
    fyn_snapshot!(context.filters(), context.python_upgrade().arg("3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
// Tests that `fyn python upgrade 3.12` will warn if trying to install over non-managed
// interpreter.
#[test]
fn python_upgrade_force_install() -> Result<()> {
    ...
    ----- stderr -----
    warning: Executable already exists at `[BIN]/python3.12` but is not managed by fyn; use `fyn python install 3.12 --force` to replace it
    Installed Python 3.12.[LATEST] in [TIME]
     + cpython-3.12.[LATEST]-[PLATFORM]
    ");

    // Force the `bin` install.
    fyn_snapshot!(context.filters(), context.python_install().arg("3.12").arg("--force").arg("3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
