...
/// Read from a `fyn.toml` file in the current directory.
#[test]
#[cfg_attr(
    windows,
...
fn resolve_uv_toml() -> anyhow::Result<()> {
    ...
    // Write a `fyn.toml` file to the directory.
    let config = context.temp_dir.child("fyn.toml");
    ...
    fyn_snapshot!(context.filters(), add_shared_args(context.pip_compile())
        .arg("--show-settings")
...
///
/// We prefer `fyn.toml` when both are present, but respect `pyproject.toml` otherwise.
#[test]
#[cfg_attr(
    windows,
    ignore = "Configuration tests are not yet supported on Windows"
...
/// Merge configuration between the top-level `tool.fyn` and the more specific `tool.fyn.pip`.
#[test]
#[cfg_attr(
    windows,
    ignore = "Configuration tests are not yet supported on Windows"
...
fn resolve_top_level() -> anyhow::Result<()> {
    ...
    // Write out to the top-level (`tool.fyn`, rather than `tool.fyn.pip`).
    let pyproject = context.temp_dir.child("pyproject.toml");
    pyproject.write_str(indoc::indoc! {r#"
        [project]
        name = "example"
    ...
    // Write out to both the top-level (`tool.fyn`) and the pip section (`tool.fyn.pip`). The
    // `tool.fyn.pip` section should take precedence when combining.
    pyproject.write_str(indoc::indoc! {r#"
        [project]
        name = "example"
        version = "0.0.0"
...
fn resolve_user_configuration() -> anyhow::Result<()> {
    ...
    // However, the user-level `tool.fyn.pip` settings override the project-level `tool.fyn` settings.
    // This is awkward, but we merge the user configuration into the workspace configuration, so
    // the resulting configuration has both `tool.fyn.pip.resolution` (from the user configuration)
    // and `tool.fyn.resolution` (from the workspace settings), so we choose the former.
    let config = context.temp_dir.child("fyn.toml");
    config.write_str(indoc::indoc! {r#"
        resolution = "highest"
    "#})?;
...
/// When running a user-level command (like `fyn tool install`), we should read user configuration,
...
fn resolve_tool() -> anyhow::Result<()> {
    ...
    // If we're running a user-level command, like `fyn tool install`, we should use lowest direct,
    // but retain build isolation (since we ignore the local configuration).
    fyn_snapshot!(context.filters(), add_shared_args(context.tool_install())
        .arg("--show-settings")
        .arg("requirements.in")
        .env(EnvVars::XDG_CONFIG_HOME, xdg.path()), @r#"
    success: true
...
/// Read from both a `fyn.toml` and `pyproject.toml` file in the current directory.
///
/// Some fields in `[tool.fyn]` are masked by `fyn.toml` being defined, and should be warned about.
#[test]
#[cfg_attr(
    windows,
    ignore = "Configuration tests are not yet supported on Windows"
...
fn resolve_both() -> anyhow::Result<()> {
    ...
    warning: Found both a `fyn.toml` file and a supported tool section in an adjacent `pyproject.toml`. The following fields from `pyproject.toml` will be ignored in favor of the `fyn.toml` file:
    - offline
    - pip
    "#
    );
...
/// Read from both a `fyn.toml` and `pyproject.toml` file in the current directory.
///
/// But the fields `[tool.fyn]` defines aren't allowed in `fyn.toml` so there's no warning.
#[test]
#[cfg_attr(
    windows,
    ignore = "Configuration tests are not yet supported on Windows"
...
fn invalid_conflicts() -> anyhow::Result<()> {
    ...
    fyn_snapshot!(context.filters(), add_shared_args(context.lock()), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn resolve_config_file() -> anyhow::Result<()> {
    ...
    // Write a `fyn.toml` to a temporary location. (Use the cache directory for convenience, since
    // it's already obfuscated in the fixtures.)
    let config_dir = &context.cache_dir;
    let config = config_dir.child("fyn.toml");
    config.write_str(indoc::indoc! {r#"
        [pip]
        resolution = "lowest-direct"
        generate-hashes = true
    ...
    ----- stderr -----
    warning: The `--config-file` argument expects to receive a `fyn.toml` file, not a `pyproject.toml`. If you're trying to run a command from another project, use the `--project` argument instead.
...
fn resolve_skip_empty() -> anyhow::Result<()> {
    ...
    // Set `lowest-direct` in a `fyn.toml`.
    let config = context.temp_dir.child("fyn.toml");
    config.write_str(indoc::indoc! {r#"
        [pip]
        resolution = "lowest-direct"
    ...
    // Adding a `tool.fyn` section should cause us to ignore the `fyn.toml`.
    pyproject.write_str(indoc::indoc! {r#"
        [project]
        name = "child"
        dependencies = [
...
fn verify_hashes() -> anyhow::Result<()> {
    ...
    fyn_snapshot!(context.filters(), add_shared_args(context.pip_install())
        .arg("-r")
        .arg("requirements.in")
        .arg("--show-settings"), @r#"
    success: true
...
fn preview_features() {
    ...
    fyn_snapshot!(context.filters(), cmd().arg("--preview"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    GlobalSettings {
    ...
    fyn_snapshot!(context.filters(), cmd().arg("--preview").arg("--no-preview"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    GlobalSettings {
    ...
    fyn_snapshot!(context.filters(), cmd().arg("--preview").arg("--preview-features").arg("python-install-default"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    GlobalSettings {
    ...
    fyn_snapshot!(context.filters(), cmd().arg("--preview-features").arg("python-install-default,python-upgrade"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    GlobalSettings {
    ...
    fyn_snapshot!(context.filters(), cmd().arg("--preview-features").arg("python-install-default").arg("--preview-feature").arg("python-upgrade"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    GlobalSettings {
...
/// Track the interactions between `upgrade` and `upgrade-package` across the `fyn pip` CLI and a
/// configuration file.
#[test]
#[cfg_attr(
    windows,
...
fn build_isolation_override() -> anyhow::Result<()> {
    ...
    // Write a `fyn.toml` file to disable build isolation.
    let uv_toml = context.temp_dir.child("fyn.toml");
