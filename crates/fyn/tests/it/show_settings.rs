...
fn resolve_both() -> anyhow::Result<()> {
    ...
    warning: Found both a `fyn.toml` file and a `[tool.fyn]` section in an adjacent `pyproject.toml`. The following fields from `[tool.fyn]` will be ignored in favor of the `fyn.toml` file:
    - offline
    - pip
    "#
    );
