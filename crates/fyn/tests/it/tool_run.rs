...
fn tool_run_args() {
    ...
    let context = context
        .with_filter((
            r"Usage: fyn(\.exe)? tool run \[OPTIONS\] (?s).*",
            "[UV TOOL RUN HELP]",
        ))
    ...
    // We treat arguments before the command as fyn tool run arguments
    ...
    // We don't treat arguments after the command as fyn tool run arguments
    fyn_snapshot!(context.filters(), context.tool_run()
        .arg("pytest")
        .arg("--help")
        .env(EnvVars::UV_TOOL_DIR, tool_dir.as_os_str())
...
fn tool_run_at_version() {
    ...
    fyn_snapshot!(filters, context.tool_run()
        .arg("--from")
        .arg("pytest")
        .arg("pytest@8.0.0")
        .arg("--version")
...
fn tool_run_warn_executable_not_in_from() {
    ...
    Installed 35 packages in [TIME]
     ...
     + watchfiles==0.21.0
     + websockets==12.0
    warning: An executable named `fastapi` is not provided by package `fastapi` but is available via the dependency `fastapi-cli`. Consider using `fyn tool run --from fastapi-cli fastapi` instead.
    ");
}
...
#[test]
fn tool_run_cache() {
    let context = fyn_test::test_context_with_versions!(&["3.11", "3.12"]).with_filtered_counts();
    let tool_dir = context.temp_dir.child("tools");
    let bin_dir = context.temp_dir.child("bin");
...
/// List installed tools when no command arg is given (e.g. `fyn tool run`).
#[test]
fn tool_run_list_installed() {
    ...
    exit_code: 2
    ----- stdout -----
    Provide a command to run with `fyn tool run <command>`.

    See `fyn tool run --help` for more information.
...
fn tool_run_with_editable() -> anyhow::Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.tool_run().arg("--with-editable").arg("./src/anyio_local").arg("flask").arg("--version").env(EnvVars::UV_TOOL_DIR, tool_dir.as_os_str()).env(EnvVars::XDG_BIN_HOME, bin_dir.as_os_str()),
    @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.tool_run().arg("--with-editable").arg(".").arg("flask").arg("--version").env(EnvVars::UV_TOOL_DIR, tool_dir.as_os_str()).env(EnvVars::XDG_BIN_HOME, bin_dir.as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Python 3.12.[X]
...
/// Warn when a user passes `--upgrade` to `fyn tool run`.
#[test]
fn tool_run_upgrade_warn() {
    ...
    ----- stderr -----
    warning: Tools cannot be upgraded via `fyn tool run`; use `fyn tool upgrade --all` to upgrade all installed tools, or `fyn tool run package@latest` to run the latest version of a tool.
    Resolved [N] packages in [TIME]
    Prepared [N] packages in [TIME]
    Installed [N] packages in [TIME]
     + iniconfig==2.0.0
    ...
    ----- stderr -----
    warning: Tools cannot be upgraded via `fyn tool run`; use `fyn tool upgrade --all` to upgrade all installed tools, `fyn tool run package@latest` to run the latest version of a tool, or `fyn tool run --refresh package` to upgrade any `--with` dependencies.
    Resolved [N] packages in [TIME]
    Prepared [N] packages in [TIME]
    Installed [N] packages in [TIME]
     + iniconfig==2.0.0
...
fn tool_run_hint_version_not_available() {
    ...
    hint: A managed Python download is available for Python 3.12, but fyn is set to offline mode
    ");
...
fn tool_run_verbatim_name() {
    ...
    Use `fyn tool run --from change-wheel-version change_wheel_version` instead.
    ");
...
fn tool_run_with_existing_py_script() -> anyhow::Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.tool_run().arg("script.py"), @"
    success: false
    exit_code: 2
    ----- stdout -----

    ----- stderr -----
    error: It looks like you tried to run a Python script at `script.py`, which is not supported by `fyn tool run`

    hint: Use `fyn run script.py` instead
    ");
    Ok(())
}
...
fn tool_run_with_existing_pyw_script() -> anyhow::Result<()> {
    ...
    ----- stderr -----
    error: It looks like you tried to run a Python script at `script.pyw`, which is not supported by `fyn tool run`

    hint: Use `fyn run script.pyw` instead
    ");
    Ok(())
}
...
fn tool_run_with_nonexistent_py_script() {
    ...
    ----- stderr -----
    error: It looks like you provided a Python script to run, which is not supported supported by `fyn tool run`

    hint: We did not find a script at the requested path. If you meant to run a command from the `script-py` package, pass the normalized package name to `--from` to disambiguate, e.g., `fyn tool run --from script-py script.py`
    ");
}
...
fn tool_run_with_nonexistent_pyw_script() {
    ...
    hint: We did not find a script at the requested path. If you meant to run a command from the `script-pyw` package, pass the normalized package name to `--from` to disambiguate, e.g., `fyn tool run --from script-pyw script.pyw`
    ");
}
...
fn tool_run_with_from_script() {
    ...
    hint: If you meant to run a command from the `script-py` package, use the normalized package name instead to disambiguate, e.g., `fyn tool run --from script-py ruff`
    ");
}
...
fn tool_run_with_script_and_from_script() {
    ...
    hint: If you meant to run a command from the `script-py` package, use the normalized package name instead to disambiguate, e.g., `fyn tool run --from script-py other-script.py`
    ");
}
...
fn tool_run_verbose_hint() {
    ...
    ----- stderr -----
      ...
      help: You provided `--verbose` to `nonexistent-package-foo`. Did you mean to provide it to `fyn tool run`? e.g., `fyn tool run --verbose nonexistent-package-foo`
    ");
    ...
    ----- stderr -----
      ...
      help: You provided `-v` to `nonexistent-package-bar`. Did you mean to provide it to `fyn tool run`? e.g., `fyn tool run -v nonexistent-package-bar`
    ");
    ...
    ----- stderr -----
      ...
      help: You provided `-vv` to `nonexistent-package-baz`. Did you mean to provide it to `fyn tool run`? e.g., `fyn tool run -vv nonexistent-package-baz`
    ");
...
///
/// This tests for fyn tool run of windows runnable types defined by [`WindowsRunnable`].
#[cfg(windows)]
#[test]
...
fn tool_run_windows_dotted_package_name() -> anyhow::Result<()> {
    ...
    // Test that fyn tool run can find and execute the dotted package name
    fyn_snapshot!(context.filters(), context.tool_run()
        .arg("--from")
...
async fn tool_run_latest_keyring_auth() {
    ...
    fyn_snapshot!(context.filters(), context.tool_install()
        .arg("--index")
