...
fn workspace_dir_simple() {
    ...
    fyn_snapshot!(context.filters(), context.workspace_dir().current_dir(&workspace), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/foo
...
fn workspace_dir_specific_package() {
    ...
    // with --package bar
    fyn_snapshot!(context.filters(), context.workspace_dir().arg("--package").arg("bar").current_dir(&workspace), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/foo/bar
...
fn workspace_metadata_from_member() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.workspace_dir().current_dir(&member_dir), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/workspace
...
fn workspace_metadata_no_project() {
    ...
    fyn_snapshot!(context.filters(), context.workspace_dir(), @"
    success: false
    exit_code: 2
    ----- stdout -----
