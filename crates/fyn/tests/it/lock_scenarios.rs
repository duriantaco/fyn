...
use fyn_test::{fyn_snapshot, packse_index_url};
...
fn wrong_backtracking_basic() -> Result<()> {
    ...
    // Assert the idempotence of `fyn lock` when resolving from the lockfile (`--locked`).
    context
        .lock()
        .arg("--locked")
...
///
/// In particular, this serves as a regression test. fyn used to create a
...
/// clearly wrong.
///
/// The issue was that fyn was intersecting *all* marker expressions. So
...
///
/// Since there are no other constraints on `a`, this causes fyn to select
