...
use fyn_test::{fyn_snapshot, site_packages_path};
...
// fyn, as otherwise the tests may pick up an already installed fyn
...
fn find_uv_bin_in_ephemeral_environment() -> anyhow::Result<()> {
    ...
    exit_code: 0
    ----- stdout -----
    [CACHE_DIR]/archive-v0/[HASH]/[BIN]/fyn
...
fn find_uv_bin_user_bin() {
    ...
    fs_err::remove_file(if cfg!(unix) {
        context.venv.child("bin").child("fyn")
    } else {
        context.venv.child("Scripts").child("fyn.exe")
    })
    .unwrap();
...
fn find_uv_bin_error_message() {
    let mut context = fyn_test::test_context!("3.12")
        .with_filtered_python_names()
        .with_filtered_virtualenv_bin()
        .with_filtered_exe_suffix()
        .with_filter(user_scheme_bin_filter())
...
fn find_uv_bin_py310() {
    let context = fyn_test::test_context!("3.10")
        .with_filtered_python_names()
        .with_filtered_virtualenv_bin()
        .with_filtered_exe_suffix()
        .with_filter(user_scheme_bin_filter())
