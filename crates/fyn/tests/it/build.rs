...
use fyn_test::{DEFAULT_PYTHON_VERSION, apply_filters, fyn_snapshot};
use indoc::indoc;
...
fn build_basic() -> Result<()> {
    ...
    fyn_snapshot!(&filters, context.build().arg("project"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().current_dir(project.path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build(), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("--out-dir").arg("out").current_dir(project.path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_sdist() -> Result<()> {
    ...
    // Build the specified path.
    fyn_snapshot!(&filters, context.build().arg("--sdist").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_wheel() -> Result<()> {
    ...
    // Build the specified path.
    fyn_snapshot!(&filters, context.build().arg("--wheel").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_sdist_wheel() -> Result<()> {
    ...
    // Build the specified path.
    fyn_snapshot!(&filters, context.build().arg("--sdist").arg("--wheel").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_wheel_from_sdist() -> Result<()> {
    ...
    fyn_snapshot!(&filters, context.build().arg("./dist/project-0.1.0.tar.gz").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("./dist/project-0.1.0.tar.gz").arg("--sdist").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("./dist/project-0.1.0.tar.gz").arg("--wheel").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("./dist/project-0.1.0-py3-none-any.whl").arg("--wheel").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_fail() -> Result<()> {
    ...
    // Build the specified path.
    fyn_snapshot!(&filters, context.build().arg("project"), @r#"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_workspace() -> Result<()> {
    ...
    // Build the member.
    fyn_snapshot!(&filters, context.build().arg("--package").arg("member").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Build all packages.
    fyn_snapshot!(&filters, context.build().arg("--all").arg("--no-build-logs").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("./project").arg("--package").arg("member"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // If a source is provided, discover the workspace from the source.
    fyn_snapshot!(&filters, context.build().arg("./project").arg("--all").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("--package").arg("member"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("--all"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(&filters, context.build().arg("--package").arg("fail").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_constraints() -> Result<()> {
    ...
    fyn_snapshot!(&filters, context.build().arg("--build-constraint").arg("constraints.txt").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
///
/// `fyn build --all` should respect `tool.fyn.build-constraint-dependencies` declared at the
/// workspace root.
#[test]
fn build_all_respects_workspace_build_constraint_dependencies() -> Result<()> {
    ...
    assert!(
        ...
        "expected `fyn build --all` to fail when workspace build constraints make `hatchling` \
...
/// Limitation: when `fyn build` is invoked with an explicit source path, configuration is still
...
fn build_sha() -> Result<()> {
    ...
    fyn_snapshot!(&filters, context.build().arg("--build-constraint").arg("constraints.txt").arg("--require-hashes").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_quiet() -> Result<()> {
    ...
    fyn_snapshot!(&context.filters(), context.build().arg("project").arg("-q"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_no_build_logs() -> Result<()> {
    ...
    fyn_snapshot!(&context.filters(), context.build().arg("project").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_hide_build_output_env_var() -> Result<()> {
    ...
    fyn_snapshot!(&context.filters(), context.build().arg("project").env(EnvVars::UV_HIDE_BUILD_OUTPUT, "1"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_hide_build_output_on_failure() -> Result<()> {
    ...
    fyn_snapshot!(&filters, context.build().arg("project").env(EnvVars::UV_HIDE_BUILD_OUTPUT, "1").env("FOO", "bar"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_git_boundary_in_dist_build() -> Result<()> {
    ...
    fyn_snapshot!(&context.filters(), context.build().current_dir(project.path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_non_package() -> Result<()> {
    ...
    // Build the member.
    fyn_snapshot!(&filters, context.build().arg("--package").arg("member").current_dir(&project), @r#"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    // Build all packages.
    fyn_snapshot!(&filters, context.build().arg("--all").arg("--no-build-logs").current_dir(&project), @r#"
    success: false
    exit_code: 2
    ----- stdout -----
...
/// Test the fyn fast path. Tests all four possible build plans:
/// * Defaults
/// * `--sdist`
/// * `--wheel`
/// * `--sdist --wheel`
...
fn build_fast_path() -> Result<()> {
    ...
    ----- stderr -----
    Building source distribution (fyn build backend)...
    Building wheel from source distribution (fyn build backend)...
...
fn build_list_files_errors() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.build()
        .arg(&built_by_uv)
        .arg("--out-dir")
        .arg(context.temp_dir.join("output1"))
        .arg("--list")
    ...
    Usage: fyn build --cache-dir [CACHE_DIR] --out-dir <OUT_DIR> --exclude-newer <EXCLUDE_NEWER> <SRC>

    For more information, try '--help'.
    ");

    // Not a fyn build backend package, we can't list it.
    let anyio_local = current_dir()?.join("../../test/packages/anyio_local");
    // Windows normalization
    let context = context.with_filter(("/crates/fyn/../../", "/"));
    fyn_snapshot!(context.filters(), context.build()
        .arg(&anyio_local)
        .arg("--out-dir")
        .arg(context.temp_dir.join("output2"))
    ...
    ----- stderr -----
      ...
      ╰─▶ Can only use `--list` with a compatible fyn build backend, but `[WORKSPACE]/test/packages/anyio_local` is not compatible because `build_system.build-backend` is not `fyn_build`, but `flit_core.buildapi`
    ");
    Ok(())
}
...
fn build_unconfigured_setuptools() -> Result<()> {
    ...
    // This is not technically a `fyn build` test, we use it to contrast this passing case with the
    // failing cases later.
    ...
    fyn_snapshot!(context.filters(), context.python_command().arg("-c").arg("import greet"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Greetings!
...
/// In a project layout with a virtual root, an easy mistake to make is running `fyn pip install .`
/// in the root.
#[test]
fn build_workspace_virtual_root() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.build().arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    ----- stderr -----
    Building source distribution...
    warning: `[TEMP_DIR]/` appears to be a workspace root without a Python project; consider using `fyn sync` to install the workspace, or add a `[build-system]` table to `pyproject.toml`
    Building wheel from source distribution...
...
fn build_with_nonnormalized_name() -> Result<()> {
    ...
    // Build the specified path.
    fyn_snapshot!(&filters, context.build().arg("--no-build-logs").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn force_pep517() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.build().env(EnvVars::RUST_BACKTRACE, "0"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.build().arg("--force-pep517").env(EnvVars::RUST_BACKTRACE, "0"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn test_workspace_trailing_slash() {
    ...
    fyn_snapshot!(context.filters(), context.build().arg("child"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.build().arg("./child/../child/"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_clear() -> Result<()> {
    ...
    fyn_snapshot!(&context.filters(), context.build().arg("project").arg("--clear").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
/// Test `fyn build --no-create-gitignore`.
#[test]
fn build_no_gitignore() -> Result<()> {
    ...
    fyn_snapshot!(&context.filters(), context.build().arg("project").arg("--no-create-gitignore").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
