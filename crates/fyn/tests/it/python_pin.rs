...
fn python_pin() {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("any"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Request CPython
    fyn_snapshot!(context.filters(), context.python_pin().arg("cpython"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("cpython@3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("cp3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg(&context.python_versions.first().unwrap().1), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    #[cfg(unix)]
    {
        fyn_snapshot!(context.filters(), context.python_pin().arg("pypy"), @"
        success: true
        exit_code: 0
        ----- stdout -----
    ...
    #[cfg(unix)]
    {
        fyn_snapshot!(context.filters(), context.python_pin().arg("3.7"), @"
        success: true
        exit_code: 0
        ----- stdout -----
...
fn python_pin_global_if_no_local() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("3.11").arg("--global"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn python_pin_global_use_local_if_available() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("--global"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    3.12
...
fn python_pin_compatible_with_requires_python() -> Result<()> {
    let context =
        fyn_test::test_context_with_versions!(&["3.10", "3.11"]).with_filtered_python_sources();
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("3.10"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("cpython@3.10"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("cpython@3.10").arg("--no-project"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("cpython@3.10").arg("--no-workspace"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Pinned `.python-version` to `cpython@3.10`
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg(">3.8,<3.11"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("3.13t"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("cpython@3.11"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Updating `requires-python` should affect `fyn python pin` compatibilities.
    pyproject_toml.write_str(
        r#"
        [project]
        name = "project"
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg(">3.8,<3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn warning_pinned_python_version_not_installed() -> Result<()> {
    ...
    if cfg!(windows) {
        fyn_snapshot!(context.filters(), context.python_pin(), @r###"
        success: true
        exit_code: 0
        ----- stdout -----
        3.12
...
fn python_pin_resolve_no_python() {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved").arg("3.12"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn python_pin_resolve() {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved").arg("any"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Request Python 3.13
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved").arg("3.13"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Request CPython
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved").arg("cpython"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved").arg("cpython@3.13"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Updated `.python-version` from `[PYTHON-3.12]` -> `[PYTHON-3.13]`
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved")
    ...
    // (skip on Windows because the snapshot is different and the behavior is not platform dependent)
    #[cfg(unix)]
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved").arg("pypy"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    // (skip on Windows because the snapshot is different and the behavior is not platform dependent)
    #[cfg(unix)]
    fyn_snapshot!(context.filters(), context.python_pin().arg("--resolved").arg("3.7"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn python_pin_install() {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("3.12").env(EnvVars::UV_PYTHON_DOWNLOADS, "auto"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Pinned `.python-version` to `3.12`
...
fn python_pin_rm() {
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("--rm"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fyn_snapshot!(context.filters(), context.python_pin().arg("--rm").arg("--global"), @"
    success: false
    exit_code: 2
    ----- stdout -----
