...
fn no_package() {
    ...
    fyn_snapshot!(context.filters(), context.pip_tree(), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn prune_last_in_the_subgroup() {
    ...
    fyn_snapshot!(context.filters(), context.pip_tree().arg("--prune").arg("certifi"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    requests v2.31.0
...
fn reverse() {
    ...
    fyn_snapshot!(context.filters(), context.pip_tree().arg("--reverse"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    blinker v1.7.0
...
fn invert() {
    ...
    fyn_snapshot!(context.filters(), context.pip_tree().arg("--invert"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    blinker v1.7.0
...
fn no_dedupe_and_invert() {
    ...
    fyn_snapshot!(context.filters(), context.pip_tree().arg("--no-dedupe").arg("--invert"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    six v1.16.0
...
fn with_editable() {
    ...
    fyn_snapshot!(filters, context.pip_tree(), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn show_version_specifiers_simple() {
    ...
    fyn_snapshot!(context.filters(), context.pip_tree().arg("--show-version-specifiers"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    requests v2.31.0
...
fn print_output_even_with_quite_flag() {
    ...
    context.assert_command("import requests").success();
    fyn_snapshot!(context.filters(), context.pip_tree().arg("--quiet"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn no_duplicate_dependencies_with_markers() {
    ...
    let context = fyn_test::test_context_with_versions!(&["3.12", "3.13"]).with_filtered_counts();
