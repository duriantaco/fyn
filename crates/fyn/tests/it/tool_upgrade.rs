...
fn tool_upgrade_pinned_hint() {
    ...
    hint: `babel` is pinned to `2.6.0` (installed with an exact version pin); reinstall with `fyn tool install babel@latest` to upgrade to a new version.
    ");
}
...
fn tool_upgrade_non_existing_package() {
    ...
    error: Failed to upgrade black
      Caused by: `black` is not installed; run `fyn tool install black` to install
    ");
...
fn tool_upgrade_not_stop_if_upgrade_fails() -> anyhow::Result<()> {
    ...
    error: Failed to upgrade python-dotenv
      Caused by: `python-dotenv` is missing a valid receipt; run `fyn tool install --force python-dotenv` to reinstall
    ");
...
fn tool_upgrade_constraint() {
    ...
    ----- stderr -----
    warning: `--upgrade-package` is enabled by default on `fyn tool upgrade`
    ...
    ----- stderr -----
    warning: `--upgrade` is enabled by default on `fyn tool upgrade`
    Nothing to upgrade
    ");
}
