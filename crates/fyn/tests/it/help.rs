...
fn help() {
    ...
    // The `fyn help` command should show the long help message
    fyn_snapshot!(context.filters(), context.help(), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    Python options:
          --managed-python       Require use of fyn-managed Python versions [env: UV_MANAGED_PYTHON=]
          --no-managed-python    Disable use of fyn-managed Python versions [env: UV_NO_MANAGED_PYTHON=]
    ...
    Global options:
      ...
      -v, --verbose...
          ...
          --config-file <CONFIG_FILE>
              The path to a `fyn.toml` file to use for configuration [env: UV_CONFIG_FILE=]
          --no-config
              Avoid discovering configuration files (`pyproject.toml`, `fyn.toml`) [env: UV_NO_CONFIG=]
      -h, --help
              Display the concise help for this command
      -V, --version
              Display the fyn version

    Use `fyn help <command>` for more information on a specific command.
...
fn help_flag() {
    ...
    fyn_snapshot!(context.filters(), context.command().arg("--help"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    An extremely fast Python package manager.
...
fn help_short_flag() {
    ...
    fyn_snapshot!(context.filters(), context.command().arg("-h"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    An extremely fast Python package manager.
...
fn help_subcommand() {
    ...
    fyn_snapshot!(context.filters(), context.help().arg("python"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    Usage: fyn python [OPTIONS] <COMMAND>
    ...
    Cache options:
      -n, --no-cache
          ...
          --cache-dir [CACHE_DIR]
              ...
              Defaults to `$XDG_CACHE_HOME/fyn` or `$HOME/.cache/fyn` on macOS and Linux, and
    ...
    Python options:
          --managed-python
              ...
              By default, fyn prefers using Python versions it manages. However, it will use system
              Python versions if a fyn-managed Python is not installed. This option disables use of
    ...
    Global options:
      -q, --quiet...
              ...
              Repeating this option, e.g., `-qq`, will enable a silent mode in which fyn will write no
              output to stdout.
      ...
      -v, --verbose...
          ...
          --config-file <CONFIG_FILE>
              ...
              While fyn configuration can be included in a `pyproject.toml` file, it is not allowed in
              this context.
    ...
    Use `fyn help python <command>` for more information on a specific command.
...
fn help_subsubcommand() {
    ...
    fyn_snapshot!(context.filters(), context.help().env_remove(EnvVars::UV_PYTHON_INSTALL_DIR).arg("python").arg("install"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    available Python versions are bundled with each fyn release. To install new Python versions, you may
    need upgrade fyn.

    Python versions are installed into the fyn Python directory, which can be retrieved with `fyn python
    dir`.

    By default, Python executables are added to a directory on the path with a minor version suffix,
    e.g., `python3.13`. To install `python3` and `python`, use the `--default` flag. Use `fyn python dir
    ...
    Usage: fyn python install [OPTIONS] [TARGETS]...
    ...
    Options:
      -i, --install-dir <INSTALL_DIR>
              ...
              If provided, `UV_PYTHON_INSTALL_DIR` will need to be set for subsequent operations for fyn
      ...
      -U, --upgrade
              ...
              This option is only supported for minor version requests, e.g., `3.12`; fyn will exit with
...
fn help_flag_subcommand() {
    ...
    fyn_snapshot!(context.filters(), context.command().arg("python").arg("--help"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    Manage Python versions and installations
    ...
    Use `fyn help python` for more details.
...
fn help_flag_subsubcommand() {
    ...
    fyn_snapshot!(context.filters(), context.command().arg("python").arg("install").arg("--help"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    Download and install Python versions
...
fn help_unknown_subcommand() {
    ...
    fyn_snapshot!(context.filters(), context.help().arg("foobar"), @"
    success: false
    exit_code: 2
    ----- stdout -----

    ----- stderr -----
    error: There is no command `foobar` for `fyn`. Did you mean one of:
        auth
        run
        init
        add
    ...
    fyn_snapshot!(context.filters(), context.help().arg("foo").arg("bar"), @"
    success: false
    exit_code: 2
    ----- stdout -----

    ----- stderr -----
    error: There is no command `foo bar` for `fyn`. Did you mean one of:
        auth
        run
        init
        add
...
fn help_unknown_subsubcommand() {
    ...
    fyn_snapshot!(context.filters(), context.help().arg("python").arg("foobar"), @"
    success: false
    exit_code: 2
    ----- stdout -----

    ----- stderr -----
    error: There is no command `foobar` for `fyn python`. Did you mean one of:
        list
        install
        upgrade
        find
...
fn help_with_global_option() {
    ...
    fyn_snapshot!(context.filters(), context.help().arg("--no-cache"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    An extremely fast Python package manager.
...
fn help_with_help() {
    ...
    fyn_snapshot!(context.filters(), context.help().arg("--help"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Display documentation for a command

    Usage: fyn help [OPTIONS] [COMMAND]...
...
fn help_with_version() {
    ...
    fyn_snapshot!(context.filters(), context.help().arg("--version"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    Usage: fyn help --verbose... [COMMAND]...
...
fn help_with_no_pager() {
    ...
    fyn_snapshot!(context.filters(), context.help().arg("--no-pager"), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    An extremely fast Python package manager.
