...
use fyn_test::{diff_snapshot, fyn_snapshot};
...
fn packse_add_remove_one_package() {
    ...
    let new_lock = context.read("fyn.lock");
...
fn jax_instability() -> Result<()> {
    ...
    // Back to where we started.
    //
    // Note that this is wrong! This demonstrates that `fyn` sometimes does
