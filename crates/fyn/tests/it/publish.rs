...
fn skip_existing_redirect() {
    ...
    ----- stderr -----
    error: `fyn publish` does not support `--skip-existing` because there is not a reliable way to identify when an upload fails due to an existing distribution. Instead, use `--check-url` to provide the URL to the simple API for your index. fyn will check the index for existing distributions before attempting uploads.
    "
    );
}
...
async fn check_keyring_behaviours() {
    ...
    fyn_snapshot!(context.filters(), context.publish()
        ...
        .arg(&publish_url)
        .arg(dummy_wheel())
        .env(EnvVars::KEYRING_TEST_CREDENTIALS, format!(r#"{{"{publish_url}": {{"dummy": "dummy"}}}}"#))
        .env(EnvVars::PATH, venv_bin_path(&context.venv)), @"
    success: false
    exit_code: 2
    ----- stdout -----
