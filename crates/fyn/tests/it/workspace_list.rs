...
fn workspace_list_simple() {
    ...
    fyn_snapshot!(context.filters(), context.workspace_list().current_dir(&workspace), @"
    success: true
    exit_code: 0
    ----- stdout -----
    foo
    ...
    fyn_snapshot!(context.filters(), context.workspace_list().arg("--paths").current_dir(&workspace), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/foo
...
fn workspace_list_from_member() -> Result<()> {
    ...
    fyn_snapshot!(context.filters(), context.workspace_list().current_dir(&member_dir), @"
    success: true
    exit_code: 0
    ----- stdout -----
    albatross
...
fn workspace_list_multiple_members() {
    ...
    fyn_snapshot!(context.filters(), context.workspace_list().current_dir(&workspace_root), @"
    success: true
    exit_code: 0
    ----- stdout -----
    pkg-a
    ...
    fyn_snapshot!(context.filters(), context.workspace_list().arg("--paths").current_dir(&workspace_root), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/pkg-a
...
fn workspace_list_single_project() {
    ...
    fyn_snapshot!(context.filters(), context.workspace_list().current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
    my-project
...
fn workspace_list_no_project() {
    ...
    fyn_snapshot!(context.filters(), context.workspace_list(), @"
    success: false
    exit_code: 2
    ----- stdout -----
