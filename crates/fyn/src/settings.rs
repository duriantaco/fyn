...
use fyn_pep508::{MarkerTree, RequirementOrigin};
use fyn_preview::Preview;
use fyn_pypi_types::SupportedEnvironments;
use fyn_python::{Prefix, PythonDownloads, PythonPreference, PythonVersion, Target};
...
use fyn_settings::{
    AuditOptions, Combine, EnvironmentOptions, FilesystemOptions, MalwareCheckSettings, Options,
...
use fyn_workspace::pyproject::{DependencyType, ExtraBuildDependencies};
    ...
    pub(crate) fn resolve(
        ...
        if installer.upgrade || !installer.upgrade_package.is_empty() {
            if with.is_empty() && with_requirements.is_empty() {
                warn_user_once!(
                    "Tools cannot be upgraded via `{invocation_source}`; use `fyn tool upgrade --all` to upgrade all installed tools, or `{invocation_source} package@latest` to run the latest version of a tool."
                );
            } else {
                warn_user_once!(
                    "Tools cannot be upgraded via `{invocation_source}`; use `fyn tool upgrade --all` to upgrade all installed tools, `{invocation_source} package@latest` to run the latest version of a tool, or `{invocation_source} --refresh package` to upgrade any `--with` dependencies."
                );
            }
        }
        ...
        if installer.reinstall || !installer.reinstall_package.is_empty() {
            if with.is_empty() && with_requirements.is_empty() {
                warn_user_once!(
                    "Tools cannot be reinstalled via `{invocation_source}`; use `fyn tool upgrade --all --reinstall` to reinstall all installed tools, `{invocation_source} package@latest` to run the latest version of a tool, or `fyn cache prune` to clear any cached tool environments."
                );
            } else {
                warn_user_once!(
                    "Tools cannot be reinstalled via `{invocation_source}`; use `fyn tool upgrade --all --reinstall` to reinstall all installed tools, `{invocation_source} package@latest` to run the latest version of a tool, `{invocation_source} --refresh package` to reinstall any `--with` dependencies, or `fyn cache prune` to clear any cached tool environments."
                );
            }
        }
...
impl ToolUpgradeSettings {
    ...
    pub(crate) fn resolve(
        ...
        if upgrade {
            warn_user_once!("`--upgrade` is enabled by default on `fyn tool upgrade`");
        }
        if !upgrade_package.is_empty() {
            warn_user_once!("`--upgrade-package` is enabled by default on `fyn tool upgrade`");
        }
...
impl LockDiffArgSources {
    ...
    fn retain_resolver_args(&self, args: fyn_cli::ResolverArgs) -> fyn_cli::ResolverArgs {
        ...
        fyn_cli::ResolverArgs {
            ...
            upgrade_packages: self.retain("upgrade_packages", upgrade_packages),
...
/// The resolved settings to use for an invocation of the fyn CLI when installing dependencies.
///
...
/// The resolved settings to use for an invocation of the fyn CLI when resolving dependencies.
///
/// Combines the `[tool.fyn]` persistent configuration with the command-line arguments
...
/// The resolved settings to use for an invocation of the fyn CLI with both resolver and installer
/// capabilities.
///
/// Represents the shared settings that are used across all fyn commands outside the `pip` API.
/// Analogous to the settings contained in the `[tool.fyn]` table, combined with [`ResolverInstallerArgs`].
#[derive(Debug, Clone, Default)]
...
/// settings contained in the `[tool.fyn.pip]` table.
#[derive(Debug, Clone)]
...
impl PipSettings {
    ...
    pub(crate) fn combine(
        ...
        // Merge the top-level options (`tool.fyn`) with the pip-specific options (`tool.fyn.pip`),
        // preferring the latter.
        //
        // For example, prefer `tool.fyn.pip.index-url` over `tool.fyn.index-url`.
...
/// The resolved settings to use for an invocation of the `fyn publish` CLI.
#[derive(Debug, Clone)]
...
/// The resolved settings to use for an invocation of the `fyn auth logout` CLI.
#[derive(Debug, Clone)]
...
/// The resolved settings to use for an invocation of the `fyn auth token` CLI.
#[derive(Debug, Clone)]
...
/// The resolved settings to use for an invocation of the `fyn auth set` CLI.
#[derive(Debug, Clone)]
