...
fn exec_spawn(cmd: &mut Command) -> std::io::Result<Infallible> {
    ...
    #[cfg(windows)]
    {
        fyn_windows::spawn_child(cmd, false)
    }
}

/// Assuming the binary is called something like `fynx@1.2.3(.exe)`, compute the `@1.2.3(.exe)` part
/// so that we can preferentially find `fyn@1.2.3(.exe)`, for folks who like managing multiple
/// installs in this way.
fn get_fynx_suffix(current_exe: &Path) -> Option<&str> {
    let os_file_name = current_exe.file_name()?;
    let file_name_str = os_file_name.to_str()?;
    file_name_str.strip_prefix("fynx")
}

/// Gets the path to `fyn`, given info about `fynx`
fn get_fyn_path(current_exe_parent: &Path, fynx_suffix: Option<&str>) -> std::io::Result<PathBuf> {
    // First try to find a matching suffixed `fyn`, e.g. `fyn@1.2.3(.exe)`
    let fyn_with_suffix = fynx_suffix.map(|suffix| current_exe_parent.join(format!("fyn{suffix}")));
...
fn run() -> std::io::Result<ExitStatus> {
    ...
    let Some(bin) = current_exe.parent() else {
        return Err(std::io::Error::new(
            ...
            "Could not determine the location of the `fynx` binary",
        ));
    };
    let fynx_suffix = get_fynx_suffix(&current_exe);
    let fyn = get_fyn_path(bin, fynx_suffix)?;
    let args = ["tool", "fynx"]
        .iter()
        .map(OsString::from)
        // Skip the `fynx` name
