...
use fyn_fs::{Simplified, relative_to};
...
use fyn_requirements::RequirementsSource;
use fyn_resolver::{ExcludeNewer, FlatIndex};
use fyn_settings::PythonInstallMirrors;
use fyn_types::{AnyErrorBuild, BuildContext, BuildStack, HashStrategy};
use fyn_workspace::pyproject::ExtraBuildDependencies;
use fyn_workspace::{DiscoveryOptions, Workspace, WorkspaceCache, WorkspaceError};
...
enum Error {
    ...
    Io(#[from] io::Error),
    #[error(transparent)]
    FindOrDownloadPython(#[from] fyn_python::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    BuildBackend(#[from] fyn_build_backend::Error),
    #[error(transparent)]
    BuildDispatch(AnyErrorBuild),
    #[error(transparent)]
    BuildFrontend(#[from] fyn_build_frontend::Error),
    #[error(transparent)]
    ...
    ListForcePep517,
    #[error(
        "Can only use `--list` with a compatible fyn build backend, but `{name}` is not compatible because {reason}"
    )]
    ...
    InvalidSourceDistExt(String, fyn_distribution_filename::ExtensionError),
    #[error("The built source distribution has an invalid filename")]
    InvalidBuiltSourceDistFilename(#[source] fyn_distribution_filename::SourceDistFilenameError),
    ...
    for (source, result) in results {
        match result {
            ...
            Err(err) => {
                ...
                let help = if let Error::Extract(fyn_extract::Error::Tar(err)) = &err {
...
async fn build_package(
    ...
    // Check if the build backend is matching fyn version that allows calling in the fyn build backend
    // directly.
    let build_action = if list {
        ...
        if let Err(reason) = check_direct_build(source.path(), fyn_version::version()) {
        ...
        match check_direct_build(source.path(), fyn_version::version()) {
            Ok(()) => BuildAction::DirectBuild,
            Err(reason) => {
                debug!(
                    "Not using `fyn_build` direct build for `{}` because {}",
    ...
    let build_output = match printer {
        Printer::Default | Printer::NoProgress | Printer::Verbose => {
            if build_logs && !fyn_flags::contains(fyn_flags::EnvironmentFlags::HIDE_BUILD_OUTPUT) {
                BuildOutput::Stderr
            } else {
                BuildOutput::Quiet
            }
    ...
    match plan {
        BuildPlan::SdistToWheel => {
            ...
            fyn_extract::stream::archive(path.display(), reader, ext, temp_dir.path()).await?;
        ...
        BuildPlan::WheelFromSdist => {
            ...
            fyn_extract::stream::archive(source.path().display(), reader, ext, temp_dir.path())
                .await?;
...
impl BuildAction {
    ...
    fn force_build(self) -> Self {
        match self {
            // List is only available for the fyn build backend
    ...
    let build_result = match action {
        BuildAction::List => {
            ...
            let (filename, file_list) = tokio::task::spawn_blocking(move || {
                fyn_build_backend::list_source_dist(
                    &source_tree_,
                    fyn_version::version(),
                    sources_enabled,
                )
        ...
        BuildAction::DirectBuild => {
            ...
            let filename = tokio::task::spawn_blocking(move || {
                fyn_build_backend::build_source_dist(
                    &source_tree,
                    &output_dir_,
                    fyn_version::version(),
                    sources_enabled,
