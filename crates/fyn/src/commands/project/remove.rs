...
use fyn_normalize::{DEV_DEPENDENCIES, DefaultExtras, DefaultGroups};
...
pub(crate) async fn remove(
    ...
    let target = if let Some(script) = script {
        ...
        // No workspace caching since `fyn remove` changes the workspace definition.
        ...
        match dependency_type {
            ...
            DependencyType::Dev => {
                ...
                if dev_deps.is_empty() && group_deps.is_empty() {
                    show_other_dependency_type_hint(printer, &package, &toml)?;
                    anyhow::bail!(
                        "The dependency `{package}` could not be found in `tool.fyn.dev-dependencies` or `tool.fyn.dependency-groups.dev`"
                    );
                }
            }
    ...
    for dep_ty in pyproject.find_dependency(name, None) {
        match dep_ty {
            ...
            DependencyType::Dev => writeln!(
                ...
                "hint".bold().cyan(),
                ":".bold(),
                format!("fyn remove {name} --dev`").bold()
            )?,
            DependencyType::Optional(group) => writeln!(
                ...
                "hint".bold().cyan(),
                ":".bold(),
                format!("fyn remove {name} --optional {group}").bold()
            )?,
            DependencyType::Group(group) => writeln!(
                ...
                "hint".bold().cyan(),
                ":".bold(),
                format!("fyn remove {name} --group {group}").bold()
            )?,
        }
    }
