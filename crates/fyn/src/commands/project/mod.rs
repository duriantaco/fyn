...
use fyn_distribution::{DistributionDatabase, LoweredExtraBuildDependencies, LoweredRequirement};
...
use fyn_fs::{CWD, LockedFile, LockedFileError, LockedFileMode, Simplified};
use fyn_git::ResolvedRepositoryReference;
...
use fyn_requirements::{NamedRequirementsResolver, RequirementsSpecification};
use fyn_resolver::{
    FlatIndex, Installable, Lock, OptionsBuilder, Preference, PythonRequirement,
    ResolverEnvironment, ResolverOutput,
};
use fyn_scripts::Pep723ItemRef;
use fyn_settings::PythonInstallMirrors;
use fyn_static::EnvVars;
use fyn_torch::{TorchSource, TorchStrategy};
use fyn_types::{BuildIsolation, EmptyInstalledPackages, HashStrategy};
use fyn_virtualenv::remove_virtualenv;
use fyn_warnings::{warn_user, warn_user_once};
use fyn_workspace::dependency_groups::DependencyGroupError;
use fyn_workspace::pyproject::{ExtraBuildDependency, PyProjectToml};
use fyn_workspace::{RequiresPythonSources, Workspace, WorkspaceCache};
...
pub(crate) enum ProjectError {
    #[error(
        "The lockfile at `fyn.lock` needs to be updated, but `{2}` was provided. To update the lockfile, run `fyn lock`."
    )]
    ...
    #[error(
        "The lockfile at `fyn.lock` uses an unsupported schema version (v{1}, but only v{0} is supported). Downgrade to a compatible fyn version, or remove the `fyn.lock` prior to running `fyn lock` or `fyn sync`."
    )]
    UnsupportedLockVersion(u32, u32),

    #[error(
        "Failed to parse `fyn.lock`, which uses an unsupported schema version (v{1}, but only v{0} is supported). Downgrade to a compatible fyn version, or remove the `fyn.lock` prior to running `fyn lock` or `fyn sync`."
    )]
    UnparsableLockVersion(u32, u32, #[source] toml::de::Error),

    #[error("Failed to serialize `fyn.lock`")]
    ...
    #[error(
        "Default group `{0}` (from `tool.fyn.default-groups`) is not defined in the project's `dependency-groups` table"
    )]
    ...
    #[error(transparent)]
    Python(#[from] fyn_python::Error),

    #[error(transparent)]
    Virtualenv(#[from] fyn_virtualenv::Error),
    ...
    #[error(transparent)]
    Lock(#[from] fyn_resolver::LockError),

    #[error(transparent)]
    Operation(#[from] pip::operations::Error),

    #[error(transparent)]
    Interpreter(#[from] fyn_python::InterpreterError),

    #[error(transparent)]
    Tool(#[from] fyn_tool::Error),

    #[error(transparent)]
    Name(#[from] fyn_normalize::InvalidNameError),

    #[error(transparent)]
    Requirements(#[from] fyn_requirements::Error),

    #[error(transparent)]
    Metadata(#[from] fyn_distribution::MetadataError),

    #[error(transparent)]
    Lowering(#[from] fyn_distribution::LoweringError),

    #[error(transparent)]
    Workspace(#[from] fyn_workspace::WorkspaceError),

    #[error(transparent)]
    PyprojectMut(#[from] fyn_workspace::pyproject_mut::Error),
    ...
    #[error(transparent)]
    CacheInfo(#[from] fyn_cache_info::CacheInfoError),
    ...
    #[error(transparent)]
    Accelerator(#[from] fyn_torch::AcceleratorError),
...
impl ScriptInterpreter {
    ...
    pub(crate) fn root(script: Pep723ItemRef<'_>, active: Option<bool>, cache: &Cache) -> PathBuf {
        ...
        if let Some(from_virtual_env) = from_virtual_env_variable() {
            if !fyn_fs::is_same_file_allow_missing(&from_virtual_env, &cache_env).unwrap_or(false) {
                match active {
                    Some(true) => {
                        debug!(
    ...
    pub(crate) async fn discover(
        ...
        match PythonEnvironment::from_root(&root, cache) {
            ...
            Err(fyn_python::Error::MissingEnvironment(_)) => {}
    ...
    pub(crate) async fn lock(script: Pep723ItemRef<'_>) -> Result<LockedFile, LockedFileError> {
        match script {
            Pep723ItemRef::Script(script) => {
                LockedFile::acquire(
                    std::env::temp_dir().join(format!("fyn-{}.lock", cache_digest(&script.path))),
            ...
            Pep723ItemRef::Remote(.., url) => {
                LockedFile::acquire(
                    std::env::temp_dir().join(format!("fyn-{}.lock", cache_digest(url))),
                    LockedFileMode::Exclusive,
                    url.to_string(),
                )
                .await
            ...
            Pep723ItemRef::Stdin(metadata) => {
                LockedFile::acquire(
                    std::env::temp_dir().join(format!("fyn-{}.lock", cache_digest(&metadata.raw))),
                    LockedFileMode::Exclusive,
                    "stdin".to_string(),
                )
                .await
...
impl ProjectInterpreter {
    ...
    pub(crate) async fn discover(
        ...
        match PythonEnvironment::from_root(&root, cache) {
            ...
            Err(fyn_python::Error::MissingEnvironment(_)) => {}
            Err(fyn_python::Error::InvalidEnvironment(inner)) => {
            ...
            Err(fyn_python::Error::Query(fyn_python::InterpreterError::NotFound(_))) => {}
            Err(fyn_python::Error::Query(fyn_python::InterpreterError::BrokenLink(
                BrokenLink {
                    path,
                    unix,
                    venv: _,
        ...
        .await?
        {
            ...
            ProjectInterpreter::Interpreter(interpreter) => {
                ...
                let replace = match (root.try_exists(), root.join("pyvenv.cfg").try_exists()) {
                    ...
                    (_, Err(err)) | (Err(err), _) => {
                        return Err(ProjectError::InvalidProjectEnvironmentDir(
                            root,
                            format!(
                                "it is not a compatible environment but cannot be recreated because fyn cannot determine if it is a virtual environment: {err}"
                            ),
                        ));
                    }
                };
                ...
                let prompt = workspace
                    ...
                    .map(fyn_virtualenv::Prompt::Static)
                    .unwrap_or(fyn_virtualenv::Prompt::None);

                // Under `--dry-run`, avoid modifying the environment.
                if dry_run.enabled() {
                    let temp_dir = cache.venv_dir()?;
                    let environment = fyn_virtualenv::create_venv(
                        temp_dir.path(),
                        interpreter,
                        prompt,
                        false,
                        fyn_virtualenv::OnExisting::Remove(
                            fyn_virtualenv::RemovalReason::ManagedEnvironment,
                        ),
                        false,
                        false,
                        upgradeable,
                ...
                if replace {
                    match remove_virtualenv(&root) {
                        ...
                        Err(fyn_virtualenv::Error::Io(err))
...
pub(crate) async fn resolve_names(
    ...
    preview: Preview,
    lfs: GitLfsSetting,
) -> Result<Vec<Requirement>, fyn_requirements::Error> {
    ...
    let build_isolation = match build_isolation {
        fyn_configuration::BuildIsolation::Isolate => BuildIsolation::Isolated,
        fyn_configuration::BuildIsolation::Shared => {
            environment = PythonEnvironment::from_interpreter(interpreter.clone());
            BuildIsolation::Shared(&environment)
        }
        fyn_configuration::BuildIsolation::SharedPackage(packages) => {
            environment = PythonEnvironment::from_interpreter(interpreter.clone());
...
pub(crate) async fn sync_environment(
    ...
    let build_isolation = match build_isolation {
        fyn_configuration::BuildIsolation::Isolate => BuildIsolation::Isolated,
        fyn_configuration::BuildIsolation::Shared => BuildIsolation::Shared(&venv),
        fyn_configuration::BuildIsolation::SharedPackage(packages) => {
...
pub(crate) fn default_dependency_groups(
    ...
    if let Some(defaults) = pyproject_toml.tool.as_ref().and_then(|tool| {
        tool.fyn
            .as_ref()
            .and_then(|fyn| fyn.default_groups.as_ref())
    }) {
...
pub(crate) fn script_extra_build_requires(
    ...
    // Collect any `tool.fyn.extra-build-dependencies` from the script.
    ...
    let script_extra_build_dependencies = script
        ...
        .as_ref()
        .and_then(|tool| tool.fyn.as_ref())
        .and_then(|fyn| fyn.extra_build_dependencies.as_ref())
        .unwrap_or(&empty);
...
fn warn_on_requirements_txt_setting(spec: &RequirementsSpecification, settings: &ResolverSettings) {
    ...
    if settings.index_locations.no_index() {
        ...
        warn_user_once!(
            "Ignoring `--no-index` from requirements file. Instead, use the `--no-index` command-line argument, or set `no-index` in a `fyn.toml` or `pyproject.toml` file."
        );
    } else {
        if let Some(index_url) = index_url {
            if settings.index_locations.default_index().map(Index::url) != Some(index_url) {
                warn_user_once!(
                    "Ignoring `--index-url` from requirements file: `{index_url}`. Instead, use the `--index-url` command-line argument, or set `index-url` in a `fyn.toml` or `pyproject.toml` file."
                );
            }
        }
        for extra_index_url in extra_index_urls {
            ...
            {
                warn_user_once!(
                    "Ignoring `--extra-index-url` from requirements file: `{extra_index_url}`. Instead, use the `--extra-index-url` command-line argument, or set `extra-index-url` in a `fyn.toml` or `pyproject.toml` file.`"
                );
            }
        }
        for find_link in find_links {
            ...
            {
                warn_user_once!(
                    "Ignoring `--find-links` from requirements file: `{find_link}`. Instead, use the `--find-links` command-line argument, or set `find-links` in a `fyn.toml` or `pyproject.toml` file.`"
                );
            }
        }
    }
    ...
    if !no_binary.is_none() && settings.build_options.no_binary() != no_binary {
        warn_user_once!(
            "Ignoring `--no-binary` setting from requirements file. Instead, use the `--no-binary` command-line argument, or set `no-binary` in a `fyn.toml` or `pyproject.toml` file."
        );
    }

    if !no_build.is_none() && settings.build_options.no_build() != no_build {
        warn_user_once!(
            "Ignoring `--no-binary` setting from requirements file. Instead, use the `--no-build` command-line argument, or set `no-build` in a `fyn.toml` or `pyproject.toml` file."
        );
    }
}
    ...
    if conflicts.len() == 1 {
        ...
        if let Some(group) = group {
            if workspace_non_trivial {
                return format!(
                    " (from workspace member `{package}`'s `tool.fyn.dependency-groups.{group}.requires-python`)."
                );
            }
            return format!(" (from `tool.fyn.dependency-groups.{group}.requires-python`).");
        }
        if workspace_non_trivial {
