...
use fyn_configuration::{Constraints, DependencyGroupsWithDefaults, ExtrasSpecification};
use fyn_distribution_types::Index;
use fyn_normalize::{ExtraName, PackageName};
use fyn_pypi_types::{DependencyGroupSpecifier, LenientRequirement, VerbatimParsedUrl};
use fyn_resolver::{Installable, Lock, Package};
use fyn_scripts::Pep723Script;
use fyn_workspace::Workspace;
use fyn_workspace::pyproject::{Source, Sources, ToolfynSources};
...
impl<'lock> InstallTarget<'lock> {
    ...
    pub(crate) fn requirements(
        &self,
    ) -> impl Iterator<Item = Cow<'lock, fyn_pep508::Requirement<VerbatimParsedUrl>>> {
        match self {
            Self::Project { workspace, .. }
            | Self::Projects { workspace, .. }
            | Self::Workspace { workspace, .. }
                            ...
                            dependencies
                                ...
                                .filter_map(|requires_dist| {
                                    LenientRequirement::<VerbatimParsedUrl>::from_str(requires_dist)
                                        .map(fyn_pep508::Requirement::from)
                                        .map(Cow::Owned)
                                        .ok()
                                })
