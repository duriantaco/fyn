...
use fyn_workspace::{DiscoveryOptions, MemberDiscovery, VirtualProject, Workspace, WorkspaceCache};
...
pub(crate) async fn sync(
        ...
        if !lockfile.is_file() {
            if frozen.is_some() {
                return Err(anyhow::anyhow!(
                    "`fyn sync --frozen` requires a script lockfile; run `{}` to lock the script",
                    format!("fyn lock --script {}", script.path.user_display()).green(),
                ));
            }

            if let LockCheck::Enabled(lock_check) = lock_check {
                return Err(anyhow::anyhow!(
                    "`fyn sync {lock_check}` requires a script lockfile; run `{}` to lock the script",
                    format!("fyn lock --script {}", script.path.user_display()).green(),
                ));
            }
    ...
    if let SyncTarget::Project(project) = &target {
        ...
        for (name, member) in project.workspace().packages() {
            ...
            {
                warn_user!(
                    "Skipping installation of entry points (`project.scripts`) for package `{}` because this project is not packaged; to install entry points, set `tool.fyn.package = true` or define a `build-system`",
                    name
                );
            }
        }
...
/// don't store plaintext credentials in the `fyn.lock`, we do respect credentials that are defined
...
/// A report of the fyn sync operation
#[derive(Debug, Serialize)]
#[serde(rename_all = "snake_case")]
struct Report {
...
struct PythonReport {
    path: PortablePathBuf,
    version: fyn_pep508::StringVersion,
    implementation: String,
}

impl From<&fyn_python::Interpreter> for PythonReport {
    fn from(interpreter: &fyn_python::Interpreter) -> Self {
        Self {
...
struct PackageChangeReport {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    version: Option<fyn_pep440::Version>,
