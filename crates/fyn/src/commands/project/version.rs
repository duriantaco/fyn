...
use fyn_cache::Cache;
use fyn_cli::version::ProjectVersionInfo;
use fyn_cli::{VersionBump, VersionBumpSpec, VersionFormat};
...
use fyn_normalize::DefaultExtras;
...
/// Display version information for fyn itself (`fyn self version`)
    ...
    match output_format {
        VersionFormat::Text => {
            if short {
                ...
                writeln!(printer.stdout(), "fyn {}", version_info.cyan())?;
            }
        }
        VersionFormat::Json => {
...
/// Read or update project version (`fyn version`)
...
///
/// Note that `fyn version` never needs to support PEP 723 scripts, as those are unversioned.
async fn find_target(
    ...
    // No workspace caching since `fyn version` changes the workspace definition.
    let project = if let Some(package) = package {
        VirtualProject::discover_with_package(
            project_dir,
            &DiscoveryOptions {
                project: fyn_workspace::ProjectDiscovery::Required,
