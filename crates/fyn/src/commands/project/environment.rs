...
use fyn_cache_key::{cache_digest, hash_digest};
use fyn_client::BaseClientBuilder;
use fyn_configuration::{Concurrency, Constraints, TargetTriple};
use fyn_distribution_types::{
    BuiltDist, Dist, Identifier, Node, Resolution, ResolvedDist, SourceDist,
};
use fyn_fs::PythonExt;
use fyn_preview::Preview;
use fyn_python::{Interpreter, PythonEnvironment, canonicalize_executable};
...
impl EphemeralEnvironment {
    ...
    ///
    /// Ephemeral environments created by `fyn run --with` extend a parent (virtual or system)
...
struct CachedEnvironmentDist {
    dist: ResolvedDist,
    hashes: fyn_pypi_types::HashDigests,
...
impl CachedEnvironment {
    ...
    pub(crate) async fn from_spec(
        ...
        // assumption is such that the environment will _not_ be modified by the user or fyn;
        ...
        let venv = fyn_virtualenv::create_venv(
            ...
            false,
            fyn_virtualenv::OnExisting::Remove(fyn_virtualenv::RemovalReason::TemporaryEnvironment),
            true,
            false,
            false,
        )?;
    ...
    fn cache_info(
        dist: &ResolvedDist,
    ) -> Result<Option<CacheInfo>, fyn_cache_info::CacheInfoError> {
        let path = match dist {
    ...
    fn base_interpreter(
        interpreter: &Interpreter,
        cache: &Cache,
    ) -> Result<Interpreter, fyn_python::Error> {
