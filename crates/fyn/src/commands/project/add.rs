...
use fyn_fs::{LockedFile, LockedFileError, Simplified};
use fyn_git::GIT_STORE;
use fyn_normalize::{DEV_DEPENDENCIES, DefaultExtras, DefaultGroups, ExtraName, PackageName};
use fyn_pep508::{MarkerTree, VersionOrUrl};
use fyn_preview::Preview;
use fyn_python::{
    Interpreter, PythonDownloads, PythonEnvironment, PythonPreference, PythonRequest,
};
...
use fyn_workspace::pyproject_mut::{AddBoundsKind, ArrayEdit, DependencyTarget, PyProjectTomlMut};
...
pub(crate) async fn add(
    ...
    for source in &requirements {
        match source {
            RequirementsSource::PyprojectToml(_) => {
                bail!("Adding requirements from a `pyproject.toml` is not supported in `fyn add`");
            }
            RequirementsSource::SetupPy(_) => {
                bail!("Adding requirements from a `setup.py` is not supported in `fyn add`");
            }
            RequirementsSource::Pep723Script(_) => {
                bail!("Adding requirements from a PEP 723 script is not supported in `fyn add`");
            }
            RequirementsSource::SetupCfg(_) => {
                bail!("Adding requirements from a `setup.cfg` is not supported in `fyn add`");
            }
            RequirementsSource::PylockToml(_) => {
                bail!("Adding requirements from a `pylock.toml` is not supported in `fyn add`");
            }
    ...
    let mut target = if let Some(script) = script {
        ...
        // No workspace caching since `fyn add` changes the workspace definition.
    ...
    // ensure the group/extra exists. This handles the case where `fyn add -r requirements.txt
    // --group <name>` or `fyn add -r requirements.txt --optional <extra>` is called with an empty
    // requirements file.
    if edits.is_empty() {
        match &dependency_type {
    ...
    // If `--frozen`, exit early. There's no reason to lock and sync, since we don't need a `fyn.lock`
    // to exist at all.
    if frozen.is_some() {
    ...
    for mut requirement in requirements {
        ...
        let (requirement, source) = match target {
            AddTarget::Script(_, _) | AddTarget::Project(_, _) if raw => {
                (fyn_pep508::Requirement::from(requirement), None)
            }
            AddTarget::Script(script, _) => {
        ...
        // will be checked into version control (e.g., `pyproject.toml` and `fyn.lock`). Instead,
...
fn resolve_requirement(
...
) -> Result<(fyn_pep508::Requirement, Option<Source>), anyhow::Error> {
    ...
    let mut processed_requirement = fyn_pep508::Requirement::from(requirement);
        ...
        match self {
            Self::Script(script, lock) => {
                ...
                if let Some(lock) = lock {
                    debug!("Reverting changes to `fyn.lock`");
                    fs_err::write(target.lock_path(), lock)?;
                } else {
                    debug!("Removing `fyn.lock`");
