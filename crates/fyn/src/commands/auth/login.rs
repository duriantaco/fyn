...
use fyn_auth::{
    AccessToken, AuthBackend, Credentials, Pyfynwt, PyxOAuthTokens, PyxTokenStore, PyxTokens,
    Service, TextCredentialStore, is_default_pyx_domain,
};
use fyn_client::{AuthIntegration, BaseClient, BaseClientBuilder};
use fyn_distribution_types::IndexUrl;
...
pub(crate) async fn login(
    ...
    if pyx_store.is_known_domain(service.url()) || is_default_pyx_domain(service.url()) {
        ...
        let jwt = Pyfynwt::decode(&access_token)?;
    ...
    let username = if let Some(username) = username {
        ...
        if term.is_term() {
            let prompt = "username: ";
            fyn_console::username(prompt, &term)?
        } else {
    ...
    let password = match (password, url_password, token) {
        ...
        (None, None, None) => {
            ...
            if term.is_term() {
                let prompt = "password: ";
                fyn_console::password(prompt, &term)?
            } else {
