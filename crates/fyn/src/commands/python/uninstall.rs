...
use fyn_python::downloads::PythonDownloadRequest;
use fyn_python::managed::{
    ManagedPythonInstallations, PythonMinorVersionLink, python_executable_dir,
};
use fyn_python::{PythonInstallationKey, PythonInstallationMinorVersionKey, PythonRequest};
...
pub(crate) async fn uninstall(
    ...
    if fyn_fs::directories(installations.root())?.all(|path| fyn_fs::is_temporary(&path)) {
        ...
        if let Some(top_level) = installations.root().parent() {
            ...
            if fyn_fs::directories(top_level)?.all(|path| fyn_fs::is_temporary(&path)) {
...
async fn do_uninstall(
    ...
    for (request, download_request) in requests.iter().zip(download_requests) {
        ...
        if !found {
            ...
            #[cfg(windows)]
            {
                fyn_python::windows_registry::remove_orphan_registry_entries(
    ...
    #[cfg(windows)]
    {
        fyn_python::windows_registry::remove_registry_entry(
            &matching_installations,
            all,
            &mut errors,
        );
        fyn_python::windows_registry::remove_orphan_registry_entries(&installed_installations);
    }
