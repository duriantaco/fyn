...
use fyn_shell::Shell;
use fyn_trampoline_builder::{Launcher, LauncherKind};
use fyn_warnings::{warn_user, write_error_chain};
...
impl<'a> InstallRequest<'a> {
    fn new(request: PythonRequest, download_list: &'a ManagedPythonDownloadList) -> Result<Self> {
        ...
        let download_request = PythonDownloadRequest::from_request(&request)
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "`{}` is not a valid Python download request; see `fyn help python` for supported formats and `fyn python list --only-downloads` for available versions",
            ...
            Err(downloads::Error::NoDownloadFound(request))
                if request.libc().is_some_and(Libc::is_musl)
                    && request.arch().is_some_and(|arch| {
                        arch.inner() == Arch::from(&fyn_platform_tags::Arch::Armv7L)
                    }) =>
            {
                return Err(anyhow::anyhow!(
                    "fyn does not yet provide musl Python distributions on armv7."
                ));
            }
...
pub(crate) enum PythonUpgradeSource {
    /// The user invoked `fyn python install --upgrade`
    Install,
    /// The user invoked `fyn python upgrade`
    Upgrade,
}
...
impl std::fmt::Display for PythonUpgradeSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Install => write!(f, "fyn python install --upgrade"),
            Self::Upgrade => write!(f, "fyn python upgrade"),
        }
    }
}
...
async fn perform_install(
    ...
    if requests.is_empty() {
        match upgrade {
            ...
            PythonUpgrade::Enabled(PythonUpgradeSource::Install) => {
                writeln!(
                    printer.stderr(),
                    "No Python versions specified for upgrade; did you mean `fyn python upgrade`?"
                )?;
            }
    ...
    let requested_minor_versions = requests
        ...
        .filter_map(|request| {
            ...
            {
                fyn_pep440::Version::from_str(&format!("{major}.{minor}")).ok()
            } else {
                None
            }
        })
            ...
            for installation in matching_installations {
                ...
                if matches!(&request.request, &PythonRequest::Any) {
                    ...
                    match InstallRequest::new(
                        ...
                        Err(err) => {
                            // This shouldn't really happen, but maybe a new version of fyn dropped
    ...
    for installation in &installations {
        ...
        if !matches!(registry, Some(false)) {
            #[cfg(windows)]
            {
                match fyn_python::windows_registry::create_registry_entry(installation) {
                    Ok(()) => {}
                    Err(err) => {
                        errors.push((
        ...
        for install_key in &changelog.installed {
                ...
                && !install_key.os().is_emscripten()
            {
                ...
                if let Some(not_explicit) = not_explicit {
                    ...
                    writeln!(
                        printer.stderr(),
                        "{} fyn selected a Python distribution with an emulated architecture ({}) for your platform because support for the native architecture ({}) is not yet mature; to override this behaviour, request the native architecture explicitly with: {}",
                        "note:".bold(),
                        install_key.arch(),
                        native_arch,
                        native_request
...
fn create_bin_links(
    ...
    for target in targets {
        ...
        match create_link_to_executable(&target, &executable) {
            ...
            Err(fyn_python::managed::Error::LinkExecutable(err))
                    ...
                    None => {
                        ...
                        if valid_link {
                            if !force {
                                if upgrade {
                                    warn_user!(
                                        "Executable already exists at `{}` but is not managed by fyn; use `fyn python install {}.{}{} --force` to replace it",
                                        target.simplified_display(),
                                    ...
                                    errors.push((
                                        ...
                                        installation.key().clone(),
                                        anyhow::anyhow!(
                                            "Executable already exists at `{}` but is not managed by fyn; use `--force` to replace it",
                                            target.simplified_display()
                                        ),
                                    ));
                                }
...
async fn compile_stdlib_bytecode(
    ...
    let files = fyn_installer::compile_tree(
        &stdlib_path,
        &installation.executable(false),
        concurrency,
        cache.root(),
