...
use fyn_configuration::{Concurrency, Constraints, DryRun, TargetTriple};
use fyn_distribution_types::{ExtraBuildRequires, Requirement, RequirementSource};
use fyn_fs::CWD;
use fyn_normalize::PackageName;
use fyn_pep440::{Operator, Version};
use fyn_preview::Preview;
use fyn_python::{
    EnvironmentPreference, Interpreter, PythonDownloads, PythonInstallation, PythonPreference,
    PythonRequest,
};
use fyn_requirements::RequirementsSpecification;
use fyn_settings::{Combine, PythonInstallMirrors, ResolverInstallerOptions, ToolOptions};
use fyn_tool::{InstalledTools, Tool};
...
pub(crate) async fn upgrade(
    ...
    let names: BTreeMap<PackageName, Vec<Requirement>> = {
        if names.is_empty() {
            ...
            for name in names {
                let requirement = Requirement::from(fyn_pep508::Requirement::parse(&name, &*CWD)?);
    ...
    fn print(&self, name: &PackageName, printer: Printer) -> Result<()> {
        match self {
            Self::PinnedVersion { version } => {
                ...
                let reinstall_command = format!("fyn tool install {name}@latest");
...
async fn upgrade_tool(
    ...
    let existing_tool_receipt = match installed_tools.get_tool_receipt(name) {
        ...
        Ok(None) => {
            let install_command = format!("fyn tool install {name}");
        ...
        Err(_) => {
            let install_command = format!("fyn tool install --force {name}");
            return Err(anyhow::anyhow!(
