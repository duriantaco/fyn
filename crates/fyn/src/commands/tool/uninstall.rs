...
use fyn_tool::{InstalledTools, Tool, ToolEntrypoint};
...
pub(crate) async fn uninstall(name: Vec<PackageName>, printer: Printer) -> Result<ExitStatus> {
    ...
    if fyn_fs::directories(installed_tools.root())?.all(|path| fyn_fs::is_temporary(&path)) {
        ...
        if let Some(parent) = installed_tools.root().parent() {
            if fyn_fs::directories(parent)?.all(|path| fyn_fs::is_temporary(&path)) {
        ...
        for name in names {
            let Some(receipt) = installed_tools.get_tool_receipt(&name)? else {
                ...
                match installed_tools.remove_environment(&name) {
                    ...
                    Err(fyn_tool::Error::VirtualEnvError(fyn_virtualenv::Error::Io(err)))
