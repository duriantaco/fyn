...
use fyn_distribution_types::{InstalledDist, Name};
use fyn_fs::Simplified;
#[cfg(unix)]
use fyn_fs::replace_symlink;
...
use fyn_settings::{PythonInstallMirrors, ToolOptions};
use fyn_shell::Shell;
use fyn_tool::{InstalledTools, Tool, ToolEntrypoint, entrypoint_paths};
...
pub(crate) async fn refine_interpreter(
    ...
    let pip::operations::Error::Resolve(fyn_resolver::ResolveError::NoSolution(no_solution_err)) =
        err
    else {
        return Ok(None);
    };
...
pub(crate) fn finalize_tool_install(
    ...
    let executable_directory = fyn_tool::tool_executable_dir()?;
...
fn hint_executable_from_dependency(
    ...
    match packages.as_slice() {
        [] => {}
        [package] => {
            let command = format!("fyn tool install {}", package.name());
            writeln!(
                printer.stdout(),
