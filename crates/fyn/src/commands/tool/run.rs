...
use fyn_configuration::{Concurrency, Constraints, GitLfsSetting, TargetTriple};
use fyn_distribution::LoweredExtraBuildDependencies;
use fyn_distribution_types::InstalledDist;
...
use fyn_pep440::{VersionSpecifier, VersionSpecifiers};
use fyn_pep508::MarkerTree;
use fyn_preview::Preview;
use fyn_python::PythonVersionFile;
use fyn_python::VersionFileDiscoveryOptions;
use fyn_python::{
    EnvironmentPreference, PythonDownloads, PythonEnvironment, PythonInstallation,
    PythonPreference, PythonRequest,
};
use fyn_requirements::{RequirementsSource, RequirementsSpecification};
use fyn_settings::{PythonInstallMirrors, ResolverInstallerOptions, ToolOptions};
...
pub(crate) async fn run(
    ...
    if let Some(ref from) = from {
        ...
        // If the user tries to invoke `fynx script.py`, hint them towards `fyn run`.
        if has_python_script_ext(target_path) {
            return if target_path.try_exists()? {
                Err(anyhow::anyhow!(
                    ...
                    "hint".bold().cyan(),
                    ":".bold(),
                    format!("fyn run {}", target_path.user_display()).green(),
                ))
            } else {
    ...
    // If the user tries to invoke `fynx run ruff`, hint them towards `fynx ruff`, but only if
    // the `run` package is guaranteed to come from PyPI.
    let (mut target, mut args) = (target, args);
    if from.is_none()
        && invocation_source == ToolRunCommand::Fynx
        && target == "run"
        && settings
            .resolver
            .index_locations
            ...
            .all(|index| matches!(index.url, IndexUrl::Pypi(..)))
    {
        ...
        if term.is_term() {
            ...
            let prompt = format!(
                ...
                format!("fynx run {rest}").green(),
                "run".cyan(),
                format!("fynx {rest}").green()
            );
            let confirmation = fyn_console::confirm(&prompt, &term, true)?;
            if confirmation {
    ...
    let (from, environment) = match result {
        ...
        Err(ProjectError::Operation(err)) => {
            // If the user ran `fynx run ...`, the `run` is likely a mistake. Show a dedicated hint.
            if from.is_none() && invocation_source == ToolRunCommand::Fynx && target == "run" {
                let rest = args.iter().map(|s| s.to_string_lossy()).join(" ");
    ...
    if let Some(ref provider_hints) = provider_hints {
        if provider_hints.not_from_any() {
            ...
            // It'd be a breaking change but would make `fynx` invocations safer.
        ...
        match packages.as_slice() {
            ...
            packages => {
                let provided_by = packages
                    .iter()
                    .map(fyn_distribution_types::Name::name)
...
async fn get_or_create_environment(
    ...
    let python_request = match (explicit_python_request, tool_python_request) {
        // e.g., `fynx --python 3.10 python3.12`
        ...
        // e.g, `fynx --python 3.10 ...`
        (Some(explicit), _) => Some(explicit),
        // e.g., `fynx python` or `fynx <tool>`
        ...
        ToolRequest::Package {
            ...
            let (executable, requirement) = match target {
                ...
                Target::Unspecified(requirement) => {
                    ...
                    // Prefer, in order:
                    // 1. The verbatim executable provided by the user, independent of the requirement (as in: `fynx --from package executable`).
                    // 2. The verbatim executable provided by the user as a named requirement (as in: `fynx change_wheel_version`).
                    // 3. The resolved package name (as in: `fynx git+https://github.com/pallets/flask`).
