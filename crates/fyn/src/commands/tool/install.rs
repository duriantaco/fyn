...
pub(crate) async fn install(
    ...
    let requirement = match &request {
        ...
        ToolRequest::Package {
            ...
            // Determine if it's an entirely different package (e.g., `fyn install foo --from bar`).
        ...
        ToolRequest::Python { .. } => {
            bail!(
                ...
                "fyn tool install".cyan(),
                "fyn python install".cyan(),
            );
        }
    };
...
fn existing_environment_usable(
    ...
    settings: &ResolverInstallerSettings,
    existing_tool_receipt: Option<&fyn_tool::Tool>,
    printer: Printer,
) -> bool {
