...
use fyn_distribution_types::{IndexCapabilities, RequiresPython};
    ...
    for (name, tool) in tools {
        ...
        let Ok(tool) = tool else {
            warn_user!(
                ...
                format!("fyn tool uninstall {name}").green()
            );
            continue;
        };
        ...
        let tool_env = match installed_tools.get_environment(&name, cache) {
            ...
            Ok(None) => {
                warn_user!(
                    ...
                    format!("fyn tool install {name} --reinstall").green()
                );
                continue;
            }
            Err(e) => {
        ...
        let version = match tool_env.version() {
            Ok(version) => version,
            Err(e) => {
                if let fyn_tool::Error::EnvironmentError(e) = e {
                    warn_user!(
                        "{e} (run `{}` to reinstall)",
                        format!("fyn tool install {name} --reinstall").green()
                    );
