...
use console::Term;
use fyn_auth::{Credentials, PyxTokenStore};
use fyn_cache::Cache;
use fyn_client::{
...
use fyn_distribution_types::{IndexCapabilities, IndexLocations, IndexUrl};
...
use fyn_settings::EnvironmentOptions;
use fyn_warnings::{warn_user_once, write_error_chain};
...
fn prompt_username_and_password() -> Result<(Option<String>, Option<String>)> {
    ...
    let username = fyn_console::input(username_prompt, &term).context("Failed to read username")?;
    let password =
        fyn_console::password(password_prompt, &term).context("Failed to read password")?;
