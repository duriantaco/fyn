...
use fyn_client::{MetadataFormat, RegistryClient, VersionFiles};
use fyn_distribution_filename::DistFilename;
use fyn_distribution_types::{IndexCapabilities, IndexMetadataRef, IndexUrl, RequiresPython};
...
impl LatestClient<'_> {
    ...
    ) -> Result<Option<DistFilename>, fyn_client::Error> {
            ...
            .await
        {
            ...
            Err(err) => {
                return match err.kind() {
                    fyn_client::ErrorKind::RemotePackageNotFound(_) => Ok(None),
                    fyn_client::ErrorKind::NoIndex(_) => Ok(None),
                    fyn_client::ErrorKind::Offline(_) => Ok(None),
                    _ => Err(err),
                };
            }
        };
