...
use fyn_python::{EnvironmentPreference, Prefix, PythonEnvironment, PythonPreference, Target};
        ...
        let mut fetches = futures::stream::iter(&results)
            .map(async |dist| {
                let latest = client
                    ...
                    .await?;
                Ok::<(&PackageName, Option<DistFilename>), fyn_client::Error>((dist.name(), latest))
            })
