...
use fyn_distribution_types::{Diagnostic, IndexCapabilities, IndexLocations, Name, RequiresPython};
...
use fyn_pep508::{Requirement, VersionOrUrl};
use fyn_preview::Preview;
use fyn_pypi_types::{ResolutionMetadata, ResolverMarkerEnvironment, VerbatimParsedUrl};
use fyn_python::{EnvironmentPreference, PythonEnvironment, PythonPreference, PythonRequest};
use fyn_resolver::{ExcludeNewer, PrereleaseMode};
    ...
    let latest = if outdated && !packages.is_empty() {
        ...
        let mut fetches = futures::stream::iter(&packages)
            .map(async |(name, ..)| {
                ...
                Ok::<Option<_>, fyn_client::Error>(Some((*name, filename.into_version())))
            })
