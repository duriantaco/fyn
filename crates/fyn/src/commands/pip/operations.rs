...
use fyn_cache::Cache;
use fyn_client::{BaseClientBuilder, RegistryClient};
...
use fyn_distribution::{DistributionDatabase, SourcedDependencyGroups};
...
use fyn_distribution_types::{DistributionMetadata, InstalledMetadata, Name, Resolution};
use fyn_fs::Simplified;
use fyn_install_wheel::LinkMode;
use fyn_installer::{InstallationStrategy, Plan, Planner, Preparer, SitePackages};
use fyn_normalize::PackageName;
use fyn_pep440::Version;
use fyn_pep508::{MarkerEnvironment, RequirementOrigin, VerbatimUrl};
use fyn_platform_tags::Tags;
use fyn_preview::Preview;
use fyn_pypi_types::{Conflicts, ResolverMarkerEnvironment};
use fyn_python::managed::{ManagedPythonInstallation, PythonMinorVersionLink};
use fyn_python::{PythonEnvironment, PythonInstallation};
use fyn_requirements::{
...
use fyn_types::{BuildContext, HashStrategy, InFlight, InstalledPackagesProvider};
...
async fn execute_plan(
    ...
    if !uninstalls.is_empty() {
        ...
        for dist_info in &uninstalls {
            match fyn_installer::uninstall(dist_info, &layout).await {
                ...
                Err(fyn_installer::UninstallError::Uninstall(
                    fyn_install_wheel::Error::MissingRecord(_),
                )) => {
                    warn_user!(
                ...
                Err(fyn_installer::UninstallError::Uninstall(
                    fyn_install_wheel::Error::MissingTopLevel(_),
                )) => {
                    warn_user!(
    ...
    if !installs.is_empty() {
        let start = std::time::Instant::now();
        installs = fyn_installer::Installer::new(venv, preview)
...
pub(crate) enum Error {
    ...
    #[error(transparent)]
    Uninstall(#[from] fyn_installer::UninstallError),

    #[error(transparent)]
    Hash(#[from] fyn_types::HashStrategyError),
    ...
    #[error("The environment is outdated; run `{}` to update the environment", "fyn sync".cyan())]
    OutdatedEnvironment,
}
