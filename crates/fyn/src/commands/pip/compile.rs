...
use fyn_configuration::{KeyringProviderType, TargetTriple};
...
use fyn_pypi_types::{Conflicts, SupportedEnvironments};
use fyn_python::{
    EnvironmentPreference, PythonDownloads, PythonEnvironment, PythonInstallation,
    PythonPreference, PythonRequest, PythonVersion, VersionRequest,
};
use fyn_requirements::upgrade::{LockedRequirements, read_pylock_toml_requirements};
...
pub(crate) async fn pip_compile(
    ...
    if pylock.is_some() {
        return Err(anyhow!(
            "`pylock.toml` is not a supported input format for `fyn pip compile`"
        ));
    }
    ...
    if include_header {
        writeln!(
            writer,
            "{}",
            "# This file was autogenerated by fyn via the following command:".green()
        )?;
        writeln!(
            writer,
            "{}",
...
/// Format the fyn command used to generate the output file.
fn cmd(
    include_index_url: bool,
    include_find_links: bool,
    custom_compile_command: Option<String>,
