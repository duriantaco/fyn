...
use fyn_cache::Cache;
use fyn_distribution_types::{DependencyMetadata, Diagnostic, InstalledDistKind, Name};
use fyn_fs::Simplified;
use fyn_installer::SitePackages;
use fyn_normalize::PackageName;
use fyn_preview::Preview;
use fyn_python::PythonPreference;
use fyn_python::{EnvironmentPreference, Prefix, PythonEnvironment, PythonRequest, Target};
...
pub(crate) fn pip_freeze(
        ...
        Some(paths) => {
            paths
                ...
                .filter_map(|path| {
                    environment
                        .clone()
                        .with_target(fyn_python::Target::from(path))
    ...
    site_packages
        .iter()
        .flat_map(fyn_installer::SitePackages::iter)
        .filter(|dist| {
