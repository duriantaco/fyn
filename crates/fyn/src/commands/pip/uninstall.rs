...
use fyn_configuration::{DryRun, KeyringProviderType};
use fyn_distribution_types::Requirement;
use fyn_distribution_types::{InstalledMetadata, Name, UnresolvedRequirement};
use fyn_fs::Simplified;
use fyn_pep508::UnnamedRequirement;
use fyn_preview::Preview;
use fyn_pypi_types::VerbatimParsedUrl;
use fyn_python::PythonRequest;
use fyn_python::{EnvironmentPreference, PythonPreference};
use fyn_python::{Prefix, PythonEnvironment, Target};
...
pub(crate) async fn pip_uninstall(
    ...
    if let Some(externally_managed) = environment.interpreter().is_externally_managed() {
        if break_system_packages {
            ...
            return if let Some(error) = externally_managed.into_error() {
                Err(anyhow::anyhow!(
                    "The interpreter at {} is externally managed, and indicates the following:\n\n{}\n\nConsider creating a virtual environment with `fyn venv`.",
                    environment.root().user_display().cyan(),
                    textwrap::indent(&error, "  ").green(),
                ))
            } else {
                Err(anyhow::anyhow!(
                    "The interpreter at {} is externally managed. Instead, create a virtual environment with `fyn venv`.",
                    environment.root().user_display().cyan()
                ))
            };
        }
    ...
    let site_packages = fyn_installer::SitePackages::from_environment(&environment)?;
