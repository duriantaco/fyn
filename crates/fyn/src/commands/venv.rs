...
use fyn_normalize::DefaultGroups;
...
use fyn_shell::{Shell, shlex_posix, shlex_windows};
use fyn_types::{AnyErrorBuild, BuildContext, BuildIsolation, BuildStack, HashStrategy};
use fyn_virtualenv::OnExisting;
...
enum VenvError {
    ...
    Creation(#[source] fyn_virtualenv::Error),
    ...
    Tags(#[source] fyn_platform_tags::TagsError),

    #[error("Failed to resolve `--find-links` entry")]
    FlatIndex(#[source] fyn_client::FlatIndexError),
}
...
pub(crate) async fn venv(
    ...
    let path = path.unwrap_or(
        project
            ...
            .and_then(|project| {
                ...
                // avoids a breaking change when adding project environment support to `fyn venv`.
    ...
    if seed {
        ...
        let extra_build_variables = fyn_distribution_types::ExtraBuildVariables::default();
        ...
        let requirements = if interpreter.python_tuple() >= (3, 12) {
            vec![Requirement::from(
                fyn_pep508::Requirement::from_str("pip").unwrap(),
            )]
        } else {
            // Include `setuptools` and `wheel` on Python <3.12.
            vec![
                Requirement::from(fyn_pep508::Requirement::from_str("pip").unwrap()),
                Requirement::from(fyn_pep508::Requirement::from_str("setuptools").unwrap()),
                Requirement::from(fyn_pep508::Requirement::from_str("wheel").unwrap()),
            ]
        };
