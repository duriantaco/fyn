...
/// includes targets and timestamps, along with all `fyn=debug` messages by default.
pub(crate) fn setup_logging(
    ...
    let default_directive = match level {
        ...
        Level::DebugUv => {
            ...
            Directive::from_str("fyn=debug").unwrap()
        }
        Level::TraceUv => {
            // Show `TRACE` messages from the CLI crate (and ERROR/WARN/INFO/DEBUG)
            Directive::from_str("fyn=trace").unwrap()
        }
        Level::TraceAll => {
    ...
    // (a thing we absolutely do throughout fyn), then there will still be color
