...
/// Known sources for fyn installations.
...
impl InstallSource {
    ...
    /// Detect how fyn was installed by inspecting the current executable path.
    ...
    pub(crate) fn update_instructions(self) -> &'static str {
        match self {
            Self::Homebrew => "brew update && brew upgrade fyn",
        }
    }
}
...
mod tests {
    ...
    fn detects_homebrew_cellar() {
        assert_eq!(
            InstallSource::from_path(Path::new("/opt/homebrew/Cellar/fyn/0.9.11/bin/fyn")),
    ...
    fn ignores_non_cellar_paths() {
        assert_eq!(
            InstallSource::from_path(Path::new("/usr/local/bin/fyn")),
            None
        );
    }
}
