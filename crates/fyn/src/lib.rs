...
#[cfg(feature = "self-update")]
use fyn_pep440::release_specifiers_to_ranges;
use fyn_pep508::VersionOrUrl;
use fyn_preview::{Preview, PreviewFeature};
use fyn_pypi_types::{ParsedDirectoryUrl, ParsedUrl};
...
async fn run(
    mut cli: Cli,
    lock_diff_arg_sources: settings::LockDiffArgSources,
) -> Result<ExitStatus> {
    ...
    let deprecated_isolated = if cli.top_level.global_args.isolated {
        match &*cli.command {
            ...
            Commands::Tool(ToolNamespace {
                command: ToolCommand::Fynx(_) | ToolCommand::Run(_),
            }) => false,
            ...
            Commands::Project(command) if matches!(**command, ProjectCommand::Init(_)) => {
                warn_user!(
                    "The `--isolated` flag is deprecated and has no effect. Instead, use `--no-config` to prevent fyn from discovering configuration files or `--no-workspace` to prevent fyn from adding the initialized project to the containing workspace."
                );
                false
            }
            ...
            Commands::Project(_) | Commands::Tool(_) | Commands::Python(_) => {
                warn_user!(
                    "The `--isolated` flag is deprecated and has no effect. Instead, use `--no-config` to prevent fyn from discovering configuration files."
                );
                false
            }
            ...
            _ => {
                warn_user!(
                    "The `--isolated` flag is deprecated. Instead, use `--no-config` to prevent fyn from discovering configuration files."
                );
                true
            }
        }
    ...
    // 2. The nearest configuration file (`fyn.toml` or `pyproject.toml`) above the workspace root.
    ...
    let script = if let Commands::Project(command) = &*cli.command {
        match &**command {
            ProjectCommand::Run(fyn_cli::RunArgs { .. }) => match run_command.as_ref() {
                Some(
            ...
            // For `fyn lock diff --script`, the PEP 723 tag must already exist.
            ProjectCommand::Lock(fyn_cli::LockArgs {
                script: lock_script,
                command: Some(fyn_cli::LockCommand::Diff(diff_args)),
                ..
            }) => match (lock_script.as_deref(), diff_args.script.as_deref()) {
                (Some(_), Some(_)) => {
                    bail!(
                        "`--script` cannot be provided to both `fyn lock` and `fyn lock diff`; use `fyn lock diff --script <SCRIPT>`"
                    )
                }
                (Some(script), None) | (None, Some(script)) => {
                    match Pep723Script::read(script).await {
                        Ok(Some(script)) => Some(Pep723Item::Script(script)),
                        Ok(None) => {
                            bail!(
                                ...
                                format!("fyn init --script {}", script.user_display()).green()
                            )
                        }
                        Err(Pep723Error::Io(err)) if err.kind() == std::io::ErrorKind::NotFound => {
                            bail!(
            ...
            // For `fyn add --script` and `fyn lock --script`, we'll create a PEP 723 tag if it
            // doesn't already exist.
            ProjectCommand::Add(fyn_cli::AddArgs {
                script: Some(script),
                ..
            })
            | ProjectCommand::Lock(fyn_cli::LockArgs {
            ...
            ProjectCommand::Remove(fyn_cli::RemoveArgs {
                script: Some(script),
                ..
            })
            | ProjectCommand::Sync(fyn_cli::SyncArgs {
                script: Some(script),
                ..
            })
            | ProjectCommand::Tree(fyn_cli::TreeArgs {
                script: Some(script),
                ..
            })
    ...
    } else if let Commands::Python(fyn_cli::PythonNamespace {
        command:
            PythonCommand::Find(fyn_cli::PythonFindArgs {
                script: Some(script),
                ..
            }),
    }) = &*cli.command
    ...
    let filesystem = script
        ...
        .map(|fyn| Options::simple(fyn.globals.clone(), fyn.top_level.clone()))
    ...
    if globals.preview.is_enabled(PreviewFeature::AdjustUlimit) {
        match fyn_unix::adjust_open_file_limit() {
            Ok(_) | Err(fyn_unix::OpenFileLimitError::AlreadySufficient { .. }) => {}
    ...
    fyn_configuration::RAYON_PARALLELISM.store(globals.concurrency.installs, Ordering::Relaxed);
    ...
    if cache_settings.no_cache {
        debug!("Disabling the fyn cache due to `--no-cache`");
    }
    ...
    match *cli.command {
        ...
        Commands::Auth(AuthNamespace {
            ...
            use fyn_cli::AuthHelperProtocol;
        ...
        Commands::Pip(PipNamespace {
            ...
            let args = PipDownloadSettings::resolve(args, filesystem, environment);
            show_settings!(args);
        ...
        Commands::Pip(PipNamespace {
            ...
            "pip's `debug` is unsupported (consider using `fynx pip debug` instead)"
        )),
        ...
        Commands::Venv(args) => {
            ...
            if args.no_system {
                warn_user_once!(
                    "The `--no-system` flag has no effect, `fyn venv` always ignores virtual environments when finding a Python interpreter; did you mean `--managed-python`?"
                );
            }

            if args.system {
                warn_user_once!(
                    "The `--system` flag has no effect, `fyn venv` always ignores virtual environments when finding a Python interpreter; did you mean `--no-managed-python`?"
                );
            }
            ...
            let on_existing = fyn_virtualenv::OnExisting::from_args(
                args.allow_existing,
                args.clear,
                args.no_clear,
            );
            ...
            commands::venv(
                ...
                fyn_virtualenv::Prompt::from_args(prompt),
        ...
        Commands::Tool(ToolNamespace {
            command: run_variant @ (ToolCommand::Fynx(_) | ToolCommand::Run(_)),
        }) => {
            let (args, invocation_source) = match run_variant {
                ToolCommand::Fynx(args) => (args.tool_run, ToolRunCommand::Fynx),
            ...
            if let Some(shell) = args.generate_shell_completion {
                // fynx: combine `fyn tool fynx` with the top-level arguments
                let mut uvx = Cli::command()
                    .find_subcommand("tool")
                    .unwrap()
                    .find_subcommand("fynx")
                    .unwrap()
                    .clone()
                ...
                // Copy the top-level arguments into the `fynx` command, as in `Args::augment_args`,
            ...
            let client_builder = match invocation_source {
                ToolRunCommand::Fynx => client_builder.subcommand(vec!["fynx".to_owned()]),
        ...
        Commands::Publish(args) => {
            ...
            if args.skip_existing {
                bail!(
                    "`fyn publish` does not support `--skip-existing` because there is not a \
                    ...
                    API for your index. fyn will check the index for existing distributions before \
                    attempting uploads."
                );
            }
    ...
    match *project_command {
        ProjectCommand::Init(args) => {
            ...
            if explicit_project {
                if globals.preview.is_enabled(PreviewFeature::InitProjectFlag) {
                    bail!(
                        "The `--project` option cannot be used in `fyn init`. {}",
                ...
                warn_user!(
                    "Use of the `--project` option in `fyn init` is deprecated and will be removed in a future release. {}",
                    if args.path.is_some() {
                        "Since a positional path was provided, the `--project` option has no effect. Consider using `--directory` instead."
                    } else {
                        "Consider using `fyn init <PATH>` instead."
                    }
                );
            }
        ...
        ProjectCommand::Sync(args) => {
            ...
            let script = script.map(|script| match script {
                ...
                Pep723Item::Stdin(..) => unreachable!("`fyn lock` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fyn lock` does not support remote files"),
            });
        ...
        ProjectCommand::Lock(args) => {
            ...
            if let Some(command) = command {
                match command {
                    fyn_cli::LockCommand::Diff(diff_args) => {
                        if check || locked || check_exists || dry_run {
                            bail!(
                                "`--check`, `--locked`, `--check-exists`, and `--dry-run` cannot be used with `fyn lock diff`"
                            );
                        }

                        let args = settings::LockDiffSettings::resolve(
                            *diff_args,
                            resolver,
                            build,
                            refresh,
                        ...
                        let script = script
                            .map(|script| match script {
                                Pep723Item::Script(script) => script,
                                Pep723Item::Stdin(..) => {
                                    unreachable!("`fyn lock diff` does not support stdin")
                                }
                                Pep723Item::Remote(..) => {
                                    unreachable!("`fyn lock diff` does not support remote files")
                                }
                            })
                            .map(ScriptPath::Script);

                        return Box::pin(commands::lock_diff(
                            project_dir,
                            args.refresh,
                            args.python,
                            args.install_mirrors,
                            args.settings,
                            client_builder.subcommand(vec!["lock".to_owned(), "diff".to_owned()]),
                            script,
                            globals.python_preference,
                            globals.python_downloads,
                            globals.concurrency,
            ...
            let script = script
                .map(|script| match script {
                    Pep723Item::Script(script) => script,
                    Pep723Item::Stdin(..) => unreachable!("`fyn add` does not support stdin"),
                    Pep723Item::Remote(..) => {
                        unreachable!("`fyn add` does not support remote files")
                    }
                })
                .map(ScriptPath::Script)
        ...
        ProjectCommand::Remove(args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fyn remove` does not support stdin"),
                Pep723Item::Remote(..) => {
                    unreachable!("`fyn remove` does not support remote files")
                }
            });
        ...
        ProjectCommand::Tree(args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fyn tree` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fyn tree` does not support remote files"),
            });
        ...
        ProjectCommand::Export(args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fyn export` does not support stdin"),
                Pep723Item::Remote(..) => {
                    unreachable!("`fyn export` does not support remote files")
                }
            });
        ...
        ProjectCommand::Audit(audit_args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fyn audit` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fyn audit` does not support remote files"),
            });
...
fn lock_diff_arg_sources<I, T>(args: I) -> settings::LockDiffArgSources
where
    I: IntoIterator<Item = T>,
    T: Into<OsString> + Clone,
{
    let Ok(matches) = Cli::command().try_get_matches_from(args) else {
        return settings::LockDiffArgSources::default();
    };

    let Some(("lock", lock_matches)) = matches.subcommand() else {
        return settings::LockDiffArgSources::default();
    };

    let Some(("diff", diff_matches)) = lock_matches.subcommand() else {
        return settings::LockDiffArgSources::default();
    };

    settings::LockDiffArgSources::from_arg_matches(diff_matches)
}
...
/// # Usage
///
/// This entry point is not recommended for external consumption, the fyn binary interface is the
/// official public API.
///
/// When using this entry point, fyn assumes it is running in a process it controls and that the
/// entire process lifetime is managed by fyn. Unexpected behavior may be encountered if this entry
    ...
    T: Into<OsString> + Clone,
{
                ...
                match subcommand.as_str() {
                    "compile" => {
                        err.insert(
                            ...
                            ContextValue::String("fyn pip compile".to_string()),
                        );
                    }
                    "install" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fyn pip install".to_string()),
                        );
                    }
                    "uninstall" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fyn pip uninstall".to_string()),
                        );
                    }
                    "freeze" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fyn pip freeze".to_string()),
                        );
                    }
                    "list" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fyn pip list".to_string()),
                        );
                    }
                    "show" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fyn pip show".to_string()),
                        );
                    }
                    _ => {}
                }
    ...
    let lock_diff_arg_sources = lock_diff_arg_sources(args.iter().cloned());
    ...
    let main2 = move || {
        ...
        let result = runtime.block_on(Box::pin(run(cli, lock_diff_arg_sources)));
    ...
    if let Ok(max_size_str) = std::env::var(fyn_static::EnvVars::UV_CACHE_MAX_SIZE) {
        if let Some(max_bytes) = fyn_cache::Cache::parse_size(&max_size_str) {
            if let Some(cache_dir) = fyn_dirs::user_cache_dir() {
                let cache = fyn_cache::Cache::from_path(cache_dir);
