//! This embeds a "manifest" - a special XML document - into the fyn binary on Windows builds.
//!
//! It includes reasonable defaults for Windows binaries:
//! - `System` codepage to retain backwards compatibility with previous fyn releases.
...
fn main() {
    if std::env::var_os(EnvVars::CARGO_CFG_WINDOWS).is_some() {
        let [major, minor, patch] = fyn_version::version()
            .splitn(3, '.')
            .map(str::parse)
            .collect::<Result<Vec<u16>, _>>()
            .ok()
            .and_then(|v| v.try_into().ok())
            .expect("fyn version must be in x.y.z format");
