...
use fyn_static::{EnvVars, parse_boolish_environment_variable};
use std::env;
use std::io::Write;
...
/// Entrypoint for the `fyn-build` Python package.
fn main() -> Result<()> {
    // Support configuring the log level with `RUST_LOG` (shows only the error level by default) and
    // color.
    //
    // This configuration is a simplified version of the fyn logging configuration. When using
    // fyn_build through fyn proper, the fyn logging configuration applies.
    ...
    // Ad-hoc preview features parsing due to a lack of clap CLI in fyn-build.
    ...
    match command.as_str() {
        "build-sdist" => {
            ...
            let filename = fyn_build_backend::build_source_dist(
                &env::current_dir()?,
                &sdist_directory,
                fyn_version::version(),
                false,
        ...
        "build-wheel" => {
            ...
            let filename = fyn_build_backend::build_wheel(
                &env::current_dir()?,
                &wheel_directory,
        ...
        "build-editable" => {
            let wheel_directory = PathBuf::from(args.next().context("Missing wheel directory")?);
            let metadata_directory = args.next().map(PathBuf::from);
            let filename = fyn_build_backend::build_editable(
                &env::current_dir()?,
                &wheel_directory,
                metadata_directory.as_deref(),
                fyn_version::version(),
        ...
        "prepare-metadata-for-build-wheel" => {
            let wheel_directory = PathBuf::from(args.next().context("Missing wheel directory")?);
            let filename = fyn_build_backend::metadata(
                &env::current_dir()?,
                &wheel_directory,
                fyn_version::version(),
            )?;
