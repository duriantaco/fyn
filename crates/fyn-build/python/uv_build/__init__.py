...
# Use the `fyn build-backend` command rather than `fyn-build`. This option is provided
# for downstream distributions who provide `fyn` and wish to avoid building a partially
# overlapping `fyn-build` executable.
USE_fyn_EXECUTABLE = False
...
def call(
    ...
    """Invoke a fyn subprocess and return the filename from stdout."""
    import shutil
    import subprocess
    import sys
    ...
    fyn_bin_name = "fyn" if USE_fyn_EXECUTABLE else "fyn-build"
    # Unlike `find_fyn_bin`, this mechanism must work according to PEP 517
    fyn_bin = shutil.which(fyn_bin_name)
    if fyn_bin is None:
        raise RuntimeError(f"{fyn_bin_name} was not properly installed")
    build_backend_args = ["build-backend"] if USE_fyn_EXECUTABLE else []
    # Forward stderr, capture stdout for the filename
    result = subprocess.run(
        [fyn_bin, *build_backend_args, *args], stdout=subprocess.PIPE
    )
    ...
    if not stdout:
        print(
            f"{fyn_bin_name} subprocess did not return a filename on stdout",
            file=sys.stderr,
        )
        sys.exit(1)
