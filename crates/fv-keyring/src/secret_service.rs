...
impl SsCredential {
    ...
    pub fn new_with_target(target: Option<&str>, service: &str, user: &str) -> Result<Self> {
        ...
        let attributes = HashMap::from([
            ...
            ("application".to_string(), "fv".to_string()),
        ]);
        Ok(Self {
            attributes,
            label: format!(
                "{user}@{service}:{target} (fv v{})",
    ...
    pub fn new_with_no_target(service: &str, user: &str) -> Result<Self> {
        ...
        Ok(Self {
            attributes,
            label: format!(
                "fv v{} for no target, service '{service}', user '{user}'",
                env!("CARGO_PKG_VERSION"),
            ),
            target: None,
        })
...
mod tests {
    ...
    async fn test_get_update_attributes() {
        ...
        assert_eq!(out_map["application"], "fv");
