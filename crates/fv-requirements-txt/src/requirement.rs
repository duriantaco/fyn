...
use fv_pypi_types::{ParsedDirectoryUrl, ParsedUrl, VerbatimParsedUrl};
...
///
/// Analog to `UnresolvedRequirement` but with `fv_pep508::Requirement` instead of
/// `distribution_types::Requirement`.
#[derive(Hash, Debug, Clone, Eq, PartialEq)]
pub enum RequirementsTxtRequirement {
    /// The fv-specific superset over PEP 508 requirements specifier incorporating
    /// `tool.fv.sources`.
    Named(fv_pep508::Requirement<VerbatimParsedUrl>),
...
impl RequirementsTxtRequirement {
    ...
    pub fn into_editable(self) -> Result<Self, EditableError> {
        match self {
            Self::Named(requirement) => {
                ...
                let fv_pep508::VersionOrUrl::Url(url) = version_or_url else {
                ...
                Ok(Self::Named(fv_pep508::Requirement {
                    version_or_url: Some(fv_pep508::VersionOrUrl::Url(VerbatimParsedUrl {
    ...
    pub fn parse(
        ...
        match fv_pep508::Requirement::parse(input, &working_dir) {
            Ok(requirement) => {
