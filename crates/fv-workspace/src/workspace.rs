...
use fv_configuration::DependencyGroupsWithDefaults;
use fv_distribution_types::{Index, Requirement, RequirementSource};
use fv_fs::{CWD, Simplified};
use fv_normalize::{DEV_DEPENDENCIES, GroupName, PackageName};
use fv_pep440::VersionSpecifiers;
use fv_pep508::{MarkerTree, VerbatimUrl};
use fv_pypi_types::{Conflicts, SupportedEnvironments, VerbatimParsedUrl};
...
use crate::pyproject::{
    Project, PyProjectToml, PyprojectTomlError, Source, Sources, ToolFvSources, ToolFvWorkspace,
};
...
pub enum WorkspaceError {
    ...
    #[error("Nested workspaces are not supported, but workspace member has a `tool.fv.workspace` table: {}", _0.simplified_display())]
    ...
    #[error("Directory walking failed for `tool.fv.workspace.members` glob: `{0}`")]
...
pub struct Workspace {
    ...
    /// the `fv.tool.workspace`, or the `pyproject.toml` in an implicit single workspace project.
...
impl Workspace {
    ...
    pub async fn discover(
        ...
        let path = std::path::absolute(path)
            ...
            .clone();
        // Remove `.` and `..`
        let path = fv_fs::normalize_path(&path);
        ...
        let explicit_root = pyproject_toml
            ...
            .as_ref()
            .and_then(|tool| tool.fv.as_ref())
            .and_then(|fv| fv.workspace.as_ref())
            .map(|workspace| {
                (
                    project_path.clone(),
                    workspace.clone(),
    ...
    pub fn update_member(
        ...
        if member.root == self.install_path {
            ...
            let workspace_sources = workspace_pyproject_toml
                .tool
                .clone()
                .and_then(|tool| tool.fv)
                .and_then(|fv| fv.sources)
                .map(ToolFvSources::into_inner)
                .unwrap_or_default();
    ...
    ///
    /// N.B. this checks if a workspace member is required by inspecting `tool.fv.source` entries,
                ...
                packages
                    ...
                    .filter_map(|(name, member)| {
                        member
                            ...
                            .as_ref()
                            .and_then(|tool| tool.fv.as_ref())
                            .and_then(|fv| fv.sources.as_ref())
                            .map(ToolFvSources::inner)
                            .map(move |sources| {
                                sources
                                    .iter()
        ...
        self.packages.iter().filter_map(|(name, member)| {
            ...
            let groups = {
                ...
                if member
                    ...
                    .as_ref()
                    .and_then(|tool| tool.fv.as_ref())
                    .and_then(|fv| fv.dev_dependencies.as_ref())
                    .is_some()
                {
    ...
    pub fn environments(&self) -> Option<&SupportedEnvironments> {
        self.pyproject_toml
            ...
            .as_ref()
            .and_then(|tool| tool.fv.as_ref())
            .and_then(|fv| fv.environments.as_ref())
    }
    ...
    pub fn required_environments(&self) -> Option<&SupportedEnvironments> {
        self.pyproject_toml
            ...
            .as_ref()
            .and_then(|tool| tool.fv.as_ref())
            .and_then(|fv| fv.required_environments.as_ref())
    }
    ...
    pub fn requirements(&self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
        Vec::new()
    }
    ...
    pub fn overrides(&self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
    ...
    pub fn exclude_dependencies(&self) -> Vec<fv_normalize::PackageName> {
    ...
    pub fn constraints(&self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
    ...
    pub fn build_constraints(&self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
        let Some(build_constraints) = self
            .pyproject_toml
            .tool
            .as_ref()
            .and_then(|tool| tool.fv.as_ref())
            .and_then(|fv| fv.build_constraint_dependencies.as_ref())
        else {
            return vec![];
        };
    ...
    pub fn venv(&self, active: Option<bool>) -> PathBuf {
        ...
        if let Some(from_virtual_env) = from_virtual_env_variable() {
            if !fv_fs::is_same_file_allow_missing(&from_virtual_env, &project_env).unwrap_or(false)
            {
                match active {
                    Some(true) => {
                        debug!(
    ...
    async fn collect_members(
        workspace_root: PathBuf,
        workspace_definition: ToolFvWorkspace,
        ...
        if !dev_dependencies_members.is_empty() {
            warn_user_once!(
                "The `tool.fv.dev-dependencies` field (used in {}) is deprecated and will be removed in a future release; use `dependency-groups.dev` instead",
    ...
    async fn collect_members_only(
        ...
        for member_glob in workspace_definition.clone().members.unwrap_or_default() {
            ...
            let normalized_glob = fv_fs::normalize_path(Path::new(member_glob.as_str()));
...
impl ProjectWorkspace {
    ...
    async fn from_project(
        ...
        let project_path = std::path::absolute(install_path)
            ...
            .clone();
        // Remove `.` and `..`
        let project_path = fv_fs::normalize_path(&project_path);
        // Trim trailing slashes.
...
async fn find_workspace(
    project_root: &Path,
    options: &DiscoveryOptions,
) -> Result<Option<(PathBuf, ToolFvWorkspace, PyProjectToml)>, WorkspaceError> {
...
/// Check if we're in the `tool.fv.workspace.excluded` of a workspace.
fn is_excluded_from_workspace(
    project_path: &Path,
    workspace_root: &Path,
    workspace: &ToolFvWorkspace,
    ...
    for exclude_glob in workspace.exclude.iter().flatten() {
        ...
        let normalized_glob = fv_fs::normalize_path(Path::new(exclude_glob.as_str()));
        let absolute_glob = PathBuf::from(glob::Pattern::escape(
            workspace_root.simplified().to_string_lossy().as_ref(),
        ))
        .join(normalized_glob.as_ref());
...
/// Check if we're in the `tool.fv.workspace.members` of a workspace.
...
impl VirtualProject {
    ...
    pub async fn discover(
        ...
        {
            // Otherwise, if it contains a `tool.fv.workspace` table, it's a non-project workspace
            // root.
...
mod tests {
    ...
    use fv_normalize::GroupName;
    use fv_pypi_types::DependencyGroupSpecifier;
...
"#;
    ...
    async fn nested_workspace() -> Result<()> {
        ...
        insta::with_settings!({filters => filters}, {
            assert_snapshot!(
                error,
            @"Nested workspaces are not supported, but workspace member has a `tool.fv.workspace` table: [ROOT]/packages/seeds");
        });
