...
use fv_build_backend::BuildBackendSettings;
use fv_configuration::GitLfsSetting;
use fv_distribution_types::{Index, IndexName, RequirementSource};
use fv_fs::{PortablePathBuf, relative_to};
use fv_git_types::GitReference;
use fv_macros::OptionsMetadata;
use fv_normalize::{DefaultGroups, ExtraName, GroupName, PackageName};
use fv_options_metadata::{OptionSet, OptionsMetadata, Visit};
use fv_pep440::{Version, VersionSpecifiers};
use fv_pep508::MarkerTree;
use fv_pypi_types::{
...
impl PyProjectToml {
    ...
    pub fn is_package(&self, require_build_system: bool) -> bool {
        // If `tool.fv.package` is set, defer to that explicit setting.
        if let Some(is_package) = self.tool_fv_package() {
            return is_package;
        }
    ...
    /// Returns the value of `tool.fv.package` if set.
    fn tool_fv_package(&self) -> Option<bool> {
        self.tool
            .as_ref()
            .and_then(|tool| tool.fv.as_ref())
            .and_then(|fv| fv.package)
    }
    ...
    pub fn conflicts(&self) -> Conflicts {
        ...
        let Some(toolfv) = tool.fv.as_ref() else {
            return empty;
        };
        let Some(conflicting) = toolfv.conflicts.as_ref() else {
            return empty;
        };
...
pub struct Tool {
    pub fv: Option<ToolFv>,
}

/// Validates the `tool.fv.index` field.
///
...
// `crates/fv-settings/src/settings.rs`.
...
pub struct ToolFv {
    ...
    ///
    /// `tool.fv.sources` enriches the dependency metadata with additional sources, incorporated
    ...
    pub sources: Option<ToolFvSources>,
    ...
    /// Project tasks that can be run via `fv run <name>`.
    ///
    ...
    pub tasks: Option<ToolFvTasks>,

    /// The workspace definition for the project, if any.
    #[option_group]
    pub workspace: Option<ToolFvWorkspace>,

    /// Whether the project is managed by fv. If `false`, fv will ignore the project when
    /// `fv run` is invoked.
    #[option(
        default = r#"true"#,
        value_type = "bool",
    ...
    /// to dependency groups (typically to inform fv that your dev tooling
    ...
    pub dependency_groups: Option<ToolFvDependencyGroups>,

    /// The project's development dependencies.
    ///
    /// Development dependencies will be installed by default in `fv run` and `fv sync`, but will
    ...
    /// `tool.fv.dev-dependencies` and `dependency-groups.dev` are combined to determine the final
    ...
    pub dev_dependencies: Option<Vec<fv_pep508::Requirement<VerbatimParsedUrl>>>,
    ...
    ///
    /// !!! note
    ///     In `fv lock`, `fv sync`, and `fv run`, fv will only read `override-dependencies` from
    ///     the `pyproject.toml` at the workspace root, and will ignore any declarations in other
    ///     workspace members or `fv.toml` files.
    #[cfg_attr(
        feature = "schemars",
        schemars(
            with = "Option<Vec<String>>",
    ...
    pub override_dependencies: Option<Vec<fv_pep508::Requirement<VerbatimParsedUrl>>>,
    ...
    ///
    /// !!! note
    ///     In `fv lock`, `fv sync`, and `fv run`, fv will only read `exclude-dependencies` from
    ///     the `pyproject.toml` at the workspace root, and will ignore any declarations in other
    ///     workspace members or `fv.toml` files.
    #[cfg_attr(
        feature = "schemars",
    ...
    ///
    /// !!! note
    ///     In `fv lock`, `fv sync`, and `fv run`, fv will only read `constraint-dependencies` from
    ///     the `pyproject.toml` at the workspace root, and will ignore any declarations in other
    ///     workspace members or `fv.toml` files.
    #[cfg_attr(
        feature = "schemars",
    ...
    pub constraint_dependencies: Option<Vec<fv_pep508::Requirement<VerbatimParsedUrl>>>,
    ...
    ///
    /// !!! note
    ///     In `fv lock`, `fv sync`, and `fv run`, fv will only read `build-constraint-dependencies` from
    ///     the `pyproject.toml` at the workspace root, and will ignore any declarations in other
    ///     workspace members or `fv.toml` files.
    #[cfg_attr(
        feature = "schemars",
    ...
    pub build_constraint_dependencies: Option<Vec<fv_pep508::Requirement<VerbatimParsedUrl>>>,

    /// A list of supported environments against which to resolve dependencies.
    ///
    /// By default, fv will resolve for all possible environments during a `fv lock` operation.
    ...
    ///
    /// These environments will also be respected when `fv pip compile` is invoked with the
    /// `--universal` flag.
    #[cfg_attr(
        feature = "schemars",
        schemars(
    ...
    ///
    /// By default, fv requires each package to include at least one wheel that is compatible with
    ...
    ///
    /// For example, `environments = ["sys_platform == 'darwin'"]` would limit fv to solving for
    ...
    ///
    /// By making such conflicts explicit, fv can generate a universal resolution
    ...
    /// Configuration for the fv build backend.
    ///
    /// Note that those settings only apply when using the `fv_build` backend, other build backends
...
#[cfg_attr(test, derive(Serialize))]
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
pub struct ToolFvSources(BTreeMap<PackageName, Sources>);
...
impl ToolFvSources {
    ...
    /// Convert the [`ToolFvSources`] into its inner `BTreeMap`.
    #[must_use]
...
impl<'de> serde::de::Deserialize<'de> for ToolFvSources {
...
/// A map of task names to their definitions, as declared in `[tool.fv.tasks]`.
#[derive(Default, Debug, Clone, PartialEq, Eq)]
#[cfg_attr(test, derive(Serialize))]
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
pub struct ToolFvTasks(pub BTreeMap<String, TaskDefinition>);

impl ToolFvTasks {
    /// Return a reference to the task with the given name, if it exists.
    pub fn get(&self, name: &str) -> Option<&TaskDefinition> {
        self.0.get(name)
    }

    /// Iterate over all task names and definitions.
    pub fn iter(&self) -> impl Iterator<Item = (&String, &TaskDefinition)> {
        self.0.iter()
    }

    /// Returns `true` if no tasks are defined.
    pub fn is_empty(&self) -> bool {
        self.0.is_empty()
    }
}

impl<'de> serde::de::Deserialize<'de> for ToolFvTasks {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let map = BTreeMap::<String, TaskDefinition>::deserialize(deserializer)?;
        Ok(Self(map))
    }
}

/// A task that can be run via `fv run <name>`.
///
...
impl TaskDefinition {
    /// Return a short summary for display in `--list-tasks`.
    pub fn description(&self) -> &str {
        match self {
            ...
            Self::Detailed(task) => task
                .description
                .as_deref()
                .or(task.cmd.as_deref())
                .unwrap_or("(chain)"),
        }
    }
}

impl<'de> serde::de::Deserialize<'de> for TaskDefinition {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
...
/// A detailed task definition with optional extras.
#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
#[cfg_attr(test, derive(Serialize))]
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
#[serde(rename_all = "kebab-case")]
pub struct DetailedTask {
    /// The command to run. Mutually exclusive with `chain`.
    pub cmd: Option<String>,
    /// A list of other task names to run in sequence.
    pub chain: Option<Vec<String>>,
    /// A human-readable description, shown in `--list-tasks`.
    pub description: Option<String>,
    /// Environment variables to set when running this task.
...
#[cfg_attr(test, derive(Serialize))]
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
pub struct ToolFvDependencyGroups(BTreeMap<GroupName, DependencyGroupSettings>);

impl ToolFvDependencyGroups {
    /// Returns the underlying `BTreeMap` of group names to settings.
    pub fn inner(&self) -> &BTreeMap<GroupName, DependencyGroupSettings> {
        &self.0
    }

    /// Convert the [`ToolFvDependencyGroups`] into its inner `BTreeMap`.
    #[must_use]
...
/// Ensure that all keys in the TOML table are unique.
impl<'de> serde::de::Deserialize<'de> for ToolFvDependencyGroups {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
...
pub enum ExtraBuildDependencyWire {
    Unannotated(fv_pep508::Requirement<VerbatimParsedUrl>),
    #[serde(rename_all = "kebab-case")]
    Annotated {
        requirement: fv_pep508::Requirement<VerbatimParsedUrl>,
        match_runtime: bool,
    },
}
...
pub struct ExtraBuildDependency {
    pub requirement: fv_pep508::Requirement<VerbatimParsedUrl>,
    pub match_runtime: bool,
}

impl From<ExtraBuildDependency> for fv_pep508::Requirement<VerbatimParsedUrl> {
...
pub enum Source {
    ...
    Git {
        ...
        lfs: Option<bool>,
        #[serde(
            skip_serializing_if = "fv_pep508::marker::ser::is_empty",
            serialize_with = "fv_pep508::marker::ser::serialize",
            default
        )]
        marker: MarkerTree,
...
pub enum DependencyType {
    ...
    Production,
    /// A dependency in `tool.fv.dev-dependencies`.
    Dev,
