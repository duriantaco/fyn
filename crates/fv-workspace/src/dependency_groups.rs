...
use fv_distribution_types::RequiresPython;
use fv_fs::Simplified;
use fv_normalize::{DEV_DEPENDENCIES, GroupName};
use fv_pep440::VersionSpecifiers;
use fv_pep508::Pep508Error;
use fv_pypi_types::{DependencyGroupSpecifier, VerbatimParsedUrl};

use crate::pyproject::{DependencyGroupSettings, PyProjectToml, ToolFvDependencyGroups};
...
pub struct FlatDependencyGroup {
    pub requirements: Vec<fv_pep508::Requirement<VerbatimParsedUrl>>,
...
impl FlatDependencyGroups {
    ...
    pub fn from_pyproject_toml(
        ...
        // First, collect `tool.fv.dev_dependencies`
        ...
        let empty_settings = ToolFvDependencyGroups::default();
        let group_settings = pyproject_toml
            .tool
            .as_ref()
            .and_then(|tool| tool.fv.as_ref())
            .and_then(|fv| fv.dependency_groups.as_ref())
            ...
            for specifier in *specifiers {
                match specifier {
                    DependencyGroupSpecifier::Requirement(requirement) => {
                        match fv_pep508::Requirement::<VerbatimParsedUrl>::from_str(requirement) {
...
pub enum DependencyGroupErrorInner {
    ...
    #[error(
        "Group `{0}` includes the `dev` group (`include = \"dev\"`), but only `tool.fv.dev-dependencies` was found. To reference the `dev` group via an `include`, remove the `tool.fv.dev-dependencies` section and add any development dependencies to the `dev` entry in the `[dependency-groups]` table instead."
    )]
    ...
    #[error("Failed to find group `{0}` specified in `[tool.fv.dependency-groups]`")]
    SettingsGroupNotFound(GroupName),
    #[error(
        "`[tool.fv.dependency-groups]` specifies the `dev` group, but only `tool.fv.dev-dependencies` was found. To reference the `dev` group, remove the `tool.fv.dev-dependencies` section and add any development dependencies to the `dev` entry in the `[dependency-groups]` table instead."
    )]
    SettingsDevGroupInclude,
}

impl DependencyGroupErrorInner {
    /// Enrich a [`DependencyGroupError`] with the `tool.fv.dev-dependencies` metadata, if applicable.
    #[must_use]
    pub fn with_dev_dependencies(
        self,
        dev_dependencies: Option<&Vec<fv_pep508::Requirement<VerbatimParsedUrl>>>,
    ) -> Self {
        match self {
