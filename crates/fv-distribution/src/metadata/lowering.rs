...
use fv_distribution_filename::DistExtension;
use fv_distribution_types::{
    Index, IndexLocations, IndexMetadata, IndexName, Origin, Requirement, RequirementSource,
};
use fv_git_types::{GitLfs, GitReference, GitUrl, GitUrlParseError};
use fv_normalize::{ExtraName, GroupName, PackageName};
use fv_pep440::VersionSpecifiers;
use fv_pep508::{MarkerTree, VerbatimUrl, VersionOrUrl, looks_like_git_repository};
use fv_pypi_types::{ConflictItem, ParsedGitUrl, ParsedUrlError, VerbatimParsedUrl};
use fv_redacted::{DisplaySafeUrl, DisplaySafeUrlError};
use fv_workspace::Workspace;
use fv_workspace::pyproject::{PyProjectToml, Source, Sources};
...
enum RequirementOrigin {
    /// The `tool.fv.sources` were read from the project.
    Project,
    /// The `tool.fv.sources` were read from the workspace root.
    Workspace,
}

impl LoweredRequirement {
    /// Combine `project.dependencies` or `project.optional-dependencies` with `tool.fv.sources`.
    pub(crate) fn from_requirement<'data>(
        ...
        // Identify the source from the `tool.fv.sources` table.
                    ...
                    let (source, mut marker) = match source {
                        ...
                        Source::Workspace {
                            ...
                            let source = if let Some(git_member) = &git_member {
                                ...
                                let subdirectory =
                                    fv_fs::relative_to(member.root(), git_member.fetch_root)
                                        .expect("Workspace member must be relative");
                                let subdirectory = fv_fs::normalize_path_buf(subdirectory);
    ...
    /// Lower a [`fv_pep508::Requirement`] in a non-workspace setting (for example, in a PEP 723
...
/// An error parsing and merging `tool.fv.sources` with
/// `project.{dependencies,optional-dependencies}`.
#[derive(Debug, Error)]
pub enum LoweringError {
    #[error(
        "`{0}` is included as a workspace member, but is missing an entry in `tool.fv.sources` (e.g., `{0} = {{ workspace = true }}`)"
    )]
    MissingWorkspaceSource(PackageName),
    #[error(
        "`{0}` is included as a workspace member, but references a {1} in `tool.fv.sources`. Workspace members must be declared as workspace sources (e.g., `{0} = {{ workspace = true }}`)."
    )]
    NonWorkspaceSource(PackageName, SourceKind),
    #[error(
        "`{0}` references a workspace in `tool.fv.sources` (e.g., `{0} = {{ workspace = true }}`), but is not a workspace member"
    )]
    ...
    #[error(transparent)]
    InvalidVerbatimUrl(#[from] fv_pep508::VerbatimUrlError),
...
/// (e.g., `fv.toml`) rather than in the project's `pyproject.toml`.
fn missing_index_hint(locations: &IndexLocations, index: &IndexName) -> Option<String> {
    ...
    config_index.and_then(|idx| {
        let source = match idx.origin {
            Some(Origin::User) => "a user-level `fv.toml`",
            Some(Origin::System) => "a system-level `fv.toml`",
            Some(Origin::Project) => "a project-level `fv.toml`",
            Some(Origin::Cli | Origin::RequirementsTxt) | None => return None,
        };
        Some(format!(
            "Index `{index}` was found in {source}, but indexes \
             referenced via `tool.fv.sources` must be defined in the project's \
             `pyproject.toml`"
        ))
    })
}
...
fn path_source(
    ...
    if is_dir {
        if let Some(git_member) = git_member {
            ...
            let subdirectory = fv_fs::relative_to(install_path, git_member.fetch_root)
                .expect("Workspace member must be relative");
            let subdirectory = fv_fs::normalize_path_buf(subdirectory);
        ...
        if editable == Some(true) {
            ...
            // If the `package` option is unset, check if `tool.fv.package` is set
