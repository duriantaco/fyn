...
use fv_configuration::NoSources;
use fv_distribution_types::{IndexLocations, Requirement};
use fv_normalize::{ExtraName, GroupName, PackageName};
use fv_pep508::MarkerTree;
use fv_workspace::dependency_groups::FlatDependencyGroups;
use fv_workspace::pyproject::{Sources, ToolFvSources};
use fv_workspace::{DiscoveryOptions, MemberDiscovery, ProjectWorkspace, WorkspaceCache};
...
impl RequiresDist {
    /// Lower without considering `tool.fv` in `pyproject.toml`, used for index and other archive
    /// dependencies.
    pub fn from_metadata23(metadata: fv_pypi_types::RequiresDist) -> Self {
        Self {
            name: metadata.name,
    ...
    /// Lower by considering `tool.fv` in `pyproject.toml` if present, used for Git and directory
    /// dependencies.
    pub async fn from_project_maybe_workspace(
        metadata: fv_pypi_types::RequiresDist,
        install_path: &Path,
    ...
    fn from_project_workspace(
        ...
        // Collect any `tool.fv.index` entries.
        let empty = vec![];
        let project_indexes = project_workspace
            ...
            .and_then(|fv| fv.index.as_deref())
            .unwrap_or(&empty);

        // Collect any `tool.fv.sources` and `tool.fv.dev_dependencies` from `pyproject.toml`.
    ...
    /// Validate the sources for a given [`fv_pypi_types::RequiresDist`].
    ///
...
/// Like [`fv_pypi_types::RequiresDist`], but with any recursive (or self-referential) dependencies
/// resolved.
///
/// For example, given:
/// ```toml
...
mod test {
    ...
    use fv_auth::CredentialsCache;
    use fv_configuration::NoSources;
    use fv_distribution_types::IndexLocations;
    use fv_normalize::PackageName;
    use fv_pep508::Requirement;
    use fv_workspace::{DiscoveryOptions, ProjectWorkspace, WorkspaceCache};
    ...
    async fn requires_dist_from_pyproject_toml(
        ...
        .await?;
        let pyproject_toml = fv_pypi_types::PyProjectToml::from_toml(contents, "pyproject.toml")?;
        let requires_dist = fv_pypi_types::RequiresDist::from_pyproject_toml(pyproject_toml)?;
    ...
    async fn workspace_and_url_spec() {
        ...
        error: Failed to parse entry: `tqdm`
          Caused by: `tqdm` references a workspace in `tool.fv.sources` (e.g., `tqdm = { workspace = true }`), but is not a workspace member
        ");
    }
