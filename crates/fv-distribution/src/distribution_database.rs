...
use fv_cache::{ArchiveId, CacheBucket, CacheEntry, WheelCache};
use fv_cache_info::{CacheInfo, Timestamp};
use fv_client::{
...
use fv_fs::write_atomic;
use fv_platform_tags::Tags;
use fv_pypi_types::{HashDigest, HashDigests, PyProjectToml};
use fv_redacted::DisplaySafeUrl;
use fv_types::{BuildContext, BuildStack};
...
impl<'a, Context: BuildContext> DistributionDatabase<'a, Context> {
    ...
    async fn build_wheel(
            ...
            )
        {
            if matches!(dist, SourceDist::Registry(_)) {
                ...
                warn_user_once!(
                    "{dist} uses a legacy source distribution format ('.{extension}') that is not compliant with PEP 625. A future version of fv will reject this source distribution. Consider upgrading to a newer version of {package}",
                    package = dist.name(),
                );
            } else {
                warn_user_once!(
                    "{dist} is not a standards-compliant source distribution: expected '.tar.gz' but found '.{extension}'. A future version of fv will reject source distributions that do not meet the requirements specified in PEP 625",
                );
            }
        }
    ...
    async fn get_wheel_metadata(
        ...
        // wheel.
        //
        // Historically, for `fv pip compile --universal`, we also generate hashes for
            ...
            async {
                ...
                match progress {
                    Some((reporter, progress)) => {
                        ...
                        match extension {
                            WheelExtension::Whl => {
                                fv_extract::stream::unzip(query_url, &mut reader, temp_dir.path())
                                    .await
                                    .map_err(|err| Error::Extract(filename.to_string(), err))?;
                            }
                            WheelExtension::WhlZst => {
                                fv_extract::stream::untar_zst(&mut reader, temp_dir.path())
                                    .await
                                    .map_err(|err| Error::Extract(filename.to_string(), err))?;
                            }
                        }
                    ...
                    None => match extension {
                        WheelExtension::Whl => {
                            fv_extract::stream::unzip(query_url, &mut hasher, temp_dir.path())
                                .await
                                .map_err(|err| Error::Extract(filename.to_string(), err))?;
                        }
                        WheelExtension::WhlZst => {
                            fv_extract::stream::untar_zst(&mut hasher, temp_dir.path())
                                .await
                                .map_err(|err| Error::Extract(filename.to_string(), err))?;
                        }
                    },
            ...
            async {
                ...
                let hashes = if hashes.is_none() {
                    ...
                    tokio::task::spawn_blocking({
                        ...
                        move || -> Result<(), fv_extract::Error> {
                            // Unzip the wheel into a temporary directory.
                            match extension {
                                WheelExtension::Whl => {
                                    fv_extract::unzip(file, &target)?;
                                }
                                WheelExtension::WhlZst => {
                                    fv_extract::stream::untar_zst_file(file, &target)?;
                                }
                            }
                            Ok(())
                        }
                    ...
                    let algorithms = hashes.algorithms();
                    let mut hashers = algorithms.into_iter().map(Hasher::from).collect::<Vec<_>>();
                    let mut hasher = fv_extract::hash::HashReader::new(file, &mut hashers);
    ...
    async fn load_wheel(
        ...
        if let Some(archive) = archive {
            ...
            match extension {
                WheelExtension::Whl => {
                    fv_extract::stream::unzip(path.display(), &mut hasher, temp_dir.path())
                        .await
                        .map_err(|err| Error::Extract(filename.to_string(), err))?;
                }
                WheelExtension::WhlZst => {
    ...
    async fn unzip_wheel(&self, path: &Path, target: &Path) -> Result<ArchiveId, Error> {
        let temp_dir = tokio::task::spawn_blocking({
            ...
            move || -> Result<TempDir, Error> {
                ...
                fv_extract::unzip(reader, temp_dir.path())
