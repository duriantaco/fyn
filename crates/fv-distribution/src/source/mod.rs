...
use fv_cache::{Cache, CacheBucket, CacheEntry, CacheShard, Removal, WheelCache};
use fv_cache_info::CacheInfo;
use fv_client::{
    CacheControl, CachedClientError, Connectivity, DataWithCachePolicy, RegistryClient,
};
use fv_configuration::{BuildKind, BuildOutput, NoSources};
use fv_distribution_filename::{SourceDistExtension, WheelFilename};
...
use fv_extract::hash::Hasher;
use fv_fs::{Simplified, rename_with_retry, write_atomic};
use fv_git::{GIT_LFS, GitError};
use fv_git_types::{GitHubRepository, GitOid};
use fv_metadata::read_archive_metadata;
use fv_normalize::PackageName;
use fv_pep440::{Version, release_specifiers_to_ranges};
use fv_platform_tags::Tags;
use fv_pypi_types::{HashAlgorithm, HashDigest, HashDigests, PyProjectToml, ResolutionMetadata};
use fv_redacted::DisplaySafeUrl;
use fv_types::{BuildContext, BuildKey, BuildStack, SourceBuildTrait};
use fv_workspace::pyproject::ToolFvSources;
...
impl<'a, T: BuildContext> SourceDistributionBuilder<'a, T> {
    ...
    pub(crate) async fn source_tree_requires_dist(
        ...
        match fv_pypi_types::RequiresDist::from_pyproject_toml(pyproject_toml.clone()) {
            Ok(requires_dist) => {
            ...
            Err(
                err @ (fv_pypi_types::MetadataError::Pep508Error(_)
                | fv_pypi_types::MetadataError::DynamicField(_)
                | fv_pypi_types::MetadataError::FieldNotFound(_)
                | fv_pypi_types::MetadataError::PoetrySyntax),
            ) => {
                debug!(
    ...
    async fn github_metadata(
        ...
        // have `tool.fv.sources` entries that it inherits from the workspace root.
        ...
        let pyproject_toml = match PyProjectToml::from_toml(&content, source) {
            ...
            Err(
                fv_pypi_types::MetadataError::InvalidPyprojectTomlSyntax(..)
                | fv_pypi_types::MetadataError::InvalidPyprojectTomlSchema(..),
            ) => {
        ...
        let metadata =
            match ResolutionMetadata::parse_pyproject_toml(pyproject_toml, source.version()) {
                Ok(metadata) => metadata,
                Err(
                    fv_pypi_types::MetadataError::Pep508Error(..)
                    | fv_pypi_types::MetadataError::DynamicField(..)
                    | fv_pypi_types::MetadataError::FieldNotFound(..)
                    | fv_pypi_types::MetadataError::PoetrySyntax,
                ) => {
        ...
        // Determine whether the project has `tool.fv.sources`. If the project has sources, it must
        ...
        match has_sources(&content) {
            ...
            Err(err) => {
                debug!("Failed to parse `tool.fv.sources` from GitHub API for: {url} ({err})");
                return Ok(None);
            }
        }
    ...
    async fn download_archive(
        ...
        let mut hashers = algorithms
            ...
            .map(Hasher::from)
            .collect::<Vec<_>>();
        let mut hasher = fv_extract::hash::HashReader::new(reader.compat(), &mut hashers);
        ...
        fv_extract::stream::archive(query_url, &mut hasher, ext, temp_dir.path())
            .await
        ...
        let extracted = match fv_extract::strip_component(temp_dir.path()) {
            Ok(top_level) => top_level,
            Err(fv_extract::Error::NonSingularArchive(_)) => temp_dir.keep(),
            Err(err) => {
    ...
    async fn persist_archive(
        ...
        let mut hashers = algorithms
            ...
            .map(Hasher::from)
            .collect::<Vec<_>>();
        let mut hasher = fv_extract::hash::HashReader::new(reader, &mut hashers);

        // Unzip the archive into a temporary directory.
        fv_extract::stream::archive(path.display(), &mut hasher, ext, &temp_dir.path())
            .await
        ...
        let extracted = match fv_extract::strip_component(temp_dir.path()) {
            Ok(top_level) => top_level,
            Err(fv_extract::Error::NonSingularArchive(_)) => temp_dir.path().to_path_buf(),
            Err(err) => {
                return Err(Error::Extract(
                    temp_dir.path().to_string_lossy().into_owned(),
                    err,
    ...
    async fn build_distribution(
        ...
        // Try a direct build if that isn't disabled and the fv build backend is used.
        ...
        {
            // In the fv build backend, the normalized filename and the disk filename are the same.
            name.to_string()
        } else {
            ...
            if let Some(builder) = self.build_context.build_arena().remove(&build_key) {
                ...
                let builder = self
                    ...
                    .setup_build(
                        ...
                        if fv_flags::contains(fv_flags::EnvironmentFlags::HIDE_BUILD_OUTPUT) {
                            BuildOutput::Quiet
                        } else {
                            BuildOutput::Debug
                        },
                    ...
                    for sibling in entry.path().read_dir().map_err(Error::CacheRead)? {
                        ...
                        if sibling.file_type().map_err(Error::CacheRead)?.is_dir() {
                            ...
                            if sibling_name != pointer.revision.id().as_str() {
                                ...
                                removal +=
                                    fv_cache::rm_rf(sibling.path()).map_err(Error::CacheWrite)?;
                            }
                        }
                    }
                }
    ...
    async fn read(
        ...
        match read_pkg_info(source_root, subdirectory).await {
            ...
            Err(
                ...
                | Error::PkgInfo(
                    fv_pypi_types::MetadataError::Pep508Error(_)
                    | fv_pypi_types::MetadataError::DynamicField(_)
                    | fv_pypi_types::MetadataError::FieldNotFound(_)
                    | fv_pypi_types::MetadataError::UnsupportedMetadataVersion(_),
                )),
            ) => {
...
/// Returns `true` if a `pyproject.toml` has `tool.fv.sources`.
