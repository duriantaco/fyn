...
use fv_cache::Error as CacheError;
use fv_client::WrappedReqwestError;
use fv_distribution_filename::{WheelFilename, WheelFilenameError};
use fv_distribution_types::{InstalledDist, InstalledDistError, IsBuildBackendError};
use fv_fs::Simplified;
use fv_git::GitError;
use fv_normalize::PackageName;
use fv_pep440::{Version, VersionSpecifiers};
use fv_platform_tags::Platform;
use fv_pypi_types::{HashAlgorithm, HashDigest};
use fv_redacted::DisplaySafeUrl;
use fv_types::AnyErrorBuild;
...
pub enum Error {
    ...
    // Network error
    #[error(transparent)]
    InvalidUrl(#[from] fv_distribution_types::ToUrlError),
    #[error("Expected a file URL, but received: {0}")]
    NonFileUrl(DisplaySafeUrl),
    #[error(transparent)]
    Git(#[from] fv_git::GitResolverError),
    #[error(transparent)]
    ...
    Metadata(#[from] fv_pypi_types::MetadataError),
    #[error("Failed to read metadata: `{}`", _0.user_display())]
    WheelMetadata(PathBuf, #[source] Box<fv_metadata::Error>),
    ...
    Extract(String, #[source] fv_extract::Error),
    ...
    PkgInfo(#[source] fv_pypi_types::MetadataError),
    ...
    PyprojectToml(#[source] fv_pypi_types::MetadataError),
    #[error(transparent)]
    MetadataLowering(#[from] MetadataError),
    #[error("Distribution not found at: {0}")]
    NotFound(DisplaySafeUrl),
    #[error("Attempted to re-extract the source distribution for `{}`, but the {} hash didn't match. Run `{}` to clear the cache.", _0, _1, "fv cache clean".green())]
