...
use fv_distribution_types::BuildableSource;
...
impl dyn Reporter {
    /// Converts this reporter to a [`fv_git::Reporter`].
    pub(crate) fn into_git_reporter(self: Arc<dyn Reporter>) -> Arc<dyn fv_git::Reporter> {
        Arc::new(Facade {
            reporter: self.clone(),
        })
    }
...
/// A facade for converting from [`Reporter`] to [`fv_git::Reporter`].
struct Facade {
    reporter: Arc<dyn Reporter>,
}

impl fv_git::Reporter for Facade {
    fn on_checkout_start(&self, url: &DisplaySafeUrl, rev: &str) -> usize {
