...
impl StateStore {
    ...
    /// 3. A `.fv` directory in the current working directory.
    ///
    /// Returns an absolute cache dir.
    pub fn from_settings(state_dir: Option<PathBuf>) -> Result<Self, io::Error> {
        if let Some(state_dir) = state_dir {
            Self::from_path(state_dir)
        } else if let Some(data_dir) = fv_dirs::legacy_user_state_dir().filter(|dir| dir.exists()) {
            // If the user has an existing directory at (e.g.) `/Users/user/Library/Application Support/fv`,
            // respect it for backwards compatibility. Otherwise, prefer the XDG strategy, even on
            // macOS.
            Self::from_path(data_dir)
        } else if let Some(data_dir) = fv_dirs::user_state_dir() {
            Self::from_path(data_dir)
        } else {
            Self::from_path(".fv")
        }
