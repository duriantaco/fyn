...
pub async fn compile_tree(
    ...
    for _ in 0..worker_count {
        ...
        worker_handles.push(async {
            // If the worker thread panics (process aborts with panic="abort"),
            // this recv fails, which we map to CompileError::Join.
            rx.await.map_err(|_| CompileError::Join)?
        });
    }
