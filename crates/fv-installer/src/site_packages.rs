...
use fv_pep508::VersionOrUrl;
use fv_platform_tags::Tags;
use fv_pypi_types::{ResolverMarkerEnvironment, VerbatimParsedUrl};
use fv_python::{Interpreter, PythonEnvironment};
use fv_redacted::DisplaySafeUrl;
use fv_types::InstalledPackagesProvider;
                ...
                match &requirement.requirement {
                    ...
                    UnresolvedRequirement::Unnamed(requirement) => {
                        match self.get_urls(requirement.url.verbatim.raw()).as_slice() {
                            ...
                            [distribution] => {
                                let requirement = fv_pep508::Requirement {
...
pub enum InstallationStrategy {
    ...
    /// type differs, as in the `pip` and `fv pip` CLIs.
    ///
    ...
    /// then runs `fv pip install anyio` (which depends on `idna`), the existing `idna` installation
