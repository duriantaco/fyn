...
use fv_cache_key::{CanonicalUrl, RepositoryUrl};
use fv_distribution_filename::ExpandedTags;
...
use fv_git_types::{GitLfs, GitOid};
use fv_normalize::PackageName;
use fv_pep440::Version;
use fv_platform_tags::{AbiTag, IncompatibleTag, TagCompatibility, Tags};
use fv_pypi_types::{DirInfo, DirectUrl, VcsInfo, VcsKind};
    ...
    pub(crate) fn check(
        ...
        match source {
            ...
            RequirementSource::Registry { specifier, .. } => {
                ...
                //
                // For example: the `fv pip` CLI is stateful, in that it "respects"
                // already-installed packages in the virtual environment. So if you run `fv pip
                // install ./path/to/idna`, and then `fv pip install anyio` (which depends on
                // `idna`), we'll "accept" the already-installed `idna` even though it is implicitly
                // being "required" as a registry package.
                //
                // The `fv sync` CLI is stateless, in that all requirements must be defined
                // declaratively ahead-of-time. So if you `fv sync` to install `./path/to/idna` and
                // later `fv sync` to install `anyio`, we'll know (during that second sync) if the
