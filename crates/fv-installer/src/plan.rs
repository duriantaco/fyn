...
use fv_cache::{Cache, CacheBucket, WheelCache};
use fv_cache_info::Timestamp;
use fv_configuration::{BuildOptions, Reinstall};
use fv_distribution::{
...
impl<'a> Planner<'a> {
    ...
    pub fn build(
                    ...
                    [installed] => {
                        ...
                        match RequirementSatisfaction::check(
                            ...
                            RequirementSatisfaction::OutOfDate => {
                                ...
                                // should. This is a known issue, but should only affect the `fv pip` CLI, as the
        ...
        if site_packages.any() {
            // Retain seed packages unless: (1) the virtual environment was created by fv and
            // (2) the `--seed` argument was not passed to `fv venv`.
...
mod tests {
    use super::*;
    use fv_platform_tags::{Arch, Os, Platform};
    use std::str::FromStr;
