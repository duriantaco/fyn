...
use fv_distribution_types::CachedDist;
use fv_install_wheel::{Layout, LinkMode};
...
impl<'a> Installer<'a> {
    ...
    pub fn new(venv: &'a PythonEnvironment, preview: Preview) -> Self {
        Self {
            ...
            cache: None,
            reporter: None,
            name: Some("fv".to_string()),
            metadata: true,
            preview,
        }
    }

    /// Set the [`LinkMode`][`fv_install_wheel::LinkMode`] to use for this installer.
    #[must_use]
    ...
    /// Set the `installer_name` to something other than `"fv"`.
    #[must_use]
    ...
    /// Set whether to install fv-specifier files in the dist-info directory.
    #[must_use]
...
fn install(
    ...
    // Initialize the threadpool with the user settings.
    LazyLock::force(&RAYON_INITIALIZE);
    let state = fv_install_wheel::InstallState::new(preview);
    wheels.par_iter().try_for_each(|wheel| {
        fv_install_wheel::install_wheel(
            layout,
            relocatable,
            wheel.path(),
            wheel.filename(),
            wheel
                .parsed_url()
                .map(fv_pypi_types::DirectUrl::from)
                .as_ref(),
