...
use fv_distribution::{DistributionDatabase, LocalWheel};
...
use fv_types::{BuildContext, HashStrategy, InFlight};
            ...
            let result = self
                ...
                .and_then(|wheel: LocalWheel| {
                    if wheel.satisfies(policy) {
                        ...
                        let err = fv_distribution::Error::hash_mismatch(
                            dist.to_string(),
                            policy.digests(),
                            wheel.hashes(),
                        );
            ...
            match result.as_ref() {
                Ok(cached) => {
                    ...
                    if *dist.name() != cached.filename().name {
                        let err = fv_distribution::Error::WheelMetadataNameMismatch {
                    ...
                    if let Some(version) = dist.version() {
                        ...
                        {
                            let err = fv_distribution::Error::WheelMetadataVersionMismatch {
                                given: version.clone(),
...
impl Error {
    ...
    fn from_dist(dist: Dist, err: fv_distribution::Error, resolution: &Resolution) -> Self {
        let chain =
...
impl dyn Reporter {
    /// Converts this reporter to a [`fv_distribution::Reporter`].
    pub(crate) fn into_distribution_reporter(
        self: Arc<dyn Reporter>,
    ) -> Arc<dyn fv_distribution::Reporter> {
        Arc::new(Facade {
            reporter: self.clone(),
        })
    }
...
/// A facade for converting from [`Reporter`] to [`fv_distribution::Reporter`].
struct Facade {
    reporter: Arc<dyn Reporter>,
}

impl fv_distribution::Reporter for Facade {
    fn on_build_start(&self, source: &BuildableSource) -> usize {
