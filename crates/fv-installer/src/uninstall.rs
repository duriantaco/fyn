use fv_distribution_types::{InstalledDist, InstalledDistKind, InstalledEggInfoFile};

/// Uninstall a package from the specified Python environment.
pub async fn uninstall(
    dist: &InstalledDist,
) -> Result<fv_install_wheel::Uninstall, UninstallError> {
    let uninstall = tokio::task::spawn_blocking({
        let dist = dist.clone();
        move || match dist.kind {
            InstalledDistKind::Registry(_) | InstalledDistKind::Url(_) => {
                Ok(fv_install_wheel::uninstall_wheel(dist.install_path())?)
            }
            InstalledDistKind::EggInfoDirectory(_) => {
                Ok(fv_install_wheel::uninstall_egg(dist.install_path())?)
            }
            InstalledDistKind::LegacyEditable(dist) => {
                Ok(fv_install_wheel::uninstall_legacy_editable(&dist.egg_link)?)
            }
...
pub enum UninstallError {
    ...
    #[error(transparent)]
    Uninstall(#[from] fv_install_wheel::Error),
    #[error(transparent)]
