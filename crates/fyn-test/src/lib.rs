...
use fyn_python::managed::ManagedPythonInstallations;
use fyn_python::{
...
pub const LATEST_PYTHON_3_15: &str = "3.15.0b3";
pub const LATEST_PYTHON_3_14: &str = "3.14.6";
pub const LATEST_PYTHON_3_13: &str = "3.13.14";
...
///
/// This macro captures the fyn binary path at compile time using `env!("CARGO_BIN_EXE_fyn")`,
...
macro_rules! test_context {
    ($python_version:expr) => {
        $crate::TestContext::new_with_bin(
            $python_version,
            std::path::PathBuf::from(env!("CARGO_BIN_EXE_fyn")),
        )
    };
}
...
/// Return the path to the fyn binary.
///
/// This macro captures the fyn binary path at compile time using `env!("CARGO_BIN_EXE_fyn")`,
/// which is only available in the test crate.
#[macro_export]
...
pub const INSTA_FILTERS: &[(&str, &str)] = &[
    ...
    // fyn version display
    (
        r"fyn(-.*)? \d+\.\d+\.\d+(-(alpha|beta|rc)\.\d+)?(\+\d+)?( \([^)]*\))?",
        r"fyn [VERSION] ([COMMIT] DATE)",
    ),
...
impl TestContext {
    /// Create a new test context with a virtual environment and explicit fyn binary path.
    ///
    ...
    pub fn with_pyvenv_cfg_filters(mut self) -> Self {
        let added_filters = [
            ...
            (
                r"fyn = \d+\.\d+\.\d+(-(alpha|beta|rc)\.\d+)?(\+\d+)?".to_string(),
                "fyn = [fyn_VERSION]".to_string(),
            ),
            (
    ...
    /// context, e.g., for use in `fyn python list`.
    #[must_use]
...
";
    ...
    pub fn with_cyclonedx_filters(mut self) -> Self {
        ...
        self.filters.push((
            r#""name": "fyn",\s*"version": "\d+\.\d+\.\d+(-(alpha|beta|rc)\.\d+)?(\+\d+)?""#
                .to_string(),
            r#""name": "fyn",
        "version": "[VERSION]""#
                .to_string(),
    ...
    pub fn with_python_download_cache(mut self) -> Self {
        self.extra_env.push((
            ...
            env::var_os(EnvVars::UV_PYTHON_CACHE_DIR).unwrap_or_else(|| {
                fyn_cache::Cache::from_settings(false, None)
                    .unwrap()
    ...
    /// Create a new test context with multiple Python versions and explicit fyn binary path.
    ///
    ...
    /// Create a fyn command for testing.
    ...
    pub fn add_shared_env(&self, command: &mut Command, activate_venv: bool) {
        ...
        command
            ...
            // Disable wrapping of fyn output for readability / determinism in snapshots.
            .env(EnvVars::UV_NO_WRAP, "1")
            // While we disable wrapping in fyn above, invoked tools may still wrap their output so
            ...
            // other git repos.
            //
            // If one wants to write a test specifically targeting fyn within a
    ...
    /// Create a `fyn help` command with options shared across scenarios.
    ...
    /// Create a `fyn sync` command with options shared across scenarios.
    ...
    /// Create a `fyn lock` command with options shared across scenarios.
    ...
    /// Create a `fyn workspace dir` command with options shared across scenarios.
    ...
    /// Create a `fyn workspace list` command with options shared across scenarios.
    ...
    /// Create a `fyn export` command with options shared across scenarios.
    ...
    /// Create a `fyn format` command with options shared across scenarios.
    ...
    /// Create a `fyn build` command with options shared across scenarios.
    ...
    /// Create a `fyn publish` command with options shared across scenarios.
    ...
    /// Create a `fyn python find` command with options shared across scenarios.
    ...
    /// Create a `fyn python list` command with options shared across scenarios.
    ...
    /// Create a `fyn python install` command with options shared across scenarios.
    ...
    /// Create a `fyn python uninstall` command with options shared across scenarios.
    ...
    /// Create a `fyn python upgrade` command with options shared across scenarios.
    ...
    /// Create a `fyn python pin` command with options shared across scenarios.
    ...
    /// Create a `fyn python dir` command with options shared across scenarios.
    ...
    /// Create a `fyn run` command with options shared across scenarios.
    ...
    /// Create a `fyn tool run` command with options shared across scenarios.
    ...
    /// Create a `fyn upgrade run` command with options shared across scenarios.
    ...
    /// Create a `fyn tool install` command with options shared across scenarios.
    ...
    /// Create a `fyn tool list` command with options shared across scenarios.
    ...
    /// Create a `fyn tool dir` command with options shared across scenarios.
    ...
    /// Create a `fyn tool uninstall` command with options shared across scenarios.
    ...
    /// Create a `fyn add` command for the given requirements.
    ...
    /// Create a `fyn remove` command for the given requirements.
    ...
    /// Create a `fyn tree` command with options shared across scenarios.
    ...
    /// Create a `fyn cache prune` command.
    ...
    /// Create a `fyn cache size` command.
    ...
    /// Create a `fyn build_backend` command.
    ///
    ...
    /// Create a `fyn auth login` command.
    ...
    /// Create a `fyn auth logout` command.
    ...
    /// Create a `fyn auth helper --protocol bazel get` command.
    ...
    /// Create a `fyn auth token` command.
    ...
    pub fn copy_ecosystem_project(&self, name: &str) {
        ...
        if let Err(err) = fs_err::remove_file(self.temp_dir.join("fyn.lock")) {
            assert_eq!(
                err.kind(),
                io::ErrorKind::NotFound,
                "Failed to remove fyn.lock: {err}"
            );
        }
    }
    ...
    pub fn diff_lock(&self, change: impl Fn(&Self) -> Command) -> String {
        ...
        let lock_path = ChildPath::new(self.temp_dir.join("fyn.lock"));
...
/// Helper method to apply filters to a string. Useful when `!fyn_snapshot` cannot be used.
...
pub const READ_ONLY_GITHUB_SSH_DEPLOY_KEY: &str = "";
...
pub async fn download_to_disk(url: &str, path: &Path) {
    let trusted_hosts: Vec<_> = env::var(EnvVars::UV_INSECURE_HOST)
        .unwrap_or_default()
        .split(' ')
        .map(|h| fyn_configuration::TrustedHost::from_str(h).unwrap())
        .collect();
...
#[macro_export]
macro_rules! fyn_snapshot {
    ($spawnable:expr, @$snapshot:literal) => {{
        fyn_snapshot!($crate::INSTA_FILTERS.to_vec(), $spawnable, @$snapshot)
    }};
