...
//! of inclusion (`fv pip install -r` and `fv pip compile` vs. `fv pip install -c` and
//! `fv pip compile -c`).
//!
...
//! * `fv pip install -r pyproject.toml` or `fv pip compile requirements.in`: The `pyproject.toml`
...
//! * `fv pip install .` in a directory with `pyproject.toml` or `fv pip compile requirements.in`
...
//! * `fv pip install -e`: We add the directory in `editables` instead of `requirements`. The
...
use fv_configuration::{DependencyGroups, NoBinary, NoBuild};
use fv_distribution_types::{Index, Requirement};
...
use fv_pypi_types::PyProjectToml;
use fv_redacted::DisplaySafeUrl;
use fv_requirements_txt::{RequirementsTxt, RequirementsTxtRequirement, SourceCache};
use fv_scripts::Pep723Metadata;
...
impl RequirementsSpecification {
    ...
    fn from_pep723_metadata(metadata: &Pep723Metadata) -> Self {
        ...
        if let Some(tool_fv) = metadata.tool.as_ref().and_then(|tool| tool.fv.as_ref()) {
            let constraints = tool_fv
            ...
            Self {
                ...
                extra_index_urls: tool_fv
                    ...
                    .collect(),
                no_index: tool_fv.top_level.no_index.unwrap_or_default(),
                find_links: tool_fv
                    .top_level
                    .find_links
                    .as_ref()
...
async fn read_file(path: &Path, client_builder: &BaseClientBuilder<'_>) -> Result<String> {
    ...
    let content = fv_fs::read_to_string_transcode(path).await?;
    Ok(content)
}
