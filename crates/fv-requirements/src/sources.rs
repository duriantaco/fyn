...
use fv_requirements_txt::RequirementsTxtRequirement;
...
impl RequirementsSource {
    ...
    /// package (e.g., `fv pip install flask`).
    ///
    ...
    pub fn from_package_argument(name: &str) -> Result<Self> {
        ...
        // `fv pip install requirements.txt`), prompt them to correct it.
        ...
        if (name.ends_with(".txt") || name.ends_with(".in")) && Path::new(&name).is_file() {
            ...
            if term.is_term() {
                ...
                let confirmation =
                    fv_console::confirm(&prompt, &term, true).context("Confirm prompt failed")?;
                if confirmation {
        ...
        // `fv pip install pyproject.toml`), prompt them to correct it.
    ...
    /// package (e.g., `fvx --with flask ruff`).
    ///
    /// If the user provided a value that appears to be a `requirements.txt` file or a local
    /// directory, prompt them to correct it (if the terminal is interactive).
    pub fn from_with_package_argument(name: &str) -> Result<Self> {
        ...
        // `fvx --with requirements.txt ruff`), prompt them to correct it.
        #[expect(clippy::case_sensitive_file_extension_comparisons)]
        if (name.ends_with(".txt") || name.ends_with(".in")) && Path::new(&name).is_file() {
            let term = Term::stderr();
            if term.is_term() {
        ...
        // `fvx --with pyproject.toml ruff`), prompt them to correct it.
        if (name == "pyproject.toml"
            || name == "setup.py"
            || name == "setup.cfg"
            || is_pylock_toml(name))
    ...
    /// Parse an editable [`RequirementsSource`] (e.g., `fv pip install -e .`).
    ...
    /// Parse a package [`RequirementsSource`] (e.g., `fv pip install ruff`).
