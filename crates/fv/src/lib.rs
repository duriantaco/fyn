...
use fv_configuration::min_stack_size;
use fv_flags::EnvironmentFlags;
use fv_fs::{CWD, Simplified};
#[cfg(feature = "self-update")]
use fv_pep440::release_specifiers_to_ranges;
use fv_pep508::VersionOrUrl;
use fv_preview::{Preview, PreviewFeature};
use fv_pypi_types::{ParsedDirectoryUrl, ParsedUrl};
use fv_python::PythonRequest;
use fv_requirements::{GroupsSpecification, RequirementsSource};
use fv_requirements_txt::RequirementsTxtRequirement;
use fv_scripts::{Pep723Error, Pep723Item, Pep723Metadata, Pep723Script};
use fv_settings::{Combine, EnvironmentOptions, FilesystemOptions, Options};
...
async fn run(mut cli: Cli) -> Result<ExitStatus> {
    ...
    // `fv init`, which creates the project directory (separate deprecation).
    ...
    let deprecated_isolated = if cli.top_level.global_args.isolated {
        match &*cli.command {
            ...
            Commands::Tool(ToolNamespace {
                command: ToolCommand::Fvx(_) | ToolCommand::Run(_),
            }) => false,
            ...
            Commands::Project(command) if matches!(**command, ProjectCommand::Init(_)) => {
                warn_user!(
                    "The `--isolated` flag is deprecated and has no effect. Instead, use `--no-config` to prevent fv from discovering configuration files or `--no-workspace` to prevent fv from adding the initialized project to the containing workspace."
                );
                false
            }
            ...
            Commands::Project(_) | Commands::Tool(_) | Commands::Python(_) => {
                warn_user!(
                    "The `--isolated` flag is deprecated and has no effect. Instead, use `--no-config` to prevent fv from discovering configuration files."
                );
                false
            }
            ...
            _ => {
                warn_user!(
                    "The `--isolated` flag is deprecated. Instead, use `--no-config` to prevent fv from discovering configuration files."
                );
                true
            }
        }
    ...
    // 2. The nearest configuration file (`fv.toml` or `pyproject.toml`) above the workspace root.
    //    If found, this file is combined with the user configuration file.
    // 3. The nearest configuration file (`fv.toml` or `pyproject.toml`) in the directory tree,
    ...
    let filesystem = if let Some(config_file) = cli.top_level.config_file.as_ref() {
        ...
        {
            warn_user!(
                "The `--config-file` argument expects to receive a `fv.toml` file, not a `pyproject.toml`. If you're trying to run a command from another project, use the `--project` argument instead."
            );
        }
    ...
    let run_command = if let Commands::Project(command) = &mut *cli.command {
        if let ProjectCommand::Run(fv_cli::RunArgs {
    ...
    let script = if let Commands::Project(command) = &*cli.command {
        match &**command {
            ProjectCommand::Run(fv_cli::RunArgs { .. }) => match run_command.as_ref() {
                Some(
            ...
            // For `fv add --script` and `fv lock --script`, we'll create a PEP 723 tag if it
            // doesn't already exist.
            ProjectCommand::Add(fv_cli::AddArgs {
                script: Some(script),
                ..
            })
            | ProjectCommand::Lock(fv_cli::LockArgs {
                script: Some(script),
                ..
            }) => match Pep723Script::read(&script).await {
                Ok(Some(script)) => Some(Pep723Item::Script(script)),
            ...
            ProjectCommand::Remove(fv_cli::RemoveArgs {
                script: Some(script),
                ..
            })
            | ProjectCommand::Sync(fv_cli::SyncArgs {
                script: Some(script),
                ..
            })
            | ProjectCommand::Tree(fv_cli::TreeArgs {
                script: Some(script),
                ..
            })
            | ProjectCommand::Export(fv_cli::ExportArgs {
                script: Some(script),
                ..
            }) => match Pep723Script::read(&script).await {
                Ok(Some(script)) => Some(Pep723Item::Script(script)),
                Ok(None) => {
                    bail!(
                        ...
                        format!("fv init --script {}", script.user_display()).green()
                    )
                }
                Err(Pep723Error::Io(err)) if err.kind() == std::io::ErrorKind::NotFound => {
                    bail!(
    ...
    } else if let Commands::Python(fv_cli::PythonNamespace {
        command:
            PythonCommand::Find(fv_cli::PythonFindArgs {
                script: Some(script),
                ..
            }),
    }) = &*cli.command
    ...
    let filesystem = script
        ...
        .map(|fv| Options::simple(fv.globals.clone(), fv.top_level.clone()))
    ...
    fv_flags::init(EnvironmentFlags::from(&environment))
    ...
    debug!("fv {}", fv_cli::version::uv_self_version());
    ...
    if globals.preview.is_enabled(PreviewFeature::AdjustUlimit) {
        match fv_unix::adjust_open_file_limit() {
            Ok(_) | Err(fv_unix::OpenFileLimitError::AlreadySufficient { .. }) => {}
    ...
    if let Some(required_version) = globals.required_version.as_ref() {
        ...
        if !required_version.contains(&package_version) {
            ...
            let hint = {
                ...
                {
                    // E.g., `>=1.0.0`
                    format!(". Update `fv` by running `{}`.", "fv self update".cyan())
                } else {
                    String::new()
                }
            };
            ...
            return Err(anyhow::anyhow!(
                "Required fv version `{required_version}` does not match the running version `{package_version}`{hint}",
            ));
        }
    }
    ...
    fv_configuration::RAYON_PARALLELISM.store(globals.concurrency.installs, Ordering::Relaxed);
    ...
    if cache_settings.no_cache {
        debug!("Disabling the fv cache due to `--no-cache`");
    }
    ...
    match *cli.command {
        ...
        Commands::Pip(PipNamespace {
            ...
            // reinstalled. This matches user expectations: `fv pip install .` should always
        ...
        Commands::Pip(PipNamespace {
            ...
            "pip's `debug` is unsupported (consider using `fvx pip debug` instead)"
        )),
        ...
        Commands::Venv(args) => {
            ...
            if args.no_system {
                warn_user_once!(
                    "The `--no-system` flag has no effect, `fv venv` always ignores virtual environments when finding a Python interpreter; did you mean `--managed-python`?"
                );
            }

            if args.system {
                warn_user_once!(
                    "The `--system` flag has no effect, `fv venv` always ignores virtual environments when finding a Python interpreter; did you mean `--no-managed-python`?"
                );
            }
            ...
            let on_existing = fv_virtualenv::OnExisting::from_args(
                args.allow_existing,
                args.clear,
                args.no_clear,
            );
            ...
            commands::venv(
                ...
                fv_virtualenv::Prompt::from_args(prompt),
        ...
        Commands::Self_(_) => {
            const BASE_MESSAGE: &str =
                "fv was installed through an external package manager and cannot update itself.";

            let message = match InstallSource::detect() {
                Some(source) => format!(
                    "{base}\n\n{hint}{colon} You installed fv using {}. To update fv, run `{}`",
                    source.description(),
                ...
                None => format!("{BASE_MESSAGE} Please use your package manager to update fv."),
            };
        ...
        Commands::Tool(ToolNamespace {
            command: run_variant @ (ToolCommand::Fvx(_) | ToolCommand::Run(_)),
        }) => {
            let (args, invocation_source) = match run_variant {
                ToolCommand::Fvx(args) => (args.tool_run, ToolRunCommand::Fvx),
            ...
            if let Some(shell) = args.generate_shell_completion {
                // fvx: combine `fv tool fvx` with the top-level arguments
                ...
                // Copy the top-level arguments into the `fvx` command, as in `Args::augment_args`,
            ...
            let client_builder = match invocation_source {
                ToolRunCommand::Fvx => client_builder.subcommand(vec!["fvx".to_owned()]),
        ...
        Commands::Publish(args) => {
            ...
            if args.skip_existing {
                bail!(
                    "`fv publish` does not support `--skip-existing` because there is not a \
                    ...
                    API for your index. fv will check the index for existing distributions before \
                    attempting uploads."
                );
            }
...
async fn run_project(
    ...
    match *project_command {
        ProjectCommand::Init(args) => {
            ...
            if explicit_project {
                if globals.preview.is_enabled(PreviewFeature::InitProjectFlag) {
                    bail!(
                        "The `--project` option cannot be used in `fv init`. {}",
                ...
                warn_user!(
                    "Use of the `--project` option in `fv init` is deprecated and will be removed in a future release. {}",
                    if args.path.is_some() {
                        "Since a positional path was provided, the `--project` option has no effect. Consider using `--directory` instead."
                    } else {
                        "Consider using `fv init <PATH>` instead."
                    }
                );
            }
        ...
        ProjectCommand::Run(args) => {
            ...
            if args.list_tasks {
                use fv_workspace::pyproject::PyProjectToml;
                let pyproject_path = project_dir.join("pyproject.toml");
                if let Ok(content) = fs_err::read_to_string(&pyproject_path) {
                    if let Ok(pyproject) = toml::from_str::<PyProjectToml>(&content) {
                        if let Some(tasks) = pyproject.tool.and_then(|t| t.fv).and_then(|u| u.tasks)
                        {
                            if tasks.is_empty() {
                                eprintln!("No tasks defined in [tool.fv.tasks]");
                            } else {
                            ...
                            eprintln!("No [tool.fv.tasks] section found in pyproject.toml");
                        }
                    }
                } else {
        ...
        ProjectCommand::Sync(args) => {
            ...
            let script = script.map(|script| match script {
                ...
                Pep723Item::Stdin(..) => unreachable!("`fv lock` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fv lock` does not support remote files"),
            });
        ...
        ProjectCommand::Lock(args) => {
            ...
            let script = script
                .map(|script| match script {
                    Pep723Item::Script(script) => script,
                    Pep723Item::Stdin(..) => unreachable!("`fv add` does not support stdin"),
                    Pep723Item::Remote(..) => {
                        unreachable!("`fv add` does not support remote files")
                    }
                })
        ...
        ProjectCommand::Remove(args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fv remove` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fv remove` does not support remote files"),
            });
        ...
        ProjectCommand::Tree(args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fv tree` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fv tree` does not support remote files"),
            });
        ...
        ProjectCommand::Export(args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fv export` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fv export` does not support remote files"),
            });
        ...
        ProjectCommand::Audit(audit_args) => {
            ...
            let script = script.map(|script| match script {
                Pep723Item::Script(script) => script,
                Pep723Item::Stdin(..) => unreachable!("`fv audit` does not support stdin"),
                Pep723Item::Remote(..) => unreachable!("`fv audit` does not support remote files"),
            });
...
/// The main entry point for a fv invocation.
///
/// # Usage
///
/// This entry point is not recommended for external consumption, the fv binary interface is the
/// official public API.
///
/// When using this entry point, fv assumes it is running in a process it controls and that the
/// entire process lifetime is managed by fv. Unexpected behavior may be encountered if this entry
    ...
    T: Into<OsString> + Clone,
{
                ...
                match subcommand.as_str() {
                    "compile" => {
                        err.insert(
                            ...
                            ContextValue::String("fv pip compile".to_string()),
                        );
                    }
                    "install" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fv pip install".to_string()),
                        );
                    }
                    "uninstall" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fv pip uninstall".to_string()),
                        );
                    }
                    "freeze" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fv pip freeze".to_string()),
                        );
                    }
                    "list" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fv pip list".to_string()),
                        );
                    }
                    "show" => {
                        err.insert(
                            ContextKind::SuggestedSubcommand,
                            ContextValue::String("fv pip show".to_string()),
                        );
                    }
                    _ => {}
                }
    ...
    // Enforce cache size limit if configured via UV_CACHE_MAX_SIZE.
    if let Ok(max_size_str) = std::env::var(fv_static::EnvVars::UV_CACHE_MAX_SIZE) {
        if let Some(max_bytes) = fv_cache::Cache::parse_size(&max_size_str) {
            if let Some(cache_dir) = fv_dirs::user_cache_dir() {
                let cache = fv_cache::Cache::from_path(cache_dir);
                match cache.enforce_max_size(max_bytes) {
                    Ok(Some(removal)) => {
                        debug!(
                            "Cache pruned: {} files, {} bytes freed",
                            removal.num_files, removal.total_bytes
                        );
                    }
                    Ok(None) => {}
                    Err(err) => {
                        debug!("Failed to enforce cache size limit: {err}");
                    }
                }
            }
        }
