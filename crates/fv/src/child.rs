...
pub(crate) async fn run_to_completion(mut handle: Child) -> anyhow::Result<ExitStatus> {
    ...
    // `Ctrl-C`. In general, this means that fv should ignore SIGINT, allowing the child process to
    // cleanly exit instead. If fv forwarded the SIGINT immediately, the child process would receive
    ...
    // process and fv must take ownership of forwarding the signal.
    //
    // Note the above only applies in an interactive terminal. If a signal is sent directly to the
    // fv parent process (e.g., `kill -2 <pid>`), the process group is not involved and a signal is
    // not sent to the child by default. In this context, fv must forward the signal to the child.
    // fv checks if stdin is a TTY as a heuristic to determine if fv is running in an interactive
    // terminal. When not in an interactive terminal, fv will forward SIGINT to the child.
    //
    ...
    // case, it's possible that fv will improperly send a second SIGTERM to the child process.
    // However, this seems preferable to not forwarding it in the first place. In the Docker case,
    // if `fv` is invoked directly (instead of via an init system), it's PID 1 which has a
    ...
    // SIGTERM, it is not terminated. In this context, it is essential for fv to forward the SIGTERM
