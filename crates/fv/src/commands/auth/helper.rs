...
async fn credentials_for_url(
    ...
    if pyx_store.is_known_domain(url) || is_default_pyx_domain(url) {
        ...
        let client = client_builder
            .auth_integration(fv_client::AuthIntegration::NoAuthMiddleware)
            .build();
        let token = pyx_store
            .access_token(
...
/// for that URI using fv's authentication backends, and writes a JSON response
...
pub(crate) async fn helper(
    ...
    if !preview.is_enabled(PreviewFeature::AuthHelper) {
        warn_user!(
            "The `fv auth helper` command is experimental and may change without warning. Pass `--preview-features {}` to disable this warning",
