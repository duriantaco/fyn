...
use fv_requirements::{GroupsSpecification, RequirementsSource, RequirementsSpecification};
use fv_resolver::{
...
pub(crate) async fn pip_install(
    ...
    if let Some(externally_managed) = environment.interpreter().is_externally_managed() {
        if break_system_packages {
            ...
            let error_message = if system {
                ...
                format!(
                    "{}\n{}{} Consider creating a virtual environment, e.g., with `fv venv`",
                    managed_message,
                    "hint".bold().cyan(),
                    ":".bold()
                )
    ...
    let types_build_isolation = match build_isolation {
        ...
        BuildIsolation::Shared => fv_types::BuildIsolation::Shared(&environment),
    ...
    let (resolution, hasher) = if let Some(pylock) = pylock {
        ...
        let (install_path, content) =
            if pylock.starts_with("http://") || pylock.starts_with("https://") {
                ...
                let url = fv_redacted::DisplaySafeUrl::parse(&pylock.to_string_lossy())?;
