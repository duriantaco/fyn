...
use fv_configuration::{DryRun, KeyringProviderType};
use fv_distribution_types::Requirement;
use fv_distribution_types::{InstalledMetadata, Name, UnresolvedRequirement};
use fv_fs::Simplified;
use fv_pep508::UnnamedRequirement;
use fv_preview::Preview;
use fv_pypi_types::VerbatimParsedUrl;
use fv_python::PythonRequest;
use fv_python::{EnvironmentPreference, PythonPreference};
use fv_python::{Prefix, PythonEnvironment, Target};
...
pub(crate) async fn pip_uninstall(
    ...
    let site_packages = fv_installer::SitePackages::from_environment(&environment)?;
    ...
    if !dry_run.enabled() {
        for distribution in &distributions {
            let summary = fv_installer::uninstall(distribution).await?;
            debug!(
