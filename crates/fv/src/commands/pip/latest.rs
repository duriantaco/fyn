...
use fv_client::{MetadataFormat, RegistryClient, VersionFiles};
use fv_distribution_filename::DistFilename;
use fv_distribution_types::{IndexCapabilities, IndexMetadataRef, IndexUrl, RequiresPython};
...
impl LatestClient<'_> {
    ...
    ) -> Result<Option<DistFilename>, fv_client::Error> {
            ...
            .await
        {
            ...
            Err(err) => {
                return match err.kind() {
                    fv_client::ErrorKind::RemotePackageNotFound(_) => Ok(None),
                    fv_client::ErrorKind::NoIndex(_) => Ok(None),
                    fv_client::ErrorKind::Offline(_) => Ok(None),
                    _ => Err(err),
                };
            }
        };
