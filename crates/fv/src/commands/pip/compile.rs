...
use fv_configuration::{KeyringProviderType, TargetTriple};
...
use fv_pypi_types::{Conflicts, SupportedEnvironments};
use fv_python::{
    EnvironmentPreference, PythonDownloads, PythonEnvironment, PythonInstallation,
    PythonPreference, PythonRequest, PythonVersion, VersionRequest,
};
use fv_requirements::upgrade::{LockedRequirements, read_pylock_toml_requirements};
use fv_requirements::{
...
use fv_torch::{TorchMode, TorchSource, TorchStrategy};
use fv_types::{EmptyInstalledPackages, HashStrategy};
...
pub(crate) async fn pip_compile(
    ...
    excludes_from_workspace: Vec<fv_normalize::PackageName>,
    ...
    if pylock.is_some() {
        return Err(anyhow!(
            "`pylock.toml` is not a supported input format for `fv pip compile`"
        ));
    }
    ...
    if include_header {
        writeln!(
            writer,
            "{}",
            "# This file was autogenerated by fv via the following command:".green()
        )?;
        writeln!(
            writer,
            "{}",
...
/// Format the fv command used to generate the output file.
fn cmd(
    include_index_url: bool,
    include_find_links: bool,
    custom_compile_command: Option<String>,
