...
use fv_cache::Cache;
use fv_client::{BaseClientBuilder, RegistryClient};
...
use fv_distribution::{DistributionDatabase, SourcedDependencyGroups};
...
use fv_distribution_types::{DistributionMetadata, InstalledMetadata, Name, Resolution};
use fv_fs::Simplified;
use fv_install_wheel::LinkMode;
use fv_installer::{InstallationStrategy, Plan, Planner, Preparer, SitePackages};
use fv_normalize::PackageName;
use fv_pep440::Version;
use fv_pep508::{MarkerEnvironment, RequirementOrigin, VerbatimUrl};
use fv_platform_tags::Tags;
use fv_preview::Preview;
use fv_pypi_types::{Conflicts, ResolverMarkerEnvironment};
use fv_python::managed::{ManagedPythonInstallation, PythonMinorVersionLink};
use fv_python::{PythonEnvironment, PythonInstallation};
use fv_requirements::{
...
use fv_tool::InstalledTools;
use fv_types::{BuildContext, HashStrategy, InFlight, InstalledPackagesProvider};
...
async fn execute_plan(
    ...
    if !uninstalls.is_empty() {
        ...
        for dist_info in &uninstalls {
            match fv_installer::uninstall(dist_info).await {
                Ok(summary) => {
                    debug!(
                ...
                Err(fv_installer::UninstallError::Uninstall(
                    fv_install_wheel::Error::MissingRecord(_),
                )) => {
                    warn_user!(
                ...
                Err(fv_installer::UninstallError::Uninstall(
                    fv_install_wheel::Error::MissingTopLevel(_),
                )) => {
                    warn_user!(
    ...
    if !installs.is_empty() {
        let start = std::time::Instant::now();
        installs = fv_installer::Installer::new(venv, preview)
...
pub(crate) enum Error {
    ...
    #[error(transparent)]
    Uninstall(#[from] fv_installer::UninstallError),

    #[error(transparent)]
    Hash(#[from] fv_types::HashStrategyError),
    ...
    #[error("The environment is outdated; run `{}` to update the environment", "fv sync".cyan())]
    OutdatedEnvironment,
}
