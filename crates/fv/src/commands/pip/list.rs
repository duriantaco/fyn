...
use fv_python::{EnvironmentPreference, Prefix, PythonEnvironment, PythonPreference, Target};
        ...
        let mut fetches = futures::stream::iter(&results)
            .map(async |dist| {
                let latest = client
                    ...
                    .await?;
                Ok::<(&PackageName, Option<DistFilename>), fv_client::Error>((dist.name(), latest))
            })
