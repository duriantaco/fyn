...
use fv_configuration::{Concurrency, IndexStrategy, KeyringProviderType};
use fv_distribution_types::{Diagnostic, IndexCapabilities, IndexLocations, Name, RequiresPython};
use fv_installer::SitePackages;
use fv_normalize::PackageName;
use fv_pep440::{Operator, Version, VersionSpecifier, VersionSpecifiers};
use fv_pep508::{Requirement, VersionOrUrl};
use fv_preview::Preview;
use fv_pypi_types::{ResolutionMetadata, ResolverMarkerEnvironment, VerbatimParsedUrl};
use fv_python::{EnvironmentPreference, PythonEnvironment, PythonPreference, PythonRequest};
use fv_resolver::{ExcludeNewer, PrereleaseMode};
    ...
    let latest = if outdated && !packages.is_empty() {
        ...
        let mut fetches = futures::stream::iter(&packages)
            .map(async |(name, ..)| {
                ...
                Ok::<Option<_>, fv_client::Error>(Some((*name, filename.into_version())))
            })
