...
use console::Term;
use fv_auth::{Credentials, PyxTokenStore};
use fv_cache::Cache;
use fv_client::{
...
use fv_distribution_types::{IndexCapabilities, IndexLocations, IndexUrl};
...
use fv_warnings::{warn_user_once, write_error_chain};
...
fn prompt_username_and_password() -> Result<(Option<String>, Option<String>)> {
    ...
    let username = fv_console::input(username_prompt, &term).context("Failed to read username")?;
    let password =
        fv_console::password(password_prompt, &term).context("Failed to read password")?;
