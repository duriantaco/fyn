...
use fv_cache::Cache;
use fv_cli::version::ProjectVersionInfo;
use fv_cli::{VersionBump, VersionBumpSpec, VersionFormat};
...
/// Display version information for fv itself (`fv self version`)
pub(crate) fn self_version(
    short: bool,
    output_format: VersionFormat,
    printer: Printer,
) -> Result<ExitStatus> {
    let version_info = fv_cli::version::uv_self_version();
    match output_format {
        VersionFormat::Text => {
            if short {
                writeln!(printer.stdout(), "{}", version_info.cyan())?;
            } else {
                writeln!(printer.stdout(), "fv {}", version_info.cyan())?;
            }
        }
        VersionFormat::Json => {
...
/// Read or update project version (`fv version`)
...
/// Add hint to use `fv self version` when workspace discovery fails due to missing pyproject.toml
...
fn hint_uv_self_version(err: WorkspaceError, explicit_project: bool) -> anyhow::Error {
    if matches!(err, WorkspaceError::MissingPyprojectToml) && !explicit_project {
        anyhow!(
            "{}\n\n{}{} If you meant to view fv's version, use `{}` instead",
            err,
            "hint".bold().cyan(),
            ":".bold(),
            "fv self version".green()
        )
    } else {
        err.into()
    }
...
///
/// Note that `fv version` never needs to support PEP 723 scripts, as those are unversioned.
async fn find_target(
    ...
    // No workspace caching since `fv version` changes the workspace definition.
    let project = if let Some(package) = package {
        VirtualProject::discover_with_package(
            project_dir,
            &DiscoveryOptions {
                project: fv_workspace::ProjectDiscovery::Required,
