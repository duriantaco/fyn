...
use fv_fs::{PortablePathBuf, Simplified};
use fv_installer::{InstallationStrategy, SitePackages};
...
use fv_pypi_types::{ParsedArchiveUrl, ParsedGitUrl, ParsedUrl};
use fv_python::{PythonDownloads, PythonEnvironment, PythonPreference, PythonRequest};
use fv_resolver::{FlatIndex, ForkStrategy, Installable, Lock, PrereleaseMode, ResolutionMode};
use fv_scripts::Pep723Script;
use fv_settings::PythonInstallMirrors;
use fv_types::{BuildIsolation, HashStrategy};
use fv_warnings::warn_user;
use fv_workspace::pyproject::Source;
use fv_workspace::{DiscoveryOptions, MemberDiscovery, VirtualProject, Workspace, WorkspaceCache};
...
pub(crate) async fn sync(
        ...
        if !lockfile.is_file() {
            if frozen.is_some() {
                return Err(anyhow::anyhow!(
                    "`fv sync --frozen` requires a script lockfile; run `{}` to lock the script",
                    format!("fv lock --script {}", script.path.user_display()).green(),
                ));
            }

            if let LockCheck::Enabled(lock_check) = lock_check {
                return Err(anyhow::anyhow!(
                    "`fv sync {lock_check}` requires a script lockfile; run `{}` to lock the script",
                    format!("fv lock --script {}", script.path.user_display()).green(),
                ));
            }
    ...
    if let SyncTarget::Project(project) = &target {
        ...
        for (name, member) in project.workspace().packages() {
            ...
            {
                warn_user!(
                    "Skipping installation of entry points (`project.scripts`) for package `{}` because this project is not packaged; to install entry points, set `tool.fv.package = true` or define a `build-system`",
                    name
                );
            }
        }
...
/// don't store plaintext credentials in the `fv.lock`, we do respect credentials that are defined
/// in the `pyproject.toml`.
///
/// These credentials can come from any of `tool.fv.sources`, `tool.fv.dev-dependencies`,
...
fn store_credentials_from_target(target: InstallTarget<'_>, client_builder: &BaseClientBuilder) {
    ...
    for source in target.sources() {
        match source {
            Source::Git { git, .. } => {
                fv_git::store_credentials_from_url(git);
            }
            Source::Url { url, .. } => {
    ...
    for requirement in target.requirements() {
        ...
        match &url.parsed_url {
            ParsedUrl::Git(ParsedGitUrl { url, .. }) => {
                fv_git::store_credentials_from_url(url.repository());
            }
...
/// A report of the fv sync operation
#[derive(Debug, Serialize)]
#[serde(rename_all = "snake_case")]
struct Report {
...
struct PythonReport {
    path: PortablePathBuf,
    version: fv_pep508::StringVersion,
    implementation: String,
}

impl From<&fv_python::Interpreter> for PythonReport {
    fn from(interpreter: &fv_python::Interpreter) -> Self {
        Self {
...
struct PackageChangeReport {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    version: Option<fv_pep440::Version>,
