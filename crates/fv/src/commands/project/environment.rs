...
use fv_cache_key::{cache_digest, hash_digest};
use fv_client::BaseClientBuilder;
use fv_configuration::{Concurrency, Constraints, TargetTriple};
use fv_distribution_types::{
    BuiltDist, Dist, Identifier, Node, Resolution, ResolvedDist, SourceDist,
};
use fv_fs::PythonExt;
use fv_preview::Preview;
use fv_python::{Interpreter, PythonEnvironment, canonicalize_executable};
...
impl EphemeralEnvironment {
    ...
    ///
    /// Ephemeral environments created by `fv run --with` extend a parent (virtual or system)
...
struct CachedEnvironmentDist {
    dist: ResolvedDist,
    hashes: fv_pypi_types::HashDigests,
...
impl CachedEnvironment {
    ...
    pub(crate) async fn from_spec(
        ...
        // assumption is such that the environment will _not_ be modified by the user or fv;
        ...
        let venv = fv_virtualenv::create_venv(
            ...
            false,
            fv_virtualenv::OnExisting::Remove(fv_virtualenv::RemovalReason::TemporaryEnvironment),
            true,
            false,
            false,
        )?;
    ...
    /// distribution.
    fn cache_info(dist: &ResolvedDist) -> Result<Option<CacheInfo>, fv_cache_info::CacheInfoError> {
        let path = match dist {
    ...
    fn base_interpreter(
        interpreter: &Interpreter,
        cache: &Cache,
    ) -> Result<Interpreter, fv_python::Error> {
