...
use fv_dispatch::BuildDispatch;
use fv_distribution::{DistributionDatabase, LoweredExtraBuildDependencies};
...
use fv_fs::{LockedFile, LockedFileError, Simplified};
use fv_git::GIT_STORE;
use fv_normalize::{DEV_DEPENDENCIES, DefaultExtras, DefaultGroups, ExtraName, PackageName};
use fv_pep508::{MarkerTree, VersionOrUrl};
use fv_preview::Preview;
use fv_python::{Interpreter, PythonDownloads, PythonEnvironment, PythonPreference, PythonRequest};
use fv_redacted::DisplaySafeUrl;
use fv_requirements::{NamedRequirementsResolver, RequirementsSource, RequirementsSpecification};
use fv_resolver::FlatIndex;
use fv_scripts::{Pep723Metadata, Pep723Script};
use fv_settings::PythonInstallMirrors;
use fv_types::{BuildIsolation, HashStrategy};
use fv_warnings::warn_user_once;
use fv_workspace::pyproject::{DependencyType, Source, SourceError, Sources, ToolFvSources};
use fv_workspace::pyproject_mut::{AddBoundsKind, ArrayEdit, DependencyTarget, PyProjectTomlMut};
...
pub(crate) async fn add(
    ...
    for source in &requirements {
        match source {
            RequirementsSource::PyprojectToml(_) => {
                bail!("Adding requirements from a `pyproject.toml` is not supported in `fv add`");
            }
            RequirementsSource::SetupPy(_) => {
                bail!("Adding requirements from a `setup.py` is not supported in `fv add`");
            }
            RequirementsSource::Pep723Script(_) => {
                bail!("Adding requirements from a PEP 723 script is not supported in `fv add`");
            }
            RequirementsSource::SetupCfg(_) => {
                bail!("Adding requirements from a `setup.cfg` is not supported in `fv add`");
            }
            RequirementsSource::PylockToml(_) => {
                bail!("Adding requirements from a `pylock.toml` is not supported in `fv add`");
            }
    ...
    let mut target = if let Some(script) = script {
        ...
        // No workspace caching since `fv add` changes the workspace definition.
    ...
    // ensure the group/extra exists. This handles the case where `fv add -r requirements.txt
    // --group <name>` or `fv add -r requirements.txt --optional <extra>` is called with an empty
    // requirements file.
    if edits.is_empty() {
        match &dependency_type {
    ...
    // If `--frozen`, exit early. There's no reason to lock and sync, since we don't need a `fv.lock`
    // to exist at all.
    if frozen.is_some() {
    ...
    for mut requirement in requirements {
        ...
        let (requirement, source) = match target {
            AddTarget::Script(_, _) | AddTarget::Project(_, _) if raw => {
                (fv_pep508::Requirement::from(requirement), None)
            }
            AddTarget::Script(script, _) => {
        ...
        // will be checked into version control (e.g., `pyproject.toml` and `fv.lock`). Instead,
        ...
        let source = match source {
            Some(Source::Git {
                ...
                let credentials = fv_auth::Credentials::from_url(&git);
...
fn resolve_requirement(
...
) -> Result<(fv_pep508::Requirement, Option<Source>), anyhow::Error> {
    ...
    let mut processed_requirement = fv_pep508::Requirement::from(requirement);
...
impl AddTargetSnapshot {
    /// Write the snapshot back to disk (e.g., to a `pyproject.toml` and `fv.lock`).
        ...
        match self {
            Self::Script(script, lock) => {
                ...
                if let Some(lock) = lock {
                    debug!("Reverting changes to `fv.lock`");
                    fs_err::write(target.lock_path(), lock)?;
                } else {
                    debug!("Removing `fv.lock`");
...
struct DependencyEdit {
    dependency_type: DependencyType,
    requirement: fv_pep508::Requirement,
