...
use fv_auth::CredentialsCache;
use fv_cache::{Cache, CacheBucket};
use fv_cache_key::cache_digest;
use fv_client::{BaseClientBuilder, FlatIndexClient, RegistryClientBuilder};
...
use fv_dispatch::{BuildDispatch, SharedState};
use fv_distribution::{DistributionDatabase, LoweredExtraBuildDependencies, LoweredRequirement};
use fv_distribution_types::{
    ExtraBuildRequirement, ExtraBuildRequires, Index, Requirement, RequiresPython, Resolution,
    UnresolvedRequirement, UnresolvedRequirementSpecification,
};
use fv_fs::{CWD, LockedFile, LockedFileError, LockedFileMode, Simplified};
use fv_git::ResolvedRepositoryReference;
use fv_installer::{InstallationStrategy, SatisfiesResult, SitePackages};
use fv_normalize::{DEV_DEPENDENCIES, DefaultGroups, ExtraName, GroupName, PackageName};
use fv_pep440::{TildeVersionSpecifier, Version, VersionSpecifiers};
use fv_pep508::MarkerTreeContents;
use fv_preview::{Preview, PreviewFeature};
use fv_pypi_types::{ConflictItem, ConflictKind, ConflictSet, Conflicts};
use fv_python::{
...
use fv_requirements::upgrade::{LockedRequirements, read_lock_requirements};
use fv_requirements::{NamedRequirementsResolver, RequirementsSpecification};
use fv_resolver::{
    FlatIndex, Installable, Lock, OptionsBuilder, Preference, PythonRequirement,
    ResolverEnvironment, ResolverOutput,
};
use fv_scripts::Pep723ItemRef;
use fv_settings::PythonInstallMirrors;
use fv_static::EnvVars;
use fv_torch::{TorchSource, TorchStrategy};
use fv_types::{BuildIsolation, EmptyInstalledPackages, HashStrategy};
use fv_virtualenv::remove_virtualenv;
use fv_warnings::{warn_user, warn_user_once};
use fv_workspace::dependency_groups::DependencyGroupError;
use fv_workspace::pyproject::{ExtraBuildDependency, PyProjectToml};
use fv_workspace::{RequiresPythonSources, Workspace, WorkspaceCache};
...
pub(crate) enum ProjectError {
    #[error(
        "The lockfile at `fv.lock` needs to be updated, but `{2}` was provided. To update the lockfile, run `fv lock`."
    )]
    LockMismatch(Option<Box<Lock>>, Box<Lock>, LockCheckSource),

    #[error(
        "Unable to find lockfile at `fv.lock`, but {0} was provided. To create a lockfile, run `fv lock` or `fv sync` without the flag."
    )]
    MissingLockfile(MissingLockfileSource),

    #[error(
        "The lockfile at `fv.lock` needs to be updated, but `--frozen` was provided: Missing workspace member `{0}`. To update the lockfile, run `fv lock`."
    )]
    LockWorkspaceMismatch(PackageName),

    #[error(
        "The lockfile at `fv.lock` uses an unsupported schema version (v{1}, but only v{0} is supported). Downgrade to a compatible fv version, or remove the `fv.lock` prior to running `fv lock` or `fv sync`."
    )]
    UnsupportedLockVersion(u32, u32),

    #[error(
        "Failed to parse `fv.lock`, which uses an unsupported schema version (v{1}, but only v{0} is supported). Downgrade to a compatible fv version, or remove the `fv.lock` prior to running `fv lock` or `fv sync`."
    )]
    UnparsableLockVersion(u32, u32, #[source] toml::de::Error),

    #[error("Failed to serialize `fv.lock`")]
    ...
    #[error(
        "Default group `{0}` (from `tool.fv.default-groups`) is not defined in the project's `dependency-groups` table"
    )]
    ...
    #[error(transparent)]
    Client(#[from] fv_client::Error),

    #[error(transparent)]
    Python(#[from] fv_python::Error),

    #[error(transparent)]
    Virtualenv(#[from] fv_virtualenv::Error),

    #[error(transparent)]
    HashStrategy(#[from] fv_types::HashStrategyError),

    #[error(transparent)]
    Tags(#[from] fv_platform_tags::TagsError),

    #[error(transparent)]
    FlatIndex(#[from] fv_client::FlatIndexError),

    #[error(transparent)]
    Lock(#[from] fv_resolver::LockError),

    #[error(transparent)]
    Operation(#[from] pip::operations::Error),

    #[error(transparent)]
    Interpreter(#[from] fv_python::InterpreterError),

    #[error(transparent)]
    Tool(#[from] fv_tool::Error),

    #[error(transparent)]
    Name(#[from] fv_normalize::InvalidNameError),

    #[error(transparent)]
    Requirements(#[from] fv_requirements::Error),

    #[error(transparent)]
    Metadata(#[from] fv_distribution::MetadataError),

    #[error(transparent)]
    Lowering(#[from] fv_distribution::LoweringError),

    #[error(transparent)]
    Workspace(#[from] fv_workspace::WorkspaceError),

    #[error(transparent)]
    PyprojectMut(#[from] fv_workspace::pyproject_mut::Error),

    #[error(transparent)]
    ExtraBuildRequires(#[from] fv_distribution_types::ExtraBuildRequiresError),

    #[error(transparent)]
    Fmt(#[from] std::fmt::Error),

    #[error(transparent)]
    CacheInfo(#[from] fv_cache_info::CacheInfoError),

    #[error(transparent)]
    Io(#[from] std::io::Error),

    #[error(transparent)]
    RetryParsing(#[from] fv_client::RetryParsingError),

    #[error(transparent)]
    Accelerator(#[from] fv_torch::AcceleratorError),
...
impl ScriptInterpreter {
    ...
    pub(crate) fn root(script: Pep723ItemRef<'_>, active: Option<bool>, cache: &Cache) -> PathBuf {
        ...
        if let Some(from_virtual_env) = from_virtual_env_variable() {
            if !fv_fs::is_same_file_allow_missing(&from_virtual_env, &cache_env).unwrap_or(false) {
                match active {
                    Some(true) => {
                        debug!(
    ...
    pub(crate) async fn discover(
        ...
        match PythonEnvironment::from_root(&root, cache) {
            ...
            Err(fv_python::Error::MissingEnvironment(_)) => {}
    ...
    pub(crate) async fn lock(script: Pep723ItemRef<'_>) -> Result<LockedFile, LockedFileError> {
        match script {
            Pep723ItemRef::Script(script) => {
                LockedFile::acquire(
                    std::env::temp_dir().join(format!("fv-{}.lock", cache_digest(&script.path))),
            ...
            Pep723ItemRef::Remote(.., url) => {
                LockedFile::acquire(
                    std::env::temp_dir().join(format!("fv-{}.lock", cache_digest(url))),
                    LockedFileMode::Exclusive,
                    url.to_string(),
                )
                .await
            ...
            Pep723ItemRef::Stdin(metadata) => {
                LockedFile::acquire(
                    std::env::temp_dir().join(format!("fv-{}.lock", cache_digest(&metadata.raw))),
                    LockedFileMode::Exclusive,
                    "stdin".to_string(),
                )
                .await
...
impl ProjectInterpreter {
    ...
    pub(crate) async fn discover(
        ...
        match PythonEnvironment::from_root(&root, cache) {
            ...
            Err(fv_python::Error::MissingEnvironment(_)) => {}
            Err(fv_python::Error::InvalidEnvironment(inner)) => {
            ...
            Err(fv_python::Error::Query(fv_python::InterpreterError::NotFound(_))) => {}
            Err(fv_python::Error::Query(fv_python::InterpreterError::BrokenLink(BrokenLink {
                path,
                unix,
                venv: _,
            }))) => {
        ...
        .await?
        {
            ...
            ProjectInterpreter::Interpreter(interpreter) => {
                ...
                let replace = match (root.try_exists(), root.join("pyvenv.cfg").try_exists()) {
                    ...
                    (_, Err(err)) | (Err(err), _) => {
                        return Err(ProjectError::InvalidProjectEnvironmentDir(
                            root,
                            format!(
                                "it is not a compatible environment but cannot be recreated because fv cannot determine if it is a virtual environment: {err}"
                            ),
                        ));
                    }
                };
                ...
                let prompt = workspace
                    ...
                    .map(fv_virtualenv::Prompt::Static)
                    .unwrap_or(fv_virtualenv::Prompt::None);

                // Under `--dry-run`, avoid modifying the environment.
                if dry_run.enabled() {
                    let temp_dir = cache.venv_dir()?;
                    let environment = fv_virtualenv::create_venv(
                        temp_dir.path(),
                        interpreter,
                        prompt,
                        false,
                        fv_virtualenv::OnExisting::Remove(
                            fv_virtualenv::RemovalReason::ManagedEnvironment,
                        ),
                        false,
                        false,
                        upgradeable,
                ...
                if replace {
                    match remove_virtualenv(&root) {
                        ...
                        Err(fv_virtualenv::Error::Io(err))
...
pub(crate) async fn resolve_names(
    ...
    preview: Preview,
    lfs: GitLfsSetting,
) -> Result<Vec<Requirement>, fv_requirements::Error> {
    ...
    let torch_backend = torch_backend
        .map(|mode| {
            let source = if fv_auth::PyxTokenStore::from_settings()
    ...
    let build_isolation = match build_isolation {
        fv_configuration::BuildIsolation::Isolate => BuildIsolation::Isolated,
        fv_configuration::BuildIsolation::Shared => {
            environment = PythonEnvironment::from_interpreter(interpreter.clone());
            BuildIsolation::Shared(&environment)
        }
        fv_configuration::BuildIsolation::SharedPackage(packages) => {
            environment = PythonEnvironment::from_interpreter(interpreter.clone());
...
pub(crate) async fn sync_environment(
    ...
    let build_isolation = match build_isolation {
        fv_configuration::BuildIsolation::Isolate => BuildIsolation::Isolated,
        fv_configuration::BuildIsolation::Shared => BuildIsolation::Shared(&venv),
        fv_configuration::BuildIsolation::SharedPackage(packages) => {
...
pub(crate) fn default_dependency_groups(
    ...
    if let Some(defaults) = pyproject_toml
        .tool
        .as_ref()
        .and_then(|tool| tool.fv.as_ref().and_then(|fv| fv.default_groups.as_ref()))
    {
...
pub(crate) fn script_specification(
    ...
    let constraints = script
        ...
        .tool
        .as_ref()
        .and_then(|tool| tool.fv.as_ref())
        .and_then(|fv| fv.constraint_dependencies.as_ref())
        .into_iter()
        .flatten()
        .cloned()
        .flat_map(|requirement| {
    ...
    let overrides = script
        ...
        .as_ref()
        .and_then(|tool| tool.fv.as_ref())
        .and_then(|fv| fv.override_dependencies.as_ref())
        .into_iter()
        .flatten()
        .cloned()
        .flat_map(|requirement| {
    ...
    let excludes = script
        ...
        .as_ref()
        .and_then(|tool| tool.fv.as_ref())
        .and_then(|fv| fv.exclude_dependencies.as_ref())
        .into_iter()
        .flatten()
        .cloned()
        .collect::<Vec<_>>();
...
pub(crate) fn script_extra_build_requires(
    ...
    // Collect any `tool.fv.extra-build-dependencies` from the script.
    ...
    let script_extra_build_dependencies = script
        ...
        .as_ref()
        .and_then(|tool| tool.fv.as_ref())
        .and_then(|fv| fv.extra_build_dependencies.as_ref())
        .unwrap_or(&empty);
...
fn warn_on_requirements_txt_setting(spec: &RequirementsSpecification, settings: &ResolverSettings) {
    ...
    if settings.index_locations.no_index() {
        ...
        warn_user_once!(
            "Ignoring `--no-index` from requirements file. Instead, use the `--no-index` command-line argument, or set `no-index` in a `fv.toml` or `pyproject.toml` file."
        );
    } else {
        if let Some(index_url) = index_url {
            if settings.index_locations.default_index().map(Index::url) != Some(index_url) {
                warn_user_once!(
                    "Ignoring `--index-url` from requirements file: `{index_url}`. Instead, use the `--index-url` command-line argument, or set `index-url` in a `fv.toml` or `pyproject.toml` file."
                );
            }
        }
        for extra_index_url in extra_index_urls {
            ...
            {
                warn_user_once!(
                    "Ignoring `--extra-index-url` from requirements file: `{extra_index_url}`. Instead, use the `--extra-index-url` command-line argument, or set `extra-index-url` in a `fv.toml` or `pyproject.toml` file.`"
                );
            }
        }
        for find_link in find_links {
            ...
            {
                warn_user_once!(
                    "Ignoring `--find-links` from requirements file: `{find_link}`. Instead, use the `--find-links` command-line argument, or set `find-links` in a `fv.toml` or `pyproject.toml` file.`"
                );
            }
        }
    }
    ...
    if !no_binary.is_none() && settings.build_options.no_binary() != no_binary {
        warn_user_once!(
            "Ignoring `--no-binary` setting from requirements file. Instead, use the `--no-binary` command-line argument, or set `no-binary` in a `fv.toml` or `pyproject.toml` file."
        );
    }

    if !no_build.is_none() && settings.build_options.no_build() != no_build {
        warn_user_once!(
            "Ignoring `--no-binary` setting from requirements file. Instead, use the `--no-build` command-line argument, or set `no-build` in a `fv.toml` or `pyproject.toml` file."
        );
    }
}
    ...
    if conflicts.len() == 1 {
        ...
        if let Some(group) = group {
            if workspace_non_trivial {
                return format!(
                    " (from workspace member `{package}`'s `tool.fv.dependency-groups.{group}.requires-python`)."
                );
            }
            return format!(" (from `tool.fv.dependency-groups.{group}.requires-python`).");
        }
        if workspace_non_trivial {
