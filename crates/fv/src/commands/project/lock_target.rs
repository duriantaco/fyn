...
use fv_configuration::{DependencyGroupsWithDefaults, NoSources};
use fv_distribution::LoweredRequirement;
use fv_distribution_types::{Index, IndexLocations, Requirement, RequiresPython};
use fv_normalize::{GroupName, PackageName};
use fv_pep508::RequirementOrigin;
use fv_pypi_types::{Conflicts, SupportedEnvironments, VerbatimParsedUrl};
use fv_resolver::{Lock, LockVersion, VERSION};
use fv_scripts::Pep723Script;
use fv_workspace::dependency_groups::{DependencyGroupError, FlatDependencyGroup};
use fv_workspace::{Editability, Workspace, WorkspaceMember};
...
impl<'lock> LockTarget<'lock> {
    ...
    pub(crate) fn requirements(self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
        match self {
    ...
    pub(crate) fn overrides(self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
        match self {
    ...
    pub(crate) fn exclude_dependencies(self) -> Vec<fv_normalize::PackageName> {
        match self {
    ...
    pub(crate) fn constraints(self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
        match self {
    ...
    pub(crate) fn build_constraints(self) -> Vec<fv_pep508::Requirement<VerbatimParsedUrl>> {
        match self {
        ...
        match self {
            ...
            Self::Script(script) => Either::Right(
                script
                    ...
                    .as_ref()
                    .and_then(|tool| tool.fv.as_ref())
                    .and_then(|fv| fv.top_level.index.as_deref())
                    .into_iter()
                    .flatten(),
            ),
        }
    ...
    /// defaults to `fv.lock` in the workspace root.
    pub(crate) fn lock_path(self) -> PathBuf {
        let lockfile_name = std::env::var(fv_static::EnvVars::UV_LOCKFILE)
            .ok()
            .filter(|s| !s.is_empty())
            .filter(|s| !s.contains('/') && !s.contains('\\') && !s.contains(".."))
            .unwrap_or_else(|| "fv.lock".to_string());
    ...
    pub(crate) fn lower(
        self,
        requirements: Vec<fv_pep508::Requirement<VerbatimParsedUrl>>,
        locations: &IndexLocations,
        sources: &NoSources,
        credentials_cache: &CredentialsCache,
    ) -> Result<Vec<Requirement>, fv_distribution::MetadataError> {
        match self {
            Self::Workspace(workspace) => {
                let name = workspace
                    .pyproject_toml()
                ...
                let metadata = fv_distribution::BuildRequires::from_workspace(
                    fv_pypi_types::BuildRequires {
                        name,
            ...
            Self::Script(script) => {
                // Collect any `tool.fv.index` from the script.
                ...
                // Collect any `tool.fv.sources` from the script.
                    ...
                    .flat_map(|requirement| {
                        ...
                        if sources.for_package(&requirement.name) {
                            ...
                            .map(move |requirement| match requirement {
                                ...
                                Err(err) => Err(fv_distribution::MetadataError::LoweringError(
