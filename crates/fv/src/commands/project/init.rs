...
use fv_scripts::{Pep723Script, ScriptTag};
...
use fv_workspace::{DiscoveryOptions, MemberDiscovery, Workspace, WorkspaceCache, WorkspaceError};
...
fn pyproject_build_system(package: &PackageName, build_backend: ProjectBuildBackend) -> String {
    ...
    match build_backend {
        ProjectBuildBackend::Uv => {
            ...
            let min_version = Version::from_str(fv_version::version()).unwrap();
            debug_assert!(
            ...
            indoc::formatdoc! {r#"
                [build-system]
                requires = ["fv_build>={min_version},<{max_version}"]
