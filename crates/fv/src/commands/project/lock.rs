...
use fv_types::{BuildContext, BuildIsolation, EmptyInstalledPackages, HashStrategy};
use fv_warnings::{warn_user, warn_user_once};
use fv_workspace::{DiscoveryOptions, Editability, Workspace, WorkspaceCache, WorkspaceMember};
        ...
        Ok(lock) => {
            if let Some(frozen_source) = frozen {
                match frozen_source {
                    FrozenSource::Cli => {
                        warn_user!(
                            "The lockfile at `fv.lock` was only checked for validity, not whether it is up-to-date, because `--frozen` was provided; use `--check` instead"
                        );
                    }
                    FrozenSource::Env | FrozenSource::Configuration => {
                        warn_user!(
                            "The lockfile at `fv.lock` was only checked for validity, not whether it is up-to-date, because {} was provided; use `--no-frozen` or `--check` instead",
    ...
    async fn validate<Context: BuildContext>(
            ...
            .await?
        {
            SatisfiesResult::Satisfied => {
                debug!("Existing `fv.lock` satisfies workspace requirements");
