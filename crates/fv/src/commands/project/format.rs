...
use fv_bin_install::{BinVersion, Binary, ResolvedVersion, bin_install, find_matching_version};
use fv_cache::Cache;
...
pub(crate) async fn format(
    ...
    if !preview.is_enabled(PreviewFeature::Format) {
        warn_user!(
            "`fv format` is experimental and may change without warning. Pass `--preview-features {}` to disable this warning.",
