...
use anyhow::Result;
use fv_audit::service::osv;
use fv_audit::types::{Dependency, Finding};
use fv_cache::Cache;
use fv_client::BaseClientBuilder;
use fv_configuration::{Concurrency, DependencyGroups, ExtrasSpecification, TargetTriple};
...
use fv_workspace::{DiscoveryOptions, Workspace, WorkspaceCache};
use tracing::trace;
...
pub(crate) async fn audit(
    ...
    if !preview.is_enabled(PreviewFeature::Audit) {
        warn_user!(
            "`fv audit` is experimental and may change without warning. Pass `--preview-features {}` to disable this warning.",
    ...
    let workspace_root_name = lock.root().map(fv_resolver::Package::name);
