...
use fv_distribution_types::Requirement;
use fv_fs::which::is_executable;
use fv_fs::{PythonExt, Simplified, create_symlink};
use fv_installer::{InstallationStrategy, SatisfiesResult, SitePackages};
use fv_normalize::{DefaultExtras, DefaultGroups, PackageName};
...
use fv_requirements::{RequirementsSource, RequirementsSpecification};
use fv_resolver::{Installable, Lock, Preference};
use fv_scripts::Pep723Item;
use fv_settings::PythonInstallMirrors;
use fv_shell::runnable::WindowsRunnable;
use fv_static::EnvVars;
use fv_warnings::warn_user;
use fv_workspace::{DiscoveryOptions, VirtualProject, WorkspaceCache, WorkspaceError};
...
pub(crate) async fn run(
    ...
    // for scripts with a shebang line like `#!/usr/bin/env -S fv run`, so try
    // to provide guidance for that case.
    let recursion_depth = read_recursion_depth_from_environment_variable()?;
    if recursion_depth > max_recursion_depth {
        bail!(
            r"
`fv run` was recursively invoked {recursion_depth} times which exceeds the limit of {max_recursion_depth}.
...
hint: If you are running a script with `{}` in the shebang, you may need to include the `{}` flag.",
    ...
    for source in &requirements {
        match source {
            RequirementsSource::PyprojectToml(_) => {
                bail!("Adding requirements from a `pyproject.toml` is not supported in `fv run`");
            }
            RequirementsSource::SetupPy(_) => {
                bail!("Adding requirements from a `setup.py` is not supported in `fv run`");
            }
            RequirementsSource::SetupCfg(_) => {
                bail!("Adding requirements from a `setup.cfg` is not supported in `fv run`");
            }
            ...
            .filter(|target| target.lock_path().is_file())
        {
            ...
            if let Some(spec) = script_specification(
                ...
                let environment = fv_virtualenv::create_venv(
                    temp_dir.path(),
                    interpreter,
                    fv_virtualenv::Prompt::None,
                    false,
                    fv_virtualenv::OnExisting::Remove(
                        fv_virtualenv::RemovalReason::TemporaryEnvironment,
                    ),
                    false,
                    false,
                    false,
    ...
    let base_interpreter = if let Some(script_interpreter) = script_interpreter {
        ...
        if let Some(project) = project {
            ...
            if isolated {
                ...
                temp_dir = cache.venv_dir()?;
                let venv = fv_virtualenv::create_venv(
                    temp_dir.path(),
                    interpreter,
                    fv_virtualenv::Prompt::None,
                    false,
    ...
    if let Some(ephemeral_env) = ephemeral_env.as_ref() {
        if let Some(requirements_env) = requirements_env.as_ref() {
            ...
            // For ephemeral environments created by `fv run --with`, the parent environment's
    ...
    let Some(command) = command else {
        ...
        let help = format!("See `{}` for more information.", "fv run --help".bold());
...
/// Returns `true` if we can skip creating an additional ephemeral environment in `fv run`.
    ...
    fn as_command(&self, interpreter: &Interpreter) -> Command {
        match self {
            ...
            Self::PythonPackage(target, path, args) => {
                ...
                if fv_fs::which::is_executable(&entrypoint) {
...
impl RunCommand {
    ...
    ///
    /// If the target matches a task defined in `[tool.fv.tasks]`, run that
    ...
    pub(crate) async fn from_args(
        ...
        // Check if the target matches a task in [tool.fv.tasks] before
...
/// `[tool.fv.tasks]` section that contains the given name.
fn lookup_task(name: &str, project_dir: &Path) -> Option<fv_workspace::pyproject::TaskDefinition> {
    ...
    loop {
        ...
        if pyproject_path.is_file() {
            if let Ok(content) = fs_err::read_to_string(&pyproject_path) {
                if let Ok(pyproject) =
                    toml::from_str::<fv_workspace::pyproject::PyProjectToml>(&content)
                {
                    if let Some(tool_fv) = pyproject.tool.and_then(|t| t.fv) {
                        if let Some(tasks) = tool_fv.tasks {
...
fn task_to_command(
    task: &fv_workspace::pyproject::TaskDefinition,
    extra_args: &[OsString],
) -> anyhow::Result<RunCommand> {
    use fv_workspace::pyproject::TaskDefinition;
...
enum CopyEntrypointError {
    ...
    #[cfg(windows)]
    #[error(transparent)]
    Trampoline(#[from] fv_trampoline_builder::Error),
}
...
fn copy_entrypoint(
    ...
    use fv_trampoline_builder::Launcher;
