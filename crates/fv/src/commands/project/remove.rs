...
use fv_normalize::{DEV_DEPENDENCIES, DefaultExtras, DefaultGroups};
use fv_preview::Preview;
use fv_python::{PythonDownloads, PythonPreference, PythonRequest};
use fv_scripts::{Pep723Metadata, Pep723Script};
use fv_settings::PythonInstallMirrors;
use fv_warnings::warn_user_once;
use fv_workspace::pyproject::DependencyType;
use fv_workspace::pyproject_mut::{DependencyTarget, PyProjectTomlMut};
...
pub(crate) async fn remove(
    ...
    let target = if let Some(script) = script {
        ...
        // No workspace caching since `fv remove` changes the workspace definition.
        ...
        match dependency_type {
            ...
            DependencyType::Dev => {
                ...
                if dev_deps.is_empty() && group_deps.is_empty() {
                    show_other_dependency_type_hint(printer, &package, &toml)?;
                    anyhow::bail!(
                        "The dependency `{package}` could not be found in `tool.fv.dev-dependencies` or `tool.fv.dependency-groups.dev`"
                    );
                }
            }
    ...
    // When removing from a specific group, include that group in the re-sync so
    // we don't accidentally uninstall everything the group provides.
    ...
    let groups = match &dependency_type {
        DependencyType::Group(group) => {
            DependencyGroups::from_group(group.clone()).with_defaults(default_groups)
        }
        DependencyType::Dev => {
            DependencyGroups::from_dev_mode(DevMode::Include).with_defaults(default_groups)
        }
        DependencyType::Production | DependencyType::Optional(_) => {
    ...
    for dep_ty in pyproject.find_dependency(name, None) {
        match dep_ty {
            ...
            DependencyType::Dev => writeln!(
                ...
                "hint".bold().cyan(),
                ":".bold(),
                format!("fv remove {name} --dev`").bold()
            )?,
            DependencyType::Optional(group) => writeln!(
                ...
                "hint".bold().cyan(),
                ":".bold(),
                format!("fv remove {name} --optional {group}").bold()
            )?,
            DependencyType::Group(group) => writeln!(
                ...
                "hint".bold().cyan(),
                ":".bold(),
                format!("fv remove {name} --group {group}").bold()
            )?,
        }
    }
