...
use fv_configuration::{Constraints, DependencyGroupsWithDefaults, ExtrasSpecification};
use fv_distribution_types::Index;
use fv_normalize::{ExtraName, PackageName};
use fv_pypi_types::{DependencyGroupSpecifier, LenientRequirement, VerbatimParsedUrl};
use fv_resolver::{Installable, Lock, Package};
use fv_scripts::Pep723Script;
use fv_workspace::Workspace;
use fv_workspace::pyproject::{Source, Sources, ToolFvSources};
...
impl<'lock> InstallTarget<'lock> {
    ...
    pub(crate) fn requirements(
        &self,
    ) -> impl Iterator<Item = Cow<'lock, fv_pep508::Requirement<VerbatimParsedUrl>>> {
        match self {
            Self::Project { workspace, .. }
            | Self::Projects { workspace, .. }
            | Self::Workspace { workspace, .. }
                            ...
                            dependencies
                                ...
                                .filter_map(|requires_dist| {
                                    LenientRequirement::<VerbatimParsedUrl>::from_str(requires_dist)
                                        .map(fv_pep508::Requirement::from)
                                        .map(Cow::Owned)
                                        .ok()
                                })
