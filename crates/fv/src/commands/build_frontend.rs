...
use fv_build_backend::check_direct_build;
...
use fv_distribution::LoweredExtraBuildDependencies;
...
use fv_fs::{Simplified, relative_to};
...
use fv_requirements::RequirementsSource;
use fv_resolver::{ExcludeNewer, FlatIndex};
use fv_settings::PythonInstallMirrors;
use fv_types::{AnyErrorBuild, BuildContext, BuildStack, HashStrategy};
use fv_workspace::pyproject::ExtraBuildDependencies;
use fv_workspace::{DiscoveryOptions, Workspace, WorkspaceCache, WorkspaceError};
...
enum Error {
    ...
    Io(#[from] io::Error),
    #[error(transparent)]
    FindOrDownloadPython(#[from] fv_python::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    Extract(#[from] fv_extract::Error),
    #[error(transparent)]
    Operations(#[from] operations::Error),
    #[error(transparent)]
    Join(#[from] tokio::task::JoinError),
    #[error(transparent)]
    BuildBackend(#[from] fv_build_backend::Error),
    #[error(transparent)]
    BuildDispatch(AnyErrorBuild),
    #[error(transparent)]
    BuildFrontend(#[from] fv_build_frontend::Error),
    #[error(transparent)]
    ...
    ListForcePep517,
    #[error(
        "Can only use `--list` with a compatible fv build backend, but `{name}` is not compatible because {reason}"
    )]
    ...
    InvalidSourceDistExt(String, fv_distribution_filename::ExtensionError),
    #[error("The built source distribution has an invalid filename")]
    InvalidBuiltSourceDistFilename(#[source] fv_distribution_filename::SourceDistFilenameError),
    #[error("The built wheel has an invalid filename")]
    InvalidBuiltWheelFilename(#[source] fv_distribution_filename::WheelFilenameError),
    ...
    for (source, result) in results {
        match result {
            ...
            Err(err) => {
                ...
                let help = if let Error::Extract(fv_extract::Error::Tar(err)) = &err {
...
async fn build_package(
    ...
    let types_build_isolation = match build_isolation {
        BuildIsolation::Isolate => fv_types::BuildIsolation::Isolated,
        BuildIsolation::Shared => {
            environment = PythonEnvironment::from_interpreter(interpreter.clone());
            fv_types::BuildIsolation::Shared(&environment)
        }
        BuildIsolation::SharedPackage(packages) => {
            environment = PythonEnvironment::from_interpreter(interpreter.clone());
            fv_types::BuildIsolation::SharedPackage(&environment, packages)
        }
    };
    ...
    // Check if the build backend is matching fv version that allows calling in the fv build backend
    // directly.
    let build_action = if list {
        ...
        if let Err(reason) = check_direct_build(source.path(), fv_version::version()) {
        ...
        match check_direct_build(source.path(), fv_version::version()) {
            Ok(()) => BuildAction::DirectBuild,
            Err(reason) => {
                debug!(
                    "Not using `fv_build` direct build for `{}` because {}",
    ...
    let build_output = match printer {
        Printer::Default | Printer::NoProgress | Printer::Verbose => {
            if build_logs && !fv_flags::contains(fv_flags::EnvironmentFlags::HIDE_BUILD_OUTPUT) {
                BuildOutput::Stderr
            } else {
                BuildOutput::Quiet
            }
    ...
    match plan {
        BuildPlan::SdistToWheel => {
            ...
            fv_extract::stream::archive(path.display(), reader, ext, temp_dir.path()).await?;
        ...
        BuildPlan::WheelFromSdist => {
            ...
            fv_extract::stream::archive(source.path().display(), reader, ext, temp_dir.path())
                .await?;
...
impl BuildAction {
    ...
    fn force_build(self) -> Self {
        match self {
            // List is only available for the fv build backend
    ...
    let build_result = match action {
        BuildAction::List => {
            ...
            let (filename, file_list) = tokio::task::spawn_blocking(move || {
                fv_build_backend::list_source_dist(
                    &source_tree_,
                    fv_version::version(),
                    sources_enabled,
                )
        ...
        BuildAction::DirectBuild => {
            ...
            let filename = tokio::task::spawn_blocking(move || {
                fv_build_backend::build_source_dist(
                    &source_tree,
                    &output_dir_,
                    fv_version::version(),
                    sources_enabled,
...
async fn build_wheel(
    ...
    let build_message = match action {
        BuildAction::List => {
            ...
            let sources_enabled = sources.is_none();
            let (filename, file_list) = tokio::task::spawn_blocking(move || {
                fv_build_backend::list_wheel(&source_tree_, fv_version::version(), sources_enabled)
            })
            .await??;
            let raw_filename = filename.to_string();
            BuildMessage::List {
