...
use fv_tool::{InstalledTools, Tool, ToolEntrypoint};
...
pub(crate) async fn uninstall(name: Vec<PackageName>, printer: Printer) -> Result<ExitStatus> {
    ...
    if fv_fs::directories(installed_tools.root())?.all(|path| fv_fs::is_temporary(&path)) {
        ...
        if let Some(parent) = installed_tools.root().parent() {
            if fv_fs::directories(parent)?.all(|path| fv_fs::is_temporary(&path)) {
        ...
        for name in names {
            let Some(receipt) = installed_tools.get_tool_receipt(&name)? else {
                ...
                match installed_tools.remove_environment(&name) {
                    ...
                    Err(fv_tool::Error::VirtualEnvError(fv_virtualenv::Error::Io(err)))
