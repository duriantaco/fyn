...
use fv_pep440::{VersionSpecifier, VersionSpecifiers};
use fv_pep508::MarkerTree;
use fv_preview::Preview;
use fv_python::PythonVersionFile;
use fv_python::VersionFileDiscoveryOptions;
use fv_python::{
    EnvironmentPreference, PythonDownloads, PythonEnvironment, PythonInstallation,
    PythonPreference, PythonRequest,
};
use fv_requirements::{RequirementsSource, RequirementsSpecification};
use fv_settings::{PythonInstallMirrors, ResolverInstallerOptions, ToolOptions};
use fv_shell::runnable::WindowsRunnable;
use fv_static::EnvVars;
use fv_tool::{InstalledTools, entrypoint_paths};
use fv_warnings::warn_user;
use fv_warnings::warn_user_once;
use fv_workspace::WorkspaceCache;
...
impl Display for ToolRunCommand {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Fvx => write!(f, "fvx"),
            Self::ToolRun => write!(f, "fv tool run"),
        }
    }
}
...
pub(crate) async fn run(
    ...
    if let Some(ref from) = from {
        ...
        // If the user tries to invoke `fvx script.py`, hint them towards `fv run`.
        if has_python_script_ext(target_path) {
            return if target_path.try_exists()? {
                Err(anyhow::anyhow!(
                    ...
                    "hint".bold().cyan(),
                    ":".bold(),
                    format!("fv run {}", target_path.user_display()).green(),
                ))
            } else {
    ...
    // If the user tries to invoke `fvx run ruff`, hint them towards `fvx ruff`, but only if
    // the `run` package is guaranteed to come from PyPI.
    let (mut target, mut args) = (target, args);
    if from.is_none()
        && invocation_source == ToolRunCommand::Fvx
        && target == "run"
        && settings
            .resolver
            .index_locations
            ...
            .all(|index| matches!(index.url, IndexUrl::Pypi(..)))
    {
        ...
        if term.is_term() {
            ...
            let prompt = format!(
                ...
                format!("fvx run {rest}").green(),
                "run".cyan(),
                format!("fvx {rest}").green()
            );
            let confirmation = fv_console::confirm(&prompt, &term, true)?;
            if confirmation {
    ...
    let (from, environment) = match result {
        ...
        Err(ProjectError::Operation(err)) => {
            // If the user ran `fvx run ...`, the `run` is likely a mistake. Show a dedicated hint.
            if from.is_none() && invocation_source == ToolRunCommand::Fvx && target == "run" {
                let rest = args.iter().map(|s| s.to_string_lossy()).join(" ");
    ...
    if let Some(ref provider_hints) = provider_hints {
        if provider_hints.not_from_any() {
            ...
            // It'd be a breaking change but would make `fvx` invocations safer.
        ...
        match packages.as_slice() {
            ...
            packages => {
                let provided_by = packages
                    .iter()
                    .map(fv_distribution_types::Name::name)
...
async fn get_or_create_environment(
    ...
    let python_request = match (explicit_python_request, tool_python_request) {
        // e.g., `fvx --python 3.10 python3.12`
        ...
        ToolRequest::Package {
            ...
            let (executable, requirement) = match target {
                ...
                Target::Unspecified(requirement) => {
                    ...
                    // Prefer, in order:
                    // 1. The verbatim executable provided by the user, independent of the requirement (as in: `fvx --from package executable`).
                    // 2. The verbatim executable provided by the user as a named requirement (as in: `fvx change_wheel_version`).
                    // 3. The resolved package name (as in: `fvx git+https://github.com/pallets/flask`).
