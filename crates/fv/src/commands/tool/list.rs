...
use fv_distribution_types::{IndexCapabilities, RequiresPython};
use fv_fs::Simplified;
use fv_normalize::PackageName;
use fv_python::LenientImplementationName;
use fv_settings::ResolverInstallerOptions;
    ...
    for (name, tool) in tools {
        ...
        let Ok(tool) = tool else {
            warn_user!(
                ...
                format!("fv tool uninstall {name}").green()
            );
            continue;
        };
        ...
        let tool_env = match installed_tools.get_environment(&name, cache) {
            ...
            Ok(None) => {
                warn_user!(
                    ...
                    format!("fv tool install {name} --reinstall").green()
                );
                continue;
            }
            Err(e) => {
        ...
        let version = match tool_env.version() {
            Ok(version) => version,
            Err(e) => {
                if let fv_tool::Error::EnvironmentError(e) = e {
                    warn_user!(
                        "{e} (run `{}` to reinstall)",
                        format!("fv tool install {name} --reinstall").green()
                    );
