...
use fv_configuration::{Concurrency, Constraints, DryRun, TargetTriple};
use fv_distribution_types::{ExtraBuildRequires, Requirement, RequirementSource};
use fv_fs::CWD;
use fv_normalize::PackageName;
use fv_pep440::{Operator, Version};
use fv_preview::Preview;
use fv_python::{
    EnvironmentPreference, Interpreter, PythonDownloads, PythonInstallation, PythonPreference,
    PythonRequest,
};
use fv_requirements::RequirementsSpecification;
use fv_settings::{Combine, PythonInstallMirrors, ResolverInstallerOptions, ToolOptions};
use fv_tool::{InstalledTools, Tool};
...
pub(crate) async fn upgrade(
    ...
    let names: BTreeMap<PackageName, Vec<Requirement>> = {
        if names.is_empty() {
            ...
            for name in names {
                let requirement = Requirement::from(fv_pep508::Requirement::parse(&name, &*CWD)?);
    ...
    fn print(&self, name: &PackageName, printer: Printer) -> Result<()> {
        match self {
            Self::PinnedVersion { version } => {
                ...
                let reinstall_command = format!("fv tool install {name}@latest");
...
async fn upgrade_tool(
    ...
    let existing_tool_receipt = match installed_tools.get_tool_receipt(name) {
        ...
        Ok(None) => {
            let install_command = format!("fv tool install {name}");
        ...
        Err(_) => {
            let install_command = format!("fv tool install --force {name}");
            return Err(anyhow::anyhow!(
