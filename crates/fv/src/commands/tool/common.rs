...
use fv_distribution_types::{InstalledDist, Name};
use fv_fs::Simplified;
#[cfg(unix)]
use fv_fs::replace_symlink;
...
use fv_settings::{PythonInstallMirrors, ToolOptions};
use fv_shell::Shell;
use fv_tool::{InstalledTools, Tool, ToolEntrypoint, entrypoint_paths};
...
pub(crate) async fn refine_interpreter(
    ...
    let pip::operations::Error::Resolve(fv_resolver::ResolveError::NoSolution(no_solution_err)) =
        err
    else {
        return Ok(None);
    };
...
pub(crate) fn finalize_tool_install(
    ...
    let executable_directory = fv_tool::tool_executable_dir()?;
...
fn hint_executable_from_dependency(
    ...
    match packages.as_slice() {
        [] => {}
        [package] => {
            let command = format!("fv tool install {}", package.name());
            writeln!(
                printer.stdout(),
