...
pub(crate) async fn install(
    ...
    let requirement = match &request {
        ...
        ToolRequest::Package {
            ...
            // Determine if it's an entirely different package (e.g., `fv install foo --from bar`).
        ...
        ToolRequest::Python { .. } => {
            bail!(
                ...
                "fv tool install".cyan(),
                "fv python install".cyan(),
            );
        }
    };
...
fn existing_environment_usable(
    ...
    settings: &ResolverInstallerSettings,
    existing_tool_receipt: Option<&fv_tool::Tool>,
    printer: Printer,
) -> bool {
