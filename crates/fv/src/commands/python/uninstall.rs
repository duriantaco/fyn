...
use fv_python::downloads::PythonDownloadRequest;
use fv_python::managed::{
    ManagedPythonInstallations, PythonMinorVersionLink, python_executable_dir,
};
use fv_python::{PythonInstallationKey, PythonInstallationMinorVersionKey, PythonRequest};
...
pub(crate) async fn uninstall(
    ...
    if fv_fs::directories(installations.root())?.all(|path| fv_fs::is_temporary(&path)) {
        ...
        if let Some(top_level) = installations.root().parent() {
            ...
            if fv_fs::directories(top_level)?.all(|path| fv_fs::is_temporary(&path)) {
...
async fn do_uninstall(
    ...
    for (request, download_request) in requests.iter().zip(download_requests) {
        ...
        if !found {
            ...
            #[cfg(windows)]
            {
                fv_python::windows_registry::remove_orphan_registry_entries(
    ...
    #[cfg(windows)]
    {
        fv_python::windows_registry::remove_registry_entry(
            &matching_installations,
            all,
            &mut errors,
        );
        fv_python::windows_registry::remove_orphan_registry_entries(&installed_installations);
    }
