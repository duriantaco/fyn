...
use fv_normalize::DefaultGroups;
...
use fv_shell::{Shell, shlex_posix, shlex_windows};
use fv_types::{AnyErrorBuild, BuildContext, BuildIsolation, BuildStack, HashStrategy};
use fv_virtualenv::OnExisting;
...
enum VenvError {
    ...
    Creation(#[source] fv_virtualenv::Error),
    ...
    Tags(#[source] fv_platform_tags::TagsError),

    #[error("Failed to resolve `--find-links` entry")]
    FlatIndex(#[source] fv_client::FlatIndexError),
}
...
pub(crate) async fn venv(
    ...
    let path = path.unwrap_or(
        project
            ...
            .and_then(|project| {
                ...
                // avoids a breaking change when adding project environment support to `fv venv`.
    ...
    if seed {
        ...
        let extra_build_variables = fv_distribution_types::ExtraBuildVariables::default();
        ...
        let requirements = if interpreter.python_tuple() >= (3, 12) {
            vec![Requirement::from(
                fv_pep508::Requirement::from_str("pip").unwrap(),
            )]
        } else {
            // Include `setuptools` and `wheel` on Python <3.12.
            vec![
                Requirement::from(fv_pep508::Requirement::from_str("pip").unwrap()),
                Requirement::from(fv_pep508::Requirement::from_str("setuptools").unwrap()),
                Requirement::from(fv_pep508::Requirement::from_str("wheel").unwrap()),
            ]
        };
