...
use fv_auth::Service;
use fv_cache::{CacheArgs, Refresh};
use fv_cli::comma::CommaSeparatedRequirements;
use fv_cli::{
...
use fv_pep508::{MarkerTree, RequirementOrigin};
use fv_preview::Preview;
use fv_pypi_types::SupportedEnvironments;
use fv_python::{Prefix, PythonDownloads, PythonPreference, PythonVersion, Target};
...
use fv_workspace::pyproject::{DependencyType, ExtraBuildDependencies};
    ...
    pub(crate) fn resolve(
        ...
        if installer.upgrade || !installer.upgrade_package.is_empty() {
            if with.is_empty() && with_requirements.is_empty() {
                warn_user_once!(
                    "Tools cannot be upgraded via `{invocation_source}`; use `fv tool upgrade --all` to upgrade all installed tools, or `{invocation_source} package@latest` to run the latest version of a tool."
                );
            } else {
                warn_user_once!(
                    "Tools cannot be upgraded via `{invocation_source}`; use `fv tool upgrade --all` to upgrade all installed tools, `{invocation_source} package@latest` to run the latest version of a tool, or `{invocation_source} --refresh package` to upgrade any `--with` dependencies."
                );
            }
        }
        ...
        if installer.reinstall || !installer.reinstall_package.is_empty() {
            if with.is_empty() && with_requirements.is_empty() {
                warn_user_once!(
                    "Tools cannot be reinstalled via `{invocation_source}`; use `fv tool upgrade --all --reinstall` to reinstall all installed tools, `{invocation_source} package@latest` to run the latest version of a tool, or `fv cache prune` to clear any cached tool environments."
                );
            } else {
                warn_user_once!(
                    "Tools cannot be reinstalled via `{invocation_source}`; use `fv tool upgrade --all --reinstall` to reinstall all installed tools, `{invocation_source} package@latest` to run the latest version of a tool, `{invocation_source} --refresh package` to reinstall any `--with` dependencies, or `fv cache prune` to clear any cached tool environments."
                );
            }
        }
...
impl ToolUpgradeSettings {
    ...
    pub(crate) fn resolve(
        ...
        if upgrade {
            warn_user_once!("`--upgrade` is enabled by default on `fv tool upgrade`");
        }
        if !upgrade_package.is_empty() {
            warn_user_once!("`--upgrade-package` is enabled by default on `fv tool upgrade`");
        }
...
/// The resolved settings to use for an invocation of the fv CLI when installing dependencies.
///
/// Combines the `[tool.fv]` persistent configuration with the command-line arguments
...
/// The resolved settings to use for an invocation of the fv CLI when resolving dependencies.
///
/// Combines the `[tool.fv]` persistent configuration with the command-line arguments
...
/// The resolved settings to use for an invocation of the fv CLI with both resolver and installer
/// capabilities.
///
/// Represents the shared settings that are used across all fv commands outside the `pip` API.
/// Analogous to the settings contained in the `[tool.fv]` table, combined with [`ResolverInstallerArgs`].
#[derive(Debug, Clone, Default)]
...
/// settings contained in the `[tool.fv.pip]` table.
#[derive(Debug, Clone)]
...
impl PipSettings {
    ...
    pub(crate) fn combine(
        ...
        // Merge the top-level options (`tool.fv`) with the pip-specific options (`tool.fv.pip`),
        // preferring the latter.
        //
        // For example, prefer `tool.fv.pip.index-url` over `tool.fv.index-url`.
...
/// The resolved settings to use for an invocation of the `fv publish` CLI.
#[derive(Debug, Clone)]
...
/// The resolved settings to use for an invocation of the `fv auth logout` CLI.
#[derive(Debug, Clone)]
...
/// The resolved settings to use for an invocation of the `fv auth token` CLI.
#[derive(Debug, Clone)]
...
/// The resolved settings to use for an invocation of the `fv auth set` CLI.
#[derive(Debug, Clone)]
