...
/// includes targets and timestamps, along with all `fv=debug` messages by default.
pub(crate) fn setup_logging(
    ...
    let default_directive = match level {
        ...
        Level::DebugUv => {
            ...
            Directive::from_str("fv=debug").unwrap()
        }
        Level::TraceUv => {
            // Show `TRACE` messages from the CLI crate (and ERROR/WARN/INFO/DEBUG)
            Directive::from_str("fv=trace").unwrap()
        }
        Level::TraceAll => {
    ...
    // (a thing we absolutely do throughout fv), then there will still be color
