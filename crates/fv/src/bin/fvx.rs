...
fn exec_spawn(cmd: &mut Command) -> std::io::Result<Infallible> {
    ...
    #[cfg(windows)]
    {
        fv_windows::spawn_child(cmd, false)
    }
}

/// Assuming the binary is called something like `fvx@1.2.3(.exe)`, compute the `@1.2.3(.exe)` part
/// so that we can preferentially find `fv@1.2.3(.exe)`, for folks who like managing multiple
/// installs in this way.
fn get_fvx_suffix(current_exe: &Path) -> Option<&str> {
    let os_file_name = current_exe.file_name()?;
    let file_name_str = os_file_name.to_str()?;
    file_name_str.strip_prefix("fvx")
}

/// Gets the path to `fv`, given info about `fvx`
fn get_fv_path(current_exe_parent: &Path, fvx_suffix: Option<&str>) -> std::io::Result<PathBuf> {
    // First try to find a matching suffixed `fv`, e.g. `fv@1.2.3(.exe)`
    let fv_with_suffix = fvx_suffix.map(|suffix| current_exe_parent.join(format!("fv{suffix}")));
...
fn run() -> std::io::Result<ExitStatus> {
    ...
    let Some(bin) = current_exe.parent() else {
        return Err(std::io::Error::new(
            ...
            "Could not determine the location of the `fvx` binary",
        ));
    };
    let fvx_suffix = get_fvx_suffix(&current_exe);
    let fv = get_fv_path(bin, fvx_suffix)?;
    let args = ["tool", "fvx"]
        .iter()
        .map(OsString::from)
        // Skip the `fvx` name
