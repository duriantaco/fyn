...
use fv_test::{copy_dir_ignore, fv_snapshot};
...
fn workspace_metadata_simple() {
    ...
    fv_snapshot!(context.filters(), context.workspace_metadata().current_dir(&workspace), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    {
    ...
    ----- stderr -----
    warning: The `fv workspace metadata` command is experimental and may change without warning. Pass `--preview-features workspace-metadata` to disable this warning.
    "#
    );
}
...
fn workspace_metadata_from_member() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.workspace_metadata().current_dir(&member_dir), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    {
...
fn workspace_metadata_multiple_members() {
    ...
    fv_snapshot!(context.filters(), context.workspace_metadata().current_dir(&workspace_root), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    {
...
fn workspace_metadata_single_project() {
    ...
    fv_snapshot!(context.filters(), context.workspace_metadata().current_dir(&project), @r#"
    success: true
    exit_code: 0
    ----- stdout -----
    {
...
fn workspace_metadata_no_project() {
    ...
    fv_snapshot!(context.filters(), context.workspace_metadata(), @"
    success: false
    exit_code: 2
    ----- stdout -----
