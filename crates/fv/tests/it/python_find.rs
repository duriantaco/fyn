...
use fv_platform::{Arch, Os};
use fv_static::EnvVars;

use fv_test::{fv_snapshot, venv_bin_path};

#[test]
fn python_find() {
    let mut context =
        fv_test::test_context_with_versions!(&["3.11", "3.12"]).with_filtered_python_sources();

    // No interpreters on the path
    fv_snapshot!(context.filters(), context.python_find().env(EnvVars::UV_TEST_PYTHON_PATH, ""), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    // Request Python 3.12
    fv_snapshot!(context.filters(), context.python_find().arg("3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.12]
    ...
    // Request Python 3.11
    fv_snapshot!(context.filters(), context.python_find().arg("3.11"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.11]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("cpython@3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.12]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("any-3.12-any"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.12]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("pypy"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn python_find_pin() {
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--no-config"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.11]
    ...
    fv_snapshot!(context.filters(), context.python_find().current_dir(&child_dir), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.12]
    ...
    fv_snapshot!(context.filters(), context.python_pin().arg("3.11").current_dir(&child_dir), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn python_find_pin_arbitrary_name() {
    ...
    fv_snapshot!(context.filters(), context.python_pin().arg("foo"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    // Pin to an arbitrary name, bypassing fv
    context
        .temp_dir
    ...
    fv_snapshot!(context.filters(), context.python_pin().arg("3.11"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Pinned `.python-version` to `3.11`
    ...
    fv_snapshot!(context.filters(), context.python_find().current_dir(context.temp_dir.child("foo").path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.11]
...
fn python_find_project() {
    ...
    // Unless explicitly requested
    fv_snapshot!(context.filters(), context.python_find().arg("3.10"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.10]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--no-project"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.10]
...
#[test]
fn virtual_empty() {
    // testing how `fv python find` reacts to a pyproject with no `[project]` and nothing useful to it
    let context = fv_test::test_context_with_versions!(&["3.10", "3.11", "3.12"]);
...
fn virtual_dependency_group() {
    // testing basic `fv python find` functionality
...
fn python_find_venv() {
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--python").arg("3.12").arg("-q"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--system"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.11]
    ...
    fv_snapshot!(context.filters(), context.python_find().env(EnvVars::UV_SYSTEM_PYTHON, "1"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.11]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--no-system").env(EnvVars::UV_SYSTEM_PYTHON, "1"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    Usage: fv python find --cache-dir [CACHE_DIR] [REQUEST]
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--python").arg("3.11").arg("-q").current_dir(&child_dir), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    #[cfg(not(windows))]
    fv_snapshot!(context.filters(), context.python_find().arg("child/.venv"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/child/.venv/[BIN]/[PYTHON]
    ...
    #[cfg(not(windows))]
    fv_snapshot!(context.filters(), context.python_find().env(EnvVars::VIRTUAL_ENV, child_dir.join(".venv").as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/child/.venv/[BIN]/[PYTHON]
    ...
    #[cfg(not(windows))]
    fv_snapshot!(context.filters(), context.python_find().env(EnvVars::UV_TEST_PYTHON_PATH, child_dir.join(".venv").join("bin").as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/child/.venv/[BIN]/[PYTHON]
    ...
    #[cfg(not(windows))]
    {
        ...
        fv_snapshot!(context.filters(), context.python_find().env(EnvVars::UV_TEST_PYTHON_PATH, path.as_os_str()), @"
        success: true
        exit_code: 0
        ----- stdout -----
        [TEMP_DIR]/child/.venv/[BIN]/[PYTHON]
...
fn python_find_unsupported_version() {
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("3.5"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("3.5.9"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("2.6"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("2.6.8"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("4.2"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("<3.0"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("3.12t"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn python_find_venv_invalid() {
    ...
    fv_snapshot!(context.filters(), context.python_find().env(EnvVars::VIRTUAL_ENV, context.venv.as_os_str()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [VENV]/[BIN]/[PYTHON]
...
fn python_find_managed() {
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--managed-python"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.12]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--managed-python").arg("3.11"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--no-managed-python"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.12]
    ...
    // Request an interpreter that cannot be satisfied
    fv_snapshot!(context.filters(), context.python_find().arg("--no-managed-python").arg("3.11"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--python-preference").arg("system"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.12]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--python-preference").arg("system").arg("3.11"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [PYTHON-3.11]
...
fn python_required_python_major_minor() {
    ...
    fv_snapshot!(context.filters(), context.python_find().arg(">=3.11.4, <3.12").env(EnvVars::UV_TEST_PYTHON_PATH, context.temp_dir.child("child").path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg(">3.11.255, <3.12").env(EnvVars::UV_TEST_PYTHON_PATH, context.temp_dir.child("child").path()), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn python_find_script() {
    ...
    fv_snapshot!(context.filters(), context.init().arg("--script").arg("foo.py"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.sync().arg("--script").arg("foo.py"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--script").arg("foo.py"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn python_find_script_python_not_found() {
    let context = fv_test::test_context_with_versions!(&[]).with_filtered_python_sources();
...
fn python_find_show_version() {
    ...
    fv_snapshot!(context.filters(), context.python_find().env(EnvVars::UV_TEST_PYTHON_PATH, "").arg("--show-version"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("--show-version"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    3.11.[X]
    ...
    // Request Python 3.12
    fv_snapshot!(context.filters(), context.python_find().arg("--show-version").arg("3.12"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    3.12.[X]
    ...
    // Request Python 3.11
    fv_snapshot!(context.filters(), context.python_find().arg("--show-version").arg("3.11"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    3.11.[X]
...
#[test]
fn python_find_path() {
    let context = fv_test::test_context_with_versions!(&[]).with_filtered_not_executable();

    context.temp_dir.child("foo").create_dir_all().unwrap();
    context.temp_dir.child("bar").touch().unwrap();

    // No interpreter in a directory
    fv_snapshot!(context.filters(), context.python_find().arg(context.temp_dir.child("foo").as_os_str()), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg(context.temp_dir.child("bar").as_os_str()), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg(context.temp_dir.child("foobar").as_os_str()), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn python_find_freethreaded_314() {
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("3.14"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("3.14t"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/managed/cpython-3.14+freethreaded-[PLATFORM]/[INSTALL-BIN]/[PYTHON]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("3.14+gil"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn python_find_prerelease_version_specifiers() {
    ...
    fv_snapshot!(context.filters(), context.python_find().arg(">=3.14").arg("--resolve-links"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    ----- stderr -----
    warning: You're using a pre-release version of Python (3.14.0rc3) but a stable version is available. Use `fv python upgrade 3.14` to upgrade.
    ");

    // `>3.14rc2` should not match rc2
    fv_snapshot!(context.filters(), context.python_find().arg(">3.14.0rc2").arg("--resolve-links"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/managed/cpython-3.14.0rc3-[PLATFORM]/[INSTALL-BIN]/[PYTHON]
    ...
    fv_snapshot!(context.filters(), context.python_find().arg(">3.14.0rc3"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.python_find().arg(">=3.14.0rc3").arg("--resolve-links"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    [TEMP_DIR]/managed/cpython-3.14.0rc3-[PLATFORM]/[INSTALL-BIN]/[PYTHON]
...
fn python_find_prerelease_with_patch_request() {
    ...
    fv_snapshot!(context.filters(), context.python_find().arg("3.14.0"), @"
    success: false
    exit_code: 2
    ----- stdout -----
