...
fn version_get() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.version(), @"
    success: true
    exit_code: 0
    ----- stdout -----
    myproject 1.10.31
...
/// Previously would fallback to `fv --version` if this pyproject.toml isn't usable for whatever reason
/// (In this case, because tool.fv.managed = false)
#[test]
...
fn version_get_fallback_unmanaged_short() -> Result<()> {
    ...
    let filters = context
        ...
        .collect::<Vec<_>>();
    fv_snapshot!(filters, context.version()
        .arg("--short"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
/// In tarball builds of fv, git version info is missing (distros do this)
fn git_version_info_expected() -> bool {
    // This is setup to aggressively panic to make sure this is working at all
    // If you're a packager of fv and this does indeed blow up for you, we will
    // gladly change these expects into "just return false" or something.
    let manifest_dir = std::env::var(fv_static::EnvVars::CARGO_MANIFEST_DIR)
...
fn version_get_missing_with_hint() -> Result<()> {
    ...
    hint: If you meant to view fv's version, use `fv self version` instead
    ");
...
fn self_version() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.self_version(), @"
    success: true
    exit_code: 0
    ----- stdout -----
    fv [VERSION] ([COMMIT] DATE)
...
fn self_version_short() -> Result<()> {
    ...
    let filters = context
        ...
        .collect::<Vec<_>>();
    fv_snapshot!(filters, context.self_version()
        .arg("--short"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
// `fv self version --output-format json`
// (also setup a honeypot project and make sure it's not used)
#[test]
...
fn version_get_workspace() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.version().arg("--project").arg(context.temp_dir.as_ref()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    myproject 1.10.31
    ...
    fv_snapshot!(context.filters(), context.version().arg("--project").arg(context.temp_dir.join("workspace-member")), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.version().arg("--directory").arg(context.temp_dir.as_ref()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    myproject 1.10.31
    ...
    fv_snapshot!(context.filters(), context.version().arg("--directory").arg(context.temp_dir.join("workspace-member")), @"
    success: true
    exit_code: 0
    ----- stdout -----
    workspace-member 0.1.0
...
fn version_set_workspace() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), version_cmd, @"
    success: true
    exit_code: 0
    ----- stdout -----
    child2 0.1.0 => 1.1.1
    ...
    // `fv version` implies a full lock and sync, including development dependencies.
...
///
/// The result is that `fv version --package idna x.y.z` can force the re-evaluation
...
fn virtual_empty() -> Result<()> {
    // testing how `fv version` reacts to a pyproject with no `[project]` and nothing useful to it
    let context = fv_test::test_context!("3.12");
...
fn add_virtual_dependency_group() -> Result<()> {
    // testing basic `fv version` functionality
