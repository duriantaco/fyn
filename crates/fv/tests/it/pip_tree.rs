...
fn no_package() {
    ...
    fv_snapshot!(context.filters(), context.pip_tree(), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn prune_last_in_the_subgroup() {
    ...
    fv_snapshot!(context.filters(), context.pip_tree().arg("--prune").arg("certifi"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    requests v2.31.0
...
fn reverse() {
    ...
    fv_snapshot!(context.filters(), context.pip_tree().arg("--reverse"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    blinker v1.7.0
...
fn invert() {
    ...
    fv_snapshot!(context.filters(), context.pip_tree().arg("--invert"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    blinker v1.7.0
...
fn no_dedupe_and_invert() {
    ...
    fv_snapshot!(context.filters(), context.pip_tree().arg("--no-dedupe").arg("--invert"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    six v1.16.0
...
fn with_editable() {
    ...
    fv_snapshot!(filters, context.pip_tree(), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn show_version_specifiers_simple() {
    ...
    fv_snapshot!(context.filters(), context.pip_tree().arg("--show-version-specifiers"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    requests v2.31.0
...
fn print_output_even_with_quite_flag() {
    ...
    context.assert_command("import requests").success();
    fv_snapshot!(context.filters(), context.pip_tree().arg("--quiet"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn no_duplicate_dependencies_with_markers() {
    const PY_PROJECT: &str = indoc! {r#"
        ...
        [build-system]
        requires = ["fv_build>=0.8.22,<10000"]
        build-backend = "fv_build"
    "#};

    let context = fv_test::test_context_with_versions!(&["3.12", "3.13"]).with_filtered_counts();
