...
async fn prune_force() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.prune().arg("--verbose").arg("--force"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn prune_unzipped() -> Result<()> {
    ...
    // Install a requirement, to populate the cache.
    fv_snapshot!(&filters, context.pip_install().arg("-r").arg("requirements.txt").arg("--reinstall"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.prune().arg("--ci"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    requirements_txt.write_str(indoc! { r"
        source-distribution==0.0.1
    " })?;
    fv_snapshot!(&filters, context.pip_install().arg("-r").arg("requirements.txt").arg("--offline"), @"
    success: true
    exit_code: 0
    ----- stdout -----
