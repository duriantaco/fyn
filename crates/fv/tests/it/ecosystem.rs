...
// These tests just run `fv lock` on an assorted of ecosystem
// projects.
//
...
// Currently ignored because the project doesn't build with `fv` yet.
//
// Source: https://github.com/apache/airflow/blob/c55438d9b2eb9b6680641eefdd0cbc67a28d1d29/pyproject.toml
#[test]
#[ignore = "Airflow doesn't build with `fv` yet"]
fn airflow() -> Result<()> {
...
/// root of the `fv` repository.
fn lock_ecosystem_package(python_version: &str, name: &str) -> Result<()> {
    let context = fv_test::test_context!(python_version);
    ...
    let (snapshot, _) = fv_test::run_and_format(
        &mut command,
        context.filters(),
        name,
        Some(fv_test::WindowsFilters::Platform),
        None,
    );
