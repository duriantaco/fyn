...
use fv_test::{fv_snapshot, get_bin};
...
fn check_self_update() {
    ...
    let args = RuntestArgs {
        app_name: "fv".to_owned(),
        package: "fv".to_owned(),
        owner: "astral-sh".to_owned(),
        bin: get_bin!(),
        binaries: vec!["fv".to_owned()],
    ...
    let status = Command::new(installed_bin)
        .arg("--version")
        .status()
        .expect("failed to run 'fv --version'");
    assert!(status.success(), "'fv --version' returned non-zero");
}
...
fn test_self_update_offline_error() {
    ...
    fv_snapshot!(context.self_update().arg("--offline"),
    @r"
    success: false
    exit_code: 1
    ----- stdout -----
