...
fn clean_all() -> Result<()> {
    ...
    fv_snapshot!(context.with_filtered_counts().filters(), context.clean().arg("--verbose"), @"
    success: true
    exit_code: 0
    ----- stdout -----

    ----- stderr -----
    DEBUG fv [VERSION] ([COMMIT] DATE)
...
fn clear_all_alias() -> Result<()> {
    ...
    fv_snapshot!(context.with_filtered_counts().filters(), command, @"
    success: true
    exit_code: 0
    ----- stdout -----
...
async fn clean_force() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.clean().arg("--verbose").arg("--force"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    let _cache = fv_cache::Cache::from_path(context.cache_dir.path())
        .with_exclusive_lock()
        .await;
    fv_snapshot!(context.filters(), context.clean().arg("--verbose").arg("--force"), @"
    success: true
...
fn clean_package_pypi() -> Result<()> {
    ...
    fv_snapshot!(&filters, context.clean().arg("--verbose").arg("iniconfig"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Running `fv cache prune` should have no effect.
    fv_snapshot!(&filters, context.prune().arg("--verbose"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
async fn cache_timeout() {
    ...
    // Simulate another fv process running and locking the cache, e.g., with a source build.
    let _cache = Cache::from_path(context.cache_dir.path())
        .with_exclusive_lock()
        .await;

    fv_snapshot!(context.filters(), context.clean().env(EnvVars::UV_LOCK_TIMEOUT, "1"), @"
    success: false
    exit_code: 2
    ----- stdout -----

    ----- stderr -----
    Cache is currently in-use, waiting for other fv processes to finish (use `--force` to override)
    error: Timeout ([TIME]) when waiting for lock on `[CACHE_DIR]/` at `[CACHE_DIR]/.lock`, is another fv process running? You can set `UV_LOCK_TIMEOUT` to increase the timeout.
    ");
}
