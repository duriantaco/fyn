...
use fv_test::{diff_snapshot, fv_snapshot};
...
fn packse_add_remove_one_package() {
    ...
    let new_lock = context.read("fv.lock");
...
fn jax_instability() -> Result<()> {
    ...
    // Back to where we started.
    //
    // Note that this is wrong! This demonstrates that `fv` sometimes does
