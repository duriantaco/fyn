...
use fv_test::{fv_snapshot, packse_index_url};
...
fn wrong_backtracking_basic() -> Result<()> {
    ...
    // Assert the idempotence of `fv lock` when resolving from the lockfile (`--locked`).
    context
        .lock()
        .arg("--locked")
...
fn conflict_in_fork() -> Result<()> {
    ...
    ----- stderr -----
      ...
      ╰─▶ Because only package-b==1.0.0 is available and package-b==1.0.0 depends on package-d==1, we can conclude that all versions of package-b depend on package-d==1.
          ...
          hint: The resolution failed for an environment that is not the current one, consider limiting the environments with `tool.fv.environments`.
    "
    );
...
///
/// In particular, this serves as a regression test. fv used to create a
...
/// clearly wrong.
///
/// The issue was that fv was intersecting *all* marker expressions. So
...
///
/// Since there are no other constraints on `a`, this causes fv to select
