...
use fv_test::{DEFAULT_PYTHON_VERSION, apply_filters, fv_snapshot};
use indoc::indoc;
...
fn build_basic() -> Result<()> {
    ...
    fv_snapshot!(&filters, context.build().arg("project"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().current_dir(project.path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build(), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("--out-dir").arg("out").current_dir(project.path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_sdist() -> Result<()> {
    ...
    // Build the specified path.
    fv_snapshot!(&filters, context.build().arg("--sdist").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_wheel() -> Result<()> {
    ...
    // Build the specified path.
    fv_snapshot!(&filters, context.build().arg("--wheel").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_sdist_wheel() -> Result<()> {
    ...
    // Build the specified path.
    fv_snapshot!(&filters, context.build().arg("--sdist").arg("--wheel").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_wheel_from_sdist() -> Result<()> {
    ...
    fv_snapshot!(&filters, context.build().arg("./dist/project-0.1.0.tar.gz").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("./dist/project-0.1.0.tar.gz").arg("--sdist").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("./dist/project-0.1.0.tar.gz").arg("--wheel").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("./dist/project-0.1.0-py3-none-any.whl").arg("--wheel").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_fail() -> Result<()> {
    ...
    // Build the specified path.
    fv_snapshot!(&filters, context.build().arg("project"), @r#"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_workspace() -> Result<()> {
    ...
    // Build the member.
    fv_snapshot!(&filters, context.build().arg("--package").arg("member").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Build all packages.
    fv_snapshot!(&filters, context.build().arg("--all").arg("--no-build-logs").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("./project").arg("--package").arg("member"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // If a source is provided, discover the workspace from the source.
    fv_snapshot!(&filters, context.build().arg("./project").arg("--all").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("--package").arg("member"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("--all"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(&filters, context.build().arg("--package").arg("fail").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_constraints() -> Result<()> {
    ...
    fv_snapshot!(&filters, context.build().arg("--build-constraint").arg("constraints.txt").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
///
/// `fv build --all` should respect `tool.fv.build-constraint-dependencies` declared at the
/// workspace root.
#[test]
fn build_all_respects_workspace_build_constraint_dependencies() -> Result<()> {
    ...
    assert!(
        ...
        "expected `fv build --all` to fail when workspace build constraints make `hatchling` \
...
/// Limitation: when `fv build` is invoked with an explicit source path, configuration is still
...
fn build_sha() -> Result<()> {
    ...
    fv_snapshot!(&filters, context.build().arg("--build-constraint").arg("constraints.txt").arg("--require-hashes").current_dir(&project), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_quiet() -> Result<()> {
    ...
    fv_snapshot!(&context.filters(), context.build().arg("project").arg("-q"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_no_build_logs() -> Result<()> {
    ...
    fv_snapshot!(&context.filters(), context.build().arg("project").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_hide_build_output_env_var() -> Result<()> {
    ...
    fv_snapshot!(&context.filters(), context.build().arg("project").env(EnvVars::UV_HIDE_BUILD_OUTPUT, "1"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_hide_build_output_on_failure() -> Result<()> {
    ...
    fv_snapshot!(&filters, context.build().arg("project").env(EnvVars::UV_HIDE_BUILD_OUTPUT, "1").env("FOO", "bar"), @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn build_git_boundary_in_dist_build() -> Result<()> {
    ...
    fv_snapshot!(&context.filters(), context.build().current_dir(project.path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_non_package() -> Result<()> {
    ...
    // Build the member.
    fv_snapshot!(&filters, context.build().arg("--package").arg("member").current_dir(&project), @r#"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    // Build all packages.
    fv_snapshot!(&filters, context.build().arg("--all").arg("--no-build-logs").current_dir(&project), @r#"
    success: false
    exit_code: 2
    ----- stdout -----
...
/// Test the fv fast path. Tests all four possible build plans:
/// * Defaults
/// * `--sdist`
/// * `--wheel`
/// * `--sdist --wheel`
...
fn build_fast_path() -> Result<()> {
    ...
    ----- stderr -----
    Building source distribution (fv build backend)...
    Building wheel from source distribution (fv build backend)...
    ...
    ----- stderr -----
    Building wheel (fv build backend)...
...
fn build_list_files_errors() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.build()
        .arg(&built_by_uv)
        .arg("--out-dir")
        .arg(context.temp_dir.join("output1"))
        .arg("--list")
    ...
    Usage: fv build --cache-dir [CACHE_DIR] --out-dir <OUT_DIR> --exclude-newer <EXCLUDE_NEWER> <SRC>

    For more information, try '--help'.
    ");

    // Not a fv build backend package, we can't list it.
    let anyio_local = current_dir()?.join("../../test/packages/anyio_local");
    // Windows normalization
    let context = context.with_filter(("/crates/fv/../../", "/"));
    fv_snapshot!(context.filters(), context.build()
        .arg(&anyio_local)
        .arg("--out-dir")
        .arg(context.temp_dir.join("output2"))
    ...
    ----- stderr -----
      ...
      ╰─▶ Can only use `--list` with a compatible fv build backend, but `[WORKSPACE]/test/packages/anyio_local` is not compatible because `build_system.build-backend` is not `fv_build`, but `flit_core.buildapi`
    ");
    Ok(())
}
...
fn build_unconfigured_setuptools() -> Result<()> {
    ...
    // This is not technically a `fv build` test, we use it to contrast this passing case with the
    // failing cases later.
    ...
    fv_snapshot!(context.filters(), context.python_command().arg("-c").arg("import greet"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    Greetings!
...
/// In a project layout with a virtual root, an easy mistake to make is running `fv pip install .`
/// in the root.
#[test]
fn build_workspace_virtual_root() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.build().arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    ----- stderr -----
    Building source distribution...
    warning: `[TEMP_DIR]/` appears to be a workspace root without a Python project; consider using `fv sync` to install the workspace, or add a `[build-system]` table to `pyproject.toml`
    Building wheel from source distribution...
...
fn build_with_nonnormalized_name() -> Result<()> {
    ...
    // Build the specified path.
    fv_snapshot!(&filters, context.build().arg("--no-build-logs").current_dir(&project), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn force_pep517() -> Result<()> {
    // We need to use a real `fv_build` package.
    let context = fv_test::test_context!("3.12").with_exclude_newer("2025-05-27T00:00:00Z");
    ...
    pyproject_toml.write_str(indoc! {r#"
        ...
        [build-system]
        requires = ["fv_build>=0.5.15,<10000"]
        build-backend = "fv_build"
    "#})?;

    fv_snapshot!(context.filters(), context.build().env(EnvVars::RUST_BACKTRACE, "0"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.build().arg("--force-pep517").env(EnvVars::RUST_BACKTRACE, "0"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    Error: Missing source directory at: `src`
      × Failed to build `[TEMP_DIR]/`
      ├─▶ The build backend returned an error
      ╰─▶ Call to `fv_build.build_sdist` failed (exit status: 1)
          hint: This usually indicates a problem with the package or the build environment.
    ");
...
fn test_workspace_trailing_slash() {
    ...
    fv_snapshot!(context.filters(), context.build().arg("child"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.build().arg("./child/../child/"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn build_clear() -> Result<()> {
    ...
    fv_snapshot!(&context.filters(), context.build().arg("project").arg("--clear").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
/// Test `fv build --no-create-gitignore`.
#[test]
fn build_no_gitignore() -> Result<()> {
    ...
    fv_snapshot!(&context.filters(), context.build().arg("project").arg("--no-create-gitignore").arg("--no-build-logs"), @"
    success: true
    exit_code: 0
    ----- stdout -----
