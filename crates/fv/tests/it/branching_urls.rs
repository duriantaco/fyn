...
use fv_test::{fv_snapshot, make_project};
...
fn branching_urls_disjoint() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.lock().current_dir(&context.temp_dir), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn root_package_splits_transitive_too() -> Result<()> {
    ...
    assert_snapshot!(context.read("fv.lock"), @r#"
    version = 1
    revision = 3
...
fn dont_pre_visit_url_packages() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.lock().arg("--offline").current_dir(&context.temp_dir), @"
    success: true
    exit_code: 0
    ----- stdout -----
