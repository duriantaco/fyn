...
async fn unzip(url: &str) -> anyhow::Result<(), fv_extract::Error> {
    ...
    let target = tempfile::TempDir::new().map_err(fv_extract::Error::Io)?;
    fv_extract::stream::unzip(url, reader.compat(), target.path()).await
}
