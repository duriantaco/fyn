...
use fv_test::{fv_snapshot, site_packages_path};
...
// fv, as otherwise the tests may pick up an already installed fv
...
fn find_uv_bin_in_ephemeral_environment() -> anyhow::Result<()> {
    ...
    exit_code: 0
    ----- stdout -----
    [CACHE_DIR]/archive-v0/[HASH]/[BIN]/fv
...
fn find_uv_bin_user_bin() {
    ...
    fs_err::remove_file(if cfg!(unix) {
        context.venv.child("bin").child("fv")
    } else {
        context.venv.child("Scripts").child("fv.exe")
    })
    .unwrap();
...
fn find_uv_bin_error_message() {
    let mut context = fv_test::test_context!("3.12")
        .with_filtered_python_names()
        .with_filtered_virtualenv_bin()
        .with_filtered_exe_suffix()
        .with_filter(user_scheme_bin_filter())
    ...
    Traceback (most recent call last):
      ...
      File "[SITE_PACKAGES]/uv/_find_uv.py", line 50, in find_uv_bin
        raise UvNotFound(
    uv._find_uv.UvNotFound: Could not find the fv binary in any of the following locations:
     - [VENV]/[BIN]
     - /dev/null/[BIN]
     - [SITE_PACKAGES]/[BIN]
     - [USER_SCHEME]/[BIN]
...
fn find_uv_bin_py310() {
    let context = fv_test::test_context!("3.10")
        .with_filtered_python_names()
        .with_filtered_virtualenv_bin()
        .with_filtered_exe_suffix()
        .with_filter(user_scheme_bin_filter())
