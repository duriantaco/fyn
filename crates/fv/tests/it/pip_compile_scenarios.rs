...
use fv_test::{
    TestContext, build_vendor_links_url, fv_snapshot, get_bin, packse_index_url,
...
fn compatible_python_incompatible_override() -> Result<()> {
    ...
    let output = fv_snapshot!(filters, command(&context, python_versions)
...
fn incompatible_python_compatible_override_available_no_wheels() -> Result<()> {
    ...
    #    fv pip compile requirements.in --cache-dir [CACHE_DIR] --python-version=3.11
    package-a==1.0.0
...
fn python_patch_override_no_patch() -> Result<()> {
    let context = fv_test::test_context!("3.9.21");
    let python_versions = &[];
...
fn python_patch_override_patch_compatible() -> Result<()> {
    ...
    ----- stdout -----
    # This file was autogenerated by fv via the following command:
    #    fv pip compile requirements.in --cache-dir [CACHE_DIR] --python-version=3.9.0
    package-a==1.0.0
        # via -r requirements.in
