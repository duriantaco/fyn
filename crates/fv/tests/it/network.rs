...
/// IO error.
///
/// Tests different errors paths inside fv, which retries 3 times by default, for a total for 4
/// requests.
...
/// Test that invalid proxy URL in fv.toml produces a helpful error message.
#[tokio::test]
...
/// Test that invalid proxy URL (not a URL) in fv.toml produces a helpful error message.
#[tokio::test]
...
/// Test that valid proxy URL in fv.toml routes requests through the proxy.
...
/// Test that https-proxy in fv.toml routes HTTPS requests through a CONNECT tunnel proxy.
#[cfg(feature = "test-pypi")]
#[test]
...
/// Test that no-proxy in fv.toml bypasses the proxy for specified hosts.
#[cfg(feature = "test-pypi")]
#[tokio::test]
...
/// Test that proxy URLs without a scheme in fv.toml default to http://.
#[cfg(feature = "test-pypi")]
#[tokio::test]
