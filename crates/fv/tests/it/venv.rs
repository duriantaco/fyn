...
fn create_venv_project_environment() -> Result<()> {
    ...
    // `fv venv` ignores `UV_PROJECT_ENVIRONMENT` when it's not a project
    fv_snapshot!(context.filters(), context.venv().env(EnvVars::UV_PROJECT_ENVIRONMENT, "foo"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.venv().env(EnvVars::UV_PROJECT_ENVIRONMENT, "foo").current_dir(child.path()), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.venv().arg("bar"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--clear").arg("--no-workspace"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--clear").arg("--no-project"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn virtual_empty() -> Result<()> {
    // testing how `fv venv` reacts to a pyproject with no `[project]` and nothing useful to it
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--clear"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn virtual_dependency_group() -> Result<()> {
    // testing basic `fv venv` functionality
...
fn create_venv_respects_pyproject_requires_python() -> Result<()> {
    let context = fv_test::test_context_with_versions!(&["3.11", "3.9", "3.10", "3.12"]);
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--clear").arg("--python").arg("3.11"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn create_venv_respects_group_requires_python() -> Result<()> {
    let context = fv_test::test_context_with_versions!(&["3.9", "3.10", "3.11", "3.12"]);
    ...
    ----- stderr -----
    Using CPython 3.11.[X] interpreter at: [PYTHON-3.11]
    warning: The requested interpreter resolved to Python 3.11.[X], which is incompatible with the project's Python requirement: `>=3.12` (from `tool.fv.dependency-groups.dev.requires-python`).
    Creating virtual environment at: .venv
    Activate with: source .venv/[BIN]/activate
    "
    );
...
fn seed_older_python_version() {
    let context = fv_test::test_context_with_versions!(&["3.11"]);
    fv_snapshot!(context.filters(), context.venv()
        .arg(context.venv.as_os_str())
        .arg("--seed")
        .arg("--python")
...
fn create_venv_with_invalid_http_timeout() {
    let context = fv_test::test_context_with_versions!(&["3.12"]).with_http_timeout("not_a_number");
    fv_snapshot!(context.filters(), context.venv()
        .arg(context.venv.as_os_str())
        .arg("--python")
        .arg("3.12"), @"
...
fn create_venv_with_invalid_concurrent_installs() {
    let context = fv_test::test_context_with_versions!(&["3.12"]).with_concurrent_installs("0");
    fv_snapshot!(context.filters(), context.venv()
        .arg(context.venv.as_os_str())
        .arg("--python")
        .arg("3.12"), @"
...
fn create_venv_unknown_python_minor() {
    let context = fv_test::test_context_with_versions!(&["3.12"]).with_filtered_python_sources();
    ...
    fv_snapshot!(context.filters(), &mut command, @"
    success: false
    exit_code: 2
    ----- stdout -----
...
fn create_venv_python_patch() {
    let context = fv_test::test_context_with_versions!(&["3.12.9"]);
...
/// Run `fv venv` followed by `fv venv --allow-existing`.
#[test]
...
fn windows_shims() -> Result<()> {
    let context = fv_test::test_context_with_versions!(&["3.10", "3.9"]);
...
fn verify_pyvenv_cfg() {
    ...
    // Check if "fv = version" is present in the file
    let version = env!("CARGO_PKG_VERSION").to_string();
    let search_string = format!("fv = {version}");
...
fn path_with_trailing_space_gives_proper_error() {
    ...
    fv_snapshot!(context.filters(), std::process::Command::new(fv_test::get_bin!())
        .arg("venv")
...
fn venv_python_preference() {
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--no-managed-python"), @"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--clear").arg("--no-managed-python"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    fv_snapshot!(context.filters(), context.venv().arg("--clear").arg("--managed-python"), @"
    success: true
    exit_code: 0
    ----- stdout -----
...
fn create_venv_symlink_clear_preservation() -> Result<()> {
    ...
    // Run fv venv with --clear to test symlink preservation during clear
    fv_snapshot!(context.filters(), context.venv()
        .arg(symlink_path.as_os_str())
        .arg("--clear")
        .arg("--python")
...
fn create_venv_symlink_recreate_preservation() -> Result<()> {
    ...
    // Run fv venv again with --clear to test symlink preservation during recreation
    fv_snapshot!(context.filters(), context.venv()
        .arg(symlink_path.as_os_str())
        .arg("--clear")
        .arg("--python")
...
fn create_venv_nested_symlink_preservation() -> Result<()> {
    ...
    // Run fv venv again with --clear to test nested symlink preservation during recreation
    fv_snapshot!(context.filters(), context.venv()
        .arg(symlink_path.as_os_str())
        .arg("--clear")
        .arg("--python")
...
fn no_clear_conflicts_with_allow_existing() {
    ...
    Usage: fv venv --cache-dir [CACHE_DIR] --python <PYTHON> --exclude-newer <EXCLUDE_NEWER> <PATH>
