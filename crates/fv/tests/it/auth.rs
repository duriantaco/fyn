...
async fn add_package_native_auth_realm() -> Result<()> {
    let context = fv_test::test_context!("3.12").with_real_home();
    ...
    fv_snapshot!(context.filters(), context.add().arg("anyio").arg("--default-index").arg(proxy.username_url("public", "/basic-auth/simple"))
    ...
    // Login to the domain
    fv_snapshot!(context.filters(), context.auth_login()
        .arg(proxy.uri())
        .arg("--username")
        .arg("public")
        .arg("--password")
    ...
    fv_snapshot!(context.filters(), context.auth_logout()
        .arg(proxy.uri())
        .arg("--username")
        .arg("public")
        .env(EnvVars::UV_PREVIEW_FEATURES, "native-auth"), @r"
    ...
    fv_snapshot!(context.filters(), context.add().arg("iniconfig").arg("--default-index").arg(proxy.username_url("public", "/basic-auth/simple"))
        .env(EnvVars::UV_PREVIEW_FEATURES, "native-auth"), @r"
    success: false
    exit_code: 1
    ----- stdout -----
...
async fn token_native_auth() -> Result<()> {
    let context = fv_test::test_context_with_versions!(&[]).with_real_home();
    let proxy = crate::pypi_proxy::start().await;
    ...
    fv_snapshot!(context.filters(), context.auth_token()
        .arg(proxy.url("/basic-auth/simple"))
        .env(EnvVars::UV_PREVIEW_FEATURES, "native-auth"), @r"
    success: false
    exit_code: 2
...
async fn login_native_auth() -> Result<()> {
    ...
    fv_snapshot!(context.filters(), context.auth_login(), @r"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    Usage: fv auth login --cache-dir [CACHE_DIR] <SERVICE>
...
async fn logout_native_auth() -> Result<()> {
    ...
    // Without a service name
    fv_snapshot!(context.filters(), context.auth_logout(), @r"
    success: false
    exit_code: 2
    ----- stdout -----
    ...
    Usage: fv auth logout --cache-dir [CACHE_DIR] <SERVICE>
...
fn login_native_auth_url() {
    ...
    fv_snapshot!(context.auth_login()
        .arg("example.com")
        .arg("--username")
        .arg("test")
        .arg("--password")
...
fn login_password_stdin() -> Result<()> {
    ...
    fv_snapshot!(context.auth_token()
        .arg("https://example.com/simple")
        .arg("--username")
        .arg("testuser"), @"
    success: true
...
fn logout_text_store_strips_simple_suffix() {
    ...
    fv_snapshot!(context.auth_logout()
        .arg("https://example.com/simple")
        .arg("--username")
        .arg("testuser"), @"
    success: true
...
fn bazel_helper_basic_auth() {
    ...
    fv_snapshot!(context.filters(), context.auth_helper()
        .arg("--protocol=bazel")
        .arg("get"),
    ...
    ----- stderr -----
    warning: The `fv auth helper` command is experimental and may change without warning. Pass `--preview-features auth-helper` to disable this warning
    "#
    );
}
...
/// Test that `fv auth token` recognizes `pyx.dev` even with custom `PYX_API_URL`.
#[test]
