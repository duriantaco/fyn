...
fn username_password_no_longer_supported() {
    ...
    fv_snapshot!(context.filters(), context.publish()
        .arg("-u")
        .arg("dummy")
        .arg("-p")
        .arg("dummy")
...
fn skip_existing_redirect() {
    ...
    ----- stderr -----
    error: `fv publish` does not support `--skip-existing` because there is not a reliable way to identify when an upload fails due to an existing distribution. Instead, use `--check-url` to provide the URL to the simple API for your index. fv will check the index for existing distributions before attempting uploads.
    "
    );
}
