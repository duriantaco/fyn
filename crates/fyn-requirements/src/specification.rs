...
//! of inclusion (`fyn pip install -r` and `fyn pip compile` vs. `fyn pip install -c` and
//! `fyn pip compile -c`).
//!
...
//! * `fyn pip install -r pyproject.toml` or `fyn pip compile requirements.in`: The `pyproject.toml`
...
//! * `fyn pip install .` in a directory with `pyproject.toml` or `fyn pip compile requirements.in`
...
//! * `fyn pip install -e`: We add the directory in `editables` instead of `requirements`. The
...
use fyn_configuration::{DependencyGroups, NoBinary, NoBuild};
use fyn_distribution_types::{Index, Requirement};
...
use fyn_pypi_types::PyProjectToml;
use fyn_redacted::DisplaySafeUrl;
use fyn_requirements_txt::{RequirementsTxt, RequirementsTxtRequirement, SourceCache};
use fyn_scripts::Pep723Metadata;
...
impl RequirementsSpecification {
    ...
    fn from_pep723_metadata(metadata: &Pep723Metadata) -> Self {
        ...
        if let Some(tool_fyn) = metadata.tool.as_ref().and_then(|tool| tool.fyn.as_ref()) {
            let constraints = tool_fyn
            ...
            Self {
                ...
                extra_index_urls: tool_fyn
                    ...
                    .collect(),
                no_index: tool_fyn.top_level.no_index.unwrap_or_default(),
                find_links: tool_fyn
                    .top_level
                    .find_links
                    .as_ref()
...
async fn read_file(path: &Path, client_builder: &BaseClientBuilder<'_>) -> Result<String> {
    ...
    let content = fyn_fs::read_to_string_transcode(path).await?;
    Ok(content)
}
