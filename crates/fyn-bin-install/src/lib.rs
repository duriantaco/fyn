//! Binary download and installation utilities for fyn.
//!
...
use fyn_client::retryable_on_request_failure;
use fyn_distribution_filename::SourceDistExtension;
...
use fyn_cache::{Cache, CacheBucket, CacheEntry, Error as CacheError};
use fyn_client::{BaseClient, RetriableError, fetch_with_url_fallback};
use fyn_extract::{Error as ExtractError, stream};
use fyn_pep440::{Version, VersionSpecifier, VersionSpecifiers};
...
impl Binary {
    ...
    /// allowing patch version updates without requiring a fyn release.
...
pub enum BinVersion {
    ...
    Constraint(fyn_pep440::VersionSpecifiers),
}

impl FromStr for BinVersion {
    type Err = fyn_pep440::VersionSpecifiersParseError;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let specifiers = fyn_pep440::VersionSpecifiers::from_str(s)?;
...
pub enum Error {
    ...
    UrlParse {
        url: String,
        #[source]
        source: fyn_redacted::DisplaySafeUrlError,
    },
    ...
    Platform(#[from] fyn_platform::Error),
    ...
    NoMatchingVersion {
        binary: Binary,
        constraints: fyn_pep440::VersionSpecifiers,
        platform: String,
    },
...
pub async fn find_matching_version(
    binary: Binary,
    constraints: Option<&fyn_pep440::VersionSpecifiers>,
...
#[cfg(test)]
mod tests {
    use fyn_client::{BaseClientBuilder, retryable_on_request_failure};
    use fyn_redacted::DisplaySafeUrl;
    use std::io::Write;
