...
use fyn_distribution_types::{Index, Requirement, RequirementSource};
...
pub struct Workspace {
    ...
    /// the `fyn.tool.workspace`, or the `pyproject.toml` in an implicit single workspace project.
...
impl Workspace {
    ...
    pub async fn discover(
        ...
        let path = std::path::absolute(path)
            ...
            .clone();
        // Remove `.` and `..`
        let path = fyn_fs::normalize_path(&path);
    ...
    ///
    /// N.B. this checks if a workspace member is required by inspecting `tool.fyn.source` entries,
    ...
    pub fn requirements(&self) -> Vec<fyn_pep508::Requirement<VerbatimParsedUrl>> {
        Vec::new()
    }
    ...
    pub fn venv(&self, active: Option<bool>) -> PathBuf {
        ...
        if let Some(from_virtual_env) = from_virtual_env_variable() {
            if !fyn_fs::is_same_file_allow_missing(&from_virtual_env, &project_env).unwrap_or(false)
            {
                match active {
                    Some(true) => {
                        debug!(
    ...
    async fn collect_members(
        workspace_root: PathBuf,
        workspace_definition: ToolfynWorkspace,
        ...
        if !dev_dependencies_members.is_empty() {
            warn_user_once!(
                "The `tool.fyn.dev-dependencies` field (used in {}) is deprecated and will be removed in a future release; use `dependency-groups.dev` instead",
    ...
    async fn collect_members_only(
        ...
        for member_glob in workspace_definition.clone().members.unwrap_or_default() {
            ...
            let normalized_glob = fyn_fs::normalize_path(Path::new(member_glob.as_str()));
...
impl ProjectWorkspace {
    ...
    async fn from_project(
        ...
        let project_path = std::path::absolute(install_path)
            ...
            .clone();
        // Remove `.` and `..`
        let project_path = fyn_fs::normalize_path(&project_path);
        // Trim trailing slashes.
...
async fn find_workspace(
    project_root: &Path,
    options: &DiscoveryOptions,
) -> Result<Option<(PathBuf, ToolfynWorkspace, PyProjectToml)>, WorkspaceError> {
...
fn is_excluded_from_workspace(
    ...
    for exclude_glob in workspace.exclude.iter().flatten() {
        ...
        let normalized_glob = fyn_fs::normalize_path(Path::new(exclude_glob.as_str()));
        let absolute_glob = PathBuf::from(glob::Pattern::escape(
            workspace_root.simplified().to_string_lossy().as_ref(),
        ))
        .join(normalized_glob.as_ref());
