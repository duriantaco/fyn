...
use fyn_normalize::{DEV_DEPENDENCIES, GroupName};
use fyn_pep440::VersionSpecifiers;
use fyn_pep508::Pep508Error;
use fyn_pypi_types::{DependencyGroupSpecifier, VerbatimParsedUrl};

use crate::pyproject::{DependencyGroupSettings, PyProjectToml, ToolfynDependencyGroups};
...
pub struct FlatDependencyGroup {
    pub requirements: Vec<fyn_pep508::Requirement<VerbatimParsedUrl>>,
            ...
            for specifier in *specifiers {
                match specifier {
                    DependencyGroupSpecifier::Requirement(requirement) => {
                        match fyn_pep508::Requirement::<VerbatimParsedUrl>::from_str(requirement) {
...
pub enum DependencyGroupErrorInner {
    ...
    #[error(
        "Group `{0}` includes the `dev` group (`include = \"dev\"`), but only `tool.fyn.dev-dependencies` was found. To reference the `dev` group via an `include`, remove the `tool.fyn.dev-dependencies` section and add any development dependencies to the `dev` entry in the `[dependency-groups]` table instead."
    )]
    ...
    #[error("Failed to find group `{0}` specified in `[tool.fyn.dependency-groups]`")]
    SettingsGroupNotFound(GroupName),
    #[error(
        "`[tool.fyn.dependency-groups]` specifies the `dev` group, but only `tool.fyn.dev-dependencies` was found. To reference the `dev` group, remove the `tool.fyn.dev-dependencies` section and add any development dependencies to the `dev` entry in the `[dependency-groups]` table instead."
    )]
    SettingsDevGroupInclude,
}

impl DependencyGroupErrorInner {
    /// Enrich a [`DependencyGroupError`] with the `tool.fyn.dev-dependencies` metadata, if applicable.
    #[must_use]
    pub fn with_dev_dependencies(
        self,
        dev_dependencies: Option<&Vec<fyn_pep508::Requirement<VerbatimParsedUrl>>>,
    ) -> Self {
        match self {
