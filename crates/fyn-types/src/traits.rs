...
use fyn_cache::Cache;
use fyn_configuration::{BuildKind, BuildOptions, BuildOutput, NoSources};
...
///         │                  │                   │
/// ┌───────┴────────┐ ┌───────┴────────┐ ┌────────┴────────────────┐
/// │ fyn-resolver   │ │ fyn-installer  │ │   fyn-build-frontend    │
/// └───────▲────────┘ └───────▲────────┘ └────────▲────────────────┘
///         │                  │                   │
///         └─────────────┐    │    ┌──────────────┘
///                    ┌──┴────┴────┴───┐
...
/// ```
///
/// Put in a different way, the types here allow `fyn-resolver` to depend on `fyn-build` and
/// `fyn-build-frontend` to depend on `fyn-resolver` without having actual crate dependencies between
/// them.
pub trait BuildContext {
    ...
    /// Whether to incorporate `tool.fyn.sources` when resolving requirements.
    ...
    /// Build by calling directly into the fyn build backend without PEP 517, if possible.
    ///
    /// Checks if the source tree uses fyn as build backend. If not, it returns `Ok(None)`, otherwise
...
/// A wrapper for `fyn_build::SourceBuild` to avoid cyclical crate dependencies.
///
...
pub trait SourceBuildTrait {
    /// A wrapper for `fyn_build::SourceBuild::get_metadata_without_build`.
    ///
    ...
    /// A wrapper for `fyn_build::SourceBuild::build`.
    ///
...
/// A wrapper for [`fyn_installer::SitePackages`]
...
///
/// The errors types have the same problem as [`BuildDispatch`] generally: The `fyn-resolver`,
/// `fyn-installer` and `fyn-build-frontend` error types all reference each other:
