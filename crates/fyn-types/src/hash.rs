...
use fyn_configuration::HashCheckingMode;
...
use fyn_pypi_types::{HashDigest, HashDigests, HashError, ResolverMarkerEnvironment};
...
///
/// Exact duplicates are ignored. Digests for different algorithms are accumulated. If the same
/// algorithm appears with two different values, returns
...
mod tests {
    ...
    fn from_requirements_merges_direct_url_hashes_across_fragments() {
        ...
        let hasher = HashStrategy::from_requirements(
            ...
            std::iter::empty(),
            None,
            fyn_configuration::HashCheckingMode::Require,
        )
        .unwrap();
