...
use fyn_types::{AnyErrorBuild, BuildContext, BuildIsolation, BuildStack, SourceBuildTrait};
...
static DEFAULT_BACKEND: LazyLock<Pep517Backend> = LazyLock::new(|| Pep517Backend {
    ...
    requirements: vec![Requirement::from(
        fyn_pep508::Requirement::from_str("setuptools >= 40.8.0").unwrap(),
    )],
});
...
struct BuildSystem {
    ...
    requires: Vec<fyn_pep508::Requirement<VerbatimParsedUrl>>,
    ...
    async fn extract_pep517_backend(
        ...
        if !no_sources.all()
            && pyproject_toml
                ...
                .map(|fyn| fyn.build_backend.is_some())
                .unwrap_or(false)
            && build_backend != Some("fyn_build")
                ...
                package_name.or(pyproject_toml.project.as_ref().map(|project| &project.name))
        {
            ...
            if let Some(build_backend) = build_backend {
                warn_user_once!(
                    "`{package_name}` defines settings for `fyn_build` in `tool.fyn.build-backend`, \
        ...
        let backend = if let Some(build_system) = pyproject_toml.build_system {
            ...
            {
                let build_requires = fyn_pypi_types::BuildRequires {
            ...
            {
                // Give a specific hint for `fyn pip install .` in a workspace root.
                ...
                if looks_like_workspace_root {
                    warn_user_once!(
                        ...
                        consider using `fyn sync` to install the workspace, or add a \
...
async fn create_pep517_build_environment(
    ...
    let extra_requires: Vec<fyn_pep508::Requirement<VerbatimParsedUrl>> = match read_requires_result
    {
