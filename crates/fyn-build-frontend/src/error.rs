...
use fyn_configuration::BuildOutput;
use fyn_distribution_types::IsBuildBackendError;
...
pub enum Error {
    ...
    Io(#[from] io::Error),
    #[error(transparent)]
    Lowering(#[from] fyn_distribution::MetadataError),
...
fn hint_build_dependency(
    ...
    write!(
        ...
        table_key.cyan(),
        package.cyan(),
        format!("fyn pip install {package}").green(),
...
mod test {
    ...
    fn missing_wheel_package() {
        ...
        or `fyn pip install wheel` into the environment and re-run with `--no-build-isolation`.
        "#);
    }
