...
use fyn_distribution_filename::{EggInfoFilename, ExpandedTags};
use fyn_fs::Simplified;
use fyn_install_wheel::WheelFile;
use fyn_normalize::PackageName;
use fyn_pep440::Version;
use fyn_pypi_types::{DirectUrl, MetadataError};
...
pub enum InstalledDistError {
    ...
    #[error(transparent)]
    EggInfoParse(#[from] fyn_distribution_filename::EggInfoFilenameError),

    #[error(transparent)]
    VersionParse(#[from] fyn_pep440::VersionParseError),

    #[error(transparent)]
    PackageNameParse(#[from] fyn_normalize::InvalidNameError),

    #[error(transparent)]
    WheelFileParse(#[from] fyn_install_wheel::Error),

    #[error(transparent)]
    ExpandedTagParse(#[from] fyn_distribution_filename::ExpandedTagError),
...
pub struct InstalledDist {
    ...
    metadata_cache: OnceLock<fyn_pypi_types::ResolutionMetadata>,
...
impl InstalledDist {
    ...
    /// Read the `fyn_cache.json` file from a `.dist-info` directory.
    pub fn read_cache_info(path: &Path) -> Result<Option<CacheInfo>, InstalledDistError> {
        let path = path.join("fyn_cache.json");
        let file = match fs_err::File::open(&path) {
            Ok(file) => file,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(None),
            Err(err) => return Err(err.into()),
    ...
    /// Read the `fyn_build.json` file from a `.dist-info` directory.
    pub fn read_build_info(path: &Path) -> Result<Option<BuildInfo>, InstalledDistError> {
        let path = path.join("fyn_build.json");
        let file = match fs_err::File::open(&path) {
            Ok(file) => file,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(None),
            Err(err) => return Err(err.into()),
    ...
    pub fn read_metadata(&self) -> Result<&fyn_pypi_types::ResolutionMetadata, InstalledDistError> {
        ...
        let metadata = match &self.kind {
            InstalledDistKind::Registry(_) | InstalledDistKind::Url(_) => {
                ...
                fyn_pypi_types::ResolutionMetadata::parse_metadata(&contents).map_err(|err| {
...
fn read_metadata(path: &Path) -> Option<fyn_pypi_types::Metadata10> {
    ...
    let metadata = match fyn_pypi_types::Metadata10::parse_pkg_info(&content) {
        Ok(metadata) => metadata,
        Err(err) => {
