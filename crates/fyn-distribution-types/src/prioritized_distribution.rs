...
use fyn_distribution_filename::{BuildTag, WheelFilename};
use fyn_pep440::{Version, VersionSpecifier, VersionSpecifiers};
use fyn_pep508::{MarkerExpression, MarkerOperator, MarkerTree, MarkerValueString};
use fyn_platform_tags::{AbiTag, IncompatibleTag, LanguageTag, PlatformTag, TagPriority, Tags};
use fyn_pypi_types::{HashDigest, Yanked};
...
fn implied_python_markers(filename: &WheelFilename) -> MarkerTree {
    ...
    // If any ABI tag is one of the stable ABI tags, the python tag represents a minimum version
            ...
            LanguageTag::Python { major, minor: None } | LanguageTag::CPythonMajor { major } => {
                if is_abi3 {
                    MarkerTree::expression(MarkerExpression::Version {
                        key: fyn_pep508::MarkerValueVersion::PythonVersion,
