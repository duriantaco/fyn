...
use fyn_cache_key::{CacheKey, CacheKeyHasher};
use fyn_distribution_filename::DistExtension;
use fyn_fs::{CWD, PortablePath, PortablePathBuf, try_relative_to_if};
use fyn_git_types::{GitLfs, GitOid, GitReference, GitUrl, GitUrlParseError, OidParseError};
...
pub enum RequirementError {
    #[error(transparent)]
    VerbatimUrlError(#[from] fyn_pep508::VerbatimUrlError),
    #[error(transparent)]
...
impl From<Requirement> for fyn_pep508::Requirement<VerbatimUrl> {
    /// Convert a [`Requirement`] to a [`fyn_pep508::Requirement`].
...
impl From<Requirement> for fyn_pep508::Requirement<VerbatimParsedUrl> {
    /// Convert a [`Requirement`] to a [`fyn_pep508::Requirement`].
    fn from(requirement: Requirement) -> Self {
        Self {
            name: requirement.name,
...
impl From<fyn_pep508::Requirement<VerbatimParsedUrl>> for Requirement {
    /// Convert a [`fyn_pep508::Requirement`] to a [`Requirement`].
    fn from(requirement: fyn_pep508::Requirement<VerbatimParsedUrl>) -> Self {
    ...
    pub fn to_absolute(self, root: &Path) -> Self {
        match self {
            ...
            Self::Path {
                ...
                install_path: fyn_fs::normalize_path_buf(root.join(install_path)).into_boxed_path(),
                ext,
                url,
            },
            Self::Directory {
    ...
    fn try_from(wire: RequirementSourceWire) -> Result<Self, RequirementError> {
        match wire {
            ...
            RequirementSourceWire::Path { path } => {
                ...
                let url =
                    VerbatimUrl::from_normalized_path(fyn_fs::normalize_path_buf(CWD.join(&path)))?;
                Ok(Self::Path {
            ...
            RequirementSourceWire::Directory { directory } => {
                ...
                let url = VerbatimUrl::from_normalized_path(fyn_fs::normalize_path_buf(
                    CWD.join(&directory),
                ))?;
                Ok(Self::Directory {
