...
pub struct Index {
    ...
    /// defined via `[[tool.fyn.index]]`, PyPI will still be used as a fallback for packages that
    ...
    ///
    /// When using `fyn publish --index <name>`, this URL is used for publishing.
    ///
    ...
    /// When fyn should use authentication for requests to the index.
    ///
    /// ```toml
    /// [[tool.fyn.index]]
    /// name = "my-index"
    ...
    /// Status codes that fyn should ignore when deciding whether
...
impl std::fmt::Debug for Index {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut debug = f.debug_struct("Index");
        debug
            ...
            .field("ignore_error_codes", &self.ignore_error_codes)
            .field("cache_control", &self.cache_control);
        if self.exclude_newer.is_some() {
            debug.field("exclude_newer", &self.exclude_newer);
        }
        debug.finish()
    }
}
...
impl Index {
    ...
    /// (e.g., `fyn tool upgrade`) know that authentication is required even after the credentials
...
mod tests {
    ...
    #[test]
    fn test_index_exclude_newer_disabled() {
        let toml_str = r#"
            name = "test-index"
            url = "https://test.example.com/simple"
            exclude-newer = false
