...
pub enum NoSources {
    /// Use `tool.fv.sources` when resolving dependencies.
    #[default]
    None,

    /// Ignore `tool.fv.sources` when resolving dependencies for all packages.
    All,

    /// Ignore `tool.fv.sources` when resolving dependencies for specific packages.
