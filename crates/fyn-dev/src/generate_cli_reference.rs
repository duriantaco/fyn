//! Generate a Markdown-compatible reference for the fyn command-line interface.
use std::cmp::max;
...
const REPLACEMENTS: &[(&str, &str)] = &[
    // Replace suggestions to use `fyn help python` with a link to the
    // `fyn python` section
    (
        "<code>fyn help python</code>",
        "<a href=\"#fyn-python\">fyn python</a>",
    ),
...
fn generate_command<'a>(output: &mut String, command: &'a Command, parents: &mut Vec<&'a Command>) {
    ...
    // Display the top-level `fyn` command at the same level as its children
