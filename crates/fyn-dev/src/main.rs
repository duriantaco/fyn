...
async fn main() -> ExitCode {
    ...
    // Show `INFO` messages from the fyn crate, but allow `RUST_LOG` to override.
    let default_directive = Directive::from_str("fyn=info").unwrap();
    ...
    if let Err(err) = result {
        ...
        eprintln!("{}", "fyn-dev failed".red().bold());
