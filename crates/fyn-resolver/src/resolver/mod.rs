...
use fyn_configuration::{Constraints, Excludes, Overrides};
use fyn_distribution::{ArchiveMetadata, DistributionDatabase};
...
use fyn_pep440::{MIN_VERSION, Version, VersionSpecifiers, release_specifiers_to_ranges};
use fyn_pep508::{
    MarkerEnvironment, MarkerExpression, MarkerOperator, MarkerTree, MarkerValueString,
};
use fyn_platform_tags::{IncompatibleTag, Tags};
use fyn_pypi_types::{
    ...
    fn new(
        ...
        for (name, mut deps) in name_to_deps {
            ...
            if let [dep] = deps.as_slice() {
                ...
                // resolution.
                //
                // For example, given `requires-python = ">=3.7"` and `fyn ; python_version >= "3.8"`,
                // where fyn itself only supports Python 3.8 and later, we need to fork to ensure
