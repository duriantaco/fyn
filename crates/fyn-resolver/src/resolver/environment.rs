...
use fyn_distribution_types::{RequiresPython, RequiresPythonRange};
use fyn_pep440::VersionSpecifiers;
use fyn_pep508::{MarkerEnvironment, MarkerTree};
use fyn_pypi_types::{
...
/// a specific marker environment for `fyn pip`-style resolutions and ask for a
/// universal resolution for fyn's project based commands like `fyn lock`.
///
...
enum Kind {
    ...
    ///
    /// Generally, this is what's done for `fyn pip`. For the project based
    /// commands, like `fyn lock`, we do universal resolution.
    Specific {
...
impl ResolverEnvironment {
    ...
    /// environment.
    ///
    /// This enables `fyn pip`-style resolutions. That is, the resolution
...
mod tests {
    ...
    use fyn_pep440::{LowerBound, UpperBound, Version};
    ...
    /// copied from our fyn microbenchmarks.
