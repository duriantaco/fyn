use fyn_client::MetadataFormat;
use fyn_configuration::BuildOptions;
use fyn_distribution::{ArchiveMetadata, DistributionDatabase, Reporter};
...
pub type PackageVersionsResult = Result<VersionsResponse, fyn_client::Error>;
pub type WheelMetadataResult = Result<MetadataResponse, fyn_distribution::Error>;
...
pub enum MetadataResponse {
    ...
    Error(Box<RequestedDist>, Arc<fyn_distribution::Error>),
}
...
pub enum MetadataUnavailable {
    ...
    InvalidMetadata(Arc<fyn_pypi_types::MetadataError>),
    /// The wheel metadata was found, but the metadata was inconsistent.
    InconsistentMetadata(Arc<fyn_distribution::Error>),
    /// The wheel has an invalid structure.
    InvalidStructure(Arc<fyn_metadata::Error>),
...
impl<'a, Context: BuildContext> DefaultResolverProvider<'a, Context> {
    ...
    fn effective_exclude_newer(
        ...
        index: &fyn_distribution_types::IndexUrl,
...
impl<Context: BuildContext> ResolverProvider for DefaultResolverProvider<'_, Context> {
    ...
    async fn get_package_versions<'io>(
        ...
        match result {
            ...
            Err(err) => match err.kind() {
                fyn_client::ErrorKind::RemotePackageNotFound(_) => {
                ...
                fyn_client::ErrorKind::NoIndex(_) => {
                    if let Some(flat_index) = flat_index
                        .and_then(|flat_index| flat_index.get(package_name))
                        .cloned()
                    {
                ...
                fyn_client::ErrorKind::Offline(_) => {
                    if let Some(flat_index) = flat_index
                        .and_then(|flat_index| flat_index.get(package_name))
                        .cloned()
                    {
    ...
    async fn get_or_build_wheel_metadata<'io>(&'io self, dist: &'io Dist) -> WheelMetadataResult {
            ...
            .await
        {
            ...
            Err(err) => match err {
                fyn_distribution::Error::Client(client) => {
                    ...
                    match client.into_kind() {
                        ...
                        fyn_client::ErrorKind::MetadataParseError(_, _, err) => {
                            Ok(MetadataResponse::Unavailable(
                                MetadataUnavailable::InvalidMetadata(Arc::new(*err)),
                            ))
                        }
                        fyn_client::ErrorKind::Metadata(_, err) => {
                            Ok(MetadataResponse::Unavailable(
                                MetadataUnavailable::InvalidStructure(Arc::new(err)),
                            ))
                        }
                        kind => Err(fyn_client::Error::new(kind, retries, duration).into()),
                    }
                }
                fyn_distribution::Error::WheelMetadataVersionMismatch { .. } => {
                    Ok(MetadataResponse::Unavailable(
                        MetadataUnavailable::InconsistentMetadata(Arc::new(err)),
                    ))
                }
                fyn_distribution::Error::WheelMetadataNameMismatch { .. } => {
                    Ok(MetadataResponse::Unavailable(
                        MetadataUnavailable::InconsistentMetadata(Arc::new(err)),
                    ))
                }
                fyn_distribution::Error::Metadata(err) => Ok(MetadataResponse::Unavailable(
                    MetadataUnavailable::InvalidMetadata(Arc::new(err)),
                )),
                fyn_distribution::Error::WheelMetadata(_, err) => {
                    Ok(MetadataResponse::Unavailable(
                        MetadataUnavailable::InvalidStructure(Arc::new(*err)),
                    ))
                }
                fyn_distribution::Error::RequiresPython(requires_python, version) => {
                    Ok(MetadataResponse::Unavailable(
