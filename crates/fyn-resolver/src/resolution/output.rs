...
use fyn_configuration::{Constraints, Overrides};
use fyn_distribution::Metadata;
...
use fyn_pep440::{Version, VersionSpecifier};
use fyn_pep508::{MarkerEnvironment, MarkerTree, MarkerTreeKind};
use fyn_pypi_types::{Conflicts, HashDigests, ParsedUrlError, VerbatimParsedUrl, Yanked};
...
impl ResolverOutput {
    ...
    pub(crate) fn from_state(
        ...
        // the same time. At which point, fyn will report an error,
        ...
        if conflicts.is_empty() {
            ...
            // Returning an error in that case would make `fyn` unusable whenever
            // the bug occurs, but letting it through means `fyn` *could* still be
            // usable.
...
/// Convert a [`ResolverOutput`] into a [`fyn_distribution_types::Resolution`].
///
...
impl From<ResolverOutput> for fyn_distribution_types::Resolution {
