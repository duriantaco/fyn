...
use fyn_configuration::{IndexStrategy, NoBinary, NoBuild};
...
use fyn_pep508::{MarkerEnvironment, MarkerExpression, MarkerTree, MarkerValueVersion};
use fyn_platform_tags::{AbiTag, IncompatibleTag, LanguageTag, PlatformTag, Tags};
        ...
        match derivation_tree {
            ...
            DerivationTree::External(External::NoVersions(package, set)) => {
                if let Some(name) = package.name_no_root() {
                    ...
                    if let Some((exclude_newer, source)) = options
                        .exclude_newer
                        .exclude_newer_package_for_index_with_source(
                            name,
                            fork_indexes
                                .get(name)
                                .and_then(|index| index_locations.exclude_newer_for(index.url())),
                        )
                    {
                        if self
                            .available_versions
        ...
        match self {
            ...
            Self::ExcludeNewer {
                ...
                EffectiveExcludeNewerSource::Index => write!(
                    f,
                    "{}{} `{}` was filtered by the index-specific `{}` setting to only include packages uploaded \
                        before {}. Consider updating that index's cutoff, setting it to `false`, or using `{}` to override the cutoff for this package.",
                    "hint".bold().cyan(),
                    ":".bold(),
                    package.cyan(),
                    "exclude-newer".green(),
