...
use fyn_pep508::{ExtraOperator, MarkerEnvironment, MarkerExpression, MarkerOperator, MarkerTree};
use fyn_pypi_types::{ConflictItem, ConflictKind, Conflicts, Inference};
...
pub struct UniversalMarker {
    ...
    /// ```
    ///
    /// Then, when `fyn sync --extra x1` is called, we encode that was
    ...
    /// stand-point, evaluating it requires fyn's custom encoding of extras (and
    /// groups).
    marker: MarkerTree,
...
impl ConflictMarker {
    ...
    pub fn project(package: &PackageName) -> Self {
        let operator = fyn_pep508::ExtraOperator::Equal;
        let name = fyn_pep508::MarkerValueExtra::Extra(encode_project(package));
        let expr = fyn_pep508::MarkerExpression::Extra { operator, name };
    ...
    pub fn extra(package: &PackageName, extra: &ExtraName) -> Self {
        let operator = fyn_pep508::ExtraOperator::Equal;
        let name = fyn_pep508::MarkerValueExtra::Extra(encode_package_extra(package, extra));
        let expr = fyn_pep508::MarkerExpression::Extra { operator, name };
        let marker = MarkerTree::expression(expr);
        Self { marker }
    }
    ...
    pub fn group(package: &PackageName, group: &GroupName) -> Self {
        let operator = fyn_pep508::ExtraOperator::Equal;
        let name = fyn_pep508::MarkerValueExtra::Extra(encode_package_group(package, group));
        let expr = fyn_pep508::MarkerExpression::Extra { operator, name };
        let marker = MarkerTree::expression(expr);
        Self { marker }
    }
...
mod tests {
    ...
    use fyn_pypi_types::ConflictSet;
