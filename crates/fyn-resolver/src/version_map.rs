...
use fyn_client::{FlatIndexEntry, OwnedArchive, SimpleDetailMetadata, VersionFiles};
use fyn_configuration::BuildOptions;
use fyn_distribution_filename::{DistFilename, WheelFilename};
...
use fyn_pypi_types::{HashDigest, ResolutionMetadata, Yanked};
...
impl VersionMap {
    ...
    /// Return the effective `exclude-newer` cutoff that applied to this version map, if any.
