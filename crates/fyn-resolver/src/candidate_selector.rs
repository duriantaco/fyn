...
use fyn_distribution_types::{CompatibleDist, IncompatibleDist, IncompatibleSource, IndexUrl};
use fyn_distribution_types::{DistributionMetadata, IncompatibleWheel, Name, PrioritizedDist};
use fyn_normalize::PackageName;
use fyn_pep440::Version;
use fyn_platform_tags::Tags;
use fyn_types::InstalledPackagesProvider;
...
pub(crate) enum VersionChoiceKind {
    /// A preference from an output file such as `-o requirements.txt` or `fyn.lock`.
    Preference,
...
impl DistributionMetadata for Candidate<'_> {
    fn version_or_url(&self) -> fyn_distribution_types::VersionOrUrlRef<'_> {
        fyn_distribution_types::VersionOrUrlRef::Version(self.version)
    }
}
