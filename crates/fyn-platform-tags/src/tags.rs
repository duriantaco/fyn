...
impl Tags {
    ...
    pub fn from_env(
            ...
            && variant.contains(CPythonAbiVariants::Debug)
        {
            ...
            let non_debug_abi = AbiTag::CPython {
        ...
        if let Implementation::CPython { variant } = implementation {
            for minor in (2..=python_version.1).rev() {
                let abi_tag = if variant.contains(CPythonAbiVariants::Freethreading) {
                    AbiTag::Abi3T
                } else {
                    AbiTag::Abi3
                };
    ...
    pub fn compatibility(
        ...
        // Only `none` (pure Python), `abi3t`, and free-threaded CPython ABIs (e.g., `cp313t`)
        // are compatible.
