<!-- This file is generated. DO NOT EDIT -->

# fyn-performance-memory-allocator

This crate is an internal component of [fyn](https://crates.io/crates/fyn). The Rust API exposed
here is unstable and will have frequent breaking changes.

This version (0.0.35) is a component of [fyn 0.10.14](https://crates.io/crates/fyn/0.10.14). The
source can be found
[here](https://github.com/oha/fyn/blob/0.10.14/crates/fyn-performance-memory-allocator).

See fyn's crate versioning policy for details on versioning.
