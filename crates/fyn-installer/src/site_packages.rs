...
use fyn_pep508::VersionOrUrl;
use fyn_platform_tags::Tags;
use fyn_pypi_types::{ResolverMarkerEnvironment, VerbatimParsedUrl};
                ...
                match &requirement.requirement {
                    ...
                    UnresolvedRequirement::Unnamed(requirement) => {
                        match self.get_urls(requirement.url.verbatim.raw()).as_slice() {
                            ...
                            [distribution] => {
                                let requirement = fyn_pep508::Requirement {
...
pub enum InstallationStrategy {
    ...
    /// type differs, as in the `pip` and `fyn pip` CLIs.
    ///
    ...
    /// then runs `fyn pip install anyio` (which depends on `idna`), the existing `idna` installation
