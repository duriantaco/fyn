...
use fyn_cache::Cache;
use fyn_configuration::RAYON_INITIALIZE;
use fyn_distribution_types::CachedDist;
use fyn_install_wheel::{Layout, LinkMode};
...
impl<'a> Installer<'a> {
    ...
    pub fn new(venv: &'a PythonEnvironment, preview: Preview) -> Self {
        Self {
            ...
            cache: None,
            reporter: None,
            name: Some("fyn".to_string()),
            metadata: true,
            preview,
        }
    }

    /// Set the [`LinkMode`][`fyn_install_wheel::LinkMode`] to use for this installer.
    #[must_use]
    ...
    /// Set the `installer_name` to something other than `"fyn"`.
    #[must_use]
    ...
    /// Set whether to install fyn-specifier files in the dist-info directory.
    #[must_use]
...
fn install(
    ...
    // Initialize the threadpool with the user settings.
    LazyLock::force(&RAYON_INITIALIZE);
    let state = fyn_install_wheel::InstallState::new(preview);
    wheels.par_iter().try_for_each(|wheel| {
        fyn_install_wheel::install_wheel(
            layout,
            relocatable,
            wheel.path(),
            wheel.filename(),
            wheel
                .parsed_url()
                .map(fyn_pypi_types::DirectUrl::from)
                .as_ref(),
