...
use fyn_distribution::{DistributionDatabase, LocalWheel};
...
use fyn_types::{BuildContext, HashStrategy, InFlight};
            ...
            let result = self
                ...
                .and_then(|wheel: LocalWheel| {
                    if wheel.satisfies(policy) {
                        ...
                        let err = fyn_distribution::Error::hash_mismatch(
                            dist.to_string(),
                            policy.digests(),
                            wheel.hashes(),
                        );
            ...
            match result.as_ref() {
                Ok(cached) => {
                    ...
                    if *dist.name() != cached.filename().name {
                        let err = fyn_distribution::Error::WheelMetadataNameMismatch {
                    ...
                    if let Some(version) = dist.version() {
                        ...
                        {
                            let err = fyn_distribution::Error::WheelMetadataVersionMismatch {
                                given: version.clone(),
...
pub enum Error {
    ...
    Dist(
        ...
        Box<Dist>,
        DerivationChain,
        #[source] Box<fyn_distribution::Error>,
    ),
...
impl Error {
    ...
    fn from_dist(dist: Dist, err: fyn_distribution::Error, resolution: &Resolution) -> Self {
        let chain =
...
impl dyn Reporter {
    /// Converts this reporter to a [`fyn_distribution::Reporter`].
    pub(crate) fn into_distribution_reporter(
        self: Arc<dyn Reporter>,
    ) -> Arc<dyn fyn_distribution::Reporter> {
        Arc::new(Facade {
            reporter: self.clone(),
        })
    }
...
/// A facade for converting from [`Reporter`] to [`fyn_distribution::Reporter`].
struct Facade {
    reporter: Arc<dyn Reporter>,
}

impl fyn_distribution::Reporter for Facade {
    fn on_build_start(&self, source: &BuildableSource) -> usize {
