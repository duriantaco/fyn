...
use fyn_cache_key::{CanonicalUrl, RepositoryUrl};
use fyn_distribution_filename::ExpandedTags;
    ...
    pub(crate) fn check(
        ...
        match source {
            ...
            RequirementSource::Registry { specifier, .. } => {
                ...
                //
                // For example: the `fyn pip` CLI is stateful, in that it "respects"
                // already-installed packages in the virtual environment. So if you run `fyn pip
                // install ./path/to/idna`, and then `fyn pip install anyio` (which depends on
                // `idna`), we'll "accept" the already-installed `idna` even though it is implicitly
                // being "required" as a registry package.
                //
                // The `fyn sync` CLI is stateless, in that all requirements must be defined
                // declaratively ahead-of-time. So if you `fyn sync` to install `./path/to/idna` and
                // later `fyn sync` to install `anyio`, we'll know (during that second sync) if the
