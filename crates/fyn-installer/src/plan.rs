...
use fyn_cache::{Cache, CacheBucket, WheelCache};
use fyn_cache_info::Timestamp;
use fyn_configuration::{BuildOptions, Reinstall};
use fyn_distribution::{
...
impl<'a> Planner<'a> {
    ...
    pub fn build(
                    ...
                    [installed] => {
                        ...
                        match RequirementSatisfaction::check(
                            ...
                            RequirementSatisfaction::OutOfDate => {
                                ...
                                // should. This is a known issue, but should only affect the `fyn pip` CLI, as the
        ...
        if site_packages.any() {
            // Retain seed packages unless: (1) the virtual environment was created by fyn and
            // (2) the `--seed` argument was not passed to `fyn venv`.
