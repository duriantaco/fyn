...
pub enum NoSources {
    /// Use `tool.fyn.sources` when resolving dependencies.
    #[default]
    None,

    /// Ignore `tool.fyn.sources` when resolving dependencies for all packages.
    All,

    /// Ignore `tool.fyn.sources` when resolving dependencies for specific packages.
