...
use fyn_globfilter::PortableGlobError;
use fyn_normalize::PackageName;
use fyn_pypi_types::{Identifier, IdentifierParseError};
...
fn prune_redundant_modules_warn(names: &[String], show_warnings: bool) -> Vec<String> {
    ...
    if show_warnings && names.len() != pruned.len() {
        ...
        warn_user_once!(
            "Ignoring redundant module name{s} in `tool.fyn.build-backend.module-name`: `{}`",
...
/// `tool.fyn.build-backend.module-name`.
///
...
fn find_roots(
    ...
    let relative_module_root = fyn_fs::normalize_path(relative_module_root);
    // Check that even if a path contains `..`, we only include files below the module root.
    if !fyn_fs::normalize_path(&source_tree.join(&relative_module_root))
        .starts_with(fyn_fs::normalize_path(source_tree))
    {
...
mod tests {
    ...
    use fs_err::File;
    use fyn_distribution_filename::{SourceDistFilename, WheelFilename};
    use fyn_fs::{copy_dir_all, relative_to};
    ...
    fn build(
        ...
        let (_name, direct_wheel_list_files) = {
            let _preview = fyn_preview::test::with_features(preview_features);
    ...
    fn built_by_fyn_building() {
        ...
        // This is a caveat to the determinism of the fyn build backend: When a file has the
    ...
    fn license_file_pre_pep639() {
        ...
        {
            let _preview = fyn_preview::test::with_features(&[]);
            build_wheel(
