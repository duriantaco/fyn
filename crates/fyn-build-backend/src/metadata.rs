...
use fyn_globfilter::{GlobDirFilter, PortableGlobParser};
use fyn_normalize::{ExtraName, PackageName};
...
use fyn_pypi_types::{Keywords, Metadata23, ProjectUrls, VerbatimParsedUrl};
...
/// No breaking changes were introduced to the fyn build backend since these releases, so we can use
...
/// The project is not compatible with a direct fyn build.
///
...
/// Check if the build backend is matching the currently running fyn version.
///
/// Example table compatible with fyn 0.4.21:
///
/// ```toml
/// [build-system]
...
pub fn check_direct_build(
    ...
    if pyproject_toml.build_system.build_backend.as_deref() != Some("fyn_build") {
...
impl PyProjectToml {
    ...
    pub(crate) fn parse(path: &Path) -> Result<Self, Error> {
        ...
        let pyproject_toml =
            tracing::info_span!("toml::from_str fyn build backend", path = %path.display())
    ...
    pub(crate) fn settings(&self) -> Option<&BuildBackendSettings> {
        self.tool.as_ref()?.fyn.as_ref()?.build_backend.as_ref()
    }

    /// See [`BuildSystem::check_build_system`].
    pub fn check_build_system(&self, fyn_version: &str) -> Vec<String> {
        self.build_system.check_build_system(fyn_version)
    }
...
/// The `tool.fyn` section with build configuration.
#[derive(Deserialize, Debug, Clone)]
#[serde(rename_all = "kebab-case")]
pub(crate) struct Toolfyn {
    /// Configuration for building source distributions and wheels with the fyn build backend
...
impl BuildSystem {
    /// Check if the `[build-system]` table matches the fyn build backend expectations and return
    ...
    pub(crate) fn check_build_system(&self, fyn_version: &str) -> Vec<String> {
        ...
        if self.build_backend.as_deref() != Some("fyn_build") {
            warnings.push(format!(
                r#"`build_system.build-backend` was expected to be `"fyn_build"`, not `"{}"`"#,
                self.build_backend.clone().unwrap_or_default()
            ));
        }

        let fyn_version =
            Version::from_str(fyn_version).expect("uv's own version is not PEP 440 compliant");
        let next_minor = fyn_version.release().get(1).copied().unwrap_or_default() + 1;
        ...
        let bounded = match &uv_requirement.version_or_url {
            ...
            Some(VersionOrUrl::VersionSpecifier(specifier)) => {
                ...
                // is very important to allow making breaking changes in fyn without breaking
                // the existing immutable source distributions on pypi.
                if !specifier.contains(&fyn_version) {
                    ...
                    warnings.push(format!(
                        ...
                        current fyn version {fyn_version}"#,
                    ));
                }
...
mod tests {
    ...
    fn build_system_not_uv() {
        ...
        assert_snapshot!(
            pyproject_toml.check_build_system("0.4.15+test").join("\n"),
            @r#"`build_system.build-backend` was expected to be `"fyn_build"`, not `"setuptools"`"#
        );
    }
    ...
    fn check_direct_build_wrong_backend() {
        ...
        assert_snapshot!(
            check_direct_build(temp_dir.path(), "0.10.0").unwrap_err(),
            @"`build_system.build-backend` is not `fyn_build`, but `setuptools`"
        );
    }
