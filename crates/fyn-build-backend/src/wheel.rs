...
use fyn_platform_tags::{AbiTag, LanguageTag, PlatformTag};
...
pub fn build_wheel(
    ...
    for warning in pyproject_toml.check_build_system(fyn_version) {
    ...
    let temp_file = fyn_fs::tempfile_in(wheel_dir)?;
    ...
    for exclude in excludes {
        ...
        let glob = PortableGlobParser::Uv
            ...
            .map_err(|err| Error::PortableGlob {
                field: "tool.fyn.build-backend.*-exclude".to_string(),
                source: err,
            })?;
...
fn write_dist_info(
    ...
    let wheel_info = WheelInfo::new(filename, fyn_version);
    writer.write_bytes(
        &format!("{dist_info_dir}/WHEEL"),
        wheel_info.to_string().as_bytes(),
    )?;
    if fyn_preview::is_enabled(PreviewFeature::MetadataJson) {
        writer.write_bytes(
...
impl WheelInfo {
    fn new(filename: &WheelFilename, fyn_version: &str) -> Self {
        ...
        Self {
            ...
            generator: format!("fyn {fyn_version}"),
...
mod test {
    ...
    use fyn_platform_tags::{AbiTag, PlatformTag};
    ...
    fn test_prepare_metadata() {
        ...
        Name: built-by-uv
        Version: 0.1.0
        Summary: A package to be built with the fyn build backend that uses all features exposed by the build backend
