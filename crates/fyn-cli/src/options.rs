...
use fyn_cache::Refresh;
use fyn_configuration::{BuildIsolation, Reinstall, Upgrade};
use fyn_distribution_types::{ConfigSettings, PackageConfigSettings, Requirement};
use fyn_resolver::{ExcludeNewer, ExcludeNewerPackage, PrereleaseMode};
use fyn_settings::{Combine, EnvFlag, PipOptions, ResolverInstallerOptions, ResolverOptions};
...
impl From<InstallerArgs> for PipOptions {
    fn from(args: InstallerArgs) -> Self {
        ...
        Self {
            ...
            exclude_newer_package: exclude_newer_package.map(ExcludeNewerPackage::from_iter),
            link_mode,
            dependency_guard_provider: Some(dependency_guard_provider),
            dependency_guard_command,
            dependency_guard_socket_min_score,
