...
use fyn_normalize::{ExtraName, GroupName, PackageName, PipGroupName};
use fyn_pep508::{MarkerTree, Requirement};
...
pub struct TopLevelArgs {
    ...
    ///
    /// While fyn configuration can be included in a `pyproject.toml` file, it is
...
pub struct GlobalArgs {
    ...
    ///
    /// By default, fyn prefers using Python versions it manages. However, it will use system Python
    /// versions if a fyn-managed Python is not installed. This option disables use of system Python
    /// versions.
    #[arg(
        global = true,
        long,
    ...
    /// fyn will write no output to stdout.
    ...
    ///
    /// By default, fyn loads certificates from the bundled `webpki-roots` crate. The
    /// `webpki-roots` are a reliable set of trust roots from Mozilla, and including them in fyn
    ...
    ///
    /// The following features are available: `python-upgrade`, `json-output`, `pylock`,
    /// `add-bounds`.
    #[arg(
        global = true,
    ...
    /// Avoid discovering a `pyproject.toml` or `fyn.toml` file [env: UV_ISOLATED=]
    ///
    /// Normally, configuration files are discovered in the current directory,
    /// parent directories, or user configuration directories.
    ///
    ...
    /// Skip writing `fyn` installer metadata files (e.g., `INSTALLER`, `REQUESTED`, and
...
pub enum Commands {
    ...
    #[command(
        after_help = "Use `fyn help auth` for more details.",
        after_long_help = ""
    )]
    Auth(AuthNamespace),
    ...
    #[command(
        after_help = "Use `fyn help tool` for more details.",
        after_long_help = ""
    )]
    Tool(ToolNamespace),
    ...
    ///
    /// fyn supports discovering CPython, PyPy, and GraalPy interpreters. Unsupported interpreters
    ...
    #[command(
        after_help = "Use `fyn help python` for more details.",
        after_long_help = ""
    )]
    ...
    ///
    /// When using fyn, the virtual environment does not need to be activated. fyn
    /// will find a virtual environment (named `.venv`) in the working directory
    /// or any parent directories.
    #[command(
        alias = "virtualenv",
        alias = "v",
        after_help = "Use `fyn help venv` for more details.",
        after_long_help = ""
    )]
    Venv(VenvArgs),
    ...
    #[command(
        after_help = "Use `fyn help shell` for more details.",
        after_long_help = ""
    )]
    Shell(ShellArgs),
    /// Build Python packages into source distributions and wheels.
    ///
    /// `fyn build` accepts a path to a directory or source distribution,
    /// which defaults to the current working directory.
    ///
    /// By default, if passed a directory, `fyn build` will build a source
    /// distribution ("sdist") from the source directory, and a binary
    /// distribution ("wheel") from the source distribution.
    ///
    /// `fyn build --sdist` can be used to build only the source distribution,
    /// `fyn build --wheel` can be used to build only the binary distribution,
    /// and `fyn build --sdist --wheel` can be used to build both distributions
    /// from source.
    ///
    /// If passed a source distribution, `fyn build --wheel` will build a wheel
    /// from the source distribution.
    #[command(
        after_help = "Use `fyn help build` for more details.",
        after_long_help = ""
    )]
    Build(BuildArgs),
    /// Upload distributions to an index.
    Publish(PublishArgs),
    /// Inspect fyn workspaces.
    #[command(
        after_help = "Use `fyn help workspace` for more details.",
        after_long_help = "",
        hide = true
    )]
    ...
    /// frontend (PEP 517) which calls the Python shims which calls back into fyn with this method.
    #[command(hide = true)]
    BuildBackend {
        #[command(subcommand)]
    ...
    /// Manage fyn's cache.
    #[command(
        after_help = "Use `fyn help cache` for more details.",
        after_long_help = ""
    )]
...
pub enum CacheCommand {
    ...
    /// file system as the Python environment fyn is operating on.
    Dir,
    /// Show the cache size.
    ///
...
pub struct CleanArgs {
    ...
    ///
    /// By default, `fyn cache clean` will block until no process is reading the cache. When
    /// `--force` is used, `fyn cache clean` will proceed without taking a lock.
    #[arg(long)]
    pub force: bool,
}
...
pub struct PruneArgs {
    ...
    /// Actions.
    ///
    /// By default, fyn caches both the wheels that it builds from source and the pre-built wheels
    ...
    /// modules.
    ///
    /// In `--ci` mode, fyn will prune any pre-built wheels from the cache, but retain any wheels
    ...
    /// Force removal of the cache, ignoring in-use checks.
    ///
    /// By default, `fyn cache prune` will block until no process is reading the cache. When
    /// `--force` is used, `fyn cache prune` will proceed without taking a lock.
    #[arg(long)]
    pub force: bool,
}
...
pub enum PipCommand {
    ...
    #[command(
        after_help = "Use `fyn help pip compile` for more details.",
        after_long_help = ""
    )]
    ...
    /// `pylock.toml` file will be removed. To retain extraneous packages, use `fyn pip install`
    /// instead.
    ///
    /// The input file is presumed to be the output of a `pip compile` or `fyn export` operation,
    ...
    #[command(
        after_help = "Use `fyn help pip sync` for more details.",
        after_long_help = ""
    )]
    Sync(Box<PipSyncArgs>),
    /// Install packages into an environment.
    #[command(
        after_help = "Use `fyn help pip install` for more details.",
        after_long_help = ""
    )]
    ...
    Upgrade(PipUpgradeArgs),
    /// Uninstall packages from an environment.
    #[command(
        after_help = "Use `fyn help pip uninstall` for more details.",
        after_long_help = ""
    )]
    Uninstall(PipUninstallArgs),
    /// List, in requirements format, packages installed in an environment.
    #[command(
        after_help = "Use `fyn help pip freeze` for more details.",
        after_long_help = ""
    )]
    Freeze(PipFreezeArgs),
    /// List, in tabular format, packages installed in an environment.
    #[command(
        after_help = "Use `fyn help pip list` for more details.",
        after_long_help = "",
        alias = "ls"
    )]
    List(PipListArgs),
    ...
    #[command(
        after_help = "Use `fyn help pip show` for more details.",
        after_long_help = ""
    )]
    Show(PipShowArgs),
    /// Display the dependency tree for an environment.
    #[command(
        after_help = "Use `fyn help pip tree` for more details.",
        after_long_help = ""
    )]
    Tree(PipTreeArgs),
    /// Verify installed packages have compatible dependencies.
    #[command(
        after_help = "Use `fyn help pip check` for more details.",
        after_long_help = ""
    )]
    Check(PipCheckArgs),
...
pub enum ProjectCommand {
    ...
    /// script and run with a Python interpreter, i.e., `fyn run file.py` is equivalent to `fyn run
    ...
    ///
    /// Arguments following the command (or script) are not interpreted as arguments to fyn. All
    /// options to fyn must be provided before the command, e.g., `fyn run --verbose foo`. A `--` can
    /// be used to separate the command from fyn options for clarity, e.g., `fyn run --python 3.12 --
    /// python`.
    #[command(
        after_help = "Use `fyn help run` for more details.",
        after_long_help = ""
    )]
    Run(RunArgs),
    ...
    /// (`.venv`) and lockfile (`fyn.lock`) are lazily created during the first sync.
    Init(InitArgs),
    ...
    /// `--no-sync`.
    ///
    /// If any of the requested dependencies cannot be found, fyn will exit with an error, unless the
    /// `--frozen` flag is provided, in which case fyn will add the dependencies verbatim without
    /// checking that they exist or are compatible with the project.
    ///
    /// fyn will search for a project in the current directory or any parent directory. If a project
    /// cannot be found, fyn will exit with an error.
    #[command(
        after_help = "Use `fyn help add` for more details.",
        after_long_help = ""
    )]
    Add(AddArgs),
    ...
    /// `--no-sync`.
    ///
    /// If any of the requested dependencies are not present in the project, fyn will exit with an
    /// error.
    ///
    /// If a package has been manually installed in the environment, i.e., with `fyn pip install`, it
    /// will not be removed by `fyn remove`.
    ///
    /// fyn will search for a project in the current directory or any parent directory. If a project
    /// cannot be found, fyn will exit with an error.
    #[command(
        after_help = "Use `fyn help remove` for more details.",
        after_long_help = ""
    )]
    Remove(RemoveArgs),
    ...
    /// lockfile.
    ///
    /// By default, an exact sync is performed: fyn removes packages that are not declared as
    ...
    /// Additionally, if `--no-build-isolation` is used, fyn will not remove extraneous packages to
    ...
    /// cannot be found, fyn will exit with an error.
    ///
    /// Note that, when installing from a lockfile, fyn will not provide warnings for yanked package
    /// versions.
    #[command(
        after_help = "Use `fyn help sync` for more details.",
        after_long_help = ""
    )]
    Sync(SyncArgs),
    /// Update the project's lockfile.
    ///
    /// If the project lockfile (`fyn.lock`) does not exist, it will be created. If a lockfile is
    ...
    #[command(
        after_help = "Use `fyn help lock` for more details.",
        after_long_help = ""
    )]
    Lock(LockArgs),
    ...
    #[command(
        after_help = "Use `fyn help upgrade` for more details.",
        after_long_help = ""
    )]
    Upgrade(UpgradeArgs),
    ...
    #[command(
        after_help = "Use `fyn help export` for more details.",
        after_long_help = ""
    )]
    Export(ExportArgs),
    ...
    #[command(
        after_help = "Use `fyn help format` for more details.",
        after_long_help = ""
    )]
    Format(FormatArgs),
    ...
    #[command(
        after_help = "Use `fyn help audit` for more details.",
        after_long_help = ""
    )]
    Audit(AuditArgs),
}
...
pub struct PipCompileArgs {
    ...
    ///
    /// fyn will infer the output format from the file extension of the output file, if
    ...
    ///
    /// By default, fyn strips extras, as any packages pulled in by the extras are already included
    ...
    ///
    /// By default, fyn strips environment markers, as the resolution generated by `compile` is
    ...
    ///
    /// By default, fyn uses the virtual environment in the current working directory or any parent
    ...
    ///
    /// For example, when set to `cpu`, fyn will use the CPU-only PyTorch index; when set to `cu126`,
    /// fyn will use the PyTorch index for CUDA 12.6.
    ///
...
pub struct PipSyncArgs {
    ...
    /// `pylock.toml`, `pyproject.toml`, `setup.py`, and `setup.cfg`.
    ///
    /// If a `pyproject.toml`, `setup.py`, or `setup.cfg` file is provided, fyn will
    ...
    ///
    /// By default, fyn will verify any available hashes in the requirements file, but will not
    ...
    /// Install packages into the system Python environment.
    ///
    /// By default, fyn installs into the virtual environment in the current working directory or any
    /// parent directory. The `--system` option instructs fyn to instead use the first Python found
    /// in the system `PATH`.
    ///
    ...
    /// explicitly recommend against modifications by other package managers (like fyn or `pip`).
    #[arg(
        long,
    ...
    /// If a suitable Python interpreter cannot be found, fyn will install one.
    ...
    /// `ANDROID_API_LEVEL` to specify a different minimum version, e.g., `26`.
    ///
    /// WARNING: When specified, fyn will select wheels that are compatible with the _target_
...
pub struct PipInstallArgs {
    ...
    /// When enabled, fyn will update the environment to exactly match the requirements, removing
...
pub struct PipUpgradeArgs {
    ...
    ///
    /// By default, fyn upgrades packages in the virtual environment in the current working
    /// directory or any parent directory. The `--system` option instructs fyn to instead use the
    /// first Python found in the system `PATH`.
    ///
    /// WARNING: `--system` is intended for use in continuous integration (CI) environments and
    /// should be used with caution, as it can modify the system Python installation.
    #[arg(
    ...
    /// Upgrade packages in the specified directory, rather than in the virtual or system Python
    /// environment. The packages will be installed at the top-level of the directory.
    ///
    /// Unlike other upgrade operations, this command does not require discovery of an existing
    /// Python environment and only searches for a Python interpreter to use for package
    /// resolution. If a suitable Python interpreter cannot be found, fyn will install one.
    /// To disable this, add `--no-python-downloads`.
    #[arg(short = 't', long, conflicts_with = "prefix", value_hint = ValueHint::DirPath)]
    pub target: Option<PathBuf>,

    /// Upgrade packages in `lib`, `bin`, and other top-level folders under the specified
    /// directory, as if a virtual environment were present at that location.
    ///
    /// In general, prefer the use of `--python` to upgrade an alternate environment, as scripts
    /// and other artifacts installed via `--prefix` will reference the installing interpreter,
    /// rather than any interpreter added to the `--prefix` directory, rendering them non-portable.
    ///
    /// Unlike other upgrade operations, this command does not require discovery of an existing
    /// Python environment and only searches for a Python interpreter to use for package
    /// resolution. If a suitable Python interpreter cannot be found, fyn will install one.
    ...
    /// Validate the Python environment after completing the upgrade, to detect packages with
    /// missing dependencies or other issues.
    #[arg(long, overrides_with("no_strict"))]
    pub strict: bool,
    ...
    #[arg(hide = true, long, short = 'P', help_heading = "Resolver options", value_hint = ValueHint::Other)]
    ...
    ///
    /// When enabled, fyn will process the entire site-packages directory (including packages that
...
pub struct PipUninstallArgs {
    ...
    ///
    /// By default, fyn uninstalls from the virtual environment in the current working directory or
    /// any parent directory. The `--system` option instructs fyn to instead use the first Python
...
pub struct BuildArgs {
    ...
    /// When using the fyn build backend, list the files that would be included when building.
    ///
    ...
    /// works with the fyn build backend, there is no PEP 517 file list build hook.
    ///
    ...
    ///
    /// By default, fyn won't create a PEP 517 build environment for packages using the fyn build
    ...
    ///
    /// By default, fyn creates a `.gitignore` file in the output directory to exclude build
...
pub struct VenvArgs {
    ...
    ///
    /// fyn will never search for interpreters in virtual environments when creating a virtual
    /// environment.
    #[arg(long, overrides_with("system"), hide = true)]
    pub no_system: bool,
    ...
    ///
    /// By default, fyn searches for projects in the current directory or any parent directory to
    ...
    ///
    /// By default, `fyn venv` will exit with an error if the given path is non-empty. The
    ...
    ///
    /// By default, when a TTY is available, `fyn venv` will prompt to clear a non-empty directory.
    ...
    /// The strategy to use when resolving against multiple index URLs.
    ///
    /// By default, fyn will stop at the first index on which a given package is available, and
...
pub struct ShellArgs {
    ...
    ///
    /// If not provided, fyn will look for a `.venv` directory in the current
...
pub struct InitArgs {
    ...
    ///
    /// By default, fyn will initialize a Git repository (`git`). Use `--vcs none` to explicitly
    ...
    ///
    /// By default, fyn will attempt to infer the author information from some sources (e.g., Git)
    ...
    ///
    /// By default, fyn will create a `.python-version` file containing the minor version of the
    /// discovered Python interpreter, which will cause subsequent fyn commands to use that version.
    #[arg(long)]
...
pub struct RunArgs {
    ...
    /// needs to be updated, fyn will exit with an error.
    #[arg(long, conflicts_with_all = ["frozen", "upgrade"])]
    pub locked: bool,
    ...
    /// source of truth. If the lockfile is missing, fyn will exit with an error. If the
    ...
    /// Number of times that `fyn run` will allow recursive invocations.
    ///
...
pub struct SyncArgs {
    ...
    /// May be provided more than once.
    ///
    /// When multiple extras or groups are specified that appear in `tool.fyn.conflicts`, fyn will
    /// report an error.
    ///
    ...
    /// Include all optional dependencies.
    ///
    /// When two or more extras are declared as conflicting in `tool.fyn.conflicts`, using this flag
    ...
    /// `tool.fyn.conflicts`, fyn will report an error.
    ///
    /// May be provided multiple times.
    #[arg(long, conflicts_with_all = ["only_group", "only_dev"], value_hint = ValueHint::Other)]
    pub group: Vec<GroupName>,
    ...
    /// Do not remove extraneous packages present in the environment.
    ///
    /// When enabled, fyn will make the minimum necessary changes to satisfy the requirements.
    ...
    ///
    /// In dry-run mode, fyn will resolve the project's dependencies and report on the resulting
    ...
    ///
    /// If provided, fyn will sync the dependencies based on the script's inline metadata table, in
...
pub struct LockArgs {
    ...
    /// missing or needs to be updated, fyn will exit with an error.
    ///
...
pub struct AddArgs {
    ...
    ///
    /// By default, fyn will use the `tool.fyn.sources` section to record source information for Git,
    /// local, editable, and direct URL requirements. When `--raw` is provided, fyn will add source
    /// requirements to `project.dependencies`, rather than `tool.fyn.sources`.
    ///
    /// Additionally, by default, fyn will add bounds to your dependency, e.g., `foo>=1.0.0`. When
    /// `--raw` is provided, fyn will add the dependency without bounds.
    #[arg(
        long,
    ...
    ///
    /// If provided, fyn will add the dependency to the script's inline metadata table, in adherence
    /// with PEP 723. If no such inline metadata table is present, a new one will be created and
    /// added to the script. When executed via `fyn run`, fyn will create a temporary environment for
    ...
    ///
    /// By default, fyn will add path dependencies that are within the workspace directory
    ...
    /// directory, fyn will add it as a workspace member; pass `--no-workspace` to add the package
...
pub struct RemoveArgs {
    ...
    ///
    /// If provided, fyn will remove the dependency from the script's inline metadata table, in
    /// adherence with PEP 723.
    #[arg(long, value_hint = ValueHint::FilePath)]
    pub script: Option<PathBuf>,
...
pub struct FormatArgs {
    ...
    ///
    /// For example, use `fyn format -- --line-length 100` to set the line length or
    /// `fyn format -- src/module/foo.py` to format a specific file.
...
pub struct AuditArgs {
    ...
    /// project.
    ///
    /// The specified script must be locked, i.e. with `fyn lock --script <script>`
...
pub enum ToolCommand {
    ...
    /// e.g., `fyn tool run ruff@0.3.0`. If more complex version specification is desired or if the
    /// command is provided by a different package, use `--from`.
    ///
    /// `fynx` can be used to invoke Python, e.g., with `fynx python` or `fynx python@<version>`. A
    /// Python interpreter will be started in an isolated virtual environment.
    ///
    /// If the tool was previously installed, i.e., via `fyn tool install`, the installed version
    ...
    #[command(
        after_help = "Use `fynx` as a shortcut for `fyn tool run`.\n\n\
        Use `fyn help tool run` for more details.",
        after_long_help = ""
    )]
    Run(ToolRunArgs),
    /// Hidden alias for `fyn tool run` for the `fynx` command
    #[command(
        hide = true,
        override_usage = "fynx [OPTIONS] [COMMAND]",
    ...
    ///
    /// Packages are installed into an isolated virtual environment in the fyn tools directory. The
    /// executables are linked the tool executable directory, which is determined according to the
    /// XDG standard and can be retrieved with `fyn tool dir --bin`.
    ///
    ...
    /// upgrade a tool beyond the originally provided constraints, use `fyn tool install` again.
    ///
    ...
    ///
    /// If the tool executable directory is not present on the `PATH`, fyn will attempt to add it to
    ...
    /// the path, but the directory is not present on the `PATH`, fyn will exit with an error.
    ///
    /// The tool executable directory is determined according to the XDG standard and can be
    /// retrieved with `fyn tool dir --bin`.
...
pub struct ToolRunArgs {
    ...
    /// Run with the given packages installed in editable mode
    ///
    /// When used in a project, these dependencies will be layered on top of the fyn tool's
...
pub struct ToolInstallArgs {
    ...
    /// The package to install commands from.
    ///
    /// This option is provided for parity with `fyn tool run`, but is redundant with `package`.
...
pub enum PythonCommand {
    ...
    /// `fyn python dir --bin` to see the target directory.
    ///
    ...
    ///
    /// When an upgrade is performed, virtual environments created by fyn will automatically
    ...
    ///
    /// Writes the pinned Python version to a `.python-version` file, which is used by other fyn
    /// commands to determine the required Python version.
    ///
    /// If no version is provided, fyn will look for an existing `.python-version` file and display
    /// the currently pinned version. If no `.python-version` file is found, fyn will exit with an
    /// error.
    ///
    /// See `fyn help python` to view supported request formats.
    Pin(PythonPinArgs),
    ...
    ///
    /// To view the directory where fyn installs Python executables instead, use the `--bin` flag.
...
pub struct PythonInstallCompileBytecodeArgs {
    ...
    /// faster start times.
    ///
    /// When enabled, fyn will process the Python version's `stdlib` directory. It will ignore any
    /// compilation errors.
    #[arg(
        long,
        alias = "compile",
...
pub struct PythonInstallArgs {
    ...
    ///
    /// If provided, `UV_PYTHON_INSTALL_DIR` will need to be set for subsequent operations for fyn to
    /// discover the Python installation.
    ///
    /// See `fyn python dir` to view the current Python installation directory. Defaults to
    /// `~/.local/share/fyn/python`.
    ...
    ///
    /// This is the default behavior. If this flag is provided explicitly, fyn will error if the
    ...
    ///
    /// This is the default behavior on Windows. If this flag is provided explicitly, fyn will error if the
    ...
    /// above are present, fyn will check if it has installed any Python versions. If not, it will
    ...
    ///
    /// By default, fyn will exit successfully if the version is already
    /// installed.
    #[arg(long, short)]
    pub reinstall: bool,
    ...
    ///
    /// This option is only supported for minor version requests, e.g., `3.12`; fyn will exit with an
...
pub struct PythonUpgradeArgs {
    ...
    ///
    /// By default, fyn will exit successfully if the latest patch is already
    /// installed.
    #[arg(long, short)]
    pub reinstall: bool,
...
pub struct PythonFindArgs {
    ...
    ///
    /// By default, fyn will report the first Python interpreter it would use, including those in an
    /// active virtual environment or a virtual environment in the current working directory or any
    /// parent directory.
    ///
    /// The `--system` option instructs fyn to skip virtual environment Python interpreters and
...
pub struct PythonPinArgs {
    ...
    ///
    /// fyn supports more formats than other tools that read `.python-version` files, i.e., `pyenv`.
...
pub struct AuthLogoutArgs {
    ...
    /// via an integration built into fyn.
    #[arg(
        long,
        value_enum,
        env = EnvVars::UV_KEYRING_PROVIDER,
...
/// Credential helper protocols supported by fyn
...
pub struct PublishArgs {
    ...
    ///
    /// By default, fyn attempts to upload matching PEP 740 attestations with each distribution
    /// that is published.
...
pub enum WorkspaceCommand {
    ...
    ///
    /// If used outside of a workspace, i.e., if a `pyproject.toml` cannot be found, fyn will exit with an error.
