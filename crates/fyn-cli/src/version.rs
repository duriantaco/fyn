...
use fyn_pep508::fyn_pep440::Version;

/// Information about the git repository where fyn was built from.
#[derive(Serialize)]
...
pub struct SelfVersionInfo {
    ...
    /// The target triple for which fyn was built (e.g., `x86_64-unknown-linux-gnu`).
    target_triple: String,
}

/// Version information for a project (`fyn version`).
#[derive(Serialize)]
pub struct ProjectVersionInfo {
    ...
    /// Version, such as "0.5.1".
    version: String,
    /// Information about the git commit fyn was built from.
    ///
...
pub fn fyn_self_version() -> SelfVersionInfo {
    ...
    let version = fyn_version::version().to_string();
