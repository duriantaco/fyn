...
pub enum PythonSource {
    ...
    MicrosoftStore,
    /// The Python installation was found in the fyn managed Python directory
    Managed,
    /// The Python installation was found via the invoking interpreter i.e. via `python -m fyn ...`
    ParentInterpreter,
}
...
/// The following sources are supported:
///
/// - Managed Python installations (e.g. `fyn python install`)
...
fn python_executables_from_installed<'a>(
    ...
    match preference {
        PythonPreference::OnlyManaged => {
            ...
            // search path.
            if std::env::var(fyn_static::EnvVars::UV_INTERNAL__TEST_PYTHON_MANAGED).is_ok() {
            ...
            Self::Query(err, _, source) => match &**err {
                ...
                | InterpreterError::BrokenLink(BrokenLink { path, .. }) => {
                    ...
                    if matches!(source, PythonSource::ActiveEnvironment)
                        && fyn_fs::is_virtualenv_executable(path)
                    {
                        true
                    } else {
...
pub(crate) async fn find_best_python_installation(
        ...
        .enumerate()
    {
            ...
            && let Some(download_request) = PythonDownloadRequest::from_request(request)
        {
            ...
            // Additionally, fyn did not previously attempt downloads in this
            ...
            if let Err(error) = result {
                ...
                fyn_warnings::write_error_chain(
                    error.as_ref(),
                    &mut error_chain,
                    "warning",
                    AnsiColors::Yellow,
...
/// Display a warning if the Python version of the [`Interpreter`] is unsupported by fyn.
fn warn_on_unsupported_python(interpreter: &Interpreter) {
    ...
    if interpreter.python_tuple() < (3, 8) {
        warn_user_once!(
            "fyn is only compatible with Python >=3.8, found Python {}",
...
impl PythonRequest {
    ...
    /// Try to parse a tool name as a Python version, e.g. `fynx python311`.
    ///
    /// The `PythonRequest::parse` constructor above is intended for the `--python` flag, where the
    /// value is unambiguously a Python version. This alternate constructor is intended for `fynx`
    /// or `fynx --from`, where the executable could be either a Python version or a package name.
...
impl PythonSource {
    ...
    /// This enables targeting the virtual environment with fyn by putting its `bin/` on the `PATH`
...
impl VersionRequest {
    ...
    /// Check if the request is for a version supported by fyn.
    ///
        ...
        match self {
            ...
            Self::Range(specifiers, variant) => {
                ...
                let version = if specifiers
                    .iter()
                    .any(fyn_pep440::VersionSpecifier::any_prerelease)
                {
...
fn parse_version_specifiers_request(
    ...
    if let [specifier] = specifiers.iter().as_slice() {
        if specifier.operator() == &fyn_pep440::Operator::Equal {
...
mod tests {
    ...
    use fyn_distribution_types::RequiresPython;
    use fyn_pep440::{Prerelease, PrereleaseKind, Version, VersionSpecifiers};
