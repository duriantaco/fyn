...
use fyn_cache::{Cache, CacheBucket, CachedByTimestamp, Freshness};
...
use fyn_pep508::{MarkerEnvironment, StringVersion};
use fyn_platform::{Arch, Libc, Os};
use fyn_platform_tags::{Platform, Tags, TagsError, TagsOptions};
use fyn_pypi_types::{ResolverMarkerEnvironment, Scheme};

use crate::implementation::LenientImplementationName;
use crate::managed::ManagedPythonInstallations;
use crate::pointer_size::PointerSize;
use crate::{
    Prefix, PythonInstallationKey, PythonVariant, PythonVersion, PyvenvConfiguration, Target,
...
impl Interpreter {
    ...
    pub fn key(&self) -> PythonInstallationKey {
        PythonInstallationKey::new(
            ...
            fyn_platform::Platform::new(self.os(), self.arch(), self.libc()),
            self.variant(),
        )
    }
    ...
    /// Returns `true` if this interpreter is managed by fyn.
    ///
    /// Returns `false` if we cannot determine the path of the fyn managed Python interpreters.
    pub fn is_managed(&self) -> bool {
        if let Ok(test_managed) =
            std::env::var(fyn_static::EnvVars::UV_INTERNAL__TEST_PYTHON_MANAGED)
        {
    ...
    pub async fn lock(&self) -> Result<LockedFile, LockedFileError> {
        if let Some(target) = self.target() {
            ...
            LockedFile::acquire(
                env::temp_dir().join(format!("fyn-{}.lock", cache_digest(&self.sys_executable))),
                LockedFileMode::Exclusive,
                self.sys_prefix.user_display(),
            )
            .await
...
pub fn canonicalize_executable(path: impl AsRef<Path>) -> std::io::Result<PathBuf> {
    ...
    #[cfg(windows)]
    {
        if let Ok(Some(launcher)) = fyn_trampoline_builder::Launcher::try_from_path(path) {
...
impl InterpreterInfo {
        ...
        if !output.status.success() {
            ...
            if python_home(interpreter).is_some_and(|home| !home.exists()) {
                return Err(Error::BrokenLink(BrokenLink {
                    path: interpreter.to_path_buf(),
                    unix: false,
                    venv: fyn_fs::is_virtualenv_executable(interpreter),
                }));
            }
    ...
    pub(crate) fn query_cached(executable: &Path, cache: &Cache) -> Result<Self, Error> {
            ...
            if err.kind() == io::ErrorKind::NotFound {
                    ...
                    .is_ok_and(|metadata| metadata.is_symlink())
                {
                    Error::BrokenLink(BrokenLink {
                        ...
                        unix: true,
                        venv: fyn_fs::is_virtualenv_executable(executable),
                    })
                } else {
        ...
        let cache_entry = cache.entry(
            ...
            cache_digest(&(
                ARCH,
                fyn_platform::host::OsType::from_env()
                    .map(|os_type| os_type.to_string())
                    .unwrap_or_default(),
                fyn_platform::host::OsRelease::from_env()
...
fn find_base_python(
    ...
    loop {
        ...
        let resolved = fyn_fs::normalize_absolute_path(&resolved)?;
...
mod tests {
    ...
    async fn test_cache_invalidation() {
        ...
        let json = indoc! {r##"
        {
            ...
            "sys_prefix": "/home/ferris/projects/fyn/.venv",
