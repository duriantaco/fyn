...
pub struct PyvenvConfiguration {
    ...
    /// Was the virtual environment created with the `fyn` package?
    pub(crate) uv: bool,
...
pub(crate) fn virtualenv_from_working_dir() -> Result<Option<PathBuf>, Error> {
    ...
    for dir in current_dir.ancestors() {
        ...
        if fyn_fs::is_virtualenv_base(dir) {
        ...
        if dot_venv.is_dir() {
            if !fyn_fs::is_virtualenv_base(&dot_venv) {
...
impl PyvenvConfiguration {
    ...
    /// Returns true if the virtual environment was created with the fyn package.
