...
use fyn_platform::{Error as PlatformError, Os};
use fyn_platform::{LibcDetectionError, Platform};
use fyn_state::{StateBucket, StateStore};
...
pub enum Error {
    ...
    #[error(transparent)]
    ExtractError(#[from] fyn_extract::Error),
    #[error(transparent)]
...
/// A collection of fyn-managed Python installations installed on the current system.
...
impl ManagedPythonInstallations {
    ...
    /// 3. A directory in the system-appropriate user-level data directory, e.g., `~/.local/fyn/python`.
    /// 4. A directory in the local data directory, e.g., `./.fyn/python`.
    ...
    pub fn init(self) -> Result<Self, Error> {
                ...
                .is_some_and(|parent| parent.join("toolchains").exists())
        {
            ...
            fyn_fs::replace_symlink(root, &deprecated)?;
        } else {
...
Error=This Python installation is managed by fyn and should not be modified.
";

/// A fyn-managed Python installation on the current system.
...
impl PythonMinorVersionLink {
    ...
    ///
    /// ## Unix
    /// For a Python 3.10.8 installation in `/path/to/fyn/python/cpython-3.10.8-macos-aarch64-none/bin/python3.10`,
    /// the symlink directory would be `/path/to/fyn/python/cpython-3.10-macos-aarch64-none` and the executable path including the
    /// symlink directory would be `/path/to/fyn/python/cpython-3.10-macos-aarch64-none/bin/python3.10`.
    ///
    /// ## Windows
    ...
    /// target, whether fyn created it as a junction or as a directory symlink under Wine.
    pub fn read_target(&self) -> Option<PathBuf> {
        ...
        #[cfg(windows)]
        {
            fyn_fs::read_link(&self.symlink_directory).ok()
        }
    }
}
...
pub fn create_link_to_executable(link: &Path, executable: &Path) -> Result<(), Error> {
    ...
    if cfg!(unix) {
        ...
        use fyn_trampoline_builder::windows_python_launcher;
...
pub fn replace_link_to_executable(link: &Path, executable: &Path) -> Result<(), Error> {
    ...
    if cfg!(unix) {
        ...
        fyn_fs::write_atomic_sync(link, &*launcher).map_err(Error::LinkExecutable)
    } else {
        unimplemented!("Only Windows and Unix are supported.")
    }
}
...
pub fn python_executable_dir() -> Result<PathBuf, Error> {
    fyn_dirs::user_executable_directory(Some(EnvVars::UV_PYTHON_BIN_DIR))
...
mod tests {
    ...
    use fyn_pep440::{Prerelease, PrereleaseKind};
    use fyn_platform::Platform;
    ...
    fn test_find_version_matching() {
        ...
        temp_env::with_var(
            fyn_static::EnvVars::UV_PYTHON_INSTALL_DIR,
            Some(temp_dir.path()),
            || {
    ...
    fn test_relative_install_dir_resolves_against_pwd() {
        ...
        temp_env::with_vars(
            [
                ...
                (fyn_static::EnvVars::PWD, Some(workdir.as_os_str())),
            ],
            || {
                let installations = ManagedPythonInstallations::from_settings(None).unwrap();
                assert_eq!(
