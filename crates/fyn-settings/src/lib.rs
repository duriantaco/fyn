...
impl FilesystemOptions {
    ...
    pub fn user() -> Result<Option<Self>, Error> {
        ...
        let root = dir.join("fyn");
        let file = root.join("fyn.toml");
    ...
    ///
    /// The search starts at the given path and goes up the directory tree until a `fyn.toml` file or
    ...
    /// Load a [`FilesystemOptions`] from a directory, preferring a `fyn.toml` file over a
    /// `pyproject.toml` file.
    pub fn from_directory(dir: &Path) -> Result<Option<Self>, Error> {
        // Read a `fyn.toml` file in the current directory.
        let path = dir.join("fyn.toml");
    ...
    /// Load a [`FilesystemOptions`] from a `fyn.toml` file.
...
/// Load [`Options`] from a `fyn.toml` file.
...
fn validate_uv_toml(path: &Path, options: &Options) -> Result<(), Error> {
    ...
    // The `fyn.toml` format is not allowed to include any of the following, which are
...
/// Validate that an [`Options`] contains no fields that `fyn.toml` would mask
///
    ...
    let Options {
        ...
        top_level:
            ResolverInstallerSchema {
                ...
                link_mode,
                dependency_guard_provider: _,
                dependency_guard_command: _,
                dependency_guard_socket_min_score: _,
                compile_bytecode,
                no_sources,
...
pub enum Error {
    ...
    #[error(transparent)]
    Index(#[from] fyn_distribution_types::IndexUrlError),
    ...
    pub fn new() -> Result<Self, Error> {
        ...
        Ok(Self {
            ...
            http_retries: parse_integer_environment_variable(EnvVars::UV_HTTP_RETRIES, None)?
                .unwrap_or(fyn_client::DEFAULT_RETRIES),
            #[cfg(feature = "tracing-durations-export")]
