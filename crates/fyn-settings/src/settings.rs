...
use fyn_macros::{CombineOptions, OptionsMetadata};
use fyn_normalize::{ExtraName, PackageName, PipGroupName};
use fyn_pep508::Requirement;
use fyn_pypi_types::{SupportedEnvironments, VerbatimParsedUrl};
...
pub struct Options {
    ...
    /// modified. By default, fyn will rebuild a project whenever the `pyproject.toml`, `setup.py`,
    ...
    /// Note that the use of globs can be expensive, as fyn may need to walk the filesystem to
    ...
    // NOTE(charlie): These fields are shared with `Toolfyn` in
    // `crates/fyn-workspace/src/pyproject.rs`. The documentation lives on that struct.
    // They're respected in both `pyproject.toml` and `fyn.toml` files.
    #[cfg_attr(feature = "schemars", schemars(skip))]
    pub override_dependencies: Option<Vec<Requirement<VerbatimParsedUrl>>>,

    #[cfg_attr(feature = "schemars", schemars(skip))]
    pub exclude_dependencies: Option<Vec<fyn_normalize::PackageName>>,
    ...
    // NOTE(charlie): These fields should be kept in-sync with `Toolfyn` in
    // `crates/fyn-workspace/src/pyproject.rs`. The documentation lives on that struct.
    // They're only respected in `pyproject.toml` files, and should be rejected in `fyn.toml` files.
    #[cfg_attr(feature = "schemars", schemars(skip))]
...
pub struct GlobalOptions {
    ...
    ///
    /// If the version of fyn does not meet the requirement at runtime, fyn will exit
    /// with an error.
    ///
    ...
    ///
    /// Defaults to `$XDG_CACHE_HOME/fyn` or `$HOME/.cache/fyn` on Linux and macOS, and
    ...
    /// those that are downloaded and installed by fyn.
    #[option(
        default = "\"managed\"",
        value_type = "str",
        example = r#"
    ...
    /// The maximum number of in-flight concurrent downloads that fyn will perform at any given
    /// time.
    #[option(
        default = "50",
        value_type = "int",
    ...
    /// The maximum number of source distributions that fyn will build concurrently at any given
    /// time.
    ///
...
/// The JSON schema for the `[tool.fyn]` section of a `pyproject.toml` file.
...
pub struct ResolverInstallerSchema {
    ...
    ///
    /// To control fyn's resolution strategy when multiple indexes are present, see
    ...
    ///
    /// At present, only `--keyring-provider subprocess` is supported, which configures fyn to
    ...
    ///
    /// By default, fyn will accept pre-releases for packages that _only_ publish pre-releases,
...
///
/// These values will be ignored when running commands outside the `fyn pip` namespace (e.g.,
/// `fyn lock`, `fynx`).
#[derive(Debug, Clone, Default, Deserialize, CombineOptions, OptionsMetadata)]
...
pub struct PipOptions {
    ...
    ///
    /// By default, fyn installs into the virtual environment in the current working directory or
    ...
    /// Allow `fyn pip sync` with empty requirements, which will clear the environment of all
    /// packages.
    #[option(
        default = "false",
        value_type = "bool",
