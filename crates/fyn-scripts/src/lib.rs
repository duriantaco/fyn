...
use fyn_settings::{GlobalOptions, ResolverInstallerSchema};
use fyn_warnings::warn_user;
use fyn_workspace::pyproject::{ExtraBuildDependency, Sources};
...
impl Pep723ItemRef<'_> {
    ...
    pub fn indexes(&self, source_strategy: &NoSources) -> &[fyn_distribution_types::Index] {
...
impl Pep723Script {
    ...
    pub async fn create(
        ...
        let script = if let Some(existing_contents) = existing_contents {
            ...
            if !shebang.is_empty() {
                shebang.push_str("\n#\n");
                // If the shebang doesn't contain `fyn`, it's probably something like
                ...
                // `fyn run` without `--script` when the file doesn't end in `.py`.
                if !regex::Regex::new(r"\bfyn\b").unwrap().is_match(&shebang) {
                    warn_user!(
                        "If you execute {} directly, it might ignore its inline metadata.\nConsider replacing its shebang with: {}",
                        file.to_string_lossy().cyan(),
                        "#!/usr/bin/env -S fyn run --script".cyan(),
                    );
                }
            }
            indoc::formatdoc! {r"
...
pub struct Pep723Metadata {
    pub dependencies: Option<Vec<fyn_pep508::Requirement<VerbatimParsedUrl>>>,
...
mod tests {
    ...
    fn script_init_empty() {
        ...
        let (prelude, metadata, postlude) =
            Pep723Script::init_metadata(contents, &fyn_pep440::VersionSpecifiers::default())
                .unwrap();
        assert_eq!(prelude, "");
        assert_eq!(
            metadata.raw,
    ...
    fn script_init_requires_python() {
        ...
        let (prelude, metadata, postlude) = Pep723Script::init_metadata(
            contents,
            &fyn_pep440::VersionSpecifiers::from_str(">=3.8").unwrap(),
        )
        .unwrap();
        assert_eq!(prelude, "");
        assert_eq!(
