//! Avoid cyclic crate dependencies between [resolver][`fv_resolver`],
//! [installer][`fv_installer`] and [build][`fv_build`] through [`BuildDispatch`]
...
use fv_build_frontend::{SourceBuild, SourceBuildContext};
use fv_cache::Cache;
use fv_client::RegistryClient;
use fv_configuration::{BuildKind, BuildOptions, Constraints, IndexStrategy, NoSources, Reinstall};
use fv_configuration::{BuildOutput, Concurrency};
use fv_distribution::DistributionDatabase;
use fv_distribution_filename::DistFilename;
...
use fv_git::GitResolver;
use fv_installer::{InstallationStrategy, Installer, Plan, Planner, Preparer, SitePackages};
use fv_preview::Preview;
use fv_pypi_types::Conflicts;
use fv_python::{Interpreter, PythonEnvironment};
use fv_resolver::{
...
pub enum BuildDispatchError {
    ...
    #[error(transparent)]
    Resolve(#[from] fv_resolver::ResolveError),
    ...
    #[error(transparent)]
    Prepare(#[from] fv_installer::PrepareError),
}
...
pub struct BuildDispatch<'a> {
    ...
    link_mode: fv_install_wheel::LinkMode,
...
impl BuildContext for BuildDispatch<'_> {
    ...
    async fn install<'data>(
        ...
        for dist in &remote {
            ...
            if build_stack.contains(&id) {
                return Err(BuildDispatchError::BuildFrontend(
                    fv_build_frontend::Error::CyclicBuildDependency(dist.name().clone()).into(),
                ));
            }
        }
        ...
        if !reinstalls.is_empty() {
            for dist_info in &reinstalls {
                let summary = fv_installer::uninstall(dist_info)
                    .await
    ...
    async fn setup_build<'data>(
    ...
    ) -> Result<SourceBuild, fv_build_frontend::Error> {
        let dist_name = dist.map(fv_distribution_types::Name::name);
        let dist_version = dist
            .map(fv_distribution_types::DistributionMetadata::version_or_url)
            ...
            && !matches!(build_kind, BuildKind::Editable)
        {
            let err = if let Some(dist) = dist {
                fv_build_frontend::Error::NoSourceDistBuild(dist.name().clone())
            } else {
                fv_build_frontend::Error::NoSourceDistBuilds
            };
            return Err(err);
        }
    ...
    async fn direct_build<'data>(
        ...
        // Only perform the direct build if the backend is fv in a compatible version.
        let source_tree_str = source_tree.display().to_string();
        let identifier = version_id.unwrap_or_else(|| &source_tree_str);
        if let Err(reason) = check_direct_build(&source_tree, fv_version::version()) {
        ...
        let filename = tokio::task::spawn_blocking(move || -> Result<_> {
            let filename = match build_kind {
                BuildKind::Wheel => {
                    let wheel = fv_build_backend::build_wheel(
                        &source_tree,
                        &output_dir,
                        None,
                        fv_version::version(),
                ...
                BuildKind::Sdist => {
                    let source_dist = fv_build_backend::build_source_dist(
                        &source_tree,
                        &output_dir,
                        fv_version::version(),
                        sources.is_none(),
                ...
                BuildKind::Editable => {
                    let wheel = fv_build_backend::build_editable(
                        &source_tree,
                        &output_dir,
                        None,
                        fv_version::version(),
