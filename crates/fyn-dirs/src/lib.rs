...
///
/// Corresponds to `$XDG_CACHE_HOME/fyn` on Unix.
pub fn user_cache_dir() -> Option<PathBuf> {
    etcetera::base_strategy::choose_base_strategy()
        .ok()
        .map(|dirs| dirs.cache_dir().join("fyn"))
}

/// Returns the legacy cache directory path.
///
/// Uses `/Users/user/Library/Application Support/fyn` on macOS, in contrast to the new preference
...
///
/// Corresponds to `$XDG_DATA_HOME/fyn` on Unix.
pub fn user_state_dir() -> Option<PathBuf> {
    etcetera::base_strategy::choose_base_strategy()
        .ok()
        .map(|dirs| dirs.data_dir().join("fyn"))
}
...
/// for use with fyn-specific override variables that are not subject to the XDG specification.
...
fn locate_system_config_xdg(value: Option<&str>) -> Option<PathBuf> {
    ...
    for dir in config_dirs.split(':').take_while(|s| !s.is_empty()) {
        let uv_toml_path = Path::new(dir).join("fyn").join("fyn.toml");
...
fn locate_system_config_windows(system_drive: impl AsRef<Path>) -> Option<PathBuf> {
    // On Windows, use `%SYSTEMDRIVE%\ProgramData\fyn\fyn.toml` (e.g., `C:\ProgramData`).
...
/// `/etc/xdg/fyn/fyn.toml` if unset or empty) and then `/etc/fyn/fyn.toml`
///
/// On Windows, uses `%SYSTEMDRIVE%\ProgramData\fyn\fyn.toml`.
pub fn system_config_file() -> Option<PathBuf> {
    ...
    #[cfg(not(windows))]
    {
        ...
        // Fallback to `/etc/fyn/fyn.toml` if `XDG_CONFIG_DIRS` is not set or no valid
        // path is found.
        let candidate = Path::new("/etc/fyn/fyn.toml");
...
mod test {
    ...
    fn test_locate_system_config_xdg() -> Result<(), FixtureError> {
        // Write a `fyn.toml` to a temporary directory.
        let context = assert_fs::TempDir::new()?;
        context.child("fyn").child("fyn.toml").write_str(indoc! {
            r#"
            [pip]
        ...
        assert_eq!(
            ...
            context.child("fyn").child("fyn.toml").path()
        );

        // Write a separate `fyn.toml` to a different directory.
        let first = context.child("first");
        let first_config = first.child("fyn").child("fyn.toml");
    ...
    fn test_locate_system_config_xdg_unix_permissions() -> Result<(), FixtureError> {
        let context = assert_fs::TempDir::new()?;
        let config = context.child("fyn").child("fyn.toml");
        config.write_str("")?;
