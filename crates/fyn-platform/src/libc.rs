...
impl From<&fyn_platform_tags::Os> for Libc {
    fn from(value: &fyn_platform_tags::Os) -> Self {
        match value {
            fyn_platform_tags::Os::Manylinux { .. } => Self::Some(target_lexicon::Environment::Gnu),
            fyn_platform_tags::Os::Musllinux { .. } => {
                Self::Some(target_lexicon::Environment::Musl)
            }
            fyn_platform_tags::Os::Pyodide { .. } => Self::Some(target_lexicon::Environment::Musl),
            _ => Self::None,
        }
    }
}
