...
use fv_install_wheel::LinkMode;
use fv_macros::{CombineOptions, OptionsMetadata};
use fv_normalize::{ExtraName, PackageName, PipGroupName};
use fv_pep508::Requirement;
use fv_pypi_types::{SupportedEnvironments, VerbatimParsedUrl};
...
use fv_workspace::pyproject::ExtraBuildDependencies;
use fv_workspace::pyproject_mut::AddBoundsKind;

/// A `pyproject.toml` with an (optional) `[tool.fv]` section.
#[allow(dead_code)]
...
pub struct Options {
    ...
    /// modified. By default, fv will rebuild a project whenever the `pyproject.toml`, `setup.py`,
    ...
    /// Note that the use of globs can be expensive, as fv may need to walk the filesystem to
    ...
    // NOTE(charlie): These fields are shared with `ToolFv` in
    // `crates/fv-workspace/src/pyproject.rs`. The documentation lives on that struct.
    // They're respected in both `pyproject.toml` and `fv.toml` files.
    #[cfg_attr(feature = "schemars", schemars(skip))]
    pub override_dependencies: Option<Vec<Requirement<VerbatimParsedUrl>>>,

    #[cfg_attr(feature = "schemars", schemars(skip))]
    pub exclude_dependencies: Option<Vec<fv_normalize::PackageName>>,
    ...
    // NOTE(charlie): These fields should be kept in-sync with `ToolFv` in
    // `crates/fv-workspace/src/pyproject.rs`. The documentation lives on that struct.
    // They're only respected in `pyproject.toml` files, and should be rejected in `fv.toml` files.
    #[cfg_attr(feature = "schemars", schemars(skip))]
    ...
    #[cfg_attr(feature = "schemars", schemars(skip))]
    pub tasks: Option<serde::de::IgnoredAny>,
...
pub struct GlobalOptions {
    /// Enforce a requirement on the version of fv.
    ///
    /// If the version of fv does not meet the requirement at runtime, fv will exit
    /// with an error.
    ///
    ...
    ///
    /// Defaults to `$XDG_CACHE_HOME/fv` or `$HOME/.cache/fv` on Linux and macOS, and
    ...
    /// those that are downloaded and installed by fv.
    #[option(
        default = "\"managed\"",
        value_type = "str",
        example = r#"
    ...
    /// The maximum number of in-flight concurrent downloads that fv will perform at any given
    /// time.
    #[option(
        default = "50",
        value_type = "int",
    ...
    /// The maximum number of source distributions that fv will build concurrently at any given
    /// time.
    ///
...
/// The JSON schema for the `[tool.fv]` section of a `pyproject.toml` file.
...
pub struct ResolverInstallerSchema {
    ...
    /// [`extra_index_url`](#extra-index-url). fv will only consider the first index that contains
    /// a given package, unless an alternative [index strategy](#index-strategy) is specified.
    ///
    /// If an index is marked as `explicit = true`, it will be used exclusively for those
    /// dependencies that select it explicitly via `[tool.fv.sources]`, as in:
    ///
    /// ```toml
    /// [[tool.fv.index]]
    /// name = "pytorch"
    ...
    ///
    /// To control fv's resolution strategy when multiple indexes are present, see
    ...
    ///
    /// At present, only `--keyring-provider subprocess` is supported, which configures fv to
    ...
    ///
    /// By default, fv will accept pre-releases for packages that _only_ publish pre-releases,
    ...
    pub no_sources: Option<bool>,
    /// Ignore `tool.fv.sources` for the specified packages.
    #[option(
        default = "[]",
        value_type = "list[str]",
        example = r#"
    ...
    ///
    /// This setting is only respected by `fv pip` commands.
    ///
...
pub struct PythonInstallMirrors {
    ...
    #[option(
        ...
        uv_toml_only = true,
        example = r#"
            python-downloads-json-url = "/etc/fv/python-downloads.json"
        "#
    )]
...
/// Settings that are specific to the `fv pip` command-line interface.
///
/// These values will be ignored when running commands outside the `fv pip` namespace (e.g.,
/// `fv lock`, `fvx`).
#[derive(Debug, Clone, Default, Deserialize, CombineOptions, OptionsMetadata)]
...
pub struct PipOptions {
    ...
    ///
    /// By default, fv installs into the virtual environment in the current working directory or
    ...
    /// Allow `fv pip sync` with empty requirements, which will clear the environment of all
    /// packages.
    #[option(
        default = "false",
        value_type = "bool",
    ...
    pub dependency_metadata: Option<Vec<StaticMetadata>>,
    /// Write the requirements generated by `fv pip compile` to the given `requirements.txt` file.
    ///
    ...
    /// Include environment markers in the output file generated by `fv pip compile`.
    ///
    ...
    /// Exclude the comment header at the top of output file generated by `fv pip compile`.
    #[option(
        default = r#"false"#,
        value_type = "bool",
        example = r#"
    ...
    #[option(
        ...
        value_type = "str",
        example = r#"
            custom-compile-command = "./custom-fv-compile.sh"
        "#
    )]
    ...
    /// Include `--index-url` and `--extra-index-url` entries in the output file generated by `fv pip compile`.
    #[option(
        default = "false",
        value_type = "bool",
        example = r#"
    ...
    /// Include `--find-links` entries in the output file generated by `fv pip compile`.
    #[option(
        default = "false",
        value_type = "bool",
        example = r#"
    ...
    /// Include `--no-binary` and `--only-binary` entries in the output file generated by `fv pip compile`.
    #[option(
        default = "false",
        value_type = "bool",
        example = r#"
...
pub struct OptionsWire {
    ...
    // Tasks (parsed in pyproject.rs, ignored here)
    tasks: Option<serde::de::IgnoredAny>,
...
pub struct PublishOptions {
    ...
    ///
    /// fv's supported environments for trusted publishing include GitHub Actions and GitLab CI/CD.
    #[option(
        default = "automatic",
        value_type = "str",
        example = r#"
