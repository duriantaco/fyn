use fv_client::{DEFAULT_CONNECT_TIMEOUT, DEFAULT_READ_TIMEOUT, DEFAULT_READ_TIMEOUT_UPLOAD};
use fv_dirs::{system_config_file, user_config_dir};
use fv_distribution_types::Origin;
use fv_flags::EnvironmentFlags;
use fv_fs::Simplified;
use fv_static::{EnvVars, InvalidEnvironmentVariable, parse_boolish_environment_variable};
...
impl FilesystemOptions {
    ...
    pub fn user() -> Result<Option<Self>, Error> {
        ...
        let root = dir.join("fv");
        let file = root.join("fv.toml");
    ...
    ///
    /// The search starts at the given path and goes up the directory tree until a `fv.toml` file or
    ...
    /// Load a [`FilesystemOptions`] from a directory, preferring a `fv.toml` file over a
    /// `pyproject.toml` file.
    pub fn from_directory(dir: &Path) -> Result<Option<Self>, Error> {
        // Read a `fv.toml` file in the current directory.
        let path = dir.join("fv.toml");
        match fs_err::read_to_string(&path) {
            Ok(content) => {
                let options =
                    info_span!("toml::from_str filesystem options fv.toml", path = %path.display())
                        .in_scope(|| toml::from_str::<Options>(&content))
                        .map_err(|err| Error::UvToml(path.clone(), Box::new(err)))?
                        .relative_to(&std::path::absolute(dir)?)?;

                // If the directory also contains a `[tool.fv]` table in a `pyproject.toml` file,
                // warn.
                ...
                if let Ok(content) = fs_err::read_to_string(&pyproject) {
                    ...
                    if let Some(options) =
                        result.and_then(|pyproject| pyproject.tool.and_then(|tool| tool.fv))
                    {
        ...
        match fs_err::read_to_string(&path) {
            Ok(content) => {
                // Parse, but skip any `pyproject.toml` that doesn't have a `[tool.fv]` section.
                let pyproject =
                ...
                let Some(options) = tool.fv else {
                    tracing::debug!(
                        "Skipping `pyproject.toml` in `{}` (no `[tool.fv]` section)",
                        dir.display()
                    );
                    return Ok(None);
                };
    ...
    /// Load a [`FilesystemOptions`] from a `fv.toml` file.
...
/// Load [`Options`] from a `fv.toml` file.
fn read_file(path: &Path) -> Result<Options, Error> {
    let content = fs_err::read_to_string(path)?;
    let options = info_span!("toml::from_str filesystem options fv.toml", path = %path.display())
        .in_scope(|| toml::from_str::<Options>(&content))
...
/// Validate that an [`Options`] schema is compatible with `fv.toml`.
fn validate_uv_toml(path: &Path, options: &Options) -> Result<(), Error> {
    ...
    // The `fv.toml` format is not allowed to include any of the following, which are
    // permitted by the schema since they _can_ be included in `pyproject.toml` files
    // (and we want to use `deny_unknown_fields`).
    if tasks.is_some() {
        return Err(Error::PyprojectOnlyField(path.to_path_buf(), "tasks"));
    }
...
/// Validate that an [`Options`] contains no fields that `fv.toml` would mask
///
...
fn warn_uv_toml_masked_fields(options: &Options) {
    ...
    if !masked_fields.is_empty() {
        ...
        warn_user!(
            "Found both a `fv.toml` file and a `[tool.fv]` section in an adjacent `pyproject.toml`. The following fields from `[tool.fv]` will be ignored in favor of the `fv.toml` file:\n- {}",
            field_listing,
        );
    }
}
...
pub enum Error {
    ...
    #[error(transparent)]
    Index(#[from] fv_distribution_types::IndexUrlError),
    ...
    #[error("Failed to parse: `{}`. The `{}` field is not allowed in a `fv.toml` file. `{}` is only applicable in the context of a project, and should be placed in a `pyproject.toml` file instead.", _0.user_display(), _1, _1
    )]
    ...
    pub fn new() -> Result<Self, Error> {
        ...
        Ok(Self {
            ...
            http_retries: parse_integer_environment_variable(EnvVars::UV_HTTP_RETRIES, None)?
                .unwrap_or(fv_client::DEFAULT_RETRIES),
            #[cfg(feature = "tracing-durations-export")]
