...
use fv_cache::{Cache, CacheBucket, CachedByTimestamp, Freshness};
...
use fv_install_wheel::Layout;
use fv_pep440::Version;
use fv_pep508::{MarkerEnvironment, StringVersion};
use fv_platform::{Arch, Libc, Os};
use fv_platform_tags::{Platform, Tags, TagsError};
use fv_pypi_types::{ResolverMarkerEnvironment, Scheme};
...
impl Interpreter {
    ...
    pub fn key(&self) -> PythonInstallationKey {
        PythonInstallationKey::new(
            ...
            fv_platform::Platform::new(self.os(), self.arch(), self.libc()),
            self.variant(),
        )
    }
    ...
    /// Returns `true` if this interpreter is managed by fv.
    ///
    /// Returns `false` if we cannot determine the path of the fv managed Python interpreters.
    pub fn is_managed(&self) -> bool {
        if let Ok(test_managed) =
            std::env::var(fv_static::EnvVars::UV_INTERNAL__TEST_PYTHON_MANAGED)
        {
    ...
    pub async fn lock(&self) -> Result<LockedFile, LockedFileError> {
        if let Some(target) = self.target() {
            ...
            LockedFile::acquire(
                env::temp_dir().join(format!("fv-{}.lock", cache_digest(&self.sys_executable))),
                LockedFileMode::Exclusive,
                self.sys_prefix.user_display(),
            )
            .await
...
pub fn canonicalize_executable(path: impl AsRef<Path>) -> std::io::Result<PathBuf> {
    ...
    #[cfg(windows)]
    {
        if let Ok(Some(launcher)) = fv_trampoline_builder::Launcher::try_from_path(path) {
...
impl InterpreterInfo {
        ...
        if !output.status.success() {
            ...
            if python_home(interpreter).is_some_and(|home| !home.exists()) {
                return Err(Error::BrokenLink(BrokenLink {
                    path: interpreter.to_path_buf(),
                    unix: false,
                    venv: fv_fs::is_virtualenv_executable(interpreter),
                }));
            }
    ...
    pub(crate) fn query_cached(executable: &Path, cache: &Cache) -> Result<Self, Error> {
            ...
            if err.kind() == io::ErrorKind::NotFound {
                    ...
                    .is_ok_and(|metadata| metadata.is_symlink())
                {
                    Error::BrokenLink(BrokenLink {
                        ...
                        unix: true,
                        venv: fv_fs::is_virtualenv_executable(executable),
                    })
                } else {
        ...
        let cache_entry = cache.entry(
            ...
            cache_digest(&(
                ARCH,
                fv_platform::host::OsType::from_env()
                    .map(|os_type| os_type.to_string())
                    .unwrap_or_default(),
                fv_platform::host::OsRelease::from_env()
...
fn find_base_python(
    ...
    loop {
        ...
        let resolved = fv_fs::normalize_absolute_path(&resolved)?;
...
mod tests {
    ...
    async fn test_cache_invalidation() {
        ...
        let json = indoc! {r##"
        {
            ...
            "sys_prefix": "/home/ferris/projects/fv/.venv",
