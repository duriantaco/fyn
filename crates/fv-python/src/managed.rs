...
use fv_platform::{Error as PlatformError, Os};
use fv_platform::{LibcDetectionError, Platform};
...
pub enum Error {
    ...
    #[error(transparent)]
    ExtractError(#[from] fv_extract::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    LauncherError(#[from] fv_trampoline_builder::Error),
...
/// A collection of fv-managed Python installations installed on the current system.
...
impl ManagedPythonInstallations {
    ...
    /// 3. A directory in the system-appropriate user-level data directory, e.g., `~/.local/fv/python`.
    /// 4. A directory in the local data directory, e.g., `./.fv/python`.
    ...
    pub fn init(self) -> Result<Self, Error> {
                ...
                .is_some_and(|parent| parent.join("toolchains").exists())
        {
            ...
            fv_fs::replace_symlink(root, &deprecated)?;
        } else {
...
Error=This Python installation is managed by fv and should not be modified.
";

/// A fv-managed Python installation on the current system.
...
impl PythonMinorVersionLink {
    ...
    ///
    /// ## Unix
    /// For a Python 3.10.8 installation in `/path/to/fv/python/cpython-3.10.8-macos-aarch64-none/bin/python3.10`,
    /// the symlink directory would be `/path/to/fv/python/cpython-3.10-macos-aarch64-none` and the executable path including the
    /// symlink directory would be `/path/to/fv/python/cpython-3.10-macos-aarch64-none/bin/python3.10`.
    ///
    /// ## Windows
...
pub fn create_link_to_executable(link: &Path, executable: &Path) -> Result<(), Error> {
    ...
    if cfg!(unix) {
        ...
        use fv_trampoline_builder::windows_python_launcher;
...
pub fn replace_link_to_executable(link: &Path, executable: &Path) -> Result<(), Error> {
    ...
    if cfg!(unix) {
        ...
        fv_fs::write_atomic_sync(link, &*launcher).map_err(Error::LinkExecutable)
    } else {
        unimplemented!("Only Windows and Unix are supported.")
    }
}
...
pub fn python_executable_dir() -> Result<PathBuf, Error> {
    fv_dirs::user_executable_directory(Some(EnvVars::UV_PYTHON_BIN_DIR))
...
mod tests {
    ...
    use fv_pep440::{Prerelease, PrereleaseKind};
    use fv_platform::Platform;
    ...
    fn test_find_version_matching() {
        ...
        temp_env::with_var(
            fv_static::EnvVars::UV_PYTHON_INSTALL_DIR,
            Some(temp_dir.path()),
            || {
    ...
    fn test_relative_install_dir_resolves_against_pwd() {
        ...
        temp_env::with_vars(
            [
                ...
                (fv_static::EnvVars::PWD, Some(workdir.as_os_str())),
            ],
            || {
                let installations = ManagedPythonInstallations::from_settings(None).unwrap();
                assert_eq!(
