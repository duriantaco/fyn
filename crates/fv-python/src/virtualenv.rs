...
pub struct PyVenvConfiguration {
    ...
    /// Was the virtual environment created with the `fv` package?
    pub(crate) uv: bool,
...
pub(crate) fn virtualenv_from_working_dir() -> Result<Option<PathBuf>, Error> {
    ...
    for dir in current_dir.ancestors() {
        ...
        if fv_fs::is_virtualenv_base(dir) {
        ...
        if dot_venv.is_dir() {
            if !fv_fs::is_virtualenv_base(&dot_venv) {
...
impl PyVenvConfiguration {
    ...
    /// Returns true if the virtual environment was created with the fv package.
