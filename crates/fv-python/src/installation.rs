...
use fv_cache::Cache;
use fv_client::{BaseClient, BaseClientBuilder};
use fv_pep440::{Prerelease, Version};
...
impl PythonInstallation {
    ...
    pub async fn find_or_download(
            ...
            Ok(Err(downloads::Error::NoDownloadFound(_))) => {
                if downloads_enabled {
                    ...
                    return Err(err.with_missing_python_hint(
                        "fv embeds available Python downloads and may require an update to install new versions. Consider retrying on a newer version of fv."
                            .to_string(),
                    ));
                }
                None
        ...
        if !downloads_enabled {
            ...
            match python_downloads {
                ...
                PythonDownloads::Manual => {
                    return Err(err.with_missing_python_hint(format!(
                        "A managed Python download is available{for_request}, but Python downloads are set to 'manual', use `fv python install {}` to install the required version",
            ...
            if !client_builder.connectivity.is_online() {
                return Err(err.with_missing_python_hint(format!(
                    "A managed Python download is available{for_request}, but fv is set to offline mode"
                )));
            }
    ...
    /// Returns `true` if this is a managed (fv-installed) Python installation.
    ///
    ...
    /// build can be installed via `fv python upgrade`.
    pub(crate) fn warn_if_outdated_prerelease(
        ...
        {
            warn_user!(
                "You're using a pre-release version of Python ({}) but a stable version is available. Use `fv python upgrade {}` to upgrade.",
                version,
                upgrade_request
            );
        } else {
            warn_user!(
                "You're using a pre-release version of Python ({}) but a stable version is available. Run `fv python upgrade` to update your managed interpreters.",
                version,
            );
        }
    }
