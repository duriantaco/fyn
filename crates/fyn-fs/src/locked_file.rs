...
pub enum LockedFileError {
    #[error(
        "Timeout ({}s) when waiting for lock on `{}` at `{}`, is another fyn process running? You can set `{}` to increase the timeout.",
        timeout.as_secs(),
        resource,
        path.user_display(),
        EnvVars::UV_LOCK_TIMEOUT
...
impl LockedFileMode {
    ...
    ///
    /// On Android, [`std::fs::File::try_lock`] is not supported, so we use
    /// [`rustix::fs::flock`] directly.
    ...
    ///
    /// On Android, [`std::fs::File::lock`] is not supported, so we use
    /// [`rustix::fs::flock`] directly.
    #[cfg(not(target_os = "android"))]
...
impl LockedFile {
    ...
    /// Unlock the file.
    ///
    /// On Android, [`std::fs::File::unlock`] is not supported, so we use
    /// [`rustix::fs::flock`] directly.
    #[cfg(not(target_os = "android"))]
