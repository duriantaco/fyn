...
pub fn uninstall_legacy_editable(egg_link: &Path) -> Result<Uninstall, Error> {
    ...
    // Since fv has an environment lock, it's enough to add a mutex here to ensure we never
    // lose writes to `easy-install.pth` (this is the only place in fv where `easy-install.pth`
    // is modified).
