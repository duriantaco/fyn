...
use fv_fs::{Simplified, persist_with_retry_sync, relative_to};
use fv_normalize::PackageName;
use fv_pypi_types::DirectUrl;
use fv_shell::escape_posix_for_single_quotes;
use fv_trampoline_builder::windows_script_launcher;
...
fn install_script(
    ...
    let size_and_encoded_hash = if start == placeholder_python {
        ...
        let mut target = fv_fs::tempfile_in(&layout.scheme.scripts)?;
        ...
        #[cfg(unix)]
        {
            ...
            if permissions.mode() & 0o111 == 0o111 {
                ...
                fv_fs::copy_atomic_sync(&path, &script_absolute)?;
        ...
        #[cfg(not(unix))]
        {
            ...
            match fv_fs::with_retry_sync(&path, &script_absolute, "renaming", || {
                fs_err::rename(&path, &script_absolute)
            }) {
                Ok(()) => (),
                Err(err) => {
                    debug!("Failed to rename, falling back to copy: {err}");
                    fv_fs::with_retry_sync(&path, &script_absolute, "copying", || {
...
pub(crate) fn write_file_recorded(
    ...
    fv_fs::write_atomic_sync(site_packages.join(relative_path), content.as_ref())?;
...
pub(crate) fn write_installer_metadata<Cache: serde::Serialize, Build: serde::Serialize>(
    ...
    if let Some(cache_info) = cache_info {
        write_file_recorded(
            site_packages,
            &dist_info_dir.join("fv_cache.json"),
    ...
    if let Some(build_info) = build_info {
        write_file_recorded(
            site_packages,
            &dist_info_dir.join("fv_build.json"),
