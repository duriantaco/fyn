...
async fn main() -> ExitCode {
    ...
    // Show `INFO` messages from the fv crate, but allow `RUST_LOG` to override.
    let default_directive = Directive::from_str("fv=info").unwrap();
    ...
    if let Err(err) = result {
        ...
        eprintln!("{}", "fv-dev failed".red().bold());
