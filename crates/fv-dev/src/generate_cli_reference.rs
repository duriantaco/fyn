//! Generate a Markdown-compatible reference for the fv command-line interface.
use std::cmp::max;
...
const REPLACEMENTS: &[(&str, &str)] = &[
    // Replace suggestions to use `fv help python` with a link to the
    // `fv python` section
    (
        "<code>fv help python</code>",
        "<a href=\"#fv-python\">fv python</a>",
    ),
...
fn generate_command<'a>(output: &mut String, command: &'a Command, parents: &mut Vec<&'a Command>) {
    ...
    // Display the top-level `fv` command at the same level as its children
