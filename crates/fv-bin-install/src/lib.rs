//! Binary download and installation utilities for fv.
//!
...
use fv_client::retryable_on_request_failure;
use fv_distribution_filename::SourceDistExtension;
...
use fv_cache::{Cache, CacheBucket, CacheEntry, Error as CacheError};
use fv_client::{BaseClient, RetriableError, fetch_with_url_fallback};
use fv_extract::{Error as ExtractError, stream};
use fv_pep440::{Version, VersionSpecifier, VersionSpecifiers};
...
impl Binary {
    ...
    /// allowing patch version updates without requiring a fv release.
    ...
    /// Return the manifest URLs to try.
    fn manifest_urls(self) -> Vec<DisplaySafeUrl> {
        let name = self.name();
        // These are static strings so parsing cannot fail.
        vec![DisplaySafeUrl::parse(&format!("{VERSIONS_MANIFEST_URL}/{name}.ndjson")).unwrap()]
    }

    /// Given a canonical artifact URL, return the list of URLs to try.
...
pub enum BinVersion {
    ...
    Constraint(fv_pep440::VersionSpecifiers),
}

impl FromStr for BinVersion {
    type Err = fv_pep440::VersionSpecifiersParseError;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let specifiers = fv_pep440::VersionSpecifiers::from_str(s)?;
...
pub enum Error {
    ...
    UrlParse {
        url: String,
        #[source]
        source: fv_redacted::DisplaySafeUrlError,
    },
    ...
    Platform(#[from] fv_platform::Error),
    ...
    NoMatchingVersion {
        binary: Binary,
        constraints: fv_pep440::VersionSpecifiers,
        platform: String,
    },
...
pub async fn find_matching_version(
    binary: Binary,
    constraints: Option<&fv_pep440::VersionSpecifiers>,
...
#[cfg(test)]
mod tests {
    use fv_client::{BaseClientBuilder, retryable_on_request_failure};
    use fv_redacted::DisplaySafeUrl;
    use std::io::Write;
