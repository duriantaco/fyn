...
use fyn_pypi_types::{ParsedDirectoryUrl, ParsedUrl, VerbatimParsedUrl};
...
///
/// Analog to `UnresolvedRequirement` but with `fyn_pep508::Requirement` instead of
/// `distribution_types::Requirement`.
#[derive(Hash, Debug, Clone, Eq, PartialEq)]
pub enum RequirementsTxtRequirement {
    /// The fyn-specific superset over PEP 508 requirements specifier incorporating
    /// `tool.fyn.sources`.
    Named(fyn_pep508::Requirement<VerbatimParsedUrl>),
...
impl RequirementsTxtRequirement {
    ...
    pub fn into_editable(self) -> Result<Self, EditableError> {
        match self {
            Self::Named(requirement) => {
                ...
                let fyn_pep508::VersionOrUrl::Url(url) = version_or_url else {
                ...
                Ok(Self::Named(fyn_pep508::Requirement {
                    version_or_url: Some(fyn_pep508::VersionOrUrl::Url(VerbatimParsedUrl {
    ...
    pub fn parse(
        ...
        match fyn_pep508::Requirement::parse(input, &working_dir) {
            Ok(requirement) => {
