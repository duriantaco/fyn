...
use fyn_fs::{CWD, Simplified, cachedir};
...
use fyn_python::{Interpreter, VirtualEnvironment};
...
pub(crate) fn create(
    ...
    // using a fyn-managed Python (in which case, we can do better for symlinked executables).
    ...
    match location.metadata() {
        ...
        Ok(metadata) if metadata.is_dir() => {
            let is_virtualenv = fyn_fs::is_virtualenv_base(location);
    ...
    #[cfg(unix)]
    {
        fyn_fs::replace_symlink(&executable_target, &executable)?;
        fyn_fs::replace_symlink(
            "python",
        ...
        if interpreter.markers().implementation_name() == "pypy" {
            ...
            fyn_fs::replace_symlink("python", scripts.join("pypy"))?;
        }

        if interpreter.markers().implementation_name() == "graalpy" {
            fyn_fs::replace_symlink("python", scripts.join("graalpy"))?;
        }
    }
    ...
    let mut pyvenv_cfg_data: Vec<(String, String)> = vec![
        ...
        ("fyn".to_string(), version().to_string()),
        (
...
fn confirm_clear(location: &Path, name: &'static str) -> Result<Option<bool>, io::Error> {
    ...
    if term.is_term() {
        ...
        Ok(Some(fyn_console::confirm_with_hint(
...
pub fn remove_virtualenv(location: &Path) -> Result<(), Error> {
    ...
    // fyn can still identify it as a Python virtual environment that can be deleted.
...
pub enum RemovalReason {
    ...
    /// The environment can be removed because it is managed by fyn, e.g., a project or tool
    /// environment.
    ManagedEnvironment,
}
...
impl std::fmt::Display for RemovalReason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ManagedEnvironment => f.write_str("environment is managed by fyn"),
