...
pub enum AuthPolicy {
    ...
    /// be attempted first. If the request fails, fv will search for credentials. If credentials are
    /// found, an authenticated request will be attempted.
    #[default]
    Auto,
    /// Always authenticate.
    ///
    /// If credentials are not provided, fv will eagerly search for credentials. If credentials
    /// cannot be found, fv will error instead of attempting an unauthenticated request.
    Always,
    /// Never authenticate.
    ///
    /// If credentials are provided, fv will error. fv will not search for credentials.
    Never,
}
...
// TODO(john): We are not using `fv_distribution_types::Index` directly
