...
impl KeyringProvider {
    ...
    async fn store_native(
        ...
        let entry = fv_keyring::Entry::new(&prefixed_service, username)?;
    ...
    async fn fetch_native(
        ...
        let Ok(entry) = fv_keyring::Entry::new(&prefixed_service, username) else {
            return None;
        };
        match entry.get_password().await {
            Ok(password) => return Some((username.to_string(), password)),
            Err(fv_keyring::Error::NoEntry) => {
