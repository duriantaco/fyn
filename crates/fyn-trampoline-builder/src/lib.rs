...
// Resource IDs matching fyn-trampoline
#[cfg(windows)]
...
impl Launcher {
    ...
    pub fn write_to_file(self, file: &mut File, is_gui: bool) -> Result<(), Error> {
        ...
        let temp_file = temp_dir
            .path()
            .join(format!("fyn-trampoline-{}.exe", std::process::id()));
...
///
/// See [`fyn-trampoline::bounce::TrampolineKind`].
...
def make_gui() -> None:
    ...
    root = Tk()
    root.title("fyn Test App")
    frm = ttk.Frame(root, padding=10)
    frm.grid()
    ttk.Label(frm, text="Hello from fyn-trampoline-gui.exe").grid(column=0, row=0)
    root.mainloop()
...
def main_console() -> None:
    print("Hello from fyn-trampoline-console.exe", file=sys.stdout)
    print("Hello from fyn-trampoline-console.exe", file=sys.stderr)
...
"#
    ...
    /// See [`fyn-install-wheel::wheel::format_shebang`].
    ...
    fn create_temp_certificate(temp_dir: &tempfile::TempDir) -> Result<(PathBuf, PathBuf)> {
        ...
        params
            .subject_alt_names
            .push(SanType::DnsName("fyn-test-signer".try_into()?));

        let private_key = KeyPair::generate()?;
        let public_cert = params.self_signed(&private_key)?;

        let public_cert_path = temp_dir.path().join("fyn-trampoline-test.crt");
        let private_key_path = temp_dir.path().join("fyn-trampoline-test.key");
    ...
    fn console_script_launcher() -> Result<()> {
        ...
        let stdout_predicate = "Hello from fyn-trampoline-console.exe\r\n";
        let stderr_predicate = "Hello from fyn-trampoline-console.exe\r\n";
