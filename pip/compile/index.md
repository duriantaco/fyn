...
Note by default the `fyn pip compile` output is just displayed and `--output-file` / `-o` argument is needed to write to a file.
...
```
$ fyn pip compile pyproject.toml requirements-dev.in -o requirements-dev.txt
```

fyn also supports legacy `setup.py` and `setup.cfg` formats. To lock dependencies declared in a `setup.py`:

```
$ fyn pip compile setup.py -o requirements.txt
```

To lock dependencies from stdin, use `-`:

```
$ echo "ruff" | fyn pip compile -
```

To lock with optional dependencies enabled, e.g., the "foo" extra:

```
$ fyn pip compile pyproject.toml --extra foo
```

To lock with all optional dependencies enabled:

```
$ fyn pip compile pyproject.toml --all-extras
```
...
```
$ fyn pip compile --project some/path/ --group foo --group bar
```

Alternatively, you can specify a path to a `pyproject.toml` for each group:

```
$ fyn pip compile --group some/path/pyproject.toml:foo --group other/pyproject.toml:bar
```

Note

`--group` flags do not apply to other specified sources. For instance, `fyn pip compile some/path/pyproject.toml --group foo` sources `foo` from `./pyproject.toml` and **not** `some/path/pyproject.toml`.

## [Upgrading requirements](#upgrading-requirements)

When using an output file, fyn will consider the versions pinned in an existing output file. If a dependency is pinned it will not be upgraded on a subsequent compile run. For example:

```
$ echo "ruff==0.3.0" > requirements.txt
$ echo "ruff" | fyn pip compile - -o requirements.txt
# This file was autogenerated by fyn via the following command:
#    fyn pip compile - -o requirements.txt
ruff==0.3.0
```
...
```
$ fyn pip compile - -o requirements.txt --upgrade-package ruff
```
...
Dependencies can be installed directly from their definition files or from compiled `requirements.txt` files with `fyn pip install`. See the documentation on [installing packages from files](../packages/#installing-packages-from-files) for more details.

When installing with `fyn pip install`, packages that are already installed will not be removed unless they conflict with the lockfile. This means that the environment can have dependencies that aren't declared in the lockfile, which isn't great for reproducibility. To ensure the environment exactly matches the lockfile, use `fyn pip sync` instead.
...
```
$ fyn pip compile requirements.in --constraint constraints.txt
```

Note that multiple constraints can be defined in each file and multiple files can be used.

fyn will also read `constraint-dependencies` from the `pyproject.toml` at the workspace root, and append them to those specified in the constraints file.
...
fyn will also read `build-constraint-dependencies` from the `pyproject.toml` at the workspace root, and append them to those specified in the build constraints file.
...
```
$ fyn pip compile requirements.in --override overrides.txt
```
