# [The fyn build backend](#the-fyn-build-backend)

A build backend transforms a source tree (i.e., a directory) into a source distribution or a wheel.

fyn supports all build backends (as specified by [PEP 517](https://peps.python.org/pep-0517/)), but also provides a native build backend (`fyn_build`) that integrates tightly with fyn to improve performance and user experience.
...
The fyn build backend currently **only supports pure Python code**. An alternative backend is required to build a [library with extension modules](../projects/init/#projects-with-extension-modules).
...
## [Using the fyn build backend](#using-the-fyn-build-backend)

To use fyn as a build backend in an existing project, add `fyn_build` to the [`[build-system]`](../projects/config/#build-systems) section in your `pyproject.toml`:
...
The fyn build backend follows the same [versioning policy](../../reference/policies/versioning/) as fyn. Including an upper bound on the `fyn_build` version ensures that your package continues to build correctly as new versions are released.
...
When the project is built, e.g., with [`fyn build`](../../guides/package/), the fyn build backend will be used to create the source distribution and wheel.
...
- The `pyproject.toml`
- The [module](#modules) under [`tool.fyn.build-backend.module-root`](../../reference/settings/#build-backend_module-root).
- The files referenced by `project.license-files` and `project.readme`.
- All directories under [`tool.fyn.build-backend.data`](../../reference/settings/#build-backend_data).
- All files matching patterns from [`tool.fyn.build-backend.source-include`](../../reference/settings/#build-backend_source-include).

From these, items matching [`tool.fyn.build-backend.source-exclude`](../../reference/settings/#build-backend_source-exclude) and the [default excludes](../../reference/settings/#build-backend_default-excludes) are removed.
...
- All directories under [`tool.fyn.build-backend.data`](../../reference/settings/#build-backend_data), which are copied into the `.data` directory.

From these, [`tool.fyn.build-backend.source-exclude`](../../reference/settings/#build-backend_source-exclude), [`tool.fyn.build-backend.wheel-exclude`](../../reference/settings/#build-backend_wheel-exclude) and the default excludes are removed. The source dist excludes are applied to avoid source tree to wheel builds including more files than source tree to source distribution to wheel build.
