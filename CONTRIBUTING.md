...
Testing fyn requires multiple specific Python versions; they can be installed with:
...
fn test_add() {
    ...
    fyn_snapshot!(context.filters(), context.add().arg("requests"), @"");
}
```
...
A subset of fyn tests require both [Git](https://git-scm.com) and [Git LFS](https://git-lfs.com/) to
execute properly.

These tests can be disabled by turning off either `git` or `git-lfs` fyn features.

### Local testing

You can invoke your development version of fyn with `cargo run -- <args>`. For example:
...
```console
$ docker build -t fyn-builder -f crates/fyn-dev/builder.dockerfile --load .
# Build for musl to avoid glibc errors, might not be required with your OS version
cargo build --target x86_64-unknown-linux-musl --profile profiling
docker run --rm -it -v $(pwd):/app fyn-builder /app/target/x86_64-unknown-linux-musl/profiling/fyn-dev resolve-many --cache-dir /app/cache-docker /app/scripts/popular_packages/pypi_10k_most_dependents.txt
```
...
visualize parallel requests and find any spots where fyn is CPU-bound. Example usage, with `fyn` and
`fyn-dev` respectively:

```shell
RUST_LOG=fyn=info TRACING_DURATIONS_FILE=target/traces/jupyter.ndjson cargo run --features tracing-durations-export --profile profiling -- pip compile test/requirements/jupyter.in
```

```shell
RUST_LOG=fyn=info TRACING_DURATIONS_FILE=target/traces/jupyter.ndjson cargo run --features tracing-durations-export --bin fyn-dev --profile profiling -- resolve jupyter
```
...
3. Run the development server with:
   ...
   ```shell
   fyn run --only-group docs mkdocs serve -f mkdocs.yml
   ```

The documentation should then be available locally at
[http://127.0.0.1:8000/fyn/](http://127.0.0.1:8000/fyn/).
