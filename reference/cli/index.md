...
[`fyn why`](#fyn-why) : Explain why a package is included in the project
...
[`--filter`](#fyn-run--filter) *filter* : Restrict a workspace task run to the named package.
...
[`--include-dependencies`](#fyn-run--include-dependencies) : Include active transitive workspace dependencies of filtered packages

[`--include-dependents`](#fyn-run--include-dependents) : Include active transitive workspace dependents of filtered packages
...
[`--sequential`](#fyn-run--sequential) : Run workspace tasks one at a time.
...
[`--upgrade-packages`](#fyn-run--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--workspace`](#fyn-run--workspace) : Run a named task in every child workspace member that defines it.

```
Tasks are executed in dependency order. Independent tasks run in parallel by default, after the workspace environment has been synchronized once with all packages installed.
```
...
[`--upgrade-packages`](#fyn-add--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-remove--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-version--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-sync--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
```
fyn lock [OPTIONS] [COMMAND]
```

### Commands

[`fyn lock diff`](#fyn-lock-diff) : Show the lockfile changes that would be produced by locking

### [fyn lock diff](#fyn-lock-diff)
...
```
fyn lock diff [OPTIONS]
```

### Options

[`--allow-insecure-host`](#fyn-lock-diff--allow-insecure-host), `--trusted-host` *allow-insecure-host* : Allow insecure connections to a host.
...
[`--cache-dir`](#fyn-lock-diff--cache-dir) *cache-dir* : Path to the cache directory.
...
[`--color`](#fyn-lock-diff--color) *color-choice* : Control the use of color in output.
...
[`--config-file`](#fyn-lock-diff--config-file) *config-file* : The path to a `fyn.toml` file to use for configuration.
...
[`--config-setting`](#fyn-lock-diff--config-setting), `--config-settings`, `-C` *config-setting* : Settings to pass to the PEP 517 build backend, specified as `KEY=VALUE` pairs

[`--config-settings-package`](#fyn-lock-diff--config-settings-package), `--config-settings-package` *config-settings-package* : Settings to pass to the PEP 517 build backend for a specific package, specified as `PACKAGE:KEY=VALUE` pairs

[`--default-index`](#fyn-lock-diff--default-index) *default-index* : The URL of the default package index (by default: <https://pypi.org/simple>).
...
[`--directory`](#fyn-lock-diff--directory) *directory* : Change to the given directory prior to running the command.
...
[`--exclude-newer`](#fyn-lock-diff--exclude-newer) *exclude-newer* : Limit candidate packages to those that were uploaded prior to the given date.
...
[`--exclude-newer-package`](#fyn-lock-diff--exclude-newer-package) *exclude-newer-package* : Limit candidate packages for specific packages to those that were uploaded prior to the given date.
...
[`--extra-index-url`](#fyn-lock-diff--extra-index-url) *extra-index-url* : (Deprecated: use `--index` instead) Extra URLs of package indexes to use, in addition to `--index-url`.
...
[`--find-links`](#fyn-lock-diff--find-links), `-f` *find-links* : Locations to search for candidate distributions, in addition to those found in the registry indexes.
...
[`--fork-strategy`](#fyn-lock-diff--fork-strategy) *fork-strategy* : The strategy to use when selecting multiple versions of a given package across Python versions and platforms.
...
[`--help`](#fyn-lock-diff--help), `-h` : Display the concise help for this command

[`--index`](#fyn-lock-diff--index) *index* : The URLs to use when resolving dependencies, in addition to the default index.
...
[`--index-strategy`](#fyn-lock-diff--index-strategy) *index-strategy* : The strategy to use when resolving against multiple index URLs.
...
[`--index-url`](#fyn-lock-diff--index-url), `-i` *index-url* : (Deprecated: use `--default-index` instead) The URL of the Python package index (by default: <https://pypi.org/simple>).
...
[`--keyring-provider`](#fyn-lock-diff--keyring-provider) *keyring-provider* : Attempt to use `keyring` for authentication for index URLs.
...
[`--link-mode`](#fyn-lock-diff--link-mode) *link-mode* : The method to use when installing packages from the global cache.
...
[`--managed-python`](#fyn-lock-diff--managed-python) : Require use of fyn-managed Python versions [env: UV_MANAGED_PYTHON=]

```
By default, fyn prefers using Python versions it manages. However, it will use system Python versions if a fyn-managed Python is not installed. This option disables use of system Python versions.
```

[`--native-tls`](#fyn-lock-diff--native-tls), `--system-certs` : Whether to load TLS certificates from the platform's native store [env: UV_NATIVE_TLS=]
...
[`--no-binary`](#fyn-lock-diff--no-binary) : Don't install pre-built wheels.
...
[`--no-binary-package`](#fyn-lock-diff--no-binary-package) *no-binary-package* : Don't install pre-built wheels for a specific package

```
May also be set with the `UV_NO_BINARY_PACKAGE` environment variable.
```

[`--no-build`](#fyn-lock-diff--no-build) : Don't build source distributions.
...
[`--no-build-isolation`](#fyn-lock-diff--no-build-isolation) : Disable isolation when building source distributions.
...
[`--no-build-isolation-package`](#fyn-lock-diff--no-build-isolation-package) *no-build-isolation-package* : Disable isolation when building source distributions for a specific package.

```
Assumes that the packages' build dependencies specified by PEP 518 are already installed.
```

[`--no-build-package`](#fyn-lock-diff--no-build-package) *no-build-package* : Don't build source distributions for a specific package

```
May also be set with the `UV_NO_BUILD_PACKAGE` environment variable.
```

[`--no-cache`](#fyn-lock-diff--no-cache), `--no-cache-dir`, `-n` : Avoid reading from or writing to the cache, instead using a temporary directory for the duration of the operation

```
May also be set with the `UV_NO_CACHE` environment variable.
```

[`--no-config`](#fyn-lock-diff--no-config) : Avoid discovering configuration files (`pyproject.toml`, `fyn.toml`).
...
[`--no-index`](#fyn-lock-diff--no-index) : Ignore the registry index (e.g., PyPI), instead relying on direct URL dependencies and those provided via `--find-links`

[`--no-managed-python`](#fyn-lock-diff--no-managed-python) : Disable use of fyn-managed Python versions [env: UV_NO_MANAGED_PYTHON=]

```
Instead, fyn will search for a suitable Python version on the system.
```

[`--no-progress`](#fyn-lock-diff--no-progress) : Hide all progress outputs [env: UV_NO_PROGRESS=]

```
For example, spinners or progress bars.
```

[`--no-python-downloads`](#fyn-lock-diff--no-python-downloads) : Disable automatic downloads of Python.

[`--no-sources`](#fyn-lock-diff--no-sources) : Ignore the `tool.fyn.sources` table when resolving dependencies. Used to lock against the standards-compliant, publishable package metadata, as opposed to using any workspace, Git, URL, or local path sources

```
May also be set with the `UV_NO_SOURCES` environment variable.
```

[`--no-sources-package`](#fyn-lock-diff--no-sources-package) *no-sources-package* : Don't use sources from the `tool.fyn.sources` table for the specified packages

```
May also be set with the `UV_NO_SOURCES_PACKAGE` environment variable.
```

[`--offline`](#fyn-lock-diff--offline) : Disable network access [env: UV_OFFLINE=]

```
When disabled, fyn will only use locally cached data and locally available files.
```

[`--prerelease`](#fyn-lock-diff--prerelease) *prerelease* : The strategy to use when considering pre-release versions.
...
[`--project`](#fyn-lock-diff--project) *project* : Discover a project in the given directory.
...
[`--python`](#fyn-lock-diff--python), `-p` *python* : The Python interpreter to use during resolution.
...
[`--quiet`](#fyn-lock-diff--quiet), `-q` : Use quiet output.

```
Repeating this option, e.g., `-qq`, will enable a silent mode in which fyn will write no output to stdout.
```

[`--refresh`](#fyn-lock-diff--refresh) : Refresh all cached data

[`--refresh-package`](#fyn-lock-diff--refresh-package) *refresh-package* : Refresh cached data for a specific package

[`--resolution`](#fyn-lock-diff--resolution) *resolution* : The strategy to use when selecting between the different compatible versions for a given package requirement.
...
[`--script`](#fyn-lock-diff--script) *script* : Diff the lockfile for the specified Python script, rather than the current project.

```
If provided, fyn will resolve the dependencies based on its inline metadata table, in adherence with PEP 723, without writing the script lockfile.
```

[`--upgrade`](#fyn-lock-diff--upgrade), `-U` : Allow package upgrades, ignoring pinned versions in any existing output file. Implies `--refresh`

[`--upgrade-package`](#fyn-lock-diff--upgrade-package), `-P` *upgrade-package* : Allow upgrades for a specific package, ignoring pinned versions in any existing output file. Implies `--refresh-package`

[`--upgrade-packages`](#fyn-lock-diff--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`

[`--verbose`](#fyn-lock-diff--verbose), `-v` : Use verbose output.
...
[`--compile-bytecode`](#fyn-upgrade--compile-bytecode), `--compile` : Compile Python files to bytecode after installation.
...
[`--config-setting`](#fyn-upgrade--config-setting), `--config-settings`, `-C` *config-setting* : Settings to pass to the PEP 517 build backend, specified as `KEY=VALUE` pairs

[`--config-settings-package`](#fyn-upgrade--config-settings-package), `--config-settings-package` *config-settings-package* : Settings to pass to the PEP 517 build backend for a specific package, specified as `PACKAGE:KEY=VALUE` pairs

[`--default-index`](#fyn-upgrade--default-index) *default-index* : The URL of the default package index (by default: <https://pypi.org/simple>).
...
[`--exclude`](#fyn-upgrade--exclude) *exclude* : Exclude the named package from upgrades

[`--exclude-newer`](#fyn-upgrade--exclude-newer) *exclude-newer* : Limit candidate packages to those that were uploaded prior to the given date.
...
[`--exclude-newer-package`](#fyn-upgrade--exclude-newer-package) *exclude-newer-package* : Limit candidate packages for specific packages to those that were uploaded prior to the given date.
...
[`--extra-index-url`](#fyn-upgrade--extra-index-url) *extra-index-url* : (Deprecated: use `--index` instead) Extra URLs of package indexes to use, in addition to `--index-url`.
...
[`--find-links`](#fyn-upgrade--find-links), `-f` *find-links* : Locations to search for candidate distributions, in addition to those found in the registry indexes.
...
[`--fork-strategy`](#fyn-upgrade--fork-strategy) *fork-strategy* : The strategy to use when selecting multiple versions of a given package across Python versions and platforms.
...
[`--index`](#fyn-upgrade--index) *index* : The URLs to use when resolving dependencies, in addition to the default index.
...
[`--index-strategy`](#fyn-upgrade--index-strategy) *index-strategy* : The strategy to use when resolving against multiple index URLs.
...
[`--index-url`](#fyn-upgrade--index-url), `-i` *index-url* : (Deprecated: use `--default-index` instead) The URL of the Python package index (by default: <https://pypi.org/simple>).
...
[`--keyring-provider`](#fyn-upgrade--keyring-provider) *keyring-provider* : Attempt to use `keyring` for authentication for index URLs.
...
[`--link-mode`](#fyn-upgrade--link-mode) *link-mode* : The method to use when installing packages from the global cache.
...
[`--no-binary`](#fyn-upgrade--no-binary) : Don't install pre-built wheels.
...
[`--no-binary-package`](#fyn-upgrade--no-binary-package) *no-binary-package* : Don't install pre-built wheels for a specific package

```
May also be set with the `UV_NO_BINARY_PACKAGE` environment variable.
```

[`--no-build`](#fyn-upgrade--no-build) : Don't build source distributions.
...
[`--no-build-isolation`](#fyn-upgrade--no-build-isolation) : Disable isolation when building source distributions.
...
[`--no-build-isolation-package`](#fyn-upgrade--no-build-isolation-package) *no-build-isolation-package* : Disable isolation when building source distributions for a specific package.

```
Assumes that the packages' build dependencies specified by PEP 518 are already installed.
```

[`--no-build-package`](#fyn-upgrade--no-build-package) *no-build-package* : Don't build source distributions for a specific package
...
[`--no-index`](#fyn-upgrade--no-index) : Ignore the registry index (e.g., PyPI), instead relying on direct URL dependencies and those provided via `--find-links`
...
[`--no-sources`](#fyn-upgrade--no-sources) : Ignore the `tool.fyn.sources` table when resolving dependencies. Used to lock against the standards-compliant, publishable package metadata, as opposed to using any workspace, Git, URL, or local path sources

```
May also be set with the `UV_NO_SOURCES` environment variable.
```

[`--no-sources-package`](#fyn-upgrade--no-sources-package) *no-sources-package* : Don't use sources from the `tool.fyn.sources` table for the specified packages
...
[`--prerelease`](#fyn-upgrade--prerelease) *prerelease* : The strategy to use when considering pre-release versions.
...
[`--refresh`](#fyn-upgrade--refresh) : Refresh all cached data

[`--refresh-package`](#fyn-upgrade--refresh-package) *refresh-package* : Refresh cached data for a specific package

[`--reinstall`](#fyn-upgrade--reinstall), `--force-reinstall` : Reinstall all packages, regardless of whether they're already installed. Implies `--refresh`

[`--reinstall-package`](#fyn-upgrade--reinstall-package) *reinstall-package* : Reinstall a specific package, regardless of whether it's already installed. Implies `--refresh-package`

[`--resolution`](#fyn-upgrade--resolution) *resolution* : The strategy to use when selecting between the different compatible versions for a given package requirement.
...
[`--upgrade`](#fyn-upgrade--upgrade), `-U` : Allow package upgrades, ignoring pinned versions in any existing output file. Implies `--refresh`

[`--upgrade-package`](#fyn-upgrade--upgrade-package), `-P` *upgrade-package* : Allow upgrades for a specific package, ignoring pinned versions in any existing output file. Implies `--refresh-package`

[`--upgrade-packages`](#fyn-upgrade--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-export--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-tree--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
```
fyn why [OPTIONS] <PACKAGE>
```

### Arguments

[PACKAGE](#fyn-why--package) : The package to explain

### Options

[`--all-groups`](#fyn-why--all-groups) : Include dependencies from all dependency groups.

```
`--no-group` can be used to exclude specific groups.
```

[`--allow-insecure-host`](#fyn-why--allow-insecure-host), `--trusted-host` *allow-insecure-host* : Allow insecure connections to a host.
...
[`--cache-dir`](#fyn-why--cache-dir) *cache-dir* : Path to the cache directory.
...
[`--color`](#fyn-why--color) *color-choice* : Control the use of color in output.
...
[`--config-file`](#fyn-why--config-file) *config-file* : The path to a `fyn.toml` file to use for configuration.
...
[`--config-setting`](#fyn-why--config-setting), `--config-settings`, `-C` *config-setting* : Settings to pass to the PEP 517 build backend, specified as `KEY=VALUE` pairs

[`--config-settings-package`](#fyn-why--config-settings-package), `--config-settings-package` *config-settings-package* : Settings to pass to the PEP 517 build backend for a specific package, specified as `PACKAGE:KEY=VALUE` pairs

[`--default-index`](#fyn-why--default-index) *default-index* : The URL of the default package index (by default: <https://pypi.org/simple>).
...
[`--depth`](#fyn-why--depth), `-d` *depth* : Maximum dependency path depth

```
[default: 255]
```

[`--directory`](#fyn-why--directory) *directory* : Change to the given directory prior to running the command.
...
[`--exclude-newer`](#fyn-why--exclude-newer) *exclude-newer* : Limit candidate packages to those that were uploaded prior to the given date.
...
[`--exclude-newer-package`](#fyn-why--exclude-newer-package) *exclude-newer-package* : Limit candidate packages for specific packages to those that were uploaded prior to the given date.
...
[`--extra-index-url`](#fyn-why--extra-index-url) *extra-index-url* : (Deprecated: use `--index` instead) Extra URLs of package indexes to use, in addition to `--index-url`.
...
[`--find-links`](#fyn-why--find-links), `-f` *find-links* : Locations to search for candidate distributions, in addition to those found in the registry indexes.
...
[`--fork-strategy`](#fyn-why--fork-strategy) *fork-strategy* : The strategy to use when selecting multiple versions of a given package across Python versions and platforms.
...
[`--frozen`](#fyn-why--frozen) : Display the requirements without locking the project [env: UV_FROZEN=]

```
If the lockfile is missing, fyn will exit with an error.
```

[`--group`](#fyn-why--group) *group* : Include dependencies from the specified dependency group.

```
May be provided multiple times.
```

[`--help`](#fyn-why--help), `-h` : Display the concise help for this command

[`--index`](#fyn-why--index) *index* : The URLs to use when resolving dependencies, in addition to the default index.
...
[`--index-strategy`](#fyn-why--index-strategy) *index-strategy* : The strategy to use when resolving against multiple index URLs.
...
[`--index-url`](#fyn-why--index-url), `-i` *index-url* : (Deprecated: use `--default-index` instead) The URL of the Python package index (by default: <https://pypi.org/simple>).
...
[`--keyring-provider`](#fyn-why--keyring-provider) *keyring-provider* : Attempt to use `keyring` for authentication for index URLs.
...
[`--link-mode`](#fyn-why--link-mode) *link-mode* : The method to use when installing packages from the global cache.
...
[`--locked`](#fyn-why--locked) : Assert that the `fyn.lock` will remain unchanged [env: UV_LOCKED=]

```
Requires that the lockfile is up-to-date. If the lockfile is missing or needs to be updated, fyn will exit with an error.
```

[`--managed-python`](#fyn-why--managed-python) : Require use of fyn-managed Python versions [env: UV_MANAGED_PYTHON=]

```
By default, fyn prefers using Python versions it manages. However, it will use system Python versions if a fyn-managed Python is not installed. This option disables use of system Python versions.
```

[`--native-tls`](#fyn-why--native-tls), `--system-certs` : Whether to load TLS certificates from the platform's native store [env: UV_NATIVE_TLS=]
...
[`--no-binary`](#fyn-why--no-binary) : Don't install pre-built wheels.
...
[`--no-binary-package`](#fyn-why--no-binary-package) *no-binary-package* : Don't install pre-built wheels for a specific package

```
May also be set with the `UV_NO_BINARY_PACKAGE` environment variable.
```

[`--no-build`](#fyn-why--no-build) : Don't build source distributions.
...
[`--no-build-isolation`](#fyn-why--no-build-isolation) : Disable isolation when building source distributions.
...
[`--no-build-isolation-package`](#fyn-why--no-build-isolation-package) *no-build-isolation-package* : Disable isolation when building source distributions for a specific package.

```
Assumes that the packages' build dependencies specified by PEP 518 are already installed.
```

[`--no-build-package`](#fyn-why--no-build-package) *no-build-package* : Don't build source distributions for a specific package

```
May also be set with the `UV_NO_BUILD_PACKAGE` environment variable.
```

[`--no-cache`](#fyn-why--no-cache), `--no-cache-dir`, `-n` : Avoid reading from or writing to the cache, instead using a temporary directory for the duration of the operation

```
May also be set with the `UV_NO_CACHE` environment variable.
```

[`--no-config`](#fyn-why--no-config) : Avoid discovering configuration files (`pyproject.toml`, `fyn.toml`).
...
[`--no-default-groups`](#fyn-why--no-default-groups) : Ignore the default dependency groups.
...
[`--no-dev`](#fyn-why--no-dev) : Disable the development dependency group [env: UV_NO_DEV=]

```
This option is an alias of `--no-group dev`. See `--no-default-groups` to disable all default groups instead.
```

[`--no-group`](#fyn-why--no-group) *no-group* : Disable the specified dependency group.
...
[`--no-index`](#fyn-why--no-index) : Ignore the registry index (e.g., PyPI), instead relying on direct URL dependencies and those provided via `--find-links`

[`--no-managed-python`](#fyn-why--no-managed-python) : Disable use of fyn-managed Python versions [env: UV_NO_MANAGED_PYTHON=]

```
Instead, fyn will search for a suitable Python version on the system.
```

[`--no-progress`](#fyn-why--no-progress) : Hide all progress outputs [env: UV_NO_PROGRESS=]

```
For example, spinners or progress bars.
```

[`--no-python-downloads`](#fyn-why--no-python-downloads) : Disable automatic downloads of Python.

[`--no-sources`](#fyn-why--no-sources) : Ignore the `tool.fyn.sources` table when resolving dependencies. Used to lock against the standards-compliant, publishable package metadata, as opposed to using any workspace, Git, URL, or local path sources

```
May also be set with the `UV_NO_SOURCES` environment variable.
```

[`--no-sources-package`](#fyn-why--no-sources-package) *no-sources-package* : Don't use sources from the `tool.fyn.sources` table for the specified packages

```
May also be set with the `UV_NO_SOURCES_PACKAGE` environment variable.
```

[`--offline`](#fyn-why--offline) : Disable network access [env: UV_OFFLINE=]

```
When disabled, fyn will only use locally cached data and locally available files.
```

[`--only-dev`](#fyn-why--only-dev) : Only include the development dependency group.
...
[`--only-group`](#fyn-why--only-group) *only-group* : Only include dependencies from the specified dependency group.
...
[`--prerelease`](#fyn-why--prerelease) *prerelease* : The strategy to use when considering pre-release versions.
...
[`--project`](#fyn-why--project) *project* : Discover a project in the given directory.
...
[`--python`](#fyn-why--python), `-p` *python* : The Python interpreter to use for locking and filtering.
...
[`--python-platform`](#fyn-why--python-platform) *python-platform* : The platform to use when filtering dependency paths.

```
For example, pass `--platform windows` to display the paths that would be included when installing on Windows.
...
[`--python-version`](#fyn-why--python-version) *python-version* : The Python version to use when filtering dependency paths.

```
For example, pass `--python-version 3.10` to display the paths that would be included when installing on Python 3.10.

Defaults to the version of the discovered Python interpreter.
```

[`--quiet`](#fyn-why--quiet), `-q` : Use quiet output.

```
Repeating this option, e.g., `-qq`, will enable a silent mode in which fyn will write no output to stdout.
```

[`--resolution`](#fyn-why--resolution) *resolution* : The strategy to use when selecting between the different compatible versions for a given package requirement.
...
[`--script`](#fyn-why--script) *script* : Explain dependencies for the specified PEP 723 Python script, rather than the current project.

```
If provided, fyn will resolve the dependencies based on its inline metadata table, in adherence with PEP 723.
```

[`--universal`](#fyn-why--universal) : Show platform-independent dependency paths.
...
[`--upgrade`](#fyn-why--upgrade), `-U` : Allow package upgrades, ignoring pinned versions in any existing output file. Implies `--refresh`

[`--upgrade-package`](#fyn-why--upgrade-package), `-P` *upgrade-package* : Allow upgrades for a specific package, ignoring pinned versions in any existing output file. Implies `--refresh-package`

[`--upgrade-packages`](#fyn-why--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`

[`--verbose`](#fyn-why--verbose), `-v` : Use verbose output.
...
[`--direct-only`](#fyn-audit--direct-only) : Only audit the project's direct dependencies.

```
Transitive dependencies are excluded, so only vulnerabilities in packages that the project (or the selected groups and extras) depends on directly are reported.
```
...
[`--explain`](#fyn-audit--explain) : Show dependency paths for audit findings
...
[`--upgrade-packages`](#fyn-audit--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-tool-run--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-tool-install--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--upgrade-packages`](#fyn-pip-compile--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--no-editable`](#fyn-pip-install--no-editable) : Install any editable dependencies as non-editable [env: UV_NO_EDITABLE=]
...
[`--upgrade-packages`](#fyn-pip-install--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
...
[`--force`](#fyn-venv--force) : Allow `--clear` to remove a non-virtual environment directory.
...
[`--upgrade-packages`](#fyn-build--upgrade-packages) *upgrade-packages* : Allow upgrades for specific packages, ignoring pinned versions in any existing output file. Implies `--refresh-package`
