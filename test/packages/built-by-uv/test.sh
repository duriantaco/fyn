...
# to use the debug build of fyn, so we use the internal API instead.
...
mkdir -p dist
rm -f dist/*
../../../target/debug/fyn build-backend build-sdist dist/
rm -rf build-root
mkdir build-root
cd build-root
...
../../../../../target/debug/fyn build-backend build-wheel ../../dist
unzip -l ../../dist/built_by_uv-0.1.0-py3-none-any.whl
cd ../..
fyn pip install -q pytest dist/built_by_uv-0.1.0-py3-none-any.whl
pytest
