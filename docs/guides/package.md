...
description: A guide to using fv to build and publish Python packages to a package index, like PyPI.
---

# Building and publishing a package

fv supports building Python packages into source and binary distributions via `fv build` and
uploading them to a registry with `fv publish`.
...
If your project does not include a `[build-system]` definition in the `pyproject.toml`, fv will not
build it during `fv sync` operations in the project, but will fall back to the legacy setuptools
build system during `fv build`.
...
Build your package with `fv build`:
...
Alternatively, `fv build <SRC>` will build the package in the specified directory, while
`fv build --package <PACKAGE>` will build the specified package within the current workspace.

!!! info

    By default, `fv build` respects `tool.fv.sources` when resolving build dependencies from the
    `build-system.requires` section of the `pyproject.toml`. When publishing a package, we recommend
    running `fv build --no-sources` to ensure that the package builds correctly when `tool.fv.sources`
    is disabled, as is the case when using other build tools, like [`pypa/build`](https://github.com/pypa/build).

## Updating your version

The `fv version` command provides conveniences for updating the version of your package before you
publish it.
...
```console
$ fv version --bump patch --bump dev=66463664
...
```console
$ fv version --bump patch --bump beta
hello-world 1.3.0 => 1.3.1b1
$ fv version --bump major --bump alpha
...
```console
$ fv version --bump stable
hello-world 1.3.1b2 => 1.3.1
```

!!! info

    By default, when `fv version` modifies the project it will perform a lock and sync. To
...
Publish your package with `fv publish`:
...
If you're using a custom index through `[[tool.fv.index]]`, add `publish-url` and use
`fv publish --index <name>`. For example:
...
!!! note
    ...
    When using `fv publish --index <name>`, the `pyproject.toml` must be present, i.e., you need to
    have a checkout step in a publish CI job.

Even though `fv publish` retries failed uploads, it can happen that publishing fails in the middle,
...
using `--index`, the index URL is used as check URL. fv will skip uploading files that are identical
...
!!! note
    ...
    `UV_PUBLISH_NO_ATTESTATIONS` to disable fv's default behavior.

!!! tip

    `fv publish` does not currently generate attestations; attestations must
    be created separately before publishing.

`fv publish` supports uploading [attestations](https://peps.python.org/pep-0740/) to registries that
support them, like PyPI.

fv will automatically discover and match attestations. For example, given the following `dist/`
directory, `fv publish` will upload the attestations along with their corresponding distributions:
...
Test that the package can be installed and imported with `fv run`:

```console
$ fv run --with <PACKAGE> --no-project -- python -c "import <PACKAGE>"
```
...
Or, read on for [guides](./integration/index.md) on integrating fv with other software.
