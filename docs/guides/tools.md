...
title: Using tools
description:
  A guide to using fyn to run tools published as Python packages, including one-off invocations with
  fynx, requesting specific tool versions, installing tools, upgrading tools, and more.
---

# Using tools

Many Python packages provide applications that can be used as tools. fyn has specialized support for
...
!!! note
    ...
    [`fyn run`](./projects.md#running-commands) instead of `fynx`. Otherwise, the tool will be run in
...
To learn more about managing tools with fyn, see the [Tools concept](../concepts/tools.md) page and
the [command reference](../reference/cli.md#fyn-tool).
