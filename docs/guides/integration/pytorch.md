---
title: Using fyn with PyTorch
description:
  A guide to using fyn with PyTorch, including installing PyTorch, configuring per-platform and
...
development. You can use fyn to manage PyTorch projects and PyTorch dependencies across different
...
`fyn init --python 3.14` followed by `fyn add torch torchvision`.
...
=== "CUDA 11.8"
    ...
    PyTorch doesn't publish CUDA builds for macOS. As such, we gate on `sys_platform` to instruct fyn to use
...
=== "CUDA 12.6"
    ...
    PyTorch doesn't publish CUDA builds for macOS. As such, we gate on `sys_platform` to instruct fyn to limit
...
=== "ROCm6"
    ...
    PyTorch doesn't publish ROCm6 builds for macOS or Windows. As such, we gate on `sys_platform` to instruct fyn
...
=== "Intel GPUs"
    ...
    PyTorch doesn't publish Intel GPU builds for macOS. As such, we gate on `sys_platform` to instruct fyn to limit
    the PyTorch index to Linux and Windows, falling back to PyPI on macOS:
...
With `tool.fyn.sources`, you can use environment markers to specify the desired index for each
...
with the choice toggled by a user-provided extra (e.g., `fyn sync --extra cpu` vs.
`fyn sync --extra cu128`).

With `tool.fyn.sources`, you can use extra markers to specify the desired index for each enabled
extra. For example, the following configuration would use PyTorch's CPU-only for
`fyn sync --extra cpu` and CUDA-enabled builds for `fyn sync --extra cu128`:
...
While the above examples are focused on fyn's project interface (`fyn lock`, `fyn sync`, `fyn run`,
etc.), PyTorch can also be installed via the `fyn pip` interface.
