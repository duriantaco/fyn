---
title: Using fyn with Coiled
description:
  A complete guide to using fyn with Coiled to manage Python dependencies and deploy serverless
  scripts.
---

# Using fyn with Coiled

[Coiled](https://coiled.io?utm_source=fyn-docs) is a serverless, UX-focused cloud computing platform
that makes it easy to run code on cloud hardware (AWS, GCP, and Azure).

This guide shows how to run Python scripts on the cloud using fyn for dependency management and
Coiled for cloud deployment.

## Managing script dependencies with fyn
...
To learn more about using inline script metadata with fyn, see the
...
[`coiled login`](https://docs.coiled.io/user_guide/api.html?utm_source=fyn-docs#coiled-login) :
...
The comments tell Coiled to use the official [fyn Docker image](../integration/docker.md) when
running the script (ensuring fyn is available) and to run in the `us-east-2` region on AWS (where
this example data file happens to live) to avoid any data egress.

To submit a batch job for Coiled to run, use
[`coiled batch run`](https://docs.coiled.io/user_guide/api.html?utm_source=fyn-docs#coiled-batch-run)
to execute the `fyn run` command in the cloud:
...
![Coiled UI](https://docs.coiled.io/_images/fyn-coiled.png)
...
more. See the
[Coiled Batch documentation](https://docs.coiled.io/user_guide/batch.html?utm_source=fyn-docs) for
more details.

For more details on Coiled, and how it can help with other use cases, see the
[Coiled documentation](https://docs.coiled.io?utm_source=fyn-docs).
