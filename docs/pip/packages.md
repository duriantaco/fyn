...
```console
$ # Install a tag
$ fv pip install "git+https://github.com/astral-sh/ruff@v0.2.0"

$ # Install a commit
$ fv pip install "git+https://github.com/astral-sh/ruff@1fadefa67b26508cc59cf38e6130bde2243c929d"

$ # Install a branch
$ fv pip install "git+https://github.com/astral-sh/ruff@main"
```
...
```console
$ fv pip install -e "ruff @ ./project/ruff"
```
...
```console
$ fv pip install -r requirements.txt
```

See the [`fv pip compile`](./compile.md) documentation for more information on `requirements.txt`
files.

Install from a `pyproject.toml` file:

```console
$ fv pip install -r pyproject.toml
```

Install from a `pyproject.toml` file with optional dependencies enabled, e.g., the "foo" extra:

```console
$ fv pip install -r pyproject.toml --extra foo
```

Install from a `pyproject.toml` file with all optional dependencies enabled:

```console
$ fv pip install -r pyproject.toml --all-extras
```
...
```console
$ fv pip install --group foo
```

To specify the project directory where groups should be sourced from:

```console
$ fv pip install --project some/path/ --group foo --group bar
```

Alternatively, you can specify a path to a `pyproject.toml` for each group:

```console
$ fv pip install --group some/path/pyproject.toml:foo --group other/pyproject.toml:bar
```

!!! note

    As in pip, `--group` flags do not apply to other sources specified with flags like `-r` or `-e`.
    For instance, `fv pip install -r some/path/pyproject.toml --group foo` sources `foo`
...
```console
$ fv pip uninstall flask
```

To uninstall multiple packages, e.g., Flask and Ruff:

```console
$ fv pip uninstall flask ruff
```
