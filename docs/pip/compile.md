...
fyn allows dependencies to be locked in the `requirements.txt` format. It is recommended to use the
...
fyn also supports legacy `setup.py` and `setup.cfg` formats. To lock dependencies declared in a
`setup.py`:
...
!!! note
    ...
    `fyn pip compile some/path/pyproject.toml --group foo` sources `foo`
    from `./pyproject.toml` and **not** `some/path/pyproject.toml`.

## Upgrading requirements

When using an output file, fyn will consider the versions pinned in an existing output file. If a
...
`requirements.txt` files with `fyn pip install`. See the documentation on
[installing packages from files](packages.md#installing-packages-from-files) for more details.

When installing with `fyn pip install`, packages that are already installed will not be removed
...
exactly matches the lockfile, use `fyn pip sync` instead.
...
fyn will also read `constraint-dependencies` from the `pyproject.toml` at the workspace root, and
...
fyn will also read `build-constraint-dependencies` from the `pyproject.toml` at the workspace root,
