...
fv allows dependencies to be locked in the `requirements.txt` format. It is recommended to use the
...
```console
$ fv pip compile pyproject.toml -o requirements.txt
```

Note by default the `fv pip compile` output is just displayed and `--output-file` / `-o` argument is
...
```console
$ fv pip compile pyproject.toml requirements-dev.in -o requirements-dev.txt
```

fv also supports legacy `setup.py` and `setup.cfg` formats. To lock dependencies declared in a
`setup.py`:

```console
$ fv pip compile setup.py -o requirements.txt
```

To lock dependencies from stdin, use `-`:

```console
$ echo "ruff" | fv pip compile -
```

To lock with optional dependencies enabled, e.g., the "foo" extra:

```console
$ fv pip compile pyproject.toml --extra foo
```

To lock with all optional dependencies enabled:

```console
$ fv pip compile pyproject.toml --all-extras
```
...
```console
$ fv pip compile - -o requirements.txt --upgrade-package ruff
```
...
`requirements.txt` files with `fv pip install`. See the documentation on
[installing packages from files](packages.md#installing-packages-from-files) for more details.

When installing with `fv pip install`, packages that are already installed will not be removed
...
exactly matches the lockfile, use `fv pip sync` instead.
...
```console
$ fv pip compile requirements.in --constraint constraints.txt
```

Note that multiple constraints can be defined in each file and multiple files can be used.

fv will also read `constraint-dependencies` from the `pyproject.toml` at the workspace root, and
...
fv will also read `build-constraint-dependencies` from the `pyproject.toml` at the workspace root,
...
```console
$ fv pip compile requirements.in --override overrides.txt
```
