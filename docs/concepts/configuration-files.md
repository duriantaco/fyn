...
Specifically, fyn will search for a `pyproject.toml` or `fyn.toml` file in the current directory, or
...
`fyn run` can load environment variables from dotenv files (e.g., `.env`, `.env.local`,
