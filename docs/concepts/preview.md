...
fv includes opt-in preview features to provide an opportunity for community feedback and increase
...
can use `fv pip install` with a `pylock.toml` file without additional configuration because
...
- `add-bounds`: Allows configuring the
  [default bounds for `fv add`](../reference/settings.md#add-bounds) invocations.
- `json-output`: Allows `--output-format json` for various fv commands.
...
- `format`: Allows using `fv format`.
- `native-auth`: Enables storage of credentials in a
  [system-native location](../concepts/authentication/http.md#the-fv-credentials-store).
- `workspace-metadata`: Allows using `fv workspace metadata`.
- `workspace-dir`: Allows using `fv workspace dir`.
- `workspace-list`: Allows using `fv workspace list`.
