...
As such, `fv lock` operates on the entire workspace at once, while `fv run` and `fv sync` operate on
...
To create a workspace, add a `tool.fv.workspace` table to a `pyproject.toml`, which will implicitly
create a workspace rooted at that package.

!!! tip

    By default, running `fv init` inside an existing package will add the newly created member to the workspace, creating a `tool.fv.workspace` table in the workspace root if it doesn't already exist.
...
By default, `fv run` and `fv sync` operates on the workspace root. For example, in the above
example, `fv run` and `fv run --package albatross` would be equivalent, while
`fv run --package bird-feeder` would run the command in the `bird-feeder` package.

## Workspace sources

Within a workspace, dependencies on workspace members are facilitated via
[`tool.fv.sources`](./dependencies.md), as in:
...
the workspace. The `workspace = true` key-value pair in the `tool.fv.sources` table indicates the
...
Any `tool.fv.sources` definitions in the workspace root apply to all members, unless overridden in
the `tool.fv.sources` of a specific member. For example, given the following `pyproject.toml`:
...
overrides the `tqdm` entry in its own `tool.fv.sources` table.

!!! note

    If a workspace member provides `tool.fv.sources` for some dependency, it will ignore any
    `tool.fv.sources` for the same dependency in the workspace root, even if the member's source is
...
Workspaces help enforce isolation and separation of concerns. For example, in fv, we have separate
...
dependencies in `tool.fv.sources`:
...
dependency resolution and virtual environment management (with the downside that `fv run --package`
is no longer available; instead, commands must be run from the relevant package directory).

Finally, fv's workspaces enforce a single `requires-python` for the entire workspace, taking the
intersection of all members' `requires-python` values. If you need to support testing a given member
on a Python version that isn't supported by the rest of the workspace, you may need to use `fv pip`
to install that member in a separate virtual environment.

!!! note

    As Python does not provide dependency isolation, fv can't ensure that a package uses its declared dependencies and nothing else. For workspaces specifically, fv can't ensure that packages don't import dependencies declared by another workspace member.
