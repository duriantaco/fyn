...
fyn supports modifying the project's dependencies with `fyn add` and `fyn remove`, but dependency
...
When adding a dependency from a source other than a package registry, fyn will add an entry in the
...
Requesting a different dependency source will update the `tool.fyn.sources` table, e.g., to use
...
The `tool.fyn.sources` table extends the standard dependency tables with alternative dependency
...
!!! important
    ...
    Sources are only respected by fyn. If another tool is used, only the definitions in the standard
...
be used for packages that explicitly specify it in `tool.fyn.sources`. If `explicit` is not set,
...
!!! important
    ...
    When using a directory as a path dependency, fyn will attempt to build and install the target as
...
By specifying the marker on the source, fyn will still include `httpx` on all platforms, but will
...
The use of `--no-sources` will also prevent fyn from discovering any
...
fyn requires that all dependency groups are compatible with each other and resolves all groups
together when creating the lockfile.

If dependencies declared in one group are not compatible with those in another group, fyn will fail
...
!!! tip
    ...
    To disable this behaviour during `fyn run` or `fyn sync`, use `--no-default-groups`.
...
specify a `requires-python` for the group in `[tool.fyn.dependency-groups]`, e.g.:
...
Before `[dependency-groups]` was standardized, fyn used the `tool.fyn.dev-dependencies` field to
...
!!! note
    ...
    If a `tool.fyn.dev-dependencies` field exists, `fyn add --dev` will use the existing section
...
When publishing a package, we recommend running `fyn build --no-sources` to ensure that the package
builds correctly when `tool.fyn.sources` is disabled, as is the case when using other build tools,
...
fyn allows dependencies to be "virtual", in which the dependency itself is not installed as a
...
[`tool.fyn.package = false`](../../reference/settings.md#package). Without this setting, fyn treats
...
If a dependency sets `tool.fyn.package = false`, it can be overridden by declaring `package = true`
on the source:
...
explicitly sets [`tool.fyn.package = false`](../../reference/settings.md#package). Without this
