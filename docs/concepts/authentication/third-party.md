...
fv supports automatic authentication for the Hugging Face Hub. Specifically, if the `HF_TOKEN`
environment variable is set, fv will propagate it to requests to `huggingface.co`.
...
```console
$ HF_TOKEN=hf_... fv run https://huggingface.co/datasets/<user>/<name>/resolve/<branch>/main.py
```
