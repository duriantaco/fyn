...
By default, fyn loads certificates from the bundled `webpki-roots` crate. The `webpki-roots` are a
reliable set of trust roots from Mozilla, and including them in fyn improves portability and
...
variable to the path of the certificate bundle, to instruct fyn to use that file instead of the
...
certificate verification, you can instruct fyn to allow insecure connections to dedicated hosts via
