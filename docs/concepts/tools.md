...
## The `fyn tool` interface

fyn includes a dedicated interface for interacting with tools. Tools can be invoked without
installation using `fyn tool run`, in which case their dependencies are installed in a temporary
virtual environment isolated from the current project.

Because it is very common to run tools without installing them, a `fynx` alias is provided for
`fyn tool run` — the two commands are exactly equivalent. For brevity, the documentation will mostly
refer to `fynx` instead of `fyn tool run`.

Tools can also be installed with `fyn tool install`, in which case their executables are
...
In most cases, executing a tool with `fynx` is more appropriate than installing the tool. Installing
...
When running a tool with `fynx`, a virtual environment is stored in the fyn cache directory and is
treated as disposable, i.e., if you run `fyn cache clean` the environment will be deleted. The
...
When installing a tool with `fyn tool install`, a virtual environment is created in the
[fyn tools directory](../reference/storage.md#tools). The environment will not be removed unless the
...
`fyn tool install` will also respect the `{package}@{version}` and `{package}@latest` specifiers, as
in:
...
Tool environments may be upgraded via `fyn tool upgrade`, or re-created entirely via subsequent
`fyn tool install` operations.
...
`fyn tool install black >=23,<24` followed by `fyn tool upgrade black` will upgrade Black to the
...
`fyn tool install black --prerelease allow` followed by `fyn tool upgrade black` will retain the
...
created by fyn, but will ignore non-global Python version requests like `.python-version` files and
...
previously installed by fyn. For example, if `pipx` has been used to install a tool,
`fyn tool install` will fail. The `--force` flag can be used to override this behavior.

## Relationship to `fyn run`
...
- If a tool is already installed, `fyn tool run` will use the installed version but `fyn run` will
  not.

If the tool should not be isolated from the project, e.g., when running `pytest` or `mypy`, then
`fyn run` should be used instead of `fyn tool run`.
