...
fyn uses aggressive caching to avoid re-downloading (and re-building) dependencies that have already
...
- To force fyn to ignore existing installed versions, pass `--reinstall` to any installation command
  (e.g., `fyn sync --reinstall` or `fyn pip install --reinstall ...`). (Consider running
  `fyn cache clean <package-name>` first, to ensure that the cache is cleared prior to
  reinstallation.)

As a special case, fyn will always rebuild and reinstall any local directory dependencies passed
explicitly on the command-line (e.g., `fyn pip install .`).

## Dynamic metadata

By default, fyn will _only_ rebuild and reinstall local directory dependencies (e.g., editables) if
...
entries under [`tool.fyn.cache-keys`](../reference/settings.md#cache-keys), which covers both file
paths and Git commit hashes. Setting [`tool.fyn.cache-keys`](../reference/settings.md#cache-keys)
...
!!! note
    ...
    The use of globs can be expensive, as fyn may need to walk the filesystem to determine whether any files have changed.
...
This will force fyn to rebuild and reinstall `my-package` on every run, regardless of whether the
...
- `fyn cache clean` removes _all_ cache entries from the cache directory, clearing it out entirely.
- `fyn cache clean ruff` removes all cache entries for the `ruff` package, useful for invalidating
  the cache for a single or finite set of packages.
- `fyn cache prune` removes all _unused_ cache entries. For example, the cache directory may contain
  entries created in previous fyn versions that are no longer necessary and can be safely removed.
  `fyn cache prune` is safe to run periodically, to keep the cache directory clean.

fyn blocks cache-modifying operations while other fyn commands are running. By default, those
`fyn cache` commands have a 5 min timeout waiting for other fyn processes to terminate to avoid
...
other fyn processes are reading or writing from the cache, `--force` can be used to ignore the lock.
...
By default, fyn caches both the wheels that it builds from source and the pre-built wheels that it
...
With fyn, it turns out that it's often faster to _omit_ pre-built wheels from the cache (and instead
...
To support this caching strategy, fyn provides a `fyn cache prune --ci` command, which removes all
pre-built wheels and unzipped source distributions from the cache, but retains any wheels that were
built from source. We recommend running `fyn cache prune --ci` at the end of your continuous
...
!!! note
    ...
    fyn _always_ requires a cache directory. When `--no-cache` is requested, fyn will still use
...
Python environment fyn is operating on. Otherwise, fyn will not be able to link files from the cache
...
of fyn can safely read and write to the same cache directory. However, if the cache version changed
between a given pair of fyn releases, then those releases may not be able to share the same
underlying cache entries.

For example, it's safe to use a single shared cache for fyn 0.4.12 and fyn 0.4.13, though the cache
