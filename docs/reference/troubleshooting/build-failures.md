...
fyn needs to build packages when there is not a compatible wheel (a pre-built distribution of the
...
```console
$ fyn pip install -p 3.13 'numpy<1.20'
Resolved 1 package in 62ms
  ...
  ╰─▶ Call to `setuptools.build_meta:__legacy__.build_wheel()` failed (exit status: 1)
      ...
      Traceback (most recent call last):
        ...
        File "/home/konsti/.cache/fyn/builds-v0/.tmpi4bgKb/lib/python3.13/site-packages/setuptools/__init__.py", line 9, in <module>
...
was used for the build. The error logs are not from fyn itself.

The message following the `╰─▶` is a hint provided by fyn, to help resolve common build failures. A
...
Traceback (most recent call last):
  ...
  File "/Users/example/.cache/fyn/archive-v0/3783IbOdglemN3ieOULx2/lib/python3.13/site-packages/pip/_vendor/pyproject_hooks/_impl.py", line 321, in _call_hook
    raise BackendUnavailable(data.get('traceback', ''))
pip._vendor.pyproject_hooks._impl.BackendUnavailable: Traceback (most recent call last):
  File "/Users/example/.cache/fyn/archive-v0/3783IbOdglemN3ieOULx2/lib/python3.13/site-packages/pip/_vendor/pyproject_hooks/_in_process/_in_process.py", line 77, in _build_backend
    obj = import_module(mod_path)
  File "/Users/example/.local/share/fyn/python/cpython-3.13.0-macos-aarch64-none/lib/python3.13/importlib/__init__.py", line 88, in import_module
...
!!! important
    ...
    build isolation behavior. fyn always uses [build isolation by default](../../pip/compatibility.md#pep-517-build-isolation).

    We also recommend including the `--force-reinstall` and `--no-cache` options when reproducing
    failures.

Since this build failure occurs in pip too, it is not likely to be a bug with fyn.
...
When installing, fyn needs to have a wheel for the current platform for each package. If no matching
wheel exists in the index, fyn tries to build the source distribution.
...
╰─▶ Call to `setuptools.build_meta:__legacy__.build_wheel` failed (exit status: 1)
    ...
    gcc -Wno-unused-result -Wsign-compare -DNDEBUG -g -fwrapv -O3 -Wall -fPIC -DPY_WITH_KECCAK=1 -I/root/.cache/fyn/builds-v0/.tmp8V4iEk/include -I/usr/local/include/python3.10 -c
...
!!! tip
    ...
    When using the fyn-managed Python versions, it's common to need `clang` installed instead of
    `gcc`.
...
╰─▶ Call to `setuptools.build_meta.build_wheel` failed (exit status: 1)
  ...
  running build_py
  ...
  gcc -fno-strict-overflow -Wsign-compare -DNDEBUG -g -O3 -Wall -fPIC -DSWIG_PYTHON_STRICT_BYTE_CHAR -I/root/.cache/fyn/builds-v0/.tmpgLYPe0/include -I/usr/local/include/python3.12 -c pygraphviz/graphviz_wrap.c -o
...
```hl_lines="7"
  ...
  ╰─▶ Call to `setuptools.build_meta:__legacy__.build_wheel` failed (exit status: 1)
    ...
    Traceback (most recent call last):
      ...
      File "/root/.cache/fyn/builds-v0/.tmpvvHaxI/lib/python3.12/site-packages/setuptools/build_meta.py", line 334, in get_requires_for_build_wheel
        return self._get_build_requires(config_settings, requirements=[])
                ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      File "/root/.cache/fyn/builds-v0/.tmpvvHaxI/lib/python3.12/site-packages/setuptools/build_meta.py", line 304, in _get_build_requires
        self.run_setup()
      File "/root/.cache/fyn/builds-v0/.tmpvvHaxI/lib/python3.12/site-packages/setuptools/build_meta.py", line 522, in run_setup
        super().run_setup(setup_script=setup_script)
      File "/root/.cache/fyn/builds-v0/.tmpvvHaxI/lib/python3.12/site-packages/setuptools/build_meta.py", line 320, in run_setup
        exec(code, locals())
...
```console
$ fyn pip install pip setuptools
$ fyn pip install chumpy --no-build-isolation-package chumpy
```
...
lower bound (e.g., `numpy>=1.17`). Sometimes, due to algorithmic limitations, the fyn resolver tries
to find a fitting version using unreasonably old packages, which can be prevented by using lower
bounds.

For example, when resolving the following dependencies on Python 3.10, fyn attempts to build an old
...
fyn will avoid using an old version of `apache-beam`.
