...
fyn has Tier 1 support for the following platforms:
...
fyn is continuously built, tested, and developed against its Tier 1 platforms. Inspired by the Rust
project, Tier 1 can be thought of as
["guaranteed to work"](https://doc.rust-lang.org/beta/rustc/platform-support.html#tier-1).

fyn has Tier 2 support
...
Tier 3 platforms may not be built or tested, but fyn will accept patches to fix bugs.

## Linux versions

On Linux, compatibility is determined by libc version.

fyn publishes both glibc-based and musl-based distributions.

For glibc-based Linux distributions, fyn publishes
...
fyn's official glibc-based wheels and binaries are published for the following targets:
...
fyn also publishes musl-based wheels and fully statically linked binaries for the following targets:
...
However, the embedded `fyn` binaries are fully statically linked and do not require musl libc on the
host system.

The official [Docker images](../../guides/integration/docker.md) include these fully statically
linked musl fyn binaries for amd64 and arm64.
...
fyn supports macOS 13+ (Ventura).

fyn is known to work on macOS 12, but requires installation of a `realpath` executable.
