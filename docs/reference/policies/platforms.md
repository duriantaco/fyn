...
fv has Tier 1 support for the following platforms:
...
fv provides official binaries on GitHub and pre-built wheels on [PyPI](https://pypi.org/project/fv/)
for its Tier 1 and Tier 2 platforms.

Tier 2 platforms are continuously built, but the fv test suite is not run on them and stability may
vary in practice.

Tier 3 platforms may not be built or tested, but fv will accept patches to fix bugs.

## Linux versions

On Linux, compatibility is determined by libc version.

fv publishes both glibc-based and musl-based distributions.

For glibc-based Linux distributions, fv publishes
...
fv's official glibc-based wheels and binaries are published for the following targets:
...
fv also publishes musl-based wheels and fully statically linked binaries for the following targets:
...
However, the embedded `fv` binaries are fully statically linked and do not require musl libc on the
host system.

The official [Docker images](../../guides/integration/docker.md) include these fully statically
linked musl fv binaries for amd64 and arm64.
...
fv supports macOS 13+ (Ventura).

fv is known to work on macOS 12, but requires installation of a `realpath` executable.
