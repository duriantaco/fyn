...
uv's changelog can be [viewed on GitHub](https://github.com/oha/fyn/blob/main/CHANGELOG.md).

## Crate versioning

fyn's crates are published to [crates.io](https://crates.io). The following crates follow the normal
fyn versioning policy:

- `fyn`
- `fyn-build`
- `fyn-version`

The `fyn` and `fyn-build` crates are versioned by the binary command-line interface. The Rust
interface of these crates does not follow semantic versioning.

The remainder of fyn's crates provide **no stability guarantees**. The Rust interface is considered
internal and unstable. Consequently, they are versioned as `0.0.x`. The patch version is incremented
on every fyn release, regardless of changes to the crate.
...
The `fyn.lock` schema version is considered part of the public API, and so will only be incremented
