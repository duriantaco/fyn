...
=== "macOS and Linux"
    ...
    ```console
    $ curl -LsSf https://astral.sh/fyn/install.sh | env UV_INSTALL_DIR="/custom/path" sh
    ```

=== "Windows"

    ```pwsh-session
    PS> powershell -ExecutionPolicy ByPass -c {$env:UV_INSTALL_DIR = "C:\Custom\Path";irm https://astral.sh/fyn/install.ps1 | iex}
    ```
...
```console
$ curl -LsSf https://astral.sh/fyn/install.sh | sh -s -- --help
```
