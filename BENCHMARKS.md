...
- Benchmark performance may vary dramatically across different operating systems and filesystems. In
  particular, fyn uses different installation strategies based on the underlying filesystem's
  capabilities. (For example, fyn uses reflinking on macOS, and hardlinking on Linux.)
...
Benchmarking package installation (e.g., `fyn sync`) with a warm cache. This is equivalent to
...
Benchmarking dependency resolution (e.g., `fyn lock`) with a warm cache, but no existing lockfile.
This is equivalent to blowing away an existing `requirements.txt` file to regenerate it from a
`requirements.in` file.

## Cold Resolution

Benchmarking dependency resolution (e.g., `fyn lock`) with a cold cache. This is equivalent to
running `fyn lock` on a new machine or in CI (assuming that the package manager cache is not shared
across runs).

## Reproduction

All benchmarks can be generated using the `scripts/benchmark` package, which wraps
[`hyperfine`](https://github.com/sharkdp/hyperfine) to facilitate benchmarking fyn against a variety
of other tools.

The benchmark script itself has several requirements:

- A local fyn release build (`cargo build --release`).
...
```shell
cargo run -p fyn-dev --all-features render-benchmarks resolve-warm.json --title "Warm Resolution"
cargo run -p fyn-dev --all-features render-benchmarks resolve-cold.json --title "Cold Resolution"
cargo run -p fyn-dev --all-features render-benchmarks install-warm.json --title "Warm Installation"
cargo run -p fyn-dev --all-features render-benchmarks install-cold.json --title "Cold Installation"
```
