# [Using fyn with Coiled](#using-fyn-with-coiled)

[Coiled](https://coiled.io?utm_source=fyn-docs) is a serverless, UX-focused cloud computing platform that makes it easy to run code on cloud hardware (AWS, GCP, and Azure).

This guide shows how to run Python scripts on the cloud using fyn for dependency management and Coiled for cloud deployment.

## [Managing script dependencies with fyn](#managing-script-dependencies-with-fyn)

Note

We'll use this concrete example throughout this guide, but any Python script can be used with fyn and Coiled.
...
fyn will automatically create a virtual environment and installs its dependencies.

To learn more about using inline script metadata with fyn, see the [script guide](../../scripts/#declaring-script-dependencies).
...
First, authenticate with Coiled using [`coiled login`](https://docs.coiled.io/user_guide/api.html?utm_source=fyn-docs#coiled-login) :
...
The comments tell Coiled to use the official [fyn Docker image](../docker/) when running the script (ensuring fyn is available) and to run in the `us-east-2` region on AWS (where this example data file happens to live) to avoid any data egress.

To submit a batch job for Coiled to run, use [`coiled batch run`](https://docs.coiled.io/user_guide/api.html?utm_source=fyn-docs#coiled-batch-run) to execute the `fyn run` command in the cloud:

```
$ fynx coiled batch run \
    fyn run process.py
```
