# [Using fyn with AWS Lambda](#using-fyn-with-aws-lambda)
...
You can use fyn with AWS Lambda to manage your Python dependencies, build your deployment package, and deploy your Lambda functions.

Tip

Check out the upstream [`uv-aws-lambda-example`](https://github.com/astral-sh/uv-aws-lambda-example) project for an example of best practices when using fyn to deploy an application to AWS Lambda.
...
```
FROM ghcr.io/duriantaco/fyn:0.10.17 AS fyn
...
# Bundle the dependencies into the Lambda task root via `fyn pip install --target`.
#
# Omit any local packages (`--no-emit-workspace`) and development dependencies (`--no-dev`).
# This ensures that the Docker layer cache is only invalidated when the `pyproject.toml` or `fyn.lock`
# files change, but remains robust to changes in the application code.
RUN --mount=from=fyn,source=/fyn,target=/bin/fyn \
    --mount=type=cache,target=/root/.cache/fyn \
    --mount=type=bind,source=fyn.lock,target=fyn.lock \
    --mount=type=bind,source=pyproject.toml,target=pyproject.toml \
    fyn export --frozen --no-emit-workspace --no-dev --no-editable -o requirements.txt && \
    fyn pip install -r requirements.txt --target "${LAMBDA_TASK_ROOT}"
...
1. **Minimal image size.** By using a multi-stage build, we can ensure that the final image only includes the application code and dependencies. For example, the fyn binary itself is not included in the final image.
...
```
 ...
 => CACHED [builder 2/2] RUN --mount=from=fyn,source=/fyn,target=/bin/fyn     --mount=type=cache,target=/root/.cache/u  0.0s
...
```
$ fyn init --lib library
$ fyn add ./library
```

Running `fyn init` within the `project` directory will automatically convert `project` to a workspace and add `library` as a workspace member:
...
By default, `fyn init --lib` will create a package that exports a `hello` function. We'll modify the application source code to call that function:
...
RUN --mount=from=fyn,source=/fyn,target=/bin/fyn \
    ...
    fyn export --frozen --no-dev --no-editable -o requirements.txt && \
    fyn pip install -r requirements.txt --target "${LAMBDA_TASK_ROOT}"
