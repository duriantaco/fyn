"""Create a mirror of Python distributions for use with fyn.

Example usage:
    fyn run ./scripts/create-python-mirror.py --name cpython --arch x86_64 --os linux
"""
...
VERSIONS_FILE = REPO_ROOT / "crates" / "fyn-python" / "download-metadata.json"
PREFIXES = [
    ...
    try:
        ...
        if errors:
            ...
            for url, error in errors:
                print(f"- {url}: {error}")
        print(
            f"Example usage: `UV_PYTHON_INSTALL_MIRROR='file://{target.absolute()}' fyn python install 3.13`"
        )
    except Exception as e:
