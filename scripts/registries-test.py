#!/usr/bin/env python3
"""
Test `fyn add` against multiple Python package registries.
...
def run_test(
    ...
    if auth_method == "env":
        ...
        # Use fyn's text store for authentication
    ...
    with tempfile.TemporaryDirectory() as project_dir:
        ...
        try:
            ...
            print(f"{Fore.RED}{registry_name}: ERROR{Fore.RESET} - fyn not found")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Test fyn add command against multiple registries",
    ...
    parser.add_argument(
        "--uv",
        type=str,
        help="specify a path to the fyn binary (default: fyn command)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
...
def main() -> None:
    ...
    if args.uv:
        ...
        uv = cwd.parent.joinpath(f"target/debug/fyn{executable_suffix}")
